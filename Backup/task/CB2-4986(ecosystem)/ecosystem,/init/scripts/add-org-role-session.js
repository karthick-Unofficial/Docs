...
module.exports.applyScript = async function () {
	const scriptName = "add-org-role-session";

	try {
		// Custom code ---------------------------------------------------

		const orgRoleUpdate = r.table("sys_orgRoles")
			.filter(
				r.row.hasFields("session").not()
			)
			.update({
				"session": {
					"concurrent": false,
		...
		console.log(`${scriptName} orgRole update result: `, orgRoleUpdate);
