...
export const startLiveCameraStream = (
	...
	return async (dispatch) => {
		try {
			const response = await cameraService.streamVideo(
				contextId,
				videoProfile
			);
			dispatch(
 ...
 * @param: subscriberRef -- reference to component that is using feed
 */
export const startGateRunnerActivityStream = (
	id,
	targetType,
	filterOptions,
	subscriberRef
 ...
 */
export const startFloorPlanCameraStream = (
	facilityId,
	floorPlanId,
	subscriberRef
) => {
 ...
 * @param {string} floorPlanId
 */
export const startFloorPlanAccessPointsStream = (
	facilityId,
	floorPlanId,
	subscriberRef
) => {
...
const getItemsInFOV = (contextId, subscriberRef, poly) => {
	return (dispatch, getState) => {
		...
		const integrations = Object.values(state.session.userFeeds).filter(
			(int) => int.canView
		);
...
export const startTrackHistoryStream = (
	contextEntity,
	subscriberRef,
	duration,
	forReplay
			...
			.streamRules(entityId, (err, res) => {
				if (err) console.log(err);
					...
					if (rule.deleted === true && res.old_val) {
						...
						dispatch(
							subscriptionDataReceived(entityId, rule, "rules")
						);
					}
				}
			})
...
export const startProximityEntitiesStream = (
	contextId,
	geometry,
	radiuses,
	subscriberRef
	...
	return (dispatch, getState) => {
		...
		const integrations = state.session.user.profile.integrations.filter(
			(int) =>
				int.config.canView &&
				proximityEntityTypes.includes(int.entityType)
		);
		integrations.forEach((int) => {
			eventService
				.streamProximityEntities(
					contextId,
					int.feedId,
					geometry,
					radiuses,
