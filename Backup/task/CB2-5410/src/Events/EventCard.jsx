...
const EventCard = (props) => {
	...
	let secondaryText = "";
	let favTemplates = [];
	let isFavTemplate = false;

	if (isTemplate) {
		secondaryText = description;
		favTemplates = useSelector((state) => state?.appState?.userGlobal?.eventTemplateFavorites || []);
		if (favTemplates.includes(id)) {
			isFavTemplate = true;
		}
	} else {
			...
			default:
				if (isTemplate) {
					return (
						...
						>
							{isFavTemplate && <StarIcon style={{ fontSize: "1.5rem", color: "#F8F29F" }} />}
							<DashboardCustomize style={{ fontSize: "1.5rem" }} />
						</div>
					);
				} else {
