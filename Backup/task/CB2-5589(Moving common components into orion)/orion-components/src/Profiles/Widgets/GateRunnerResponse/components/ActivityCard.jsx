...
const ActivityCard = ({
	...
	useEffect(() => {
		if (context && context.attachments) {
			const filteredImage = context.attachments.filter((attachment) => attachment.filename.includes("plate")).map((filteredAttachment) => filteredAttachment.handle);
			...
			>
				<div style={styles.lprCard}>
					<div style={styles.lprImageWrapper}>
						<img alt={"Liscence Plate"} style={styles.img} src={carImage} />
					</div>
					<Typography style={styles.gate}>{activity.CarNumber}</Typography>
				</div>
				<div style={styles.selectedGateWrapper}>
					<Typography style={{ fontSize: "14px", color: "#fff", lineHeight: "unset", paddingBottom: "10px", textTransform: "uppercase" }}>{activity.CameraName}</Typography>
