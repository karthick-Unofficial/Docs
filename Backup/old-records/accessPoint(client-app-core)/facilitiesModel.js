 ...
 * @param {string} facilityId 
 */
FacilitiesModel.prototype.getFacilityById = async function (userId, orgId, facilityId) {
	try {
 ...
 */
FacilitiesModel.prototype.addCameraToFloorplan = async function (userId, orgId, facilityId, floorplanId, cameraId, geo) {
	try {
		// contribute
		const authorizedFacility = await feedModel.getEntityWithAuthorization(userId, facilityId, "facility");
		if (!authorizedFacility) {
 ...
 */
 FacilitiesModel.prototype.getAccessPointsForPlacing = async function (userId, orgId) {
	try {
		...
		const accessPoints = await accessPointModel.getAll(orgId);
