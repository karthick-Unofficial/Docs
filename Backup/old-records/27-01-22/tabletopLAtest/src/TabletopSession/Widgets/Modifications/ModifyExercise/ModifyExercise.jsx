...
const ModifyExercise = ( { isController, userInfo, users, sessionId, simulations, simId, simTime, simTimePrecision, baseSimName, agents, agentGroups, simulationData,
	...
	const displayNotification = () =>{
		...
		if (!isController){
			...
			if (pencilsDown.endModifications){
				msgToDisplay = "The Controller has ended the modifications process for this exercise.\n No further modifications may be submitted.";
				return true;
			} 

			if (!pencilsDown.endModifications && pencilsDown.restartModification){
				msgToDisplay = "The Controller has restarted the modifications process for this exercise.\n Please submit your modifications.";
				return true;
			} 
			...
			if (getMyModification().length< 1 && getMyPendingModification().length <1 && getAccepted_RejectedModification().length <1){
				msgToDisplay = "The Controller has paused the current exercise for modifications.\n Please submit your modifications.";
				return true;
			}
		}
		msgToDisplay ="";
	...
	const displayModifications = () =>{
		...
		if (!isController){
			if (getMyModification().length< 1 && getMyPendingModification().length <1 && getAccepted_RejectedModification().length <1){
				infoMsg = "There are no modifications to submit at this time.";
				return false;
			} else{
				infoMsg ="";
				return true;
			...
			if (getNonControllerModifications().length<1){
				infoMsg = "There are no Player submitted modifications at this time." ;
				return false;
			} else {
				infoMsg ="";
				return true;
		...
		<div>
			...
			}
			{
				...
				<div className="modificationActionsRight">
					...
					<Button variant="contained" color="primary" disabled={disabledSubmit()} 
						...
						onClick={isController? submitExercise : submitModification}>{isController? "Generate New Exercise": "Submit Modifications"}</Button>
				</div>
			}
			{
				...
				<div>
					{
						...
						<div className="modificationflexColumn" style={{marginTop:20}}>
							<div style={{width: "100%"}}>
								<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{"My Modifications"}</Typography>
							</div>
					...
					}
					{
						...
						<div className="modificationflexColumn" style={{marginTop:20}}>
							<div style={{width: "100%"}}>
								<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{"Accepted/Rejected Modifications"}</Typography>
							</div>
							<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
								{
			...
			}
			{
				...
				<div>
					<div className="modificationflexColumn" style={{marginTop:20}}>
						<div style={{width: "100%"}}>
							<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{`${users[userInfo.userId].name} (CONTROLLER)`}</Typography>
						</div>
						<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
							{
								agents && getMyModification().map((modObj, index) => {
			...
			{confirmCancel && 
				<ConfirmationDialog
					open={true}
					title=""
					content={"Unsubmitted modifications exist. Are you sure you want to cancel?"}
