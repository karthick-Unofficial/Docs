...
const AccessControlWidget = ({ accessPoint, enabled, order, dir }) => {
	const { status } = accessPoint.entityData.properties;

	const controlClicked = (feature) => {
		integrationService.postExternalSystemResource("udc_accesscontrol", `${feature.command}?proxyName=Lenel&sourceId=${accessPoint.sourceId}`, null, (err, result) => {
			if (err) {
