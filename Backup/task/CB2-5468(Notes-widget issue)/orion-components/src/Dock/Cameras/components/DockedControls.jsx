...
const DockedControls = ({
	...
	return (
		<div>
			...
			>
				{cameraView && (
					<Button style={controlStyles.addCamera}
						primary={
					...
					>
						<MapMarkerRadiusIcon style={dir == "rtl" ? { marginLeft: 0, marginRight: 12 } : { marginRight: 0, marginLeft: 12 }} />
						<span style={controlStyles.dockedControls}>{findNearestMode[cameraPosition] &&
							findNearestPosition === cameraPosition
							? getTranslation("global.dock.cameras.dockedControls.selectNearestCam")
							: getTranslation("global.dock.cameras.dockedControls.mapLocation")}</span>
					</Button>
				)}
				{cameraView && <p style={{ color: "#fff" }}> <Translate value="global.dock.cameras.dockedControls.or" /> </p>}
				<div style={{ width: 256 }}>
					<Autocomplete
						freeSolo
						id="free-solo"
