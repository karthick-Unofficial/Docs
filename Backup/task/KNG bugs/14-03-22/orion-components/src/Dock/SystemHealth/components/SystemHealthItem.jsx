	...
	return (
		<Fragment>
			<div style={{display: "flex", flexWrap: "wrap", justifyContent: "space-between", width: "90%", padding: "5px 0 5px 0"}}>
				<p style={{...styles.text, ...(!passed && {color: "#be3e49"})}}>{check.name}</p>
				...
				{errorMessage && <p style={styles.subtext}>{errorMessage}</p>}
				{errorMessage && <p style={{...styles.subtext, color: "#6e7376"}}><Translate value="global.dock.systemHealth.systemHealthItem.lastUpdate"/>{moment(check.lastUpdated).fromNow()}</p>}
			</div>
		</Fragment>
	);
};
