 ...
 * 
 */
UserModel.prototype.getAll = async function (orgId, fromOrg) {
	try {
		const users = await r
			.table(USER_TABLE)
			.filter(
...
UserModel.prototype.addSync = async function (_user) {
	const self = this;
	try {
		const result = await r
			.table(USER_TABLE)
...
UserModel.prototype.verifyReset = async function (resetToken) {
	const now = new Date();
...
UserModel.prototype.checkAppIntegration = async function (userId, appId, handler) {
	try {
		try {
			const result = this.getById(userId);
			if (result) {
