...
SpotlightMode.addHandles = function (coordinates) {
	const handles = {};
	const getMidpoint = (indexA, indexB) => midpoint(point(coordinates[indexA]), point(coordinates[indexB]));
...
SpotlightMode.onDrag = function (state, e) {
	...
	if (dragSpotlight) {
		...
		const getHandle = (index) => Object.values(handles).find((handle) => handle.properties.index === index);

		const preIndex = index === 0 ? 3 : index - 1;
		const postIndex = index === 3 ? 0 : index + 1;
		const longitude = (index) => (movement === "lng" ? lng : state.spotlight.coordinates[0][index][0]);
		const latitude = (index) => (movement === "lat" ? lat : state.spotlight.coordinates[0][index][1]);
		state.spotlight.updateCoordinate(`0.${index}`, longitude(index), latitude(index));
		state.spotlight.updateCoordinate(`0.${postIndex}`, longitude(postIndex), latitude(postIndex));
		const getNewMid = (index) => {
			const newMid =
				index === 3
					? midpoint(point(state.spotlight.coordinates[0][index]), point(state.spotlight.coordinates[0][0]))
					: midpoint(
...
SpotlightMode.onMouseMove = function (state, e) {
	...
	if (featureTarget) {
		...
		if (handle) {
			this.map.getCanvas().style.cursor = handle.properties.movement === "lng" ? "ew-resize" : "ns-resize";
		} else {
