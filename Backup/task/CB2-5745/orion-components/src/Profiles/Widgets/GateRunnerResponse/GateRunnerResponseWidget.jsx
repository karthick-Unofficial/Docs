...
const GateRunnerResponseWidget = (props) => {
	...
	const applications = useSelector(
		(state) => state.session.user.profile.applications
	);
	...
	const selectedPlate =
		event.additionalProperties && event.additionalProperties.licensePlate;
	...
	useEffect(() => {
		if (prevState) {
			if (!isEqual(activitiesState, prevState.activitiesState)) {
				const filtered = activitiesState.filter((activity) =>
					filterOptions.types.includes(activity.type)
				);
				const sortedActivities =
					size(filtered) &&
					orderBy(filtered, ["activityDate"], ["desc"]);

				setFilteredActivities(sortedActivities);

				const firstActivity = last(sortedActivities);
				const recentActivity = first(sortedActivities);
				setInitialActivityDate(
					firstActivity && firstActivity.activityDate
				);
				...
				if (selectedPlate && firstActivity) {
					...
					dispatch(
						startAttachmentStream(firstActivity.id, "profile")
					);
				}
			}
			if (
				size(unitSettings) &&
				!isEqual(unitSettings, prevState.unitSettings)
			) {
			...
			if (
				!isEmpty(cardCollection) &&
				!isEqual(cardCollection, prevState.cardCollection)
			) {
				const entityDetails = [];
		...
		>
			<div className="widget-header">
				<div className="cb-font-b2" style={{ fontSize: 14 }}>
					<Translate
						value={
							"global.profiles.widgets.gateRunnerWidget.main.title"
						}
					/>
				</div>
