...
const importConnection = (state, setState, establishConnection) => {
	const { importId, importRow } = state;

	establishConnection(importId, (err, result) => {
		if (err) {
			console.log(err);
		}
		else {
		...
		return (
			<Fragment>
				<Row
					...
					handleClear={() => disconnect(connection.id)}
					disabled={true}
					toolTipCheck={true}
					...
					<div style={{ color: "white", marginTop: 40 }}>
						<div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
							<Button
								...
								style={dir == "rtl" ?  { marginLeft: 26, paddingRight: 0, backgroundColor: "transparent" } :{ marginRight: 26, paddingLeft: 0, backgroundColor: "transparent" }}
						...
						<Fragment>
							...
							<div style={{ display: "flex" }}>
								...
								<Button
									size="medium"
									color="primary"
									onClick={() => importConnection(state, setState, establishConnection)}
								>
