...
const ActiveFloorPlans = (props) => {
	...
	useEffect(() => {
		if (selectedFloors) {
			...
			setSelectedFloor(Object.keys(filteredFps).length ? filteredFps : null);
		}
	}, [selectedFloors]);
	...
	return selectedFloorsData && styleReloaded && Object.keys(selectedFloorsData).length ? (
		<Fragment>
			{Object.keys(selectedFloorsData).map((facilityId) => {
				return (
					<FloorPlan
						...
						coordinates={selectedFloorsData[facilityId].geometry.coordinates[0]}
