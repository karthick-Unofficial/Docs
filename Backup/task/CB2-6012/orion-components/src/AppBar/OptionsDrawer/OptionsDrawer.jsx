...
import { persistedState, mapSettingsSelector } from "orion-components/AppState/Selectors";
...
const OptionsDrawer = ({
	...
	useEffect(() => {
		EventBus.subscribe("overlays", handleOverlaysUpdated);
		return () => {
			EventBus.unsubscribe("overlays", handleOverlaysUpdated);
		};
	}, []);

	const handleOverlaysUpdated = (data) => {
		const rasterOverlays = [{
			layerType: "roadsAndLabels",
			allowOpacity: true
		}];
		...
		const mergedOverlays = [...data, ...rasterOverlays];
		...
		>
			...
			<List>
				...
				{overlays.length > 0 &&
					overlays
						.sort((a, b) => {
							return a.layerType.toLowerCase() > b.layerType.toLowerCase() ? 1 : -1;
						})
						.map((overlay, index) => (
							<LayerToggle
								key={index}
								...
								withOpacity={overlay.allowOpacity}
								dir={dir}
								setLayerState={setLayerState}
							/>
						))}
