...
class FullscreenVideoModal extends Component {
	constructor(props) {
		super(props);
		this.state = {};
	}
	...
	closeFullscreen = () => {
		const { closeDialog, dialogKey, setCameraPriority } = this.props;
	...
	render() {
		const { dialogKey, dialog, camera, canControl, children, dir } = this.props;
		...
		return (
			<Dialog fullScreen key={dialogKey} open={dialog === dialogKey}>
				...
				>
					{children}
					{(canControl && this.hasCapability("control") && !this.hasCapability("ribbon")) && (
										...
										<List style={{width: "100%", position: "absolute", top: "50%", transform: "translateY(-50%)"}}>
											...
											{ buttonFeatures.map(feat => (
												<ListItem button style={{paddingTop: 8, paddingBottom: 8, paddingLeft: 16, paddingRight: 16}}>
													...
													<ListItemSecondaryAction onClick={e => this.execFeatureCommand(feat.auxCmd)}>
