...
const ModificationEntry = (props) => {
	const {entities, geometries, history, isController, entryMode, description, existingcomment, onEdit, onDelete, onAccept, onReject, onUndo, dir } = props;
		...
		<div className="modificationItemContainer" style={{marginTop:10, marginBottom:10, minHeight:70, width: "98%"}}>
			...
			<div className="modificationflexColumn" style={dir == "rtl" ? {margin:5, marginRight:10, width:"100%"} : {margin:5, marginLeft:10, width:"100%"}}>
				<Typography variant="body1" className="b2-gray" 
					style={dir == "rtl" ? {textAlign:"right", textDecoration:(mode === "REJECTED")? "line-through": "none"} : {textAlign:"left", textDecoration:(mode === "REJECTED")? "line-through": "none"}}>{description}</Typography>
				{
					...
					<div className="modificationCommentsField">
						<TextField
							...
							placeholder={placeholderConverter("tableopSession.panels.modifications.modificationEntry.addComment")}
							multiline={false}
				...
				}
				{
					...
					<div className="modificationflex" style={{marginTop:0, justifyContent: "start", alignItems: "center"}}>
						...
						<Typography variant="body1" className="b2-gray" style={dir == "rtl" ? {textAlign:"right", marginRight:10} : {textAlign:"left", marginLeft:10}}>{comment}</Typography>
					</div>
				}
			</div>
			...
			<div className="" style={{margin:5, alignSelf: "center"}}>
				{ /* PENDING Button */
					!isController && (mode === "SUBMITTED" || mode === "RESUBMITTED") && <Typography variant="body1" className="b2-gray" style={dir == "rtl" ? {textAlign:"right"} : {textAlign:"left"}}>{<Translate value="tableopSession.panels.modifications.modificationEntry.addComment"/>}</Typography>
				}
			</div>
