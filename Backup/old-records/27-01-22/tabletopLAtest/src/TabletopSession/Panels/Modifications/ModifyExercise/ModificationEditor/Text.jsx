...
const Text = React.forwardRef(({ id, desc, dataType, required, value, disabled, itemSelectionHandler, paramType, dir}, ref) => {
	...
	useEffect(() => {
		...
		if (required){
			if (paramType && paramType === "system"){ 
				...
				if (currentValue <= 0){
					setTextError(<Translate value="tableopSession.panels.modifications.modificationEditor.text.valueGreater"/>);
				} else{
					setTextError(null);
				}
			} else{
				if (textValue.length<1){
					setTextError(<Translate value="tableopSession.panels.modifications.modificationEditor.text.enterDynamic" count={desc}/>);
				}
			}
		}
