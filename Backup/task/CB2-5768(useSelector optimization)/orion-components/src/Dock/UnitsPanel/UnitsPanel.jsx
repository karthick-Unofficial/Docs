...
const UnitsPanel = ({ unitSettings, dir }) => {
	...
	const getCountryCodes = () => {
		unitService.getAppSettingsByKey("units-app", "countryCodes", (err, response) => {
			if (err) {
				console.log("ERROR:", err);
			} else {
				if (response.value) {
	...
	return (
		<UnitsPanelContext.Provider value={{ memberRoleTypes, unitStatusTypes, countryCodesArray }}>
			<div style={styles.container}>
				<h3 style={styles.title}>
					<Translate value="global.dock.unitsPanel.title" />
				</h3>
				{unitSettings && unitSettings.length > 0 ? (
					<Units feedSettings={unitSettings} units={units} memberRoleTypes={memberRoleTypes} />
				) : null}
			</div>
