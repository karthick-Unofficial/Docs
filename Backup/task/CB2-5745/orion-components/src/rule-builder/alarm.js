...
const alarmBuilder = (rule, collections) => {
	...
	if (rule.alarmFeeds && rule.alarmFeeds.length > 0) {
		...
		if (!feed || feed === "") {
			...
			const article =
				filter === "" || VOWELS.includes(filter.charAt(0)) ? "an" : "a";
			return (
				<div id="rule-statement" className="cb-font-b9">
					<Translate
...
const _getFeed = (rule) => {
	const selectedFeed = rule.alarmFeeds.filter(
		(feed) => feed.value === rule.feed
	)[0];
	if (selectedFeed) {
...
const _getFilter = (rule) => {
	...
	if (selectedFeed) {
		const selectedFilter = selectedFeed.filters.filter(
			(filter) => filter.value === rule.filter
		)[0];
		const selectedSubFilter = selectedFilter
			? selectedFilter.subFilters.filter(
					(subFilter) => subFilter.value === rule.subFilter
			  )[0]
			: null;

		return selectedSubFilter
			? selectedSubFilter.label
			: selectedFilter
