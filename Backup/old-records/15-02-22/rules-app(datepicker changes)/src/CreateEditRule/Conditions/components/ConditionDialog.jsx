...
class ConditionDialog extends Component {
	...
	handleStartDateSelect = (event, date) => {
		this.setState({
			startDateValue: parseDate(date)
		});
	};

	handleEndDateSelect = (event, date) => {
		this.setState({
			endDateValue: parseDate(date)
		});
	};
	...
	handleCondition = () => {
		...
		switch (this.state.conditionTypeValue) {
			case "time": {
				...
				if (timeRuleExists) {
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.onlyOneAllowed"/>
					});
				} else if (!afterTime || !beforeTime) {
					// Check to see if user entered a valid time
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.validTimeValue"/>
					});
				} else if (!this.state.conditionAnyTimeValue && afterTime._i === beforeTime._i){
					//Check to see if times are different.
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.startEndTimeDiff"/>
					});
					...
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.startBeforeEndTime"/>
					});
				} else if (
					...
					this.setState({
						dialogError:
						<Translate value="createEditRule.conditions.conditionDialog.dialogError.fallAfterStart"/>
					});
				} else if (this.state.weekdaysValue.length === 0) {
					this.setState({
						dialogError:
						<Translate value="createEditRule.conditions.conditionDialog.dialogError.atleastOneDay"/>
					});
				} else if (
					...
					this.setState({
						dialogError:
						<Translate value="createEditRule.conditions.conditionDialog.dialogError.atleastOneLimitingFactor"/>
					});
				} else {
					const newCondition = {
						type: "time",
			...
			case "speed": {
				if (
					...
					this.setState({
						dialogError:
						<Translate value="createEditRule.conditions.conditionDialog.dialogError.slowerFasterThanVal"/>
					});
				} else if (
					...
					this.setState({
						dialogError:
						<Translate value="createEditRule.conditions.conditionDialog.dialogError.fasterThanGreater"/>
					});
				} else {
					const newCondition = {
						type: "speed",
			...
			case "in-collection": {
				if (!this.state.collectionValue) {
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.selectAColl"/>
					});
				} else {
					const newCondition = {
						type: "in-collection",
			...
			case "duration": {
				...
				if (durationRuleExists) {
					this.setState({
						dialogError: <Translate value="createEditRule.conditions.conditionDialog.dialogError.onlyOneDuration"/>
					});
				} else {
					const newCondition = {
						type: "duration",
	...
	render() {
		...
		const conditionsAddActions = [
			<FlatButton
				style={buttonStyles}
				label={<Translate value="createEditRule.conditions.conditionDialog.cancel"/>}
				onClick={this.handleConditionDialogClose}
				primary={true}
			/>,
			<FlatButton
				style={buttonStyles}
				label={editing ? <Translate value="createEditRule.conditions.conditionDialog.update"/> : <Translate value="createEditRule.conditions.conditionDialog.add"/>}
		...
		const unitOptions = [
			{ id: "kn", value: "kn", label: <Translate value="createEditRule.conditions.conditionDialog.kn"/> },
			{
				id: landUnitSystem === "metric" ? "kph" : "mph",
				value: landUnitSystem === "metric" ? "kph" : "mph",
				label: landUnitSystem === "metric" ? <Translate value="createEditRule.conditions.conditionDialog.kph"/> : <Translate value="createEditRule.conditions.conditionDialog.mph"/>
			}
		];
		...
		return (
			<Dialog
				...
				title={this.placeholderConverter("createEditRule.conditions.conditionDialog.addCondition")}
			...
			>
				<div className="condition-dialog">
					<SelectField
						...
						hintText={<Translate value="createEditRule.conditions.conditionDialog.selectCondition"/>}
						...
						<div className="speed-condition">
							<div className="speed-wrapper">
								...
								<SelectField
									...
									hintText={<Translate value="createEditRule.conditions.conditionDialog.unit"/>}
									hintStyle={dir == "rtl" ? { color: "#828283", marginRight: "24px" } : { color: "#828283", marginLeft: "24px" }}
									listStyle={{ backgroundColor: "#1f1f21" }}
									labelStyle={dir == "rtl" ? { marginRight: "24px", paddingRight: 0, paddingLeft: 56 } : { marginLeft: "24px" }}
									underlineStyle={{ opacity: 0 }}
					...
					{this.state.conditionTypeValue === "time" && (
						<div className="time-condition">
							...
							>
								<RadioButton
									label={<Translate value="createEditRule.conditions.conditionDialog.indefinite"/>}
									value="indefinite"
									labelStyle={{
								...
								/>
								<RadioButton
									label={<Translate value="createEditRule.conditions.conditionDialog.setRange"/>}
									value="set-range"
									labelStyle={{
										color: !this.state.conditionIndefiniteValue
											? "white"
							...
							<div className="date-pickers-wrapper">
								...
								>
									<DatePicker
										...
										textFieldStyle={
											this.state.conditionIndefiniteValue
												? datePickerTextStylesDisabled
												: datePickerTextStyles
										}
										minDate={new Date()}
										autoOk={true}
										disabled={this.state.conditionIndefiniteValue}
										formatDate={date => {
											return timeConversion.convertToUserTime(date, "L");
										}}
										DateTimeFormat= {locale == "en" ? undefined : DateTimeFormat}
										locale={locale}
									/>
								...
								>
									<RadioButton
										label={<Translate value="createEditRule.conditions.conditionDialog.allDay"/>}
										value="all-day"
										labelStyle={{
					...
					{this.state.conditionTypeValue === "in-collection" && (
						<div className="collection-dialog">
							<SelectField
								...
								hintText={<Translate value="createEditRule.conditions.conditionDialog.selectCollection"/>}
