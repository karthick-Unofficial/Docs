...
module.exports = function (app) {
	...
	restServer.put("/facilities/:facilityId/floorplan/:floorplanId/camera/:cameraId", async function (req, res) {
		try {
			...
			const locale = config.environment.locale;
			const translations = await geti18n(app.appRequest, locale);
			const result = await model.addCameraToFloorplan(userId, orgId, facilityId, floorplanId, cameraId, geo, translations);
			res.send(result);
		}
		catch (ex) {
			res.err(ex.message, ex);
	...
	restServer.put("/facilities/:facilityId/floorplan/:floorplanId/accessPoint/:accessPointId", async function (req, res) {
		try {
			const { userId, orgId } = req.identity;
			const { facilityId, floorplanId, accessPointId } = req.routeVars;
			const { geo } = req.body;

			const result = await model.addAccessPointToFloorplan(userId, orgId, facilityId, floorplanId, accessPointId, geo);
			res.send(result);
		}
		catch (ex) {
			res.err(ex.message, ex);
	...
	restServer.get("/facilities/facility/:facilityId", async function (req, res) {
		try {
			...
			const { facilityId } = req.routeVars;
			const result = await model.getFacilityById(userId, facilityId);
			res.send(result);
		}
		catch (ex) {
			res.err(ex.message, ex);
	...
	realtimeServer.pubsub("/floorplanCameras", async function (sub) {
		try {
			const result = await model.streamCamerasByFloorplan(
				sub.identity.userId,
				sub.identity.orgId,
				sub.args.facilityId,
	...
	realtimeServer.pubsub("/floorplanAccessPoints", async function (sub) {
		try {
			const result = await model.streamAccessPointsByFloorplan(
				sub.identity.userId,
				sub.identity.orgId,
				sub.args.facilityId,
				sub.args.floorplanId,
				function (err, record) {
					if (err) {
						logger.error(
							...
							"Unhandled error occurred while streaming floorplan accessPoints",
							{ err: { message: err.message, code: err.code } }
						);
					}
					sub.pub(record);
	...
	restServer.get("/floorplan/facilityFeedId", async function (req, res) {
		try {
			const result = await model.getAllFloorplansWithFacilityFeedId();
			res.send(result);
		}
		catch (ex) {
			res.err(ex.message, ex);
