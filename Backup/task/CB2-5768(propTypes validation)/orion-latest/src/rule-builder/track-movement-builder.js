...
const trackMovementBuilder = (rule, collections, targetAction, hasLinks) => {
	...
	return (
		<div id="rule-statement" className="cb-font-b9">
			<Translate value="global.ruleBuilder.trackMovementBuilder.alertMe" />
			{_getSubjects(rule, collections, targetAction, hasLinks)}
			<span>
				{" " + rule.trigger}
...
const _getSubjects = (rule, collections, targetAction, hasLinks) => {
	if (rule.subject.length === 0) {
		...
		if (rule.subject[0].entityType === "feed") {
			...
			const subjectStrings = ["any track in "];
			rule.subject.forEach((feed, index) => {
				subjectStrings.push(
					<span key={index}>
						{index > 0 && <Translate value="global.ruleBuilder.trackMovementBuilder.or" />}
						{feed.name}
					</span>
				);
			});
			...
			return rule.subject.map((track, index) => {
				return (
					<span key={track.id} className="rule-statement-container">
						{index > 0 && (
							<span>
								<Translate value="global.ruleBuilder.trackMovementBuilder.or" />
							</span>
						)}
						<span
...
const _getTargets = (rule, collections, targetAction, hasLinks) => {
	if (rule.targets.length === 0) {
		return rule.trigger == "cross" ? (
			<Translate value="global.ruleBuilder.trackMovementBuilder.anyLine" />
		) : (
			<Translate value="global.ruleBuilder.trackMovementBuilder.anyPolygon" />
		);
	} else {
