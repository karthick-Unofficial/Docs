...
const NewReplay = ({ dialog, openDialog, closeDialog, dir }) => {
	...
	return (
		<Fragment>
							...
							<Container component={"section"} style={{
								...
								<section style={styles.archiveTable}>
									<div style={{ display: "flex", justifyContent: "space-between", paddingLeft: 50, paddingRight: 50, paddingTop: 25 }}>
										...
										<div>
											<SearchField dir={dir} value={searchFilter} handleChange={(e) => updateSearchFilter(e.target.value)} handleClear={(e) => updateSearchFilter("")} id={"archive-search"} placeholder={"Search Archive"} />
										</div>
									</div>
									<List>
			...
			<Dialog
				...
				title={`${placeholderConverter("newReplay.replayArchive.deleteReplay.title.areYouSure")} ${selectedForDeletionCount} ${placeholderConverter("newReplay.replayArchive.deleteReplay.title.selectedReplay")}${selectedForDeletionCount > 1 ? `${placeholderConverter("newReplay.replayArchive.deleteReplay.title.s")}` : ""}?`}
				abort={{
					label: `${placeholderConverter("newReplay.replayArchive.cancel")}`,
					action: () => {
						closeDialog("deleteReplay");
					}
				}}
				confirm={{
					label: `${placeholderConverter("newReplay.replayArchive.confirm")}`,
					action: () => {
			...
			<Dialog
				...
				title={placeholderConverter("newReplay.replayArchive.downloadReplay")}
				confirm={{
					label: `${placeholderConverter("newReplay.replayArchive.ok")}`,
					action: () => {
