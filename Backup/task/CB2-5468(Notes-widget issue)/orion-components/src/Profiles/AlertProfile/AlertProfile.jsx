...
import { userFeedsSelector, notificationById } from "orion-components/GlobalData/Selectors";
...
const AlertProfile = (props) => {
	...
	const renderTabs = () => {
		...
		const cameraList = _.uniq([...(_.isArray(camerasInRange) && camerasInRange || []), ...(_.isArray(contextEntities) && contextEntities || []).filter(ent => ent.entityType === "camera")]);
					...
					<Card style={{ marginBottom: 12, borderRadius: 5 }}>
						<CardHeader
							...
							titleTypographyProps={{
								style: { lineHeight: "1.25em", fontSize: "14px" },
							}}
							subheader={
								<Fragment >
									...
									{!closed && spotlightGeo && !!spotlights && (
										...
										>
											<Link
												onClick={() =>
													dispatch(addSpotlight(spotlightGeo))
												}
												underline="none"
