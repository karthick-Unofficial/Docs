...
import { PlayCircleFilledWhite, SlowMotionVideo, PauseCircleFilled } from "@mui/icons-material";
		...
		return {
			...
			label: (
				<div style={{ marginLeft: 12 }}>
					{moment(timestamp).valueOf() <= moment(playBarValue).valueOf() ? (
						<Alert />
					) : (
						<Alert customStyles={{ errorStyle: { color: "#B5B9BE" } }} />
					)}
				</div>
			)
		};
...
const PlayBar = ({ startDate, endDate }) => {
	...
	const { clearPlayBarValue, setPlayBarValue, updatePlaying } = actionCreators;
		...
		<ErrorBoundary>
			<div
				style={{
					...
					width:
						secondaryOpen || secondaryExpanded
							? `calc(100% - ${secondaryExpanded ? "600px" : "360px"})`
							: "100%"
				}}
			>
				<div
				...
				>
					...
					>
						<IconButton
							onClick={(e) => updateMenuElement(e.currentTarget)}
							style={{
				...
				>
					<div style={{ height: 19, marginBottom: 10, marginTop: 10 }}>
						<div style={{ float: "left", color: "white" }}>
							{moment(startDate).locale(locale).format("h:mm A")}
						</div>
