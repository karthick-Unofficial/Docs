...
import { mapSettingsSelector, disabledFeedsSelector, persistedState } from "orion-components/AppState/Selectors";
import { layerSourcesSelector, userFeedsSelector, alertSelector } from "orion-components/GlobalData/Selectors";
...
import { subscribeFloorPlansWithFacilityFeedId } from "orion-components/GlobalData/Actions";
...
const MapLayer = (props) => {
	...
	const feed = useSelector((state) => _.find(userFeedsSelector(state), (feed) => feed.feedId === props.feedId));
	...
	const mapToolsRef = useRef(mapTools);
	const clusteringEnabled = useSelector((state) =>
		typeof state.clientConfig.mapSettings.clusteringEnabled !== "undefined"
			? state.clientConfig.mapSettings.clusteringEnabled
			: true
	);
	const clusterMaxZoomLevel = useSelector((state) => state.clientConfig.mapSettings.clusterMaxZoomLevel || 11);
	const silhouetteMinZoomLevel = useSelector((state) => state.clientConfig.mapSettings.silhouetteMinZoomLevel || 11);

	const [labelsVisible, setLabelVisible] = useState(false);
	const [canView, setCanView] = useState(false);
	const [mapIconTemplate, setMapIconTemplate] = useState(null);
	const [renderSilhouette, setRenderSilhouette] = useState(null);
	const [feedEntType, setFeedEntType] = useState("");

	useEffect(() => {
		setLabelVisible(settings.entityLabelsVisible);
		setCanView(feed ? feed.canView : true);
		setMapIconTemplate(feed ? feed.mapIconTemplate : null);
		setRenderSilhouette(feed && feed.renderSilhouette);
		setFeedEntType(userFeed ? userFeed.entityType : "track");
	}, [feed, settings, userFeed]);
	...
	const handleFeatureClick = (e) => {
		// Do nothing if drawing or using distance tool
		if (mapToolsRef.current.type) return;

		captureUserInteraction("MapLayer handleFeatureClick");
		let features = map.queryRenderedFeatures(e.point).filter((feature) => feature.source !== "composite");

		// Prevent click through on cluster click
		if (_.find(features, (feature) => _.includes(feature.layer.id, "cluster"))) {
			return;
		}
	...
	useEffect(() => {
		if (feedEntType === "camera" || feedEntType === "accessPoint") {
	...
	useEffect(() => {
		...
		const filteredData = entities.filter((entity) => {
			if (
				(feedEntType === "camera" && entity.entityData.displayType === "facility") ||
				(feedEntType === "accessPoint" && entity.entityData.displayType === "facility")
			) {
				let activated = false;
				if (selectedFloors) {
				...
				return !!entity.entityData.geometry;
			}
		});
		...
		dispatch(
			setMapEntities({
				[feedId]: filteredData.reduce((a, b) => ((a[b.id] = b), a), {})
			})
		);
	...
	const handleClusterClick = (e) => {
		...
		const nextStop = stops
			.map((stop) => {
				let i = 0;
				stop[0] < pointCount ? i++ : null; //eslint-disable-line no-unused-expressions
				return i;
			})
			.reduce((a, b) => {
				return a + b;
		...
		const bounds = [_.values(map.unproject(coordsA)), _.values(map.unproject(coordsB))];
	...
	const renderLayers = () => {
		...
		const entities = _.compact(_.values(data));
		const entityTypes = _.compact(_.uniq(_.map(entities, "entityData.geometry.type")));
				...
				_.filter(entities, (entity) => {
					if (entity.entityData) {
						if (
							...
							if (selectedFloors) {
								Object.keys(selectedFloors).some((facilityId) => {
									if (
										selectedFloors[facilityId] &&
										!disabledFeeds.includes(selectedFloors[facilityId].feedId) &&
										selectedFloors[facilityId].id === entity.entityData.displayTargetId
									) {
										activated = true;
										return true;
						...
						const hasAlert = _.find(
							alerts,
							(alert) => _.size(alert.object) && alert.object.id === entity.entityData.properties.id
						);
						...
						if (hasOwn(entity, "controls")) entity.entityData.properties.controls = entity.controls;
					}
		...
		const layers = _.map(
			...
			(source, index) => {
				...
				const styles = layerStyles(source.type, labelsVisible, mapTools.feature);
				const linePaint = styles.paint.line;
				const symbolLayout = styles.layout.symbol;
				const linePaintProp = linePaint
					? linePaint[_.filter(_.keys(linePaint), (key) => _.includes(feedEntType, key))[0]]
					: {};

				const layerBefore = `---ac2-${isShapeOrLine ? source.type.toLowerCase() + "s" : "feed-entities"
					}-position-end`;
				return (
					<Fragment key={index}>
						{feedEntType.toLowerCase() === "track" &&
							renderSilhouette &&
							renderSilhouettes(index, source, layerBefore)}
						<GeoJSONLayer
							key={feedId + index}
							...
							symbolLayout={
								symbolLayout
									? symbolLayout[
									_.filter(_.keys(symbolLayout), (key) => _.includes(feedEntType, key))[0]
									]
									: {}
							}
