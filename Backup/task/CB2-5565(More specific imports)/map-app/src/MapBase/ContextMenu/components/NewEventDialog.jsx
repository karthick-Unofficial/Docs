...
const NewEventDialog = ({ open, close, lngLat, loadProfile, timeFormatPreference, dir, classes, locale }) => {
	...
	const handleSave = () => {
		const entityData = point([lngLat.lng, lngLat.lat], {
			name: `${isEmpty(template) ? getTranslation("mapBase.contextMenu.newEventDialog.newEvent") : template.name} - ${formattedDateString}`
		});
				...
				{!!lngLat && (
					<Fragment>
						<Typography variant="caption">
							<Translate value="mapBase.contextMenu.newEventDialog.eventName" /></Typography>
						{isEmpty(template) ?
							<Typography variant="h6"> <Translate value="mapBase.contextMenu.newEventDialog.newEvent" />
								- {formattedDateString} </Typography> :
							<Typography variant="h6">{concat(template.name, [" - ", formattedDateString])}</Typography>}
					</Fragment>
				)}
				<SelectField
					label={getTranslation("mapBase.contextMenu.newEventDialog.selectTemplate")}
						...
						.map(template => {
							return (
								<MenuItem key={template.id ? template.id : "blank"} value={template.id}>
									{isEmpty(template) ? <Translate value="mapBase.contextMenu.newEventDialog.blank" /> : template.name}
								</MenuItem>
							);
						})}
				</SelectField>
