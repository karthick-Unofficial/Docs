...
const RobotProfile = ({
	...
	const notifications = useSelector((state) =>
		isLoaded ? state.globalData.notifications : null
	);
	const sidebarOpen = useSelector((state) =>
		isLoaded ? state.appState.dock.dockData.isOpen : null
	);
	const dockedCameras = useSelector((state) =>
		isLoaded ? state.appState.dock.cameraDock.dockedCameras : null
	);
	...
	useEffect(() => {
		...
		DEFAULT_WIDGET_CONFIG = [
			...
			},
			{
				...
				id: "activities",
				name: getTranslation(
					"global.profiles.robotDogProfile.activities"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "robotCameras",
				name: getTranslation(
					"global.profiles.robotDogProfile.robotCams"
				)
			},
			{
				enabled: true,
				id: "missionControl",
				name: getTranslation(
					"global.profiles.robotDogProfile.missionControl"
				)
			}
		];
	}, []);
			...
			if (err || response.error) {
				dispatch(
					openDialog(
						...
						getTranslation(
							"global.profiles.robotDogProfile.errorText"
						)
					)
				);
			} else if (response) {
	...
	const toggleCameraState = (id, currentState) => {
		const entity = context.entity;
		const camera = entity.entityData.properties.cameras.find(
			(robotCamera) => robotCamera.id === id
		);
		if (camera) {
			...
			const newState =
				camera.states[
					newStateIndex < camera.states.length ? newStateIndex : 0
				];
	...
	actions = [
		...
		{
			name: getTranslation(
				"global.profiles.robotDogProfile.trackHistory"
			),
	...
	const robotCamerasWidgetCameras = camerasInRange
		? camerasInRange.filter((camera) =>
				cameras.some((robotCamera) => robotCamera.id === camera.id)
		  )
		: [];
	const otherCameras = camerasInRange
		? camerasInRange.filter((camera) =>
				robotCamerasWidgetCameras.every(
					(robotCamera) => robotCamera.id !== camera.id
				)
		  )
		: [];
	...
	return (
		...
		>
			...
			<div className="widgets-container" style={widgetsContainerStyle}>
				...
				{getWidgetStatus("robotCameras") &&
					robotCamerasWidgetCameras &&
					robotCamerasWidgetCameras.length > 0 && (
						<RobotCamerasWidget
