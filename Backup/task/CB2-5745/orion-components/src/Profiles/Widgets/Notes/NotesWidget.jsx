	...
	static getDerivedStateFromProps(nextProps, prevState) {
		...
		if (
			...
			if (!prevState.updatedHTML) {
				...
				newState["updatedHTML"] = nextProps.notes.html;
				if (
					nextProps.activities[0].actor.id !== prevState.actor[0].id
				) {
								...
								{secondaryExpanded &&
									this.state.notesChanged && (
										<div className={"widget-option-button"}>
											...
											>
												{getTranslation(
													"global.profiles.widgets.notes.cancel"
												)}
											</Button>
											<Button
												variant="text"
											...
											>
												{getTranslation(
													"global.profiles.widgets.notes.save"
												)}
											</Button>
										</div>
									)}
					...
					<div id="notes-widget" className="widget-content">
						...
						<Dialog
							...
							title={getTranslation(
								"global.profiles.widgets.notes.conflictingChanges"
							)}
							textContent={
								this.state.actor[0]
									? this.state.actor.length < 2
										...
										: getTranslation(
												"global.profiles.widgets.notes.textContent2"
										  )
									: ""
							}
							confirm={{
								...
								label: getTranslation(
									"global.profiles.widgets.notes.confirm"
								)
							}}
							abort={{
								action: () =>
						...
						<Dialog
							open={
								this.props.dialog === "notesWidgetLoadingDialog"
							}
							title={getTranslation(
								"global.profiles.widgets.notes.uploading"
							)}
							textContent={getTranslation(
								"global.profiles.widgets.notes.uploadingNewImages"
							)}
							dir={dir}
						/>
					</div>
