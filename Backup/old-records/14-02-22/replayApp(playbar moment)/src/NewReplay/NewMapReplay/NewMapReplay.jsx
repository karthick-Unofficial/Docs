...
const NewMapReplay = ({
	...
	const placeholderConverter = (value) => {
		...
		const placeholderString = placeholderTxt.toString().replace(/<\/?[^>]+(>|$)/g, "");
	...
	return (
		<Dialog
			open={true}
			title={<h3><Translate value="newReplay.newMapReplay.title" /></h3>}
			titlePropStyles={{
				textAlign: "center"
			}}
			paperPropStyles={{
		...
		>
			<div style={{
				...
				<div style={{ ...styles.selectDivStyles, width: replayFrameValue === "clip-duration" ? "50%" : "100%" }}>
					<SelectField
						id="authtype"
						label={<Translate value="newReplay.newMapReplay.replayFrame" />}
						formControlProps={{
							style: {
								marginTop: 0,
								width: 120
					...
					{replayFrameValue === "clip-duration" ? (
						<Fragment>
							<div style={{ marginTop: -16, width: 208 }}>
								<DateTimePicker maxDate={moment()} dir={dir} locale={locale} format={"MMMM D, YYYY h:mm A"} handleChange={(time) => setBeginningDate(time)} value={beginningDate} label={<Translate value="newReplay.newMapReplay.startDate" />} />
							</div>
							<SelectField
								id="clip-duration"
								dir={dir}
								label={<Translate value="newReplay.newMapReplay.clipDuration" />}
								formControlProps={{
									style: {
										marginTop: 0,
										width: 140
								...
								items={durationOptions.map(option => {
									return { value: `${option} ${placeholderConverter("newReplay.newMapReplay.minutes")}`, id: option };
								})}
						...
						<Fragment>
							<div style={{ marginTop: -16, width: 208 }}>
								<DateTimePicker format={"MMMM D, YYYY h:mm A"} dir={dir} locale={locale} handleChange={(time) => setBeginningDate(time)} value={beginningDate} label={<Translate value="newReplay.newMapReplay.startDate" />} />
							</div>
							<div style={{ marginTop: -16, width: 208 }}>
								<DateTimePicker minDate={beginningDate} dir={dir} locale={locale} format={"MMMM D, YYYY h:mm A"} handleChange={(time) => setEndDate(time)} value={endDate} label={<Translate value="newReplay.newMapReplay.endDate" />} />
							</div>
							<TextField
								id="duration"
								label={<Translate value="newReplay.newMapReplay.duration" />}
								style={{
									width: 120
								}}
