...
const Settings = ({ open, closeDialog, unitMember }) => {
	...
	useEffect(() => {
		...
		const CountryCode =
			...
			countryCodesArray.find((countryCodes) =>
				includes(phone, countryCodes.code)
			);
		if (size(CountryCode)) {
		...
		<div>
			<Template
				...
				title={getTranslation(
					"global.units.components.dialog.settings.title"
				)}
			>
				<div
					style={{
				...
				>
					<div style={{ marginBottom: "25px" }}>
						<TextField
							...
							label={getTranslation(
								"global.units.components.dialog.settings.name"
							)}
							fullWidth
							inputProps={{
								readOnly: true,
						...
						<Grid container columnSpacing={3}>
							<Grid item xs={4} sm={4} md={4} lg={4}>
								<TextField
									...
									label={getTranslation(
										"global.units.components.dialog.settings.Phone"
									)}
									InputLabelProps={{
										style: {
											fontSize: "14px",
								...
								>
									{size(countryCodesArray) &&
										countryCodesArray.map((countryCode) => (
											<MenuItem
												key={`${countryCode.code}-menu-item`}
					...
					<div style={{ marginBottom: "25px" }}>
						<TextField
							...
							label={getTranslation(
								"global.units.components.dialog.settings.chooseUnit"
							)}
					...
					<div style={{ marginBottom: "25px" }}>
						...
						>
							<FormControlLabel
								...
								control={<Radio />}
								label={getTranslation(
									"global.units.components.dialog.settings.person"
								)}
								sx={{ minWidth: "150px" }}
							/>
							<FormControlLabel
								...
								control={<Radio />}
								label={getTranslation(
									"global.units.components.dialog.settings.vehicle"
								)}
								sx={{ minWidth: "150px" }}
							/>
						</RadioGroup>
					...
					<div style={{ marginTop: "10px", marginBottom: "25px" }}>
						...
						<div>
							<TextField
								...
								label={getTranslation(
									"global.units.components.dialog.settings.chooseRole"
								)}
								onChange={(e) =>
