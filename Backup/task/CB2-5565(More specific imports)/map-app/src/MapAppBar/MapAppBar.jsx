...
import { List, ListItem, ListItemText, AppBar, Drawer, ListSubheader, Switch, IconButton, Divider, Collapse } from "@mui/material";
...
const ShapesAppBar = (props) => {
	...
	const toggleSsrRadarOverlay = (mapRef, keyVal) => {
		...
		dispatch(updateSsrRadarTiles(keyVal["ssrRadarVisible"], mapRef));
	};
		...
		<div style={{ height: 48 }}>
			...
			>
				<List
					...
					subheader={
						<ListSubheader className="subheader" style={styles.listSubHeader}><Translate value="mapAppBar.subHeader.baseMap" /></ListSubheader>
					}
				>
					<TileOptions
						mapLabel={mapLabel}
				...
				<List>
					...
					>
						<ListItemText
							primary={getTranslation("mapAppBar.listItem.mapLabels")}
					...
					>
						<ListItemText
							...
							primaryTypographyProps={{ style: { fontSize: 16 }, className: `option-label ${showAllFOVs ? "toggle-on" : "toggle-off"}` }} />
						<Switch
							edge="end"
				...
				<List
					className="customList"
					subheader={
						<ListSubheader className="subheader" style={styles.listSubHeader}><Translate value="mapAppBar.subHeader.mapOverlays" /></ListSubheader>
					}
				>
					{!nauticalChartsEnabled ? null :
						<Fragment>
							...
							>
								<ListItemText
									primary={getTranslation("mapAppBar.listItem.nauticalCharts")}
					...
					>
						<ListItemText
							primary={getTranslation("mapAppBar.listItem.roadsLabels")}
					...
					{!weatherEnabled ? null :
						<Fragment>
							...
							>
								<ListItemText
									primary={getTranslation("mapAppBar.listItem.weatherRadar")}
					...
					{ssrRadarOverlayEnabled && (
						...
						>
							<ListItemText
								...
								primaryTypographyProps={{ style: { fontSize: 16 }, className: `option-label ${ssrRadarVisible ? "toggle-on" : "toggle-off"}` }} />
							<Switch
								edge="end"
								onChange={() => toggleSsrRadarOverlay(map, { ssrRadarVisible: !ssrRadarVisible })}
