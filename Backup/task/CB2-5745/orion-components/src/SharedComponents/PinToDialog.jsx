...
const PinToDialog = ({
	...
	useEffect(() => {
		if (open) {
			eventService.getEventsForPinningEntity(
				entity.id,
				(err, response) => {
			...
			entityCollection.getCollectionsForPinningEntity(
				...
				(err, response) => {
					...
					const memberships = map(
						filter(
							response,
							(collection) => collection.entityPinned
						),
	...
	const handleCollectionToggle = (collection, checked) => {
		...
		if (checked) {
			...
			const values = added.filter((value) =>
				value.id ? value.id !== collection.id : value !== collection.id
			);
			setCollectionUpdates({
				added: values,
				removed: [
	...
	const handleSubmitAddNew = () => {
		if (newCollection.length < 1) {
			setErrorText(
				getTranslation(
					"global.sharedComponents.pinToDialog.errorText.enterValidName"
				)
			);
			return;
		}
			...
			>
				<DialogTitle>
					<Typography sx={{ fontWeight: "400", fontSize: "22px" }}>
						{getTranslation(
							"global.sharedComponents.pinToDialog.pinItem"
						)}
					</Typography>
				</DialogTitle>
									...
									<FormControlLabel
										...
										control={
											<Checkbox
												...
												checked={collectionUpdates.added.some(
													(added) =>
														added.id
															? added.id ===
															  collection.id
									...
									<FormControlLabel
										...
										control={
											<Checkbox
												...
												disabled={
													!canManageEvents &&
													pinnedTo.includes(event.id)
												}
												checked={eventUpdates.added.includes(
													event.id
												)}
												onChange={(e, checked) =>
													handleEventToggle(
			...
			{canPinToCollections && (
				...
				>
					<DialogTitle sx={{ marginBottom: "1.5rem" }}>
						...
						>
							{getTranslation(
								"global.sharedComponents.pinToDialog.addToColl"
							)}
						</Typography>
					</DialogTitle>
					<DialogContent
					...
					>
						<TextField
							...
							label={getTranslation(
								"global.sharedComponents.pinToDialog.enterName"
							)}
							fullWidth={true}
