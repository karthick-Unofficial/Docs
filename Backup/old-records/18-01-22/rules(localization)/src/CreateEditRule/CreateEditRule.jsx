...
class CreateEditRule extends Component {
	...
	validateAudioSettings = () => {
		...
		if (!settings.speakAlertText && !settings.speakAlertNotification) {
			this.setState({
				audioSettingErrorText: <Translate value="createEditRule.main.errorText.atLeastTwo"/>
			});
			return false;
		}
		...
		if (settings.speakAlertText && !settings.alertText) {
			this.setState({
				audioSettingErrorText: <Translate value="createEditRule.main.errorText.alertTextReq"/>
			});
			return false;
		}
	...
	render() {
		...
		return (
			<div className="rules-wrapper" style={{ overflowY: "scroll", height: `calc(100vh - ${WavCamOpen ? "308px" : "48px"})` }}>
				...
				<DismissSettings dismissForOrg={this.state.dismissForOrg} handleDismissForOrg={this.handleDismissForOrg} />
