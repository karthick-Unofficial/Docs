...
import { eventService, integrationService } from "client-app-core";
...
import { unsubscribeFromFeed, startListStream, startEventActivityStream, startAttachmentStream, startEventPinnedItemsStream, startEventCameraStream, startProximityEntitiesStream } from "orion-components/ContextualData/Actions";
...
import { addCameraToDockMode, setCameraPriority, removeDockedCameraAndState } from "orion-components/Dock/Actions/index.js";
import {
	attachFilesToEvent,
	publishEvent,
	shareEvent,
...
const EventProfile = ({
	...
	const [vigilantData, setVigilantData] = useState(null);
	...
	if (!mounted) {
		DEFAULT_WIDGET_CONFIG = [
			...
			},
			{
				enabled: true,
				id: "equipments",
				name: getTranslation("global.profiles.eventProfile.main.equipment")
			},
			{
				enabled: true,
				id: "gate_runner_response",
				name: getTranslation("global.profiles.eventProfile.main.gateRunnerResponse")
			}
		];
		setMounted(true);
	}
	...
	useEffect(() => {
		...
		if (!forReplay) {
			...
			if (getWidgetStatus("gate_runner_response")) {
	...
	const deleteNotes = async eventId => {
		await eventService.deleteEventNotes(
			eventId,
			(err, result) => {
				if (err) {
					console.log(err);
		...
		return (
			...
			>
				...
				<div className="widgets-container" style={widgetsContainerStyle}>
					...
					{vigilantData && vigilantData.detection && getWidgetStatus("gate_runner_response") &&
						<ErrorBoundary>
							<GateRunnerResponseWidget
								...
								event={event}
								detections={vigilantData.detection}
								unsubscribeFromFeed={unsubscribeFromFeed}
								subscriberRef={"profile"}
								selectWidget={selectWidget}
