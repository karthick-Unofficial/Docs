...
const StatusCard = ({
	...
	const updateSelected = (dataIndex) => {
		return (selectedIndex) => {
			statusBoardService.updateSelectedIndex(
				card.id,
				dataIndex,
				selectedIndex,
				(err) => {
		...
		<div className="status-card-container" style={styles.container}>
			<section style={styles.contentArea}>
				...
				{card.data.map((statusControl, idx) => {
					const Component = getControlComponent(
						statusControl.type,
						statusControl,
						idx,
						dir
						...
						{card.lastUpdatedBy ? (
							<p style={styles.updatedText}>
								<Translate
									...
									count={moment(card.lastUpdateDate)
										.locale(locale)
										.fromNow()}
								/>
								<Translate
					...
					<Fragment>
							...
							>
								...
								<ListItemText
									primary={getTranslation(
										"global.sharedComponents.statusCard.main.editCard"
									)}
						...
						{!disableControls && userCanShare && (
							...
							>
								...
								<ListItemText
									primary={getTranslation(
										"global.sharedComponents.statusCard.main.share"
									)}
									style={styles.textAlignRight}
								/>
							</ListItem>
