...
const EntityProfile = ({
	...
	const trackHistDuration = useSelector(
		(state) => isLoaded && trackHistoryDuration(state)
	);
	const entityType = isLoaded && entity.entityType;
	const feedId = isLoaded && entity.feedId;
	const { displayProperties } = useSelector(
		(state) => isLoaded && feedInfoSelector(feedId)(state)
	);
	const profileIconTemplate = useSelector(
		(state) =>
			isLoaded && feedInfoSelector(feedId)(state).profileIconTemplate
	);
	const marineTrafficVisible = useSelector(
		(state) =>
			isLoaded && feedInfoSelector(feedId)(state).marineTrafficVisible
	);
	const widgetState = useSelector(
		(state) => isLoaded && widgetStateSelector(state)
	);
	const entityCollections = useSelector(
		(state) => isLoaded && collectionsSelector(state)
	);
	...
	const mapVisible = mapstatus
		? mapStatus.visible
		: isLoaded && mapstate.baseMap.visible;
	const fullscreenCamera = useSelector(
		(state) => isLoaded && fullscreenCameraOpen(state)
	);
	const appId = useSelector((state) => isLoaded && state.appId);
	const timeFormatPreference = useSelector(
		(state) => isLoaded && state.appState.global.timeFormat
	);
	...
	const [marineTrafficVesselData, setMarineTrafficVesselData] =
		useState(null);
		...
		DEFAULT_WIDGET_CONFIG = [
			{
				...
				id: "alerts",
				name: getTranslation(
					"global.profiles.entityProfile.main.alerts"
				)
			},
			{
				enabled: true,
				id: "activities",
				name: getTranslation(
					"global.profiles.entityProfile.main.activities"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "details",
				name: getTranslation(
					"global.profiles.entityProfile.main.details"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "cameras",
				name: getTranslation(
					"global.profiles.entityProfile.main.cameras"
				)
			},
			{
				enabled: false,
				...
				name: getTranslation(
					"global.profiles.entityProfile.main.marineTraffic"
				)
			},
			{
				enabled: true,
				...
				name: getTranslation(
					"global.profiles.entityProfile.main.droneAssociation"
				)
			}
		];
		setMounted(true);
			...
			if (!forReplay) {
				...
				if (
					...
					*/
					...
					integrationService.getExternalSystem(
						"zetron",
						(errExt, responseExt) => {
							if (errExt) {
								console.log(
									"An error has occurred to check external system is available to user."
								);
							} else {
								if (
									responseExt &&
									responseExt.externalSystemId
								) {
									integrationService.getExternalSystemLookup(
										"zetron",
		...
		if (radioUnitId) {
			...
			restClient.exec_post(
				...
				function (err, response) {
					...
					if (err) {
						console.log(
							"An error has occurred sending command to zetron interface.",
							err,
							response
						);
					}
	...
	const handleScroll = () => {
		$(".cb-profile-wrapper").on("resize scroll", () => {
			...
			const viewportBottom =
				viewportTop + $(".cb-profile-wrapper").height();
	...
	const getWidgetConfig = () => {
		...
		const checkDroneWidget = getCustomWidgetConfig("drone-association");
		...
		widgetConfig =
			!marineTrafficVesselData || !marineTrafficVisible
				? widgetConfig.filter(
						(widget) => widget.id !== "marineTrafficParticulars"
				  )
				: widgetConfig;
		...
		widgetConfig =
			checkDroneWidget.length === 0
				? widgetConfig.filter(
						(widget) => widget.id !== "drone-association"
				  )
				: widgetConfig;
			...
			if (err || response.error) {
				dispatch(
					openDialog(
						...
						getTranslation(
							"global.profiles.entityProfile.main.problemOccurred"
						)
					)
				);
			} else if (response) {
	...
	if (context) {
		...
		const { attachments, rules, camerasInRange, activities } = context;
		const entity = context.entity;
		...
		const rulesAppPermission = user.applications.filter((application) => {
			return (
				application.appId === "rules-app" && application.config.canView
			);
		})[0];
		const imageAttachments = attachments
			? attachments.filter((attachment) => {
					return /(jpg)|(png)|(jpeg)|(gif)|(svg)/.exec(
						attachment.mimeType
					);
			  })
			: [];
		...
		if (zetronPhoneVisible) {
			actions = [
				...
				{
					name: getTranslation(
						"global.profiles.entityProfile.main.tetraRadioCall"
					),
					nameText: "Zetron Call",
					action: () =>
						handleInitiateRadioCall(properties.radioUnitId)
				}
			];
		}
		...
		actions = [
			...
			{
				name: getTranslation(
					"global.profiles.entityProfile.main.trackHistory"
				),
			...
			{
				name: getTranslation(
					"global.profiles.entityProfile.main.pinTo"
				),
				nameText: "Pin To",
		...
		if (
			...
			actions = [
				...
				{
					name: getTranslation(
						"global.profiles.entityProfile.main.edit"
					),
					nameText: "Edit",
					action: () =>
						dispatch(
				...
				{
					name: getTranslation(
						"global.profiles.entityProfile.main.delete"
					),
					nameText: "Delete",
					action: () =>
						openDialogWithAssociation("shapeDeleteDialog", "delete")
				}
			];
		}
		actions = [
		...
		return (
			...
			>
				{!scrolledUp && (
					<ImageViewer images={imageAttachments} dir={dir} />
				)}
				<ErrorBoundary>
					<SummaryWidget
						id={contextId}
						user={
							zetronPhoneVisible
								? { ...user, zetronSystemAvailable: true }
								: user
						}
						context={context}
						name={name}
				...
				>
					...
					{getWidgetStatus("drone-association") && (
						<ErrorBoundary>
							<DroneAssociation
								key={`${contextId}-drone-association`}
								order={
									getWidgetStatus("drone-association").index
								}
								enabled={
									getWidgetStatus("drone-association").enabled
								}
								loadProfile={loadProfile}
								context={context}
							/>
				...
				<Dialog
					...
					confirm={{
						label: getTranslation(
							"global.profiles.entityProfile.main.ok"
						),
						action: () => {
