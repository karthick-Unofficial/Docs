...
const ProfileView = ({ users, locations, groups, departments, districts, settings, zones }) => {
	const {
		points_of_contact,
		location_id,
	...
	const pOContact = filter(users, (contact) => includes(points_of_contact, contact.id));
	const pDLocation = find(locations, ["id", location_id]);
	const aIndividuals = filter(users, (individual) => includes(audience_individuals, individual.id));
	const aDistricts = filter(districts, (district) => includes(audience_districts, district.id));
	const aDepartments = filter(departments, (department) => includes(audience_departments, department.id));
	const aGroups = filter(groups, (group) => includes(audience_groups, group.id));
		...
		<Fragment>
			...
			<Grid container justify="space-between" spacing={16}>
				<Grid item sm={12}>
					<NestedList
						items={aGroups}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.groups", size(aGroups))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
