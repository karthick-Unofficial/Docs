...
export const loadGISProfile = (entity, context = "primary") => {
	return (dispatch, getState) => {
		...
		Promise.all([
			...
			.then(() => {
				dispatch(
					subscriptionReceived(entity.id, null, "entity", "profile")
				);
			})
			.then(() => {
		...
		if (
			history.length < 1 ||
			history[history.length - 1].id !== entity.id
		) {
			dispatch(
				updateViewingHistory(
					entity.id,
					"gis-feature",
					entity.properties.name || entity.properties.state_name
				)
			);
		}
	};
