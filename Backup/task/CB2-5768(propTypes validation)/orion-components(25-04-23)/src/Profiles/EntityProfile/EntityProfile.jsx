...
const EntityProfile = ({
	...
	const marineTrafficVisible = useSelector((state) => entity && feedInfoSelector(feedId)(state)?.marineTrafficVisible);
	...
	const canViewRules = !readOnly && useSelector((state) => canManageByApplication(state)("rules-app", "canView", "config"));
	...
	const canAccess = !readOnly && useSelector((state) => checkPermissionBasedOnFeedId(state)(entity?.feedId, "config"));
	const canManage = !readOnly && useSelector((state) => checkPermissionBasedOnFeedId(state)(entity?.feedId, "permissions", "manage"));
	const canDeleteFiles = !readOnly && useSelector((state) => entityType !== "track" ? canManage : canManageByApplication(state)("map-app", "manage"));
	...
	if (!mounted) {
		DEFAULT_WIDGET_CONFIG = defaultEntityProfileWidgets;
		setMounted(true);
	}
	...
	const getWidgetConfig = () => {
		...
		let widgetConfig = widgetState ? unionBy(widgetState, DEFAULT_WIDGET_CONFIG, "id") : DEFAULT_WIDGET_CONFIG;
