...
export const hideFOVs = () => {
	return (dispatch, getState) => {
		...
		dispatch(
			updatePersistedState("map-app", "showAllFOVs", { showAllFOVs: false })
		);
	};
};
...
export const showFOVs = () => {
	return (dispatch, getState) => {
		...
		if (cameras) dispatch(subscribeFOVs(_.keys(cameras.data)));

		dispatch(
			updatePersistedState("map-app", "showAllFOVs", { showAllFOVs: true })
		);
	};
};
...
export const updateSsrRadarTiles = (ssrRadarVisible, map) => {
	return (dispatch, getState) => {
		if (!ssrRadarVisible) {
			dispatch(clearTiles());
		}
			...
			for (let i = 0; i < rows; i++) {
				for (let j = 0; j < columns; j++) {
					...
					currentTileWidth = (j === columns - 1) ? lastColumnWidth : tileWidth;
					currentTileHeight = (i === rows - 1) ? lastRowHeight : tileHeight;
					...
					const coords = map.unproject([(j * tileWidth) + (currentTileWidth / 2), (i * tileHeight) + (currentTileHeight / 2)]);

					// -- build out SSR data
					data.center["latitude"] = coords.lat;
					data.center["longitude"] = coords.lng;
					data.windowDimension["width"] = parseInt(currentTileWidth, 10);
					data.windowDimension["height"] = parseInt(currentTileHeight, 10);

					const radians = (coords.lat * Math.PI) / 180;
					data["metersPerPixel"] = Math.abs(156543.04 * Math.cos(radians)) / (Math.pow(2, zoomLevel));
					...
					const queryString = `tileId=${data.tileId}&centerLatitude=${data.center.latitude}&centerLongitude=${data.center.longitude}` +
						`&width=${data.windowDimension.width}&height=${data.windowDimension.height}&metersPerPixel=${data.metersPerPixel}` +
						`${data.radarImageId ? `&radarImageId=${data.radarImageId}` : ""}`;
					integrationService.getExternalSystemLookup(
						"ssr-radar",
						"setupTile",
						(err, response) => {
							if (err || !response) {
								dispatch(updatePersistedState("map-app", "mapSettings", { ssrRadarVisible: false }));
								dispatch(createUserFeedback("Failed loading SSR Radar Tiles.", null));
								return;
							}
