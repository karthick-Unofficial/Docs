...
const propTypes = {
	shared: PropTypes.string || PropTypes.bool,
	collectionMembers: PropTypes.object,
	label: PropTypes.string,
	groupId: PropTypes.string,
	createUserFeedback: PropTypes.func,
	renameCollection: PropTypes.func,
	removeFromMyItems: PropTypes.func,
	removeFromCollection: PropTypes.func,
	close: PropTypes.func,
	deleteCollection: PropTypes.func,
	isOwner: PropTypes.bool,
	createCollection: PropTypes.func,
	canRemoveFromThisCollection: PropTypes.bool,
...
const EntityCollectionEdit = ({
	...
	const rowRenderer = ({ key, index, isScrolling, isVisible, style }) => {
		...
		return (
			<div key={key} style={style}>
				<div>
					<div
						className={`ec-dialog-item dockItem ${removalArray.includes(collectionMemberArr[index].id) ? "ec-remove" : "ec-keep"
							}`}
						key={collectionMemberArr[index].id}
					>
						{getIcon(collectionMemberArr[index].entityData.properties.type)}
						<DockItemLabel
							primary={collectionMemberArr[index].entityData.properties.name}
							secondary={collectionMemberArr[index].entityData.properties.type}
						/>
						<i className="material-icons" onClick={() => handleRemoveMember(collectionMemberArr[index].id)}>
							{removalArray.includes(collectionMemberArr[index].id) ? "add_circle" : "cancel"}
						</i>
					</div>
				</div>
			</div>
	...
	const mapApp = find(user ? user.applications : {}, (app) => app.appId === "map-app");
	...
	const actions = isOwner
		? [
			...
			<Button className="customButton" onClick={handleClose} variant="text" key="close-button">
				{getTranslation("listPanel.entityCollection.entityCollectionEdit.flatButtonLabel.cancel")}
			</Button>,
			<Button className="customButton" onClick={handleSave} variant="text" key="confirm-button">
		...
		>
			<div className="dock-group-wrapper" style={{ padding: 5 }}>
				...
				{(canViewCollections || isOwner) && (
					<div className="ec-edit-controls">
						{hasItems ? (
							<a className="cb-font-link" onClick={handleCollectionDialogOpen}>
				...
				<section>
					{collectionMembersLength > 10 ? (
						<AutoSizer disableHeight>
							{({ width }) => (
								<List
									...
									width={width}
									rowHeight={66}
									height={collectionMembersLength > 8 ? 500 : collectionMembersLength * 60}
						...
						Object.values(collectionMembers).map((item) => {
							return (
								<div
									className={`ec-dialog-item dockItem ${removalArray.includes(item.id) ? "ec-remove" : "ec-keep"
										}`}
									key={item.id}
								>
									...
									/>
									{canRemoveFromThisCollection && (
										<i className="material-icons" onClick={() => handleRemoveMember(item.id)}>
											{removalArray.includes(item.id) ? "add_circle" : "cancel"}
										</i>
									)}
								</div>
							);
			...
			<EntityAddToCollection
				...
				addition={Object.values(collectionMembers).map((item) => {
					return {
						id: item.id,
						name: item.entityData.properties.name ? item.entityData.properties.name : item.id.toUpperCase(),
...
EntityCollectionEdit.propTypes = propTypes;
