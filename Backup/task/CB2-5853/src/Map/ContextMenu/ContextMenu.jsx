...
import { Video, Cctv, MapMarker, MapMarkerPath, VectorLine, Ferry, Car, Airplane, Account, HomeCity } from "mdi-material-ui";
...
const ContextMenu = ({ map, children, contextMenuOpening, contextMenuClosing, close, replayMap, getInitialPlayBarData }) => {
	...
	const facilities = useSelector((state) => {
		...
		facilityFeeds.map((feed) => {
			Object.assign(feedEntities, getFacilities(state)(state, { feedId: feed.feedId, replayMap, getInitialPlayBarData }))
		});
		return feedEntities;
	}, isEqual);
		...
		<Fragment>
			...
			>
				...
				{React.Children.map(children, (child) =>
					child && child !== null ? React.cloneElement(child, { anchorPosition, closeContextMenu: handleClose }) : null
				)}
