...
//actions
import { unsubscribeFromFeed, startAttachmentStream, startActivityStream, startCamerasInRangeStream, startTrackHistoryStream, startRulesStream, removeSubscriber } from "orion-components/ContextualData/Actions";
import { closeNotification, addCameraToDockMode, setCameraPriority, removeDockedCameraAndState } from "orion-components/Dock/Actions/index.js";
...
const EntityProfile = ({
	...
	const handleInitiateRadioCall = (radioUnitId) => {
		...
		if (radioUnitId) {
			...
			restClient.exec_post("/integration-app/api/externalSystem/zetron/resource/callRadio", dataToPost, function (err, response) {
	...
	const getCustomWidgetConfig = () => {
		const { integrations } = user;
		let customWidgetsArr = [];
		integrations.filter((customWidget) => {
			if (feedId === customWidget.feedId && customWidget.hasOwnProperty('widgets')) {
				const { widgets } = customWidget;
				widgets.forEach((element) => {
					customWidgetsArr.push(element);
				});
			}
		});
	...
	const getWidgetConfig = () => {
		...
		const getCustomWidgets = getCustomWidgetConfig();
		...
		widgetConfig = !marineTrafficVesselData || !marineTrafficVisible
			? widgetConfig.filter(widget => widget.id !== "marineTrafficParticulars")
			: widgetConfig;

		// remove drone-association widget from irrelevant feeds
		if (!(getCustomWidgets.includes("drone-association") && entityType === "track")) {
		...
		return (
			...
			>
				...
				<div className="widgets-container" style={widgetsContainerStyle}>
					...
					{marineTrafficVesselData && getWidgetStatus("marineTrafficParticulars") && (
						<ErrorBoundary>
