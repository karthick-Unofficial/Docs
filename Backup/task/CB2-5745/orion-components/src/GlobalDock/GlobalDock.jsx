...
const GlobalDock = ({
	...
	const { openPanel, closePanel, moveToOtherDock, saveDockState } =
		actionCreators;
	const globalDockState = useSelector(
		(state) =>
			state.appState &&
	...
	useEffect(() => {
		if (!globalDockState) {
			...
			keys(panelsConfig).forEach((panel) => {
				if (
					!globalDockState.leftDock.availablePanels.includes(panel) &&
	...
	useEffect(() => {
		...
		if (
			newCurrentPanel !== null &&
			!newAvailablePanels.includes(newCurrentPanel)
		) {
	...
	const { collapsedPaper, miniPaper, expandedPaper, exclusivePaper } =
		useStyles();
			...
			<div className="dockArea">
				{!exclusiveModePanel && (
					<div className="dockHeader">
						<h5 className="title">
							{panelsConfig[currentPanel].header}
						</h5>
						...
						<Tooltip title="Close" placement="bottom">
							<Close
								className="close"
								onClick={() =>
									dispatch(closePanel(appId, currentPanel))
								}
							/>
						</Tooltip>
					</div>
		...
		>
			<div className="globalDock">
				{currentPanel &&
					dockDirection === "right" &&
					(!exclusiveModePanel ||
						exclusiveModePanel === currentPanel) &&
					renderPanelArea()}
				{!exclusiveModePanel && (
