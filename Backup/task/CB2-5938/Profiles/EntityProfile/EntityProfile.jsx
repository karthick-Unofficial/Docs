	...
	const renderWidgets = (widgetName) => {
		switch (widgetName) {
			...
			case "marineTrafficParticulars":
				return <MarineTrafficParticularsWidget id={widgetName} data={marineTrafficVesselData} />;
			case "cameras":
				return (
					<CamerasWidget
						id={widgetName}
			...
			case "drone-association":
				return (
					<DroneAssociation id={widgetName} key={`${contextId}-drone-association`} contextId={contextId} />
				);
			default:
				return null;
		}
				...
				<div style={{ display: layoutControlsOpen ? "block" : "none" }}>
					<ErrorBoundary>
						<LayoutControls
							...
							profile={entityType === "shapes" ? "shapes" : "entity"}
						/>
					</ErrorBoundary>
				</div>
