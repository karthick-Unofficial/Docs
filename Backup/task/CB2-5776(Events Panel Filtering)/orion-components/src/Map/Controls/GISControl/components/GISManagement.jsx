...
const GISManagement = ({
	...
	const handleChange = (name, field, dynamicSetState) => (event) => {
		const dynamicState = name == "propertiesState" ? setPropertiesState : setAuthenticationState;
	...
	return (
		<Dialog
			confirm={{
				...
				action: handleSave,
				disabled: isEqual(properties, propertiesState) && isEqual(authentication, authenticationState)
			}}
			abort={{
