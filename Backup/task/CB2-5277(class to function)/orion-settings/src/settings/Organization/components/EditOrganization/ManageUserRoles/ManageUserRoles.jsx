...
const handleOrgRoleAppToggle = (orgId, role, roleApps, app, adding, updateRole, updateOrgRoleApps) => {
	const newRoleApps = {};
	...
	updateRole(updatedRole.id, { ...updatedRole, orgId });
};
...
const handleOrgRoleIntToggle = (orgId, role, roleInts, int, adding, updateRole, updateOrgRoleIntegrations) => {
	const newRoleInts = {};
	const updatedRole = { ...role };
	if (adding) {
...
const handlePermissionUpdates = (orgId, role, roleAppsIntegrations, appIntId, option, adding, updateRole, int) => {
	...
	updateRole(newRole.id, { ...newRole, orgId });
};
		...
		<Fragment>
					...
					<Container style={dir == "rtl" ?  {
						...
						<div style={{ width: "50%" }}>
							...
							<div style={{
								...
								<Button onClick={() => openDialog("applyToUsers")} disableTouchRipple={true} style={{ paddingLeft: 0, textTransform: "none", backgroundColor: "transparent" }} color="primary">
									<Translate value="mainContent.manageOrganization.orgSettings.manageUserRoles.applyToUsers"/>								</Button>
								{user.profile.admin && !orgRole.initialRole && (
									<Button onClick={() => openDialog("deleteRole")} disableTouchRipple={true} style={{ paddingRight: 0, textTransform: "none", backgroundColor: "transparent" }} color="primary">
										<Translate value="mainContent.manageOrganization.orgSettings.manageUserRoles.deleteRole"/>	
									</Button>
								)}
							</div>
						</div>
						<Button style={{ height: 45 }} onClick={() => openDialog("createRole")} variant="contained" color="primary">
									...
									.map((app, index) => {
													...
													control={
														<Checkbox
															...
															onChange={(e, checked) => {
																...
																handlePermissionUpdates(org.orgId, orgRole, orgRoleApps, app.appId, option, checked, updateRole);
															}}
														/>
													}
										...
										return (
											<div className="list-item-wrapper" key={app.name}>
												<ListItem
													style={
														dir === "rtl" ? stylesRTL :	styles
													}>
													<ListItemIcon>
														<span
															style={{...(dir === "rtl" ? iconStyles_RTL : iconStyles), ...{ backgroundImage: `url(${appIconSrc})` }}}
															className={"widget-icon"}
														/>
													</ListItemIcon>
													<ListItemText
														style={dir == "rtl" ? listStyles.listItemText_RTL : listStyles.listItemText}
														primary={app.name}
													/>
													{permissionOptions}
													<ListItemSecondaryAction style={dir === "rtl" ? {right: "unset", left: 16} : {}}>
														<Switch
															...
															color="primary"
															onChange={(e, checked) => {
																handleOrgRoleAppToggle(org.orgId, orgRole, orgRoleApps, app, checked, updateRole, updateOrgRoleApps);
															}
															}
														/>
													</ListItemSecondaryAction>
										...
										const permissionOptions = options.map(option => {
											...
											return (
												<FormControlLabel
													...
													style={ dir == "rtl" ? {marginRight: 0, marginLeft: 16, flex: "0 0 170px"} : listStyles.listItemText}
													control={
														<Checkbox
															checked={includesOption ? true : false}
															...
															onChange={(e, checked) => {
																...
																handlePermissionUpdates(org.orgId, orgRole, orgRoleIntegrations, int.feedId, option, checked, updateRole, true);
															}}
														/>
													}
													label={option.charAt(0).toUpperCase() + option.slice(1)}
												...
												<ListItem
													...
													<ListItemSecondaryAction style={dir == "rtl" ? {right: "unset", left: 16} : {}}>
														<Switch
															...
															color="primary"
															onChange={(e, checked) => {
																handleOrgRoleIntToggle(org.orgId, orgRole, orgRoleIntegrations, int, checked, updateRole, updateOrgRoleIntegrations);
															}}
														/>
													</ListItemSecondaryAction>
												</ListItem>
			...
			<Dialog
				...
				confirm={{
					...
					action: () => {
						deleteRole(selectOrgRole, updateChangeUserRoleList, orgRole.id, org.orgId);
					}
				}}
			>
					...
					action: () => {
						Object.keys(changeUserRoleList).forEach(userId => {
							if (changeUserRoleList[userId].original !== changeUserRoleList[userId].value)
								updateUserRole(userId, changeUserRoleList[userId].value ? orgRole.id : `${org.orgId}_org_user`);
						});
						closeDialog("applyToUsers");
					}
				}}
							...
							{filteredMembers.map((user, index) => {
								return (
									<Fragment key={`members-${index}`}>
										<ListItem style={dir == "rtl" ? listStyles.listItemRTL : listStyles.listItem}>
											<ListItemText style={listStyles.listItemText}>
												{user.name}
											</ListItemText>
											<ListItemSecondaryAction style={dir == "rtl" ? {right: "unset", left: 8} : { right: -8 }}>
												<Switch
