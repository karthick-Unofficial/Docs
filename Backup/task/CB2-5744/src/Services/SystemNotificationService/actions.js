			...
			switch (res.type) {
				case "initial": {
					...
					if (!initialBatchReceived) {
						...
						dispatch(
							systemNotificationsReceived(notificationBatch)
						);
					} else {
						notificationBatch.forEach((notification) => {
							...
							if (notification.interrupt) {
								dispatch(
									systemNotificationsReceived([notification])
								);
							}
						});
					}
				...
				default:
					console.log(
						`subscribeToSystemNotifications - Invalid type: ${res.type}`
					);
			}
		});
	};
...
export const acknowledgeSystemNotification = (notificationId) => {
	return (dispatch) => {
		systemNotificationService.acknowledgeSystemNotification(
			notificationId,
			(err, res) => {
				if (err)
					return console.log(
						"acknowledgeSystemNotification - Error:",
						err
					);
				if (!res)
					return console.log(
						`acknowledgeSystemNotification - Error - No response returned for notificationId: ${notificationId}`
					);
				if (res.success === false)
					return console.log(
						`acknowledgeSystemNotification - Unsuccessful: ${res.message}`
					);

				if (res.systemNotificationId) {
					dispatch(
						systemNotificationAcknowledged(res.systemNotificationId)
					);
				} else {
					console.log(
						"acknowledgeSystemNotification - Error: notificationId not found in response"
					);
				}
			}
		);
