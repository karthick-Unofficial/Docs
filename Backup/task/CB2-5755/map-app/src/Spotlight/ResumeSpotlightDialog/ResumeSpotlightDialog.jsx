...
const ResumeSpotlightDialog = () => {
	...
	const dialogOpen = useSelector(
		(state) => state.appState.dialog.openDialog === "resumeSpotlightDialog"
	);
	...
	return (
		<CBDialog
			...
			textContent={getTranslation(
				"spotLight.resumeSpotlightDialog.textContent"
			)}
			abort={{
				label: getTranslation("spotLight.resumeSpotlightDialog.close"),
				action: handleClose
			}}
			dir={dir}
		>
			...
			>
				{spotlights.map((spotlight, index) => {
					const isClickable = !disabledSpotlights.includes(
						spotlight.id
					);
					return (
						<Chip
							key={`spotlight-chip-${spotlight.id}`}
							label={getTranslation(
								"spotLight.resumeSpotlightDialog.spotlight",
								index + 1
							)}
							style={
								dir == "rtl"
