...
const CameraCollection = (props) => {
	...
	const cameras = useSelector((state) =>
		props.id === "all_cameras" ? feedEntitiesByTypeSelector("camera")(state) : getCollectionMember(state, props)
	);
	const collection = useSelector((state) =>
		props.id === "all_cameras" ? props.collection : getCollection(state, props)
	);
	...
	const getCollectionMembers = () => {
		const filteredMembers = pickBy(cameras, (member) => {
			...
			if (entityData) {
				...
				return `${name}|${description}|${type}`.toLowerCase().includes(searchValue);
			} else {
				return false;
			}
		});
	...
	const itemCount = `${size(members).toLocaleString(locale)} ${
		size(members) === 1
			? getTranslation("camerasListPanel.cameraCollection.item")
			: getTranslation("camerasListPanel.cameraCollection.items")
	}`;
	...
	return !size(members) && !!searchValue ? (
		...
		<Fragment>
			<Collection
				...
				filterButton={<MapFilter {...mapFilterActions} id={id || collection.members} items={members} />}
