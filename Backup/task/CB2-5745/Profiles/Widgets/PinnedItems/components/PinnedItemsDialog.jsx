		...
		if (value.length) {
			eventService.queryPinnable(contextId, value, 5, (err, response) => {
				...
				if (err) {
					setError(
						getTranslation(
							"global.profiles.widgets.pinnedItems.pinnedItemsDialog.errorText.errorOcc"
						)
					);
					setQuerying(false);
					return;
				...
				if (response instanceof Array && response.length < 1) {
					setError(
						getTranslation(
							"global.profiles.widgets.pinnedItems.pinnedItemsDialog.errorText.noItems"
						)
					);
					setQuerying(false);
				} else {
		...
		<CBDialog
			...
			confirm={{
				label: getTranslation(
					"global.profiles.widgets.pinnedItems.pinnedItemsDialog.confirm"
				),
			...
			abort={{
				label: getTranslation(
					"global.profiles.widgets.pinnedItems.pinnedItemsDialog.cancel"
				),
		...
		>
			<SearchField
				...
				placeholder={getTranslation(
					"global.profiles.widgets.pinnedItems.pinnedItemsDialog.wantToFind"
				)}
				autoFocus={true}
			/>
			<List>
