...
const GlobalDock = ({
	...
	const { openPanel, closePanel, moveToOtherDock, saveDockState } = actionCreators;
	const globalDockState = useSelector(
		(state) => state.appState && state.appState.persisted && state.appState.persisted.globalDockState
	);
					...
					<div className="dockHeader">
						...
						<Tooltip title={moveToDockTooltip} placement="bottom">
							<MoveToDockIcon
								...
								onClick={() =>
									dispatch(moveToOtherDock(appId, globalDockState, dockDirection, currentPanel))
								}
							/>
						</Tooltip>
	...
	return (
		...
		>
			<div className="globalDock">
				{currentPanel &&
					dockDirection === "right" &&
					(!exclusiveModePanel || exclusiveModePanel === currentPanel) &&
					renderPanelArea()}
				{!exclusiveModePanel && (
					<div className="iconBar">
						{availablePanels.map((panel) => {
							...
							return (
								<div
									key={panel}
									className={`iconContainer ${currentPanel === panel ? "selected" : ""}`}
								>
