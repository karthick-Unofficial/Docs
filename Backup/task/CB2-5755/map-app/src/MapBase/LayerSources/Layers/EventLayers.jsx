...
const EventLayers = ({
	...
	useEffect(() => {
		...
		if (size(eventsToDisplay)) {
			each(eventsToDisplay, (ev) => {
				...
				if (
					newEvent.entityData.properties &&
					!newEvent.entityData.properties.id
				) {
	...
	useEffect(() => {
		...
		if (
			...
			event.entityData.geometry.coordinates
		) {
					...
					{event && event.proximities ? (
						<Fragment>
							<Source
								...
								geoJsonSource={proximitiesSource}
							/>
							<Layer
