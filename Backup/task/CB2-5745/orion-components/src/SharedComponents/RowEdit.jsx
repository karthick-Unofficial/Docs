...
const RowEdit = ({
	...
	useEffect(() => {
		if (row && !adding) {
			_.each(row.data, (item, key) =>
				setState((initialState) => ({ ...initialState, [key]: item }))
			);
		}
	...
	const handleChange = (name) => (event) => {
		let lookupName = "";
		const lookupType = list.columns.find(
		...
		if (
			...
			event &&
			event.target &&
			hasOwn(event.target, "value")
		) {
			lookupName = lookupData[lookupType].find(
	...
	const handleCancel = () => {
		if (_.size(newFiles) > 0) {
			...
			_.each(removals, (file) =>
				handleDeleteFile(file.handle, file.fileId)
			);
		}
	...
	const handleConfirm = () => {
		...
		_.each(list.columns, (column) => {
			...
			if (
				!column.id.includes("_user-completed") &&
				!column.id.includes("_show-time")
			) {
				let value;
				switch (column.type) {
					case "choice":
					...
					case "checkbox":
						{
							...
							if (
								row &&
								`${column.id}_user-completed` in row.data
							) {
								if (!toggled) {
					...
					if (value !== undefined) {
						switch (type) {
							...
							case "date-time": {
								...
								const format = timeFormatPreference
	...
	return dialog === dialogRef ? (
		<Dialog
			...
			paperPropStyles={
				hasNotes ? { height: 650, maxWidth: 750, width: 750 } : {}
			}
			confirm={{
