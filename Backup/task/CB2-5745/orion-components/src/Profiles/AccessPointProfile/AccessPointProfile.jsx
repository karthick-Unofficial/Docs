...
import {
	...
	createCollection,
	updateAccesspoint
} from "../../SharedActions/accessPointProfileActions";
...
const AccessPointProfile = ({
	...
	const sidebarOpen = useSelector(
		(state) => isLoaded && state.appState.dock.dockData.isOpen
	);
	const dockedCameras = useSelector(
		(state) => isLoaded && state.appState.dock.cameraDock.dockedCameras
	);
		...
		DEFAULT_WIDGET_CONFIG = [
			{
				...
				id: "access_control",
				name: getTranslation(
					"global.profiles.accessPointProfile.accessControlWidget.title"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "activities",
				name: getTranslation(
					"global.profiles.accessPointProfile.activities"
				)
			},
			{
				enabled: true,
				id: "cameras",
				name: getTranslation(
					"global.profiles.accessPointProfile.cameras"
				)
			}
		];
		setMounted(true);
	...
	const showFloorPlanOnTargetClick = () => {
		...
		if (
			entityData.displayType === "facility" &&
	...
	if (context) {
		...
		if (
			...
			actions.push({
				name: getTranslation(
					"global.profiles.accessPointProfile.facility"
				),
				nameText: "Facility",
				action: () =>
					window.location.replace(
						`/facilities-app/#/?${entityData.displayTargetId}`
					)
			});
		}
		actions = [
			...
			{
				name: getTranslation(
					"global.profiles.accessPointProfile.pinTo"
				),
				nameText: "Pin To",
		...
		if (canManageAccessPoint) {
			actions = [
				...
				{
					name: getTranslation(
						"global.profiles.accessPointProfile.edit"
					),
					nameText: "Edit",
		...
		return (
			...
			>
				<ErrorBoundary>
					<SummaryWidget
						...
						actions={[
							...
							{
								name: getTranslation(
									"global.profiles.accessPointProfile.hide"
								),
								nameText: "Hide",
								action: () => {
									setHiding(true);
				...
				>
					...
					{getWidgetStatus("access_control") &&
						canControlAccessPoint && (
							<ErrorBoundary>
								<AccessControlWidget
									readOnly={readOnly}
									key={`${contextId}-access_control`}
									order={
										getWidgetStatus("access_control").index
									}
									enabled={
										getWidgetStatus("access_control")
											.enabled
					...
					{!disableLinkedItems && getWidgetStatus("linked_items") && (
						<ErrorBoundary>
							<LinkedItemsWidget
								...
								enabled={
									getWidgetStatus("linked_items").enabled
								}
								dialog={dialog}
								openDialog={openDialog}
					...
					<AccessPointEditDialog
						...
						accessPoint={accessPoint}
						close={() =>
							dispatch(closeDialog("accessPointEditDialog"))
						}
						update={(accessPointId, entityData) =>
							dispatch(
								updateAccesspoint(accessPointId, entityData)
							)
						}
						dir={dir}
					/>
