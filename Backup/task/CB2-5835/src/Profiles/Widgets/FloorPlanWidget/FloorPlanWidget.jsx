...
import { startFloorPlanCameraStream, startFloorPlanAccessPointsStream } from "orion-components/ContextualData/Actions";
...
const SortableItem = SortableElement(({ item, selected, onSelect, dir }) => {
	...
	return (
		<ListItem button onClick={() => onSelect(item)} style={styles.listItem}>
			...
			<ListItemText style={styles.listItemText} primary={item.name} primaryTypographyProps={{ noWrap: true }} />
	...
	return (
		<List>
			{items.map((item, index) => (
				<SortableItem key={index} index={index} item={item} selected={selected} onSelect={onSelect} dir={dir} />
			))}
		</List>
	);
});
...
const FloorPlanWidget = ({
	...
	const subscriptionsFromContext = useSelector((state) => getSelectedContextData(state)(contextId, "subscriptions"));
	const selectedFloor = useSelector((state) => floorPlanSelector(state).selectedFloor || {});
	const subscriptions = subscriptionsFromContext?.floorPlanCameras || null;
	const dir = useSelector((state) => getDir(state));
	const enabled = useSelector((state) => getWidgetState(state)(id, "enabled"));
	const cameras = useSelector((state) => getWidgetState(state)("cameras", "enabled"));
	...
	const facilityFeedId = entity?.feedId || "";
