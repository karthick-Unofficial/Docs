...
import { ArrowBackIos, ArrowForwardIos, Close, Lock, LockOpen, MoreVert, Videocam } from "@mui/icons-material";
		...
		<Card style={{ backgroundColor: "#FFFFFF00", boxShadow: "none" }}>
			...
			>
				<Switch
					...
					checked={expanded}
				/>
				<ListItemIcon className={`camera-icon ${expanded ? "enabled" : "disabled"}`}>
					{cameraIcon}
				</ListItemIcon>
				<ListItemText
					style={{ padding: 0 }}
					primary={
						expanded && loadProfile && !readOnly ? (
							...
							<div style={{ padding: "0px 12px" }}>{position || name}</div>
						)
					}
								...
								{!presetsOpen ? (
									<Fragment>
										<ListItem style={{ paddingLeft: 16, paddingRight: 16, height: 30 }}>
											...
											<ListItemSecondaryAction onClick={handleCloseMenu}>
										...
										{sidebarOpen && dockedCameras.includes(id) ? (
											...
											>
												<ListItemText primary={getTranslation("global.profiles.widgets.robotCams.robotCamDock.removeFromDock")} />
											</ListItem>
										) : (
											<ListItem
												button
											...
											>
												<ListItemText primary={getTranslation("global.profiles.widgets.robotCams.robotCamDock.addToDock")} />
											</ListItem>
										)}
										...
										>
											<ListItemText primary={getTranslation("global.profiles.widgets.robotCams.robotCamDock.presets")} />
											<ListItemSecondaryAction>
												<IconButton edge="end">
													<ArrowForwardIos style={{ color: "#FFF", fontSize: "medium" }} />
												</IconButton>
											</ListItemSecondaryAction>
										</ListItem>
										{(hasCapability("control") && buttonFeatures) && (
											<Fragment>
												<Divider style={{ background: "#626466" }} />
												{/*
									...
									<Fragment>
										...
										>
											<ArrowBackIos style={{ color: "#FFF", fontSize: "medium", marginRight: 10 }} />
											<ListItemText primary={getTranslation("global.profiles.widgets.robotCams.robotCamDock.back")} />
											<ListItemSecondaryAction onClick={handleCloseMenu}>
												<IconButton edge="end" aria-label="close">
													<Close style={{ color: "#FFF" }} />
												</IconButton>
										...
										{presets.length > 0 ? presets.map(preset => (
											...
											<ListItem
												...
												style={{ paddingTop: 8, paddingBottom: 8, paddingLeft: 16, paddingRight: 16 }}
											>
												<ListItemText primary={getTranslation("global.profiles.widgets.robotCams.robotCamDock.noPresetsAvailable")} />
											</ListItem>
										)}
									</Fragment>
								)}
