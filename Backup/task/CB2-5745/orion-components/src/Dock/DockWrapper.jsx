...
} from "../Services/SystemNotificationService/actions";
...
const DockWrapper = (props) => {
	...
	const Notifications = useSelector(
		(state) => state.globalData.notifications
	);
	...
	const hasSysHealthError = useSelector(
		(state) => state.systemHealth.hasHealthError
	);
	const externalSystems = useSelector(
		(state) =>
			(state.session &&
				state.session.organization &&
				state.session.organization.externalSystems) ||
			[]
	);
	...
	const openNotifications = useMemo(
		() => notifications.filter((item) => !item.closed),
		[notifications]
	);
			...
			return {
				...
				confirm: {
					label: getTranslation(
						"global.dock.dockWrapper.acknowledge"
					),
					action: () => {
				...
				>
					<Dialog
						...
						title={getTranslation(
							"global.dock.dockWrapper.sessionExpired"
						)}
						textContent={getTranslation(
							"global.dock.dockWrapper.redirectLogin"
						)}
						confirm={{
					...
					<Dialog
						...
						title={getTranslation(
							"global.dock.dockWrapper.userAgreement"
						)}
						confirm={{
							label: getTranslation(
								"global.dock.dockWrapper.accept"
							),
							action: handleConfirmFirstUse
						}}
						abort={{
							label: getTranslation(
								"global.dock.dockWrapper.decline"
							),
							action: () => dispatch(logOut())
						}}
						dir={dir}
					...
					{(alerts.length > 0 || hasSysHealthError) && (
						<div className="alert-bar-overlay" />
					)}
					{alerts.length > 0 && (
						<a
						...
						{componentState.newAlerts.length > 0 && (
							<div key={componentState.newAlerts[0].id}>
								<NewNotificationItem
									...
									isLonely={
										componentState.newAlerts.length === 1
									}
