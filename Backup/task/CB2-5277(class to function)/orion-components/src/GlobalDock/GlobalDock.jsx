...
const GlobalDock = ({
	...
	useEffect(() => {
		if (!globalDockState) {
			...
			const availablePanels = _.keys(panelsConfig);
			const newState = {
				leftDock: {
	...
	useEffect(() => {
		...
		if (!_.isEqual(availablePanels, newAvailablePanels)) {
		...
		if (newCurrentPanel !== null && !newAvailablePanels.includes(newCurrentPanel)) {
			closePanel(appId, newCurrentPanel);
				...
				{!exclusiveModePanel && 
					<div className="dockHeader">
						...
						<Tooltip title={moveToDockTooltip} placement="bottom">
							<MoveToDockIcon className="moveToDockIcon" onClick={() => moveToOtherDock(appId, globalDockState, dockDirection, currentPanel)} />
						</Tooltip>
						<Tooltip title="Close" placement="bottom">
							<Close className="close" onClick={() => closePanel(appId, currentPanel)} />
						</Tooltip>
					</div>
				}
