	...
	const getSearchResults = () => {
		if (searchTerms === "") {
			...
			return filter(entities, (entity) => {
				const properties = entity.entityData
				...
				properties.profileIconTemplate =
					profileIconTemplates[entity.feedId];
				...
				<div>
					<Typography variant="subtitle1" style={{ color: "#FFF" }}>
						{searchResults.length === 1 ? (
							<Translate
								value="listPanel.searchResults.matchingResult"
								count={searchResults.length}
							/>
						) : (
							<Translate
								value="listPanel.searchResults.matchingResults"
								count={searchResults.length}
							/>
						)}
					</Typography>
						...
						{searchResults.length > 10 ? (
							...
							searchResults.map((item) => {
								...
								const properties = entity.entityData
									? entity.entityData.properties
									: entity;
								const { id, name, type, subtype, entityType } =
									properties;
								return item ? (
									<CollectionItem
										key={id}
										...
										secondaryText={
											subtype
												...
												? entityType[0].toUpperCase() +
												  entityType.slice(1)
												: getTranslation(
														"listPanel.searchResults.unknown"
												  )
										}
										type={
											subtype
