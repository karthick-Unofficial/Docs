...
const ProfileView = ({
	...
	const aInds = filter(users, individual =>
		includes(audience_individuals, individual.id)
	);
	const aDists = filter(districts, district =>
		includes(audience_districts, district.id)
	);
	const aDepts = filter(departments, department =>
		includes(audience_departments, department.id)
	);
	const aGrps = filter(groups, group =>
		...
		<Fragment>
			<Grid container justify="space-between" spacing={16}>
				...
				<Grid item sm={12}>
					<NestedList
						items={pOContact}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.pointsOfContact", size(pOContact))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
			...
			<Grid container justify="space-between" spacing={16}>
				<Grid item sm={12}>
					<NestedList
						items={aGrps}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.groups", size(aGrps))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
				...
				<Grid item sm={12}>
					<NestedList
						items={aDists}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.districts", size(aDists))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
				...
				<Grid item sm={12}>
					<NestedList
						items={aDepts}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.departments", size(aDepts))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
				...
				<Grid item sm={12}>
					<NestedList
						items={aInds}
						header={getTranslation("global.profiles.widgets.shieldGroup.profileView.individuals", size(aInds))}
						dense={true}
						inset={false}
						headerStyle={{
							variant: "caption",
