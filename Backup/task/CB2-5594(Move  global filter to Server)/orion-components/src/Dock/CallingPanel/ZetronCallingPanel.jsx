...
const ZetronCallingPanel = () => {
	...
	const handleInitiateCall = (operationType, numberOrId) => {
		...
		if (contiueExecution && numberOrId) {
			...
			restClient.exec_post(`/integration-app/api/externalSystem/zetron/resource/${operationType}`, dataToPost, function (err, response) {
	...
	return (
		<div style={zetronStyles.zetronContainer}>
			...
			<div style={zetronStyles.statusContainer}>
				<div className="b2-bright-gray" style={zetronStyles.statusContainerResultStatus} >{statusInfo.message}</div>
			...
			{(statusInfo.errorMessage.length > 2) && <div style={zetronStyles.statusInfoErrorMessage}>{statusInfo.errorMessage}</div>}
						...
						return (
							...
							>
								...
								<div style={{ alignSelf: "center", marginTop: -4, marginLeft: (dir === "rtl" ? 5 : 2), visibility: `${(callButtonVisibleId === contactIndex) ? "visible" : "hidden"}` }}>
									<Button variant="contained" color="primary" startIcon={<Phone style={zetronStyles.phone} />}
