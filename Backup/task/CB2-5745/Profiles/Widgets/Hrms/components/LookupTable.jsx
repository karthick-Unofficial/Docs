...
const LookupTable = ({
	...
	useEffect(() => {
		if (lookupType === "resources") {
			setTableHeader([
				...
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.shiftEnding"
					)
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.location"
					)
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.unit"
					)
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.rank"
					)
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.name"
					)
				}
			]);
		} else {
			setTableHeader([
				...
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.category"
					)
				},
				{
					header: getTranslation(
						"global.profiles.widgets.hrms.lookupTable.count"
					)
				},
				{
					header: getTranslation(
		...
		if (assigned && !available) {
			const data = widgetData.filter((element) => {
	...
	const actions = () => {
		...
		return (
			<div
				style={{ marginTop: "5%", float: "right", marginBottom: "5%" }}
			>
				<Button onClick={closePopup}>
					<Translate value="global.profiles.widgets.hrms.lookupTable.cancel" />
				</Button>
				<Button style={style.lookupTableDone} onClick={handleSave}>
