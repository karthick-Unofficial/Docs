...
const CameraDockModule = ({
	...
	const renderCameraDisplay = (cameraReplaceMode, newCamera) => {
		...
		if (newCamera) {
			...
			if (newCamera.entityData.properties.name.toLowerCase().includes("camera")) {
				cameraTextFix = "";
			}
		}
				...
				<div style={{ position: "relative" }}>
					{cameraReplaceMode.replaceMode && (
						...
						>
							{" "}
							{cameraName ? (
								<Translate value="global.dock.cameras.camDockModule.replaceWith" count={cameraName} />
							) : (
								""
							)}{" "}
						</p>
	...
	const cameraDisplay = renderCameraDisplay(cameraReplaceMode, cameraReplaceMode.camera);
