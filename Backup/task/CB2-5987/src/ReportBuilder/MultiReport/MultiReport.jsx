...
// Material Ui
import { Table, TableBody, TableHead, TableRow, TableCell, Paper, CircularProgress, Button } from "@mui/material";
import { FileDownload } from "@mui/icons-material";
import { getTranslation } from "orion-components/i18n";
import { useSelector } from "react-redux";
import { getDir } from "orion-components/i18n/Config/selector";

const MultiReport = ({ report, handlePDFExport, eventName, time }) => {
	const reportData = useSelector((state) =>
		state.appState.reportViewer.reports.filter((reportItem) => {
			...
			<TableRow selectable={false} key={index}>
				{columns.map((column, index) => {
					...
					const td = (
						...
						>
							{prop === "email" ? (
								<a style={{ textDecoration: "none" }} href={`mailto:${row[prop]}`}>
									{row[prop]}
								</a>
							) : prop === "time" ? (
								time
				...
				switch (component.type) {
					...
					case "section": {
						if (data[component.property] && data[component.property].value)
							sections.push(
								<MultiReportSection
									key={"multi-report-section-" + index}
									value={data[component.property].value}
									template={component}
									timeFormatPreference={timeFormatPreference}
								/>
						...
						if (data[component.property] && data[component.property].value)
							longFields.push(
								<MultiReportLongField
									key={"multi-report-long-field-" + index}
									value={data[component.property].value}
									template={component}
								/>
							);
					...
					case "table": {
						if (
							...
							(data[component.property].value.rows || data[component.property].value)
						) {
							if (component.property === "lists") {
								...
								if (groups.rows) {
									...
									tables.push(
										<div
											className="mr-table-container"
											key={"mr-table-container-" + index}
										...
										>
											...
											>
												{getTranslation(`mainpage.list.${component.header}`)}
		...
		display = data.map((entity, index) => {
			return (
				<div key={`table-${index}`}>
					...
					<div className="mr-tables">{componentArray[index].tables}</div>
				</div>
			);
		});
	}
			...
			{!report.disablePDF && (
				<div className="mb5" style={{ textAlign: "right", marginBottom: 35 }}>
					{!isGeneratingFile ? (
						...
						>
							<FileDownload />
							{getTranslation("reportBuilder.components.multiReport.downloadPdf")}
						</Button>
					) : (
						<div
							style={{
