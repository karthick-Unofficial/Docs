...
export const setWidgetOrder = (appId, profile, widgets) => {
	return (dispatch) => {
		...
		if (appId === "cameras-app") {
			dispatch(
				updatePersistedState(appId, "profileWidgetOrder", keyVal, false)
			);
		} else {
...
export const addRemoveFromCollections = (
	entityId,
	added,
	removed,
	entityName,
			...
			(err, response) => {
				if (err) {
					...
					if (!undoing) {
						...
						let completeMessage = addedMessage
							? entityName + " added to " + addedMessage + "."
							: "";
						completeMessage += removedMessage
							? entityName +
							  " removed from " +
							  removedMessage +
							  "."
	...
	return function (dispatch, getState) {
		...
		entityCollection.createCollection(name, members, (err, response) => {
			if (err) {
				...
				if (
					(state.appId && state.appId === "events-app") ||
					state.appId === "map-app"
				) {
