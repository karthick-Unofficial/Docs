...
const NotificationsTab = (props) => {
	...
	const handleDismissClick = () => {
		...
		setTimeout(() => dispatch(closeBulkNotifications(activeNotifications.map((n) => n.id))), 400);
		setAnimating(true);
	};

	const handleRestoreClick = () => {
		const inactiveNotifications = notifications.filter((n) => n.closed);
		setAnimating(true);
		setTimeout(() => {
	...
	const handleArchiveClick = () => {
		...
		if (!swapped) {
			...
			setFlip("half");
			const loadTimeout = setTimeout(() => setShouldDisplayLoading(true), 300);
	...
	return (
		<div id="notification-tab-wrapper" className={"cf" + (componentState.hasError ? " adjusted" : "")}>
			<div>
				{componentState.hasError && (
					<div className="error-message-banner">
						<p>
							<Translate value="global.dock.notifications.main.errorOcc" />
						</p>
					</div>
				)}
				<div
				...
				>
					{archive ? (
						<Button color="primary" style={{ textTransform: "none" }} onClick={handleArchiveClick}>
						...
						<Fragment>
							...
							{activeNotifications.length > 0 && !readOnly && (
								<Button color="primary" style={{ textTransform: "none" }} onClick={handleDismissClick}>
			...
			<div
				className={
					"flip-container" +
					(archive && flip === "half" ? " half-flip" : archive && flip === "full" ? " flip" : "")
				}
			>
				<div className="flipper">
					<div className="front">
