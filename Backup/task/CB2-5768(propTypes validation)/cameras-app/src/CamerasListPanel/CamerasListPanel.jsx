...
const CamerasListPanel = () => {
	...
	const profileLoaded = useSelector(
		(state) => !state.appState.loading.profileLoading && !!selectedContextSelector(state)
	);
	...
	useEffect(() => {
		if (widgetLaunchData && mounted) {
			const { entityId, widget } = widgetLaunchData;
			if (entityId) {
				dispatch(loadProfile(entityId, entityId, "camera", "profile", "primary"));
	...
	}, [widgetLaunchData, profileLoaded, cameras, dispatch, mounted]);
