...
export const removeData = (id, feedId, batch, deleting = true) => {
	return (dispatch, getState) => {
		...
		if (deleting && selectedEntity && selectedEntity.id === id)
		...
		feedService
			.subscribeGlobalFeed(feedId, (err, response) => {
				...
				if (response.type) {
					...
					if (removals.length) {
						each(removals, (id) =>
							dispatch(removeData(id, feedId, batch))
						);
					}
					if (additions.length) {
			...
			.then((subscription) => {
				...
				dispatch(
					setDataSubscription(subscription.channel, feedId, "all")
				);
			});
	};
};
...
export const ignoreEntity = (entityId, entityType, feedId, appData) => {
	return (dispatch, getState) => {
		authExclusionService.ignoreEntity(
			entityId,
			entityType,
			feedId,
			(err, res) => {
					...
					if (res.result.inserted === 1) {
						...
						if (isGlobal) {
							...
							availableReducers.forEach((batch) => {
								if (feedId === "event") {
									...
									dispatch(
										removeData(entityId, feedId, batch)
									);
								}
							});
						}
						// If the data is not stored in global state, update it where it is stored per app
							...
							switch (app) {
								...
								default:
									console.log(
										`No case for ${app} exists when hiding entity`
									);
									break;
							}
						}
