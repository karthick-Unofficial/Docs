...
const MapWrapper = () => {
	...
	const settings = useSelector(state => !_.isEmpty(appState.persisted) && mapSettingsSelector(state));
	const zoom = !_.isEmpty(appState.persisted) ? settings.mapZoom : mapSettings.zoom;
	const center = !_.isEmpty(appState.persisted) ? settings.mapCenter : mapSettings.center;
	const style = !_.isEmpty(appState.persisted) ? settings.mapStyle : "satellite";
...
const MapBaseMapboxGL = memo(props => {
	...
	if (prevProps.mapRef && prevProps.WavCamOpen === nextProps.WavCamOpen) {
		if (!_.isEqual(prevProps.style, nextProps.style)) {
			return false;
		}
		return true;
	} else {
