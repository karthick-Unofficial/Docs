...
const ObjectiveEditor = ({map, data, modificationMode, createModification, updateModification, onCancelObjective, onSaveObjective, raiseError, floorPlans, dir}) => {
		...
		return(
			<div style={{flex:1, marginLeft: 10, marginRight: 10}}>
				<MapObjectSelectorContainer id="result" definitionDesc={""}
					...
					required={true}
					headerText={<Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.chooseLocation"/>}
	...
	return (
		<div>
			<div className="modificationflex">
				{
					(!isInEditMode()) && <Typography variant="body1" className="b1-white" style={{margin:10, alignSelf:"center", flex:1, fontSize:18}}>{<Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.addObj"/>}</Typography>
				}
				{
					(isInEditMode()) && <Typography variant="body1" className="b1-white" style={{margin:10, alignSelf:"center", flex:1, fontSize:18}}>{<Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.editObj"/>}</Typography>
				}

				<div className="modificationActionsRight">
					<Button style={dir == "rtl" ? { width: 74, marginRight:10, marginLeft:0} : { width: 74, marginLeft:10, marginRight:0}} onClick={handleClose}><Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.cancel"/></Button>
					<Button variant="contained" color="primary" disabled={disableSaveButton()} 
						style={{ width: 74, marginLeft:10, marginRight:10 }} onClick={saveObjective}><Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.save"/></Button>
				</div>
			</div>
			<div className="modificationflex">
				<div style={{flex:1, minWidth: 320, marginLeft: 10, marginRight: 10}}>
					<TextField 
						...
						label={<Translate value="tableopSession.panels.modifications.modificationEditor.objectiveEditor.objName"/>}
						value={objectiveName}
