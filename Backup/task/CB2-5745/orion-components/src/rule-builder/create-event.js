...
const createEventBuilder = (rule, collections) => {
	return (
...
const _getMain = (rule) => {
	...
	if (rule.eventType) {
		...
		const selectedEventType = rule.eventTypes.filter(
			(et) => et.eventTypeId === rule.eventType
		)[0];
		if (selectedEventType) {
			if (rule.eventSubtype) {
				// -- add event subtype if present
				const selectedEventSubtype = selectedEventType.subtypes.filter(
					(st) => st.id === rule.eventSubtype
				)[0];
			...
			eventTypeString = `${
				VOWELS.includes(eventTypeString.toLowerCase().charAt(0))
					? "an"
					: "a"
			} ${eventTypeString}`;
		}
	...
	if (rule.template) {
		...
		const selectedTemplate = rule.availableTemplates.filter(
			(template) => template.id === rule.template
		)[0];
		if (selectedTemplate) {
			templateString = getTranslation(
				"global.ruleBuilder.createEvent.templateString",
