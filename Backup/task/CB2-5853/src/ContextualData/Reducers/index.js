...
const contextualData = (state = initialContextualData, action) => {
	...
	switch (type) {
			...
			if (newContext.trackHistory) {
				...
				if (trackHistory[trackHistory.length - 1]) {
					...
					lastKnownPosition = JSON.stringify(
						trackHistory[trackHistory.length - 1].entityData.geometry.coordinates
					);
				} else {
		...
		case "SET_SUBSCRIPTION":
			{
				const { contextId, subscription, subscriptionId, subscriberRef } = payload;
				...
				if (!context || _.isEmpty(context)) {
					...
					return {
						...
						[contextId]: {
							...
							[subscriptionId]: context[subscriptionId] ? context[subscriptionId] : [],
							subscriptions: {
								...context.subscriptions,
								[subscriptionId]: {
									subscription: subscription,
		...
		case "ADD_SUBSCRIBER": {
			...
			const newSubscribers = [...context.subscriptions[subscriptionId].subscribers, subscriberRef];
