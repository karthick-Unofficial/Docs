...
const AccessPointWrapper = ({
	...
	const accessPoints =
		facility && facility.floorPlanAccessPoints
			? facility.floorPlanAccessPoints
			: [];
	const map = mapRef;
	...
	Object.values(userFeedsSelector).forEach((feed) => {
		if (feed.entityType === "accessPoint") {
			profileIconTemplates[feed.feedId] =
				feed.mapIconTemplate && jsonata(feed.mapIconTemplate);
		}
	});
...
const AccessPointLayer = ({
	...
	useEffect(() => {
		const filtered = {};
		if (
			!Object.values(accessPoints).length ||
			Object.values(accessPoints).length === 0
		) {
			...
			Object.keys(accessPoints).forEach((accessPoint) => {
				if (profileIconTemplates[accessPoints[accessPoint].feedId]) {
					accessPoints[accessPoint].entityData.properties.mapIcon =
						profileIconTemplates[
							accessPoints[accessPoint].feedId
						].evaluate(accessPoints[accessPoint].entityData);
				}

				if (
					!filters ||
					isEmpty(filters) ||
					includes(filters, accessPoint)
				) {
