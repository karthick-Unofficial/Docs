...
import * as actions from "./setInitialActions";
...
describe("SetInitial Async Actions.", () => {
	...
	describe("setInitialPassword", () => {
		...
		it("Should make a single put request.", () => {
			...
			store.dispatch(actions.setInitialPassword("newPass", "123"));
		...
		it("Should point to the correct endpoint.", () => {
			...
			store.dispatch(actions.setInitialPassword("newPass", "123"));
			return flushAllPromises().then(() => {
				expect(checkMockedUrl("/initial/123")).toBe(true);
			});
		});
	});
