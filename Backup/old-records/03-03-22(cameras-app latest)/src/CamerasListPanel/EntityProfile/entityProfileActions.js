...
export const attachmentDeleteSuccess = id => {
	return {
		type: t.ENTITY_ATTACHMENT_DELETE_SUCCESS,
		id
	};
};
...
export const attachFilesToEntity = (entityId, entityType, files) => {
	return dispatch => {
		attachmentService.uploadFiles(entityId, entityType, files, (err, result) => {
			if (err) {
				console.log(err);
			}
			console.log(result);
...
export const startCameraInRangeStream = (cameraId, videoProfile) => {
	return dispatch => {
		cameraService
			.streamVideo(cameraId, videoProfile, (err, response) => {
				if (err) {
					...
					dispatch(cameraInRangeVideoReceived(response, cameraId));
				}
			})
			.then(sub => {
				dispatch(cameraInRangeStreamReceived(sub));
			});
	};
};
...
export const streamCamerasContainingEntityInFov = (entity) => {
	return dispatch => {
		cameraService
			.streamCamerasInRange(entity.id, entity.entityType, (err, response) => {
				if (err) {
					console.log(err);
				} else {
					if (response.add) {
						dispatch(camerasInRangeReceived(response.add));
					} else if (response.remove) {
						dispatch(camerasInRangeRemoved(response.remove));
					}
				}
			}).then(sub => 
				dispatch(
					setCamerasInRangeSubscription({ entityId: entity.id, subscription: sub })
				)
			);
	};
};
