...
import { List, ListItem, FormControlLabel, Checkbox, CircularProgress } from "@mui/material";
...
import { linkEntities } from "orion-components/SharedActions/accessPointProfileActions";
...
const handleSubmit = (entity, linkType, linkUpdates, linkedTo, dispatch) => {
	// Filter out memberships from added collections
	const linkedEntities = linkUpdates.added.filter((value) => {
...
const LinkDialog = ({ dialog, title, closeDialog, entity, autoFocus, dir }) => {
		...
		if (query.length) {
			const serviceCall =
				entity.entityType === "camera"
					? entityStore.getEntitiesForLinking.bind(entityStore)
					: cameraService.getCamerasForLinking.bind(cameraService);
			if (serviceCall) {
				serviceCall(entity.id, entity.entityType, query, 5, (err, res) => {
					...
					if (err) {
						console.log(err);
						setSearch({
							error: getTranslation("global.sharedComponents.linkDialog.errorText.errorOccurred"),
							querying: false
						});
					}
					if (!res || !res.success) {
						...
						setSearch({
							error: getTranslation("global.sharedComponents.linkDialog.errorText.noItemsFound"),
							querying: false
						});
					} else {
						const linkedTo = map(
							filter(res.result, (camera) => camera.linkedWith),
							(camera) => {
								return {
									id: camera.id,
									entityType: "camera"
						...
						const unlinkedEntities = res.result.filter((entity) => {
							const linkedIds = linkedTo.map((link) => link.id);
	...
	const sortedEntities = entities.sort((a, b) => {
		if (
			a.entityData &&
			b.entityData &&
			a.entityData.properties &&
			b.entityData.properties &&
			a.entityData.properties.name &&
			b.entityData.properties.name
		) {
	...
	return (
		<Dialog
			...
			confirm={{
				...
				action: () => {
					handleSubmit(entity, "manually-assigned-camera", linkUpdates, linkedTo, dispatch);
					setLinkUpdates({ added: [] });
					setEntities([]);
					setLinkedTo([]);
					setSearch({ querying: false, error: null });
			...
			abort={{
				label: getTranslation("global.sharedComponents.linkDialog.cancel"),
				action: () => {
					setLinkUpdates({ added: [] });
					setEntities([]);
					setLinkedTo([]);
						...
						sortedEntities.map((entity) => {
							const checkbox = (
								<FormControlLabel
									...
									label={
										<div style={listStyles.primaryText}>{entity.entityData.properties.name}</div>
									}
									sx={{
										height: "48px",
										width: "100%",
							...
							return (
								<ListItem key={entity.id} style={listStyles.item}>
									{checkbox}
								</ListItem>
							);
						})
