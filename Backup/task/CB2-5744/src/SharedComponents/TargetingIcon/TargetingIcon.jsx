...
const TargetingIcon = (props) => {
	...
	const filters = useSelector((state) =>
		mapAppState ? mapFiltersById(state) : null
	);
	const disabledFeeds = useSelector((state) => disabledFeedsSelector(state));

	let geometry = useSelector((state) =>
		state.replayMapState ? null : props.geometry
	);
	//Get entity
	const entity = useSelector((state) =>
		mapAppState && mapAppState.entities[props.feedId]
	...
	if (
		entity &&
		entity.entityData &&
		["map", "facility"].includes(
			(entity.entityData.displayType || "map").toLowerCase()
		)
	) {
	...
	const isDisabled = !entity
		...
		? disabledFeeds.includes(
				entity.entityData.properties.type === "LineString"
					? "Line"
	...
	const shouldRender =
		...
		mapVisible &&
		geometry &&
		(!filters || !filters.length || (multipleTargets && targetIds.length > 0 ? targetIds.some(key => filters.includes(key)) : filters.includes(id)));
