...
import { mapSettingsSelector, persistedState, disabledFeedsSelector } from "orion-components/AppState/Selectors";
...
const MayLayerWrapper = (props) => {
	...
	const feed = _.find(userFeedData, (feed) => feed.feedId === props.feedId);
	...
	const timeData = useSelector(
		(state) => getInitialPlayBarData && getInitialPlayBarData(playBarValue, state.replay.timeTransactions)
	);
		...
		if (replayMap) {
			if (timeData) {
				...
				for (const key in timeData) {
					if (
						(props.feedId === "fovs" && timeData[key].entityData.properties.type === "FOV") ||
...
const MapLayer = memo(
	({
		...
		function renderLayers() {
			...
			const layers = _.map(
				...
				(source, index) => {
					...
					return (
						<Fragment key={index}>
							{feedEntType.toLowerCase() === "track" && renderSilhouettes(index, source, layerBefore)}
							<GeoJSONLayer
								key={feedId + index}
								...
								before={layerBefore}
							/>
							{source.type === "Point" && renderAlerts(layerBefore)}
		...
		function renderSilhouettes(index, source, before) {
			const vessels = {};
			...
			return (
				<Fragment key={feedId + index + "-silhouettes"}>
					<VesselPolygons vessels={vessels} before={before} />
				</Fragment>
			);
		}
		...
		function renderAlerts(layerBefore) {
			if (map && map.getSource(feedId + "Point") && !map.getLayer(feedId + "-alert-badge")) {
				map.addLayer({
...
export default MayLayerWrapper;
