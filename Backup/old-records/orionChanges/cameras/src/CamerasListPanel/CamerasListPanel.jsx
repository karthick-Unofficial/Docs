...
class CamerasListPanel extends PureComponent {
	constructor(props) {
		super(props);
		this.state = {};
	}
	...
	componentDidUpdate(prevProps, prevState) {
		const { widgetLaunchData, cameras, loadProfile, profileLoaded, setWidgetLaunchData, selectWidget } = this.props;
		...
		if (widgetLaunchData) {
			...
			if (entityId) {	
				...
				else if (Object.keys(cameras).length > 0 && cameras === prevProps.cameras) {
				...
				let selectedWidget = "Map Location and FOV";
					...
					<div id="cameras-list-wrapper" style={{
						...
						<div id="cameras-list" style={{
							...
							<SearchField
								...
								placeholder={this.placeholderConverter("camerasListPanel.placeholder.searchColl")}
								value={searchValue}
								filters={filterCount}
										...
										<List>
											<ErrorBoundary>
												<CameraCollectionContainer
													collection={{
														id: "all_cameras",
														name: this.placeholderConverter("camerasListPanel.allCams")
													}}
													id="all_cameras"
...
export default CamerasListPanel;
