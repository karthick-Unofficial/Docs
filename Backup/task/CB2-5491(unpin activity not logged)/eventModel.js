...
const ACTIVITY_TABLE = "sys_activity";
	...
	try {
		...
		const filter = r
			...
			.and(
				...
				r.row("ownerOrg").eq(orgId).default(false)
					.or(r.row("sharedWith").default([]).contains(orgId))
			);
...
EventModel.prototype.activeWithData = async function (includePinned = false) {
	try {
		let q = r.table(EVENT_TABLE)
			...
			.merge((event) => {
				return r.table(EXTERNAL_ENTITY_MAPPING_TABLE)
					.filter({
						targetId: event("id")
					})(0)
					.default({})
	...
	try {
		const filter = r
			.and(
				...
				r.or(
					r.row("ownerOrg").eq(orgId).default(false),
					r.row("sharedWith").default([]).contains(orgId)
				)
			);
...
EventModel.prototype.getByEventType = async function (
	userId,
	eventType,
	subtype
) {
	try {
		...
		const filter = r
			...
			.and(
				...
				r.branch(
					r.expr(checkSubType).ne(null),
					r.row("subtype").eq(checkSubType),
					{}
				)
			);
...
EventModel.prototype.getByIdWithAllData = async function (userId, eventId) {
	try {
		const [event, activities, attachments, pinnedItems, lists, cameras] = await Promise.all([
			...
			activityModel.getAllEntityActivities(userId, eventId, "event"),
			attachmentModel.getByTargetId(eventId),
			getPinnedItems(eventId, userId),
			r.table(LIST_TABLE).filter(r.row("targetId").eq(eventId).and(r.row("deleted").eq(false))).run(),
...
const _internalGetAllPinnedItemsBtEntityType = async function (entityType) {

	const pinnedEntities = await r
		.table(EVENT_ENTITIES_TABLE)
		.filter(r.row("entityType").eq(entityType));
...
const getPinnedItems = async function (eventId, userId) {
	const pinnedItems = [];
	const pinnedIDs = await r
		.table(EVENT_ENTITIES_TABLE)
		.filter(r.row("eventId").eq(eventId));

	for (let i = 0; i < pinnedIDs.length; i++) {
		const item = await feedModel.getEntityWithAuthorizationUseFeedId(userId, pinnedIDs[i].entityId, pinnedIDs[i].feedId);
		if (item) {
...
const _internalGetPinnedEntities = async function (eventId) {
	...
	for (let i = 0; i < Object.keys(pinnedIdsByFeedId).length; i++) {
		...
		const thisFeedsEnts = await feedModel.getFeedEntitiesInternal(feedId, entityId);
		pinnedItems = [...pinnedItems, ...thisFeedsEnts];
	}
...
EventModel.prototype.getByExternalSourceId = async function (userId, sourceId) {
	try {
		const result = await externalEntityMappingModel.getBySourceId(
			sourceId,
			"event"
		);
		if (result && result.targetId) {
			...
			logger.error("getByExternalSourceId", "There was an error while getting an external source by id", {
				err: { message: "No existing external mapping model found with sourceId: " + sourceId }
			});
			return null;
		}
	} catch (error) {
...
EventModel.prototype.getAllForPinning = async (userId, orgId, entityId) => {
	try {
		...
		// get all open events
		const activeEvents = await feedModel.getEntitiesByTypeWithAuthorization(userId, "event", true, null, true, filter);
 ...
 * @param event
 */
EventModel.prototype.create = async function (userId, orgId, event, translations) {
	try {

		// -- todo: ENTITY AUTH retrieve app Id when registering with app-gateway so not hardcoded app id
		if (!userPolicyCache.authorizeApplication(userId, "events-app", userPolicyCache.appPermissionTypes.manage)) {
			throw { message: "User is not authorized to create an event", code: 403 };
		}
		...
		if (additionalProperties) {
			...
			try {
				const result = await externalEntityMappingModel.upsert(
					null,
					null,
					newMapping);
		...
		const generatedObject = activityModel.generateObject(
			"event",
		...
		if (templatePinnedItems.length > 0) {
			try {
				...
				logger.error("pinEntites", "There was an issue pinning activity entities to escalated event.", {
 ...
 */
EventModel.prototype.escalateEvent = async function (userId, id, translations) {
	try {
		...
		if (notification.escalationEvent) {
			...
			if (templateAttachments.length > 0) {
				await attachmentModel.copyAttachment(userId, newEvent.id, notification.escalationEvent, newEvent.entityType, false);
			}
 ...
 * @param eventId
 */
EventModel.prototype.enableEvent = async function (userId, eventId, translations) {
	try {
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.manage);
		if (!event) {
		...
		const oldAdditionalProperties = externalEntityMapping
			? externalEntityMapping.additionalProperties
			: "";
 ...
 * @param update
 */
EventModel.prototype.update = async function (
	userId,
	eventId,
	update,
	translations
	...
	try {
		...
		const canManage = await feedModel.authorizeEntity(userId, eventId, "event", userPolicyCache.feedPermissionTypes.manage);

		let permittedProps = [];
		// only allow through properties that are allowed to be changed through this method
		// use redundancy to dissallow updating on properties user isn't allowed to update
		if (canManage) {
			permittedProps = [
				"desc",
				"endDate",
		...
		if (op.endDate === null) {
			...
			await r.table(EVENT_TABLE)
				.get(eventId)
				.replace(r.row.without("endDate"))
				.run();

			// Check hidden items, updating entries to allow icon change from Planned to Emergent
			await r.table(AUTH_EXCLUSION_TABLE)
				.filter({ entityId: eventId })
				.update({ "iconType": "Emergent" })
				.run();
		}
		...
		const new_valAdditionalProperties = newAdditionalProperties
			...
			: oldAdditionalProperties ? oldAdditionalProperties : "";
		const old_valAdditionalProperties = oldAdditionalProperties
...
EventModel.prototype.setActiveStatus = async function (eventId, isActive) {
	try {
		...
		const result = await r.table(EVENT_TABLE)
			.get(eventId)
		...
		const additionalProperties = await r.table(EXTERNAL_ENTITY_MAPPING_TABLE)
			.filter({
				targetId: eventId
			})(0)
			.default({})
		...
		const pinnedEnts = await this.getEventFeedCacheEntities([eventId]);
		result.changes[0].new_val["pinChanges"] = { "pinned": pinnedEnts[eventId] };
		...
		const eventShapeIds = await r.table(EVENT_ENTITIES_TABLE)
			.filter({
				eventId: eventId,
				entityType: "shapes"
			})
...
EventModel.prototype.getShapeUsageSummary = async function (shape) {
	try {
		const activeEvents = await r.table(EVENT_ENTITIES_TABLE)
			.filter({ entityId: shape.id })
			.merge((eventEntity) => {
				return r.table(EVENT_TABLE)
					.filter({
		...
		activeEvents.forEach(activeEvent => {
			if (activeEvent.isTemplate)
				usageSummary.templateShape = true;
			else
				usageSummary.inUseByActiveEvent = true;
		});
			...
			eventShapes.forEach(async (eventShape) => {
				if (eventShape.entityData.properties.displayOnEventActive) {
					...
					if (!event.isTemplate && event.template) {
						...
						if (templateShape)
							deleteShape = false; // only change shape scope
					}

					if (deleteShape)
						shapeModel.delete(userId, eventShape.id);
					else
						shapeModel.setScope(eventShape.id, false);
				}
			});
		}
 ...
 * @param entities
 */
EventModel.prototype.pinEntities = async function (
	userId,
	eventId,
	entities,
	translations
	...
	try {
		...
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.contribute);
		if (!event) {
			throw { message: "User is not authorized to access this event", code: 403 };
		}
		if (event.isDeleted) {
			throw { message: "Cannot pin entities to deleted event", code: 404 };
		}
		...
		for (let i = 0; i < entities.length; i++) {
			const ent = await feedModel.getEntityWithAuthorizationUseFeedId(
				userId,
				entities[i].id,
				entities[i].feedId
			);
		...
		if (_global.globalChangefeed) {
			if (result.changes && result.changes[0]) {
				const change = {
					new_val: { ...event, ...{ pinChanges: { pinned: authEnts } } },
					rt: true
				};
				_global.globalChangefeed.publish(change);
			}
		...
		const promises = result.changes.map(async change => {
			const entity = authEnts.find(ent => ent.id === change.new_val.entityId);
			if (entity) {
				const activity = {
					summary: "",
					type: "pinned",
 ...
 * @param entityType
 */
EventModel.prototype.unpinEntity = async function (
	userId,
	eventId,
	entityId,
	entityType,
	...
	try {
		// -- todo: ENT_AUTH previously looked like manage should be required to unpin but only collab required to pin
		...
		if (event.isDeleted) {
			throw { message: "Cannot unpin entities from deleted event", code: 404 };
		}

		// used for name in activity, and to authorize entity
		const entity = await feedModel.getEntityWithAuthorization(
			userId,
			entityId,
			entityType
		);
		...
		if (_global.globalChangefeed) {
			if (result.changes && result.changes[0]) {
				const change = {
					new_val: { ...event, ...{ pinChanges: { unpinned: [entity] } } },
					rt: true
				};
				_global.globalChangefeed.publish(change);
			}
...
EventModel.prototype.shareEvent = async function (
	userId,
	eventId,
	orgIds
) {
	try {
		...
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.share);
		if (!event) {
			throw { message: "User is not authorized to share this event", code: 403 };
		}
		if (event.isDeleted) {
...
EventModel.prototype.addComment = async function (
	userId,
	eventId,
	comment,
	translations
	...
	try {
		...
		if (event.isDeleted) {
			throw { message: "Cannot add comments to a deleted event", code: 404 };
		}
			...
			.merge(row => {
				return {
					isInScope: r
						...
						.during(
							row("startDate"),
							row("endDate").default(r.time(3000, 1, 1, "Z"))
						)
				...
				return r.expr([
					...
					r.branch(
						row("add")
							.count()
							.gt(0),
						r.table(EVENT_FEED_CACHE_TABLE).insert(row("add")),
						{
							added: 0
						}
					)
 ...
 */
EventModel.prototype.queryPinnable = async function (
	userId,
	eventId,
	query,
	pageSize
	...
	try {
		...
		for (let i = 0; i < feedIds.length; i++) {
			...
			console.log("PINNABLE FEED TYPES BY ID:", feedId);
		...
		if (uniqueRemoteEcoIds.length > 0) {
			remotePinnable = await global.ecoLinkManager.queryPinnable(uniqueRemoteEcoIds, userId, eventId, query);
		}
...
/**
 * getEventActivityStatistics
 * @param userId
 * @param orgId
 * @param eventId
 */
EventModel.prototype.getEventActivityStatistics = async function (
	userId,
	eventId
) {
	try {
		const commentCount = await r
...
EventModel.prototype.streamPinnedItems = async (
	userId,
	eventId,
	handler
) => {
	...
	if (entityCheck) {
		...
		const onFeedItem = async change => {
			...
			const pinnedEnt = await feedModel.getEntityWithAuthorization(userId, pinnedItem.entityId, pinnedItem.entityType);
			if (pinnedEnt) {
				const pinnedObj = { type: change.type };
				pinnedObj[valType] = pinnedEnt;
				if (valType === "new_val") {
					if (!pinnedEntCache.includes(pinnedItem.entityId))
						pinnedEntCache.push(pinnedItem.entityId);
				}
				else {
					if (pinnedEntCache.includes(pinnedItem.entityId))
		...
		const gcfSid = _global.globalChangefeed.subscribe({ entityType: "*" }, null, (change) => {
			const valType = change.new_val ? "new_val" : "old_val";
		...
		const cancelCF = provider.processChangefeed(
			"eventModel.streamPinnedItems",
			q,
			onFeedItem,
			onError
		);
		...
		const cancelFn = function () {
			try {
				...
				logger.error("streamPinnedItems", "Error cancelling stream", { errMessage: err.message, errStack: err.stack });
			}
		};
		...
		throw { message: "Event not found or requester does not have access", code: 404 };
	}

};


EventModel.prototype.streamPinnedItemsByType = async (
	userId,
	eventId,
	pinnedItemType,
	handler) => {
	...
	if (entityCheck) {
		...
		const onFeedItem = async change => {
			...
			switch (change.type) {
				...
				case "initial":
					valType = "new_val";
					break;
				// case "change":	// I dont think this is ever used in this scenario
				case "remove":
					valType = "old_val";
					break;
				default:
		...
		const cancelFn = provider.processChangefeed(
			"eventModel.streamPinnedItemsByType",
			q,
			onFeedItem,
			onError
		);
 ...
 */
EventModel.prototype.streamEvents = async (
	userId,
	format = "full",
	status,
	type,
	...
	if (type && type.length) {
		...
		type.forEach((value, index) => {
			...
			typeFilter = index > 0 ? typeFilter.or(
				typeQuery
			) : typeQuery;
		});
		...
		status.forEach((value, index) => {
			switch (value) {
				case "active": {
					...
					statusFilter = index > 0 ? statusFilter.or(
						statusQuery
					) : statusQuery;
	...
	const q = r
		...
		.merge(event => {
			return r.branch(event("new_val").ne(null),
				{
					new_val: {
						additionalProperties: r
							.table(EXTERNAL_ENTITY_MAPPING_TABLE)
						...
						commentCount: r
							.table(ACTIVITY_TABLE)
							.getAll([event("new_val")("id"), "comment"], { index: "targetId_type" })
							.count()
					}
				},
				{}
...
/**
 * streamAllStatistics -- stream the comment count of all events a user should have access to
 * @param userId
 * @param handler
 */
EventModel.prototype.streamAllStatistics = async function (
	userId,
	handler
) {
	try {
		const authorizedEvents = await this.getAll(userId);
		const eventIds = authorizedEvents.map(event => event.id);

		const stats = {};
		let initialComplete = false;

		// Give the stream a 1.5 seconds to finish its initial batch, then send it
		// After that send across for each update
		// -- Ideally, this would use rethinkdb's state updates,
		// -- but those don't seem to work on this changefeed
		setTimeout(() => {
			handler(null, stats);
			initialComplete = true;
		}, 1500);
		try {
			const feed = await r.table(ACTIVITY_TABLE)
				.filter(row => {
					return r.expr(eventIds).contains(row("target")("id"));
				})
				.filter({
					type: "comment"
				})
				// includeStates doesn't seem to work on this changefeed...
				.changes({
					includeInitial: true,
					includeTypes: true,
					includeStates: true
			...
			feed.each(function (err, change) {
				if (err) {
					logger.error("streamAllStatistics", "There was an error retrieving an activity feed", {
						err: { message: err }
					});
					handler(err, null);
				} else {
					...
					if (change.new_val) {
						eventIds.forEach(id => {
							if (change.new_val.target.id === id) {
								if (stats[id]) {
									stats[id]++;
								} else {
									stats[id] = 1;
					...
					// After 1.5 seconds has passed, stream one by one
					if (initialComplete) {
						handler(null, stats);
					}
				}
...
EventModel.prototype.mockUpdatePinnedItem = async (
	userId,
	eventId,
	entityId
) => {
...
EventModel.prototype.autoPinLists = async function (listIds, eventId, userId) {
	try {
		...
		if (updates.length) {
			const result = await r.table(LIST_TABLE)
				.insert(updates, { returnChanges: true })
				.run();

			logger.info("autoPinLists", "Lists were automatically pinned to a created event by subtype", { result: result });
			...
			if (result && result.changes.length) {
				...
				if (attachmentUpdates.length) {
					const attachmentResult = await r
				...
				const err = { message: "Error automatically pinning lists to event by subtype", stack: "/models/eventModel.js" };
				throw err;
			}
		}
		else {
			const err = { message: "Error generating new lists from templates", stack: "/models/eventModel.js" };
			throw err;
		}
	}
	catch (err) {
...
*/
EventModel.prototype.deleteList = async function (userId, orgId, eventId, listId, translations) {
	try {
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.manage);
		if (!event) {
			throw { message: "User is not authorized to access this event", code: 403 };
		...
		logger.error(
			"deleteList",
			"An error occurred while attempting to delete a list from an event",
			{ err: { message: error.message, stack: error.stack } }
		);
		throw error;
	}
...
*/
EventModel.prototype.updateList = async function (userId, orgId, eventId, listId, update, timezone, translations) {
	try {
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.manage);
		if (!event) {
			throw { message: "User is not authorized to access this event", code: 403 };
		...
		logger.error(
			"updateList",
			"An error occurred while attempting to update a list on an event",
			{ err: { message: err.message, stack: err.stack } }
		);
		throw err;
	}
...
* @param {string} update -- the list's update
*/
EventModel.prototype.pinList = async function (userId, orgId, eventId, listIds, type, translations) {
	try {
		const event = await feedModel.getEntityWithAuthorization(userId, eventId, "event", userPolicyCache.feedPermissionTypes.contribute);
		if (!event) {
			throw { message: "User is not authorized to access this event", code: 403 };
		...
		logger.error(
			"pinList",
			"An error occurred while attempting to pin a list(s) to an event",
			{ err: { message: error.message, stack: error.stack } }
		);
		throw error;
	}
...
EventModel.prototype.streamLists = async function (eventId, userId, expandRefs, handler) {
	try {
		...
		logger.error(
			"stream lists",
			"An error occurred while attempting to stream lists pinned to an event",
			{ err: { message: error.message, stack: error.stack } }
		);
		throw error;
	}
...
EventModel.prototype.addProximity = async function (
	userId,
	eventId,
	proximity,
	translations
...
EventModel.prototype.updateProximity = async function (
	userId,
	eventId,
	proximityId,
	proximity,
...
EventModel.prototype.deleteProximity = async function (
	userId,
	eventId,
	proximityId,
	translations
 ...
 * @param cameraId
 */
EventModel.prototype.getEventsInFov = async function (
	userId,
	cameraId
) {
	try {
 ...
 * @param eventIds
*/
EventModel.prototype.getEventFeedCacheEntities = async function (
	eventIds
) {
	try {
		const cacheEnts = await r
			.table(EVENT_FEED_CACHE_TABLE)
			.getAll(r.args(eventIds), { index: "eventId" });

		const feedGroups = {};
		for (const cacheEnt of cacheEnts) {
		...
		for (const feedId of Object.keys(feedGroups)) {
			...
			const ents = await feedModel.getFeedEntitiesInternal(feedId, entityIds);
			for (const ent of ents) {
				const cacheEnt = cacheEnts.find((ce) => ce.entityId === ent.id);
...
EventModel.prototype.getEventProximityEntities = async function (userId, feedId, geometry, radiuses, handler) {
	try {
		...
		let initialEntities = [];
		const q = r.table(feedType.sourceTable)
			.filter(
				r.and(
					...
					r.geojson(r.row("entityData")("geometry")).intersects(r.circle(geometry.coordinates, radiuses[radiuses.length - 1], { unit: "km" }))
				)
			)
		...
		const onFeedItem = (change) => {
			if (change.new_val && change.new_val.entityData) {
				if (userPolicyCache.authorizeEntity(userId, change.new_val)) {
					...
					try {
						...
						logger.error("getEventProximityEntities", "There was an error calculating distance to event point.", { err });
					}
			...
			if (change.state) {
				if (change.state === "initializing") {
					...
					// So send them across!
					handler(null, { "type": "initial-batch", "changes": initialEntities });
					initialEntities = [];
				}
			} else {
				if (isInitialState) {
					...
					// -- dont include entity if no assigned proximityId
					if (change.type === "remove" || (change.new_val && change.new_val.proximityId && change.new_val.proximityId !== -1)) {
		...
		const onError = (err) => {
			console.log("EventModel.getEventProximityEntities changefeed error", err);
			handler(err, null);
		};
		...
		logger.error("getEventProximityEntities", "There was an error while retrieving the entities within an event's proximities", {
			err: error
		});
		throw error;
	}
...
EventModel.prototype.deleteEventNotes = async function (userId, eventId) {
	...
	try {
		...
		logger.error(
			...
			{ err: { message: ex.message, code: ex.code, stack: "/models/eventModel.js" } }
		);
		throw ex;
	}
};
...
EventModel.prototype._internalGetPinnedCameras = async function () {
	...
	try {
		...
		const result = r.table(EVENT_ENTITIES_TABLE)
			.filter(r.row("entityType").eq("camera"))
			.filter((ee) => {
				return r.table(EVENT_TABLE)
					.get(ee("eventId")).hasFields("archiveDate").not();
			})
			.map((pc) => {
				return {
					"eventId": pc("eventId"),
