...
class MapLayer extends Component {
	...
	componentDidMount() {
		const { map, feedId, loadProfile, data, setMapEntities, feedEntType, selectedFloors } = this.props;
	...
	handleReorder = () => {
		...
		const layerIds = _.map(layers, "id");

		if (!mapTools.type) {
			const pointLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && _.includes(layer, "-Point-")
			);
			const pointSymbolLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && _.includes(layer, "Point-symbol")
			);
			const gisPointLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && _.includes(layer, "-Point-")
			);
			const gisPointSymbolLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && _.includes(layer, "Point-symbol")
			);
			const lineLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && (_.includes(layer, "-LineString-")  || _.includes(layer, "-MultiLineString-"))
			);
			const lineSymbolLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && _.includes(layer, "LineString-symbol")
			);
			const gisLineLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && (_.includes(layer, "-LineString-")  || _.includes(layer, "-MultiLineString-"))
			);
			const gisLineineSymbolLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && _.includes(layer, "LineString-symbol")
			);
			const polyLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && (_.includes(layer, "-Polygon-")  || _.includes(layer, "-MultiPolygon-"))
			);
			const polySymbolLayers = _.filter(
				layerIds,
				layer => !layer.startsWith("gis-") && _.includes(layer, "Polygon-symbol")
			);
			const gisPolyLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && (_.includes(layer, "-Polygon-")  || _.includes(layer, "-MultiPolygon-"))
			);
			const gisPolySymbolLayers = _.filter(
				layerIds,
				layer => layer.startsWith("gis-") && _.includes(layer, "Polygon-symbol")
			);
			const floorplanLayers = _.filter(
				layerIds,
				layer => _.includes(layer, "-floor-plan-overlay")
			);
			const layerOrder = [floorplanLayers, gisPolyLayers, gisPolySymbolLayers, polyLayers, polySymbolLayers, 
				gisLineLayers, gisLineineSymbolLayers, lineLayers, lineSymbolLayers, gisPointLayers, 
				gisPointSymbolLayers, pointLayers, pointSymbolLayers];

			// -- reorder layers in the given layer order set above
			layerOrder.forEach(layers => {
				_.each(layers, layer => {
					map.moveLayer(layer);
				});
			});
	...
	renderLayers = () => {
		...
		const layers = _.map(
			...
			(source, index) => {
				...
				return (
					<Fragment key={index}>
						{feedEntType.toLowerCase() === "track" && renderSilhouette && this.renderSilhouettes(index, source)}
						<GeoJSONLayer
							key={feedId + index}
	...
	renderSilhouettes = (index, source) => {
		const { feedId, silhouetteMinZoomLevel } = this.props;
		const vessels = {};
		...
		return (
			<Fragment key={feedId + index + "-silhouettes"}>
				<VesselPolygons vessels={vessels} minZoom={silhouetteMinZoomLevel} />
			</Fragment>
		);
	}
	...
	render() {
		const { data, canView, clusteringEnabled } = this.props;
		const { styleLoaded } = this.state;

		return data && styleLoaded && canView ? (
			<Fragment>
				{this.renderLayers()}
				{clusteringEnabled && this.renderClusters()}
			</Fragment>
		) : (
			<div />
		);
