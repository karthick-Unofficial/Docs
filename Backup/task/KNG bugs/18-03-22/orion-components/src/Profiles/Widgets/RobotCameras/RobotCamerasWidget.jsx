...
const CAMERA_POSITION_INDEX = [ "frontLeft", "frontRight", "left", "right", "back" ];
...
const RobotCamerasWidget = ({
	...
	const getRobotLayoutCameras = (hideInVisibleCameras) => {
		...
		const labels = ["FL", "FR", "L", "R", "B", "O"];
		const positions = ["frontLeft", "frontRight", "left", "right", "back", "other"];
		let camerasIntial = [];

		let otherCameraIndex = 5; //generally all position 'other' cameras will start from 5
		if (cameras && cameras.length > 0) {
			...
			labels.forEach(lbl => {
				...
				let indexTmp = (ix >= 6) ? 6 : CAMERA_POSITION_INDEX.indexOf(positions[ix - 1]);
				...
				{getRobotCameras(true).map((camera) => {
					...
					return (
						<Fragment key={camera.id}>
							<div className={`blueprint-camera ${CAMERA_POSITION_CLASS[camera.position]} ${isOutside && "outside"}`}>
