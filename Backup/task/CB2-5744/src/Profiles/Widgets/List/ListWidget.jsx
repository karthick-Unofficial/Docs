...
const SortableItem = SortableElement(({ passedProps, dir, locale }) => {
	return (
		<ListCard sortable={true} {...passedProps} dir={dir} locale={locale} />
	);
});
	...
	if (props.availableLists) {
		...
		filteredLists.forEach((list) => {
			if (
				...
				convertedCategories[list.category]
			) {
				occupiedCategories[list.category] =
					convertedCategories[list.category];
			}
			...
			<Fragment>
				<List key="add-lists-category-section">
					{Object.values(occupiedCategories).map((category) => {
						return (
							<Fragment
								key={`add-lists-category-${category.id}-fragment`}
							>
								<p
									key={`add-lists-category-${category.id}-label`}
									style={{ color: "#fff" }}
								>
									{category.name}
								</p>
					...
					>
						{getTranslation(
							"global.profiles.widgets.list.main.uncategorizedLists"
						)}
					</p>
...
const ListWidget = (props, ref) => {
	...
	useEffect(() => {
		...
		return () => {
			if (!isPrimary && !expanded) {
				dispatch(
					unsubscribeFromFeed(contextId, "lists", subscriberRef)
				);
			}
		};
	}, []);
	...
	useEffect(() => {
		if (prevProps && prevProps.lists) {
			if (
				Object.keys(prevProps.lists).length <
					Object.keys(lists).length ||
				!isEqual(prevProps.lists, lists)
			) {
				organizeSortedLists();
			}
		}
		...
		return {
			...
			...(forDialog
				? {
						...
						handleSelectList: () =>
							dialogProps.handleSelectList(item.id),
				...
				<div className="widget-header" style={{ marginTop: -8 }}>
					...
					{listAccessAndEventsManage && (
						<div className="widget-option-button">
							...
							>
								{getTranslation(
									"global.profiles.widgets.list.main.addList"
								)}
							</Button>
						</div>
					)}
			...
			<div className="widget-content">
					...
					<SortableList
						...
						shouldCancelStart={(e) => {
							...
							const header = e.path
								.slice(0, 5)
								.some((element) => {
									const classNameCheck = element.className
										.toString()
										.toLowerCase();
									return (
										classNameCheck &&
										classNameCheck.includes("muicardheader")
									);
								});
							if (!header) {
								return true;
				...
				<CBDialog
					...
					title={getTranslation(
						"global.profiles.widgets.list.main.chooseToAdd"
					)}
					confirm={{
					...
					abort={{
						label: getTranslation(
							"global.profiles.widgets.list.main.cancel"
						),
				...
				>
					<SearchField
						...
						placeholder={getTranslation(
							"global.profiles.widgets.list.main.searchLists"
						)}
						dir={dir}
					/>
					{/* <div> */}
