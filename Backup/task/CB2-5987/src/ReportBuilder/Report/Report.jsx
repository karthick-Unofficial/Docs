...
const Report = ({
	...
	const report = useSelector((state) =>
		state.appState.reportViewer.reports.filter((report) => {
	...
	const groups = page
		? useSelector((state) => state.appState.reportViewer.pages[state.appState.reportViewer.page - 1].groups)
		: [];
	...
	const pdfRequestDialogSubmitting = useSelector(
		(state) => state.appState.reportBuilder.pdfRequestDialog.isSubmitting
	);
	...
	const groupRenderer = (group) => {
		return (
			...
			>
				<TableCell
					colSpan={columns.length + columns.filter((a) => a.property === "timestamp").length}
			...
			<TableRow selectable={false} key={index}>
				{columns.map((column, index) => {
					...
					if (prop === "timestamp" || prop === "entered") {
						...
						td = (
							<TableCell colSpan={column.width} key={index} style={styles.rowRendererTableCell}>
								{row[prop]}
							</TableCell>
						);
					}
	...
	const submitButton = requestDialogSubmitting ? (
		...
		<Button color="primary" onClick={handleExportEmailRequest}>
	...
	const actions = requestDialogToast // If we've already completed submit
		? [
			<Button key="close-dialog-button" color="primary" onClick={() => dispatch(closeRequestDialog())}>
	...
	const pdfSubmitButton = pdfRequestDialogSubmitting ? (
		...
		<Button color="primary" onClick={handlePDFEmailRequest}>
			{getTranslation("reportBuilder.reportBuilder_reports.report.request")}
		</Button>
	);
	...
	const pdfActions = pdfRequestDialogToast // If we've already completed submit
		? [
			<Button key="close-dialog-button" color="primary" onClick={() => dispatch(closePDFRequestDialog())}>
				{getTranslation("reportBuilder.reportBuilder_reports.report.close")}
			</Button>,
			null
		]
		: [
			<Button key="cancel-dialog-button" color="primary" onClick={() => dispatch(closePDFRequestDialog())}>
				{getTranslation("reportBuilder.reportBuilder_reports.report.cancel")}
			</Button>,
			pdfSubmitButton
		];
		...
		<div>
									...
									{!isGeneratingFile ? (
										<Fragment>
											...
											>
												<FileDownload />
												{getTranslation(
													"reportBuilder.reportBuilder_reports.report.downloadCsv"
												)}
											</Button>
											<Button
												onClick={PDFExport}
											...
											>
												<FileDownload />
												{getTranslation(
													"reportBuilder.reportBuilder_reports.report.downloadPdf"
												)}
											</Button>
										</Fragment>
									) : (
			...
			{totalPages > 0 ? (
				<div className="navigator">
					...
					{currentPage < totalPages && (
						// eslint-disable-next-line jsx-a11y/anchor-is-valid
						<a style={{ opacity: isPaginating ? 0.2 : 1 }} onClick={NextPage}>
							{" "}
							{">"}{" "}
						</a>
					)}
				...
				<div className="navigator">
					<p>
						<Translate value="reportBuilder.reportBuilder_reports.report.noResult" />
					</p>
				</div>
			)}
							...
							<TableRow selectable={false}>
								{columns.map((column) => {
									return (
										...
										>
											{getTranslation(`reportBuilder.reportBuilder_reports.report.displayName.${column.displayName}`)}
										</TableCell>
									);
								})}
							</TableRow>
			...
			>
				<DialogTitle sx={{ color: "#4B4B4B" }} variant="h3">
					{getTranslation("reportBuilder.reportBuilder_reports.report.requestCsv")}
				</DialogTitle>
			...
			>
				...
				<DialogActions>{pdfActions}</DialogActions>
			</Dialog>
		</div>
	);
};
