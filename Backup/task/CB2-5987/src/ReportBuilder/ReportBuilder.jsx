...
const ReportBuilder = () => {
	...
	const handleExportEmailRequest = (r, valuesParam, reportParam) => {
		...
		const values = valuesParam ? valuesParam : stateRef.current.values;
		const data = submissionCheck(values, rpt);
		if (data) {
	...
	const submissionCheck = (values, report) => {
		...
		if (report.fields.length !== Object.keys(values).length) {
			report.fields.forEach((field) => {
				...
				newErrorMessage[name] = {
					data:
						!values[field.name] || !values[field.name].toString()[0]
							? getTranslation("reportBuilder.reportBuilders.completeAllFields")
							: ""
				};
			});
		}
		...
		for (const value in values) {
			newErrorMessage[value] = {
				data:
					!values[value] || !values[value].toString()[0]
						? getTranslation("reportBuilder.reportBuilders.completeAllFields")
						: ""
			};
		}
		...
		>
			<div className="rb-field-wrapper">
				...
				<div className="rb-submit-button">
					...
					>
						{downloadOnly ? (
							<Translate value="reportBuilder.reportBuilders.requestCsv" />
						) : (
							getTranslation("reportBuilder.reportBuilders.submit")
						)}
					</Button>
				</div>
			</div>
			{error && <span className="cb-font-b2 error-text">{error.message}</span>}
			{isSubmitting ? (
					...
					<div>
						<div className="rb-multi-report-wrapper">
							<ErrorBoundary>
								<MultiReport
									eventName={
										stateRef.current.values.events ? stateRef.current.values.events[0].name : ""
									}
									time={creationTime}
									report={report}
									handlePDFExport={handlePDFExport}
