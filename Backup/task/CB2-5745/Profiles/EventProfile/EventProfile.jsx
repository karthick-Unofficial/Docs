...
const EventProfile = ({
	...
	const externalSystems = useSelector(
		(state) => state.session.organization.externalSystems
	);
	...
	const lookupData = useSelector((state) =>
		state.globalData && state.globalData.listLookupData
	...
	const secondaryExpanded = useSelector(
		(state) => contextPanelState(state).secondaryExpanded
	);
		...
		DEFAULT_WIDGET_CONFIG = [
			{
				...
				id: "activities",
				name: getTranslation(
					"global.profiles.eventProfile.main.activities"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "event_lists",
				name: getTranslation(
					"global.profiles.eventProfile.main.eventLists"
				)
			},
			{
				enabled: true,
				id: "pinned_items",
				name: getTranslation(
					"global.profiles.eventProfile.main.pinnedItems"
				)
			},
			{
				enabled: true,
			...
			},
			{
				...
				id: "secure_share",
				name: getTranslation(
					"global.profiles.eventProfile.main.secureShare"
				)
			},
			{
				enabled: true,
				id: "cad_details",
				name: getTranslation(
					"global.profiles.eventProfile.main.cadDetails"
				)
			},
			{
				enabled: true,
				id: "responding_units",
				name: getTranslation(
					"global.profiles.eventProfile.main.respondingUnits"
				)
			},
			{
				enabled: true,
				id: "cameras",
				name: getTranslation(
					"global.profiles.eventProfile.main.cameras"
				)
			},
			{
				enabled: true,
				id: "proximity",
				name: getTranslation(
					"global.profiles.eventProfile.main.proximity"
				)
			},
			{
				enabled: true,
				id: "resources",
				name: getTranslation(
					"global.profiles.eventProfile.main.resources"
				)
			},
			{
				enabled: true,
				id: "equipments",
				name: getTranslation(
					"global.profiles.eventProfile.main.equipment"
				)
			},
			{
				enabled: true,
				...
				name: getTranslation(
					"global.profiles.eventProfile.main.gateRunnerResponse"
				)
			}
		];
		setMounted(true);
	...
	if (context) {
		const { attachments, activities, pinnedItems, lists, eventCameras } =
			context;
		const userId = user.id;
		const { entityData, name, desc, isPublic, type, endDate, entityType } =
			event;
		...
		return (
			...
			>
				...
				>
					...
					{$(window).width() > 1023 && getWidgetStatus("map") && (
						<MapWidget
							...
							title={getTranslation(
								"global.profiles.eventProfile.main.mapPlanner"
							)}
							dir={dir}
						/>
					)}
					...
					{getWidgetStatus("gate_runner_response") && (
						<ErrorBoundary>
							<GateRunnerResponseWidget
								order={
									getWidgetStatus("gate_runner_response")
					...
					{getWidgetStatus("responding_units") && (
						<ErrorBoundary>
							<RespondingUnitsWidget
								order={
									getWidgetStatus("responding_units").index
								}
								enabled={
									getWidgetStatus("responding_units").enabled
								}
								expanded={false}
								loadProfile={loadProfile}
						...
						<ErrorBoundary>
							<NotesWidget
								...
								updateNotes={(event, notes) =>
									dispatch(updateEventNotes(event, notes))
								}
								event={event}
								notes={
									notes && notes.html
								...
								widgetsLaunchable={
									widgetsLaunchable && appId !== "events-app"
								}
								contextId={contextId}
								entityType={entityType}
								...
								dir={dir}
								openDialog={(widget) =>
									dispatch(openDialog(widget))
								}
								closeDialog={(widget) =>
									dispatch(closeDialog(widget))
								}
							/>
						</ErrorBoundary>
					)}
					{getWidgetStatus("pinned_items") && pinnedItems && (
						<ErrorBoundary>
							<PinnedItemsWidget
								...
								enabled={
									getWidgetStatus("pinned_items").enabled
								}
						...
						(activities || forReplay) && (
							<ErrorBoundary>
								<Activities
									...
									enabled={
										getWidgetStatus("activities").enabled
									}
					...
					<Dialog
						...
						confirm={{
							label: getTranslation(
								"global.profiles.eventProfile.main.confirm"
							),
							action: handleConfirmDelete
						}}
						abort={{
							label: getTranslation(
								"global.profiles.eventProfile.main.cancel"
							),
							action: handleCloseDeleteDialog
						}}
						title={getTranslation(
							"global.profiles.eventProfile.main.confirmationText"
						)}
						dir={dir}
					/>
					<ErrorBoundary>
