...
const PTZControls = ({ instanceId, camera, dock }) => {
	...
	const handleCameraNorthWest = () => {
		...
		cameraService.moveContinuous(camera.id, instanceId, "tilt", -1, 0.5);
		cameraService.moveContinuous(camera.id, instanceId, "pan", -1, 0.5);
	};
	...
	const handleCameraZoomIn = () => {
		cameraService.moveContinuous(camera.id, instanceId, "zoom", 1, 0.5);
	};
		...
		<div className="ptz-control-main-container">
			...
			<div className={`ptz-grid ${dock ? "docked-grid" : ""}`}>
				...
				<div className="ptz-grid-item ptz-home">
					<Button variant="text" color="primary" style={{ ...buttonStyles.move }} onClick={handleCameraHome}>
						<HomeIcon />
					</Button>
				</div>
				<div className="ptz-grid-item">
