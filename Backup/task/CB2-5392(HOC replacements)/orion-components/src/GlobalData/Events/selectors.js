...
import { eventFiltersSelector, eventTemplateFiltersSelector } from "../../AppState/Selectors";
	...
	_.each(filters, (filter, key) => {
		...
		switch (key) {
			...
			case "template":
				filtered["template"] =
					activeFilter &&
					(!eventTemplate || (eventTemplate && !filter.includes(eventTemplate)));
				break;
			default:
				break;
		}
...
export const currentOwnedEventsSelector = createDeepEqualSelector(
	...
	(events, userId, filters, search) => {
		const filteredEvents = _.pickBy(events, event => {
...
export const eventsSharedFromOrgSelector = createDeepEqualSelector(
	...
	orgIdSelector,
	(events, userId, orgId) => {
		const orgEvents = _.pickBy(events, event => {
...
export const eventsSharedFromEcoSelector = createDeepEqualSelector(
	...
	orgIdSelector,
	(events, userId, orgId) => {
		const ecoEvents = _.pickBy(events, event => {
			const { ownerOrg, owner } = event;
			return ownerOrg !== orgId && owner !== userId;
		});
		return ecoEvents;
...
export const ownedTemplatesSelector = createDeepEqualSelector(
	...
	(templates, userId, search) => {
		const filteredTemplates = _.pickBy(templates, template => {
...
export const templatesSharedFromOrgSelector = createDeepEqualSelector(
	...
	(templates, userId, orgId) => {
		const orgTemplates = _.pickBy(templates, template => {
...
export const templatesSharedFromEcoSelector = createDeepEqualSelector(
	...
	orgIdSelector,
	(templates, userId, orgId) => {
		const ecoTemplates = _.pickBy(templates, template => {
			const { ownerOrg, owner } = template;
			return ownerOrg !== orgId && owner !== userId;
		});
		return ecoTemplates;
