...
const NewZoneReplay = () => {
	...
	return (
		<Dialog
			open={true}
			title={<h3>{getTranslation("newReplay.newZoneReplay.title")}</h3>}
			paperPropStyles={{
		...
		>
			<div style={{ width: "50%", marginBottom: 50 }}>
				<SearchSelectField
					id={"zone-select"}
					value={shapeValue}
					label={getTranslation("newReplay.newZoneReplay.search")}
					disabled={false}
			...
			<div style={{
				...
				<SelectField
					id="authtype"
					label={getTranslation("newReplay.newZoneReplay.replay")}
					formControlProps={{
						style: {
							marginTop: 0,
							width: 120
				...
				<div style={{ ...styles.selectDivStyles, marginTop: 30, width: replayFrameValue === "clip-duration" ? "70%" : "100%" }}>
					{replayFrameValue === "clip-duration" ? (
						<Fragment>
							<TextField
								id="date/time"
								label={getTranslation("newReplay.newZoneReplay.start")}
								style={{
									width: 170
								}}
							...
							<SelectField
								id="clip-duration"
								label={getTranslation("newReplay.newZoneReplay.clip")}
								formControlProps={{
									style: {
										marginTop: 0,
										width: 140
						...
						<Fragment>
							<div style={{ marginTop: -16, width: 170 }}>
								<DateTimePicker format={"MMMM D, YYYY h:mm A"} handleChange={(time) => setBeginningDate(time)} value={beginningDate} label={getTranslation("newReplay.newZoneReplay.start")} okLabel={getTranslation("newReplay.replayArchive.ok")} cancelLabel={getTranslation("newReplay.replayArchive.cancel")} clearLabel={getTranslation("newReplay.replayArchive.clear")}/>
							</div>
							<div style={{ marginTop: -16, width: 170 }}>
								<DateTimePicker minDate={beginningDate} format={"MMMM D, YYYY h:mm A"} handleChange={(time) => setEndDate(time)} value={endDate} label={getTranslation("newReplay.newZoneReplay.end")} okLabel={getTranslation("newReplay.replayArchive.ok")} cancelLabel={getTranslation("newReplay.replayArchive.cancel")} clearLabel={getTranslation("newReplay.replayArchive.clear")}/>
							</div>
							<TextField
								id="duration"
								label={getTranslation("newReplay.newZoneReplay.duration")}
								style={{
									width: 120
								}}
