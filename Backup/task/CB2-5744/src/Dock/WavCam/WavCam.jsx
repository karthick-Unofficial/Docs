...
// cSpell:ignore horz hfov vfov pano
...
import { AddBox, MenuOpen } from "@mui/icons-material";
import { mdiMinusBox, mdiCloseCircle } from "@mdi/js";
import { SelectField, WSImagePlayer } from "orion-components/CBComponents";
...
const WavCam = () => {
	...
	const userCameras = useSelector(
		(state) => userCamerasSelector(state),
		shallowEqual
	);
	const filteredUserCams = userCameras.filter((cam) => {
		return (
			cam.entityData.properties.features &&
			cam.entityData.properties.features.includes("ribbon")
		);
	});
	...
	const wavCamPersistedState = useSelector(
		(state) =>
			state.appState.persisted?.wavcam_pano ?? { wavCamMetadata: {} },
		shallowEqual
	);
	let selectedWavCam = null;
	if (wavCams.length > 0) {
		selectedWavCam =
			wavCamPersistedState && wavCamPersistedState.selectedWavCam
				? wavCams.find(
						(c) => c.id === wavCamPersistedState.selectedWavCam
				  )
				: wavCams[0];
	}
	const context = useSelector(
		(state) =>
			wavCams.length > 0 && selectedWavCam
				? getContext(selectedWavCam.id)(state)
				: null,
		shallowEqual
	);
	...
	const showWavCamLabels = useSelector(
		(state) => state.appState.dock.dockData.showWavCamLabels || true
	);
	...
	const getFullImageSize = (meta, newW = null) => {
		...
		const fullScaleW = Math.round(
			stateRef.current.baseWavImageW +
				stateRef.current.baseWavImageW * stateRef.current.zoom
		);
		const fullScaleH = fullScaleW * pixHPerPixW;
		const containerH = wavContainerRef.current.offsetHeight;
		const offsetTop =
			containerH > fullScaleH ? (containerH - fullScaleH) / 2 : 0;

		const maxWavImageW = stateRef.current.baseWavImageW * 2;

		const virtualXOffset =
			stateRef.current.wavX > 0
				? wavImageContainerRef.current.offsetWidth - maxWavImageW
				: 0;
		const offsetX =
			fullScaleW <= maxWavImageW
				? 0
				: (fullScaleW - virtualXOffset) *
				  stateRef.current
						.wavX; /* + stateRef.current.factoredOffsetX; */
		...
		if (meta) {
			...
			const md = {
				...
				...{
					...
					"wav-container-h":
						wavImageContainerRef.current.offsetHeight,
			...
			md["x-wavcam-lat"] =
				selectedWavCam.config.lat || md["x-wavcam-lat"];
			md["x-wavcam-lon"] =
				selectedWavCam.config.lon || md["x-wavcam-lon"];
			md["x-wavcam-az"] = selectedWavCam.config.az || md["x-wavcam-az"];
			md["x-wavcam-el"] = selectedWavCam.config.el || md["x-wavcam-el"];
			md["x-wavcam-camera-az"] =
				selectedWavCam.config.az || md["x-wavcam-camera-az"];
			md["x-wavcam-camera-el"] =
				selectedWavCam.config.el || md["x-wavcam-camera-el"];
			md["x-wavcam-alt"] =
				selectedWavCam.config.alt || md["x-wavcam-alt"];
			md["x-wavcam-height"] =
				selectedWavCam.config.height || md["x-wavcam-height"];
			md["x-wavcam-hfov"] =
				selectedWavCam.config.hfov || md["x-wavcam-hfov"];
			md["x-wavcam-vfov"] =
				selectedWavCam.config.vfov || md["x-wavcam-vfov"];
			md["x-wavcam-horizontal-extents"] =
				selectedWavCam.config.horzExtent ||
				md["x-wavcam-horizontal-extents"];
			md["x-wavcam-vertical-extents"] =
				selectedWavCam.config.vertExtent ||
				md["x-wavcam-vertical-extents"];
	...
	const calcWavSlice = function (tempMetadata) {
		...
		let wavX =
			Math.abs(stateRef.current.zoomOffsetX) < maxOffsetX
				? 0
				: (Math.abs(stateRef.current.zoomOffsetX) - maxOffsetX) /
				  fullScaleW;
		...
		if (wavX > 0) {
			...
			if (percentOffset > 1) {
				wavX = 0;
				stateRef.current.factoredOffsetX =
					stateRef.current.panOffsetX - overflowW;
		...
		let wavY =
			Math.abs(stateRef.current.zoomOffsetY) < maxOffsetY
				? 0
				: (Math.abs(stateRef.current.zoomOffsetX) - maxOffsetY) /
				  fullScaleH;
		if (wavY > 0) {
			...
			if (percentOffset > 1) {
				wavY = 0;
				stateRef.current.factoredOffsetY =
					stateRef.current.panOffsetY - overflowH;
			} else if (percentOffset < -1) {
				wavY = 1 - wavH;
				stateRef.current.factoredOffsetY =
					stateRef.current.panOffsetY + overflowH;
		...
		const winW = Math.round(
			fullScaleW < maxWavImageW ? fullScaleW : maxWavImageW
		);
		const winH = Math.round(
			fullScaleH < maxWavImageH ? fullScaleH : maxWavImageH
		);
	...
	const syncStream = function (tempMetadata) {
		...
		if (slice) {
			...
			if (
				...
				cmd !== stateRef.current.lastCommand
			) {
	...
	const handleCameraZoomIn = (evt) => {
		stateRef.current.zoom = parseFloat(
			(stateRef.current.zoom + stateRef.current.zoomFactor).toFixed(1)
		); // maybe need a max here???
	...
	const scaleImage = function (scaleDelta) {
		...
		const deltaHeight =
			fsMetadata[4] * newW -
			fsMetadata[4] * wavImageContainerRef.current.offsetWidth;

		const newH =
			wavImageContainerRef.current.offsetHeight +
			(fsMetadata[1] < stateRef.current.baseWavImageH ? 0 : deltaHeight);
		const newL = wavImageContainerRef.current.offsetLeft - deltaWidth / 2;
		const newT =
			wavImageContainerRef.current.offsetTop -
			(fsMetadata[1] < stateRef.current.baseWavImageH
				? 0
				: deltaHeight / 2);
	...
	const calcZoom = function (z) {
		const newW = Math.round(
			stateRef.current.baseWavImageW + stateRef.current.baseWavImageW * z
		);
		const fsMetadata = getFullImageSize(metadata, newW);
		const newH =
			fsMetadata[1] > stateRef.current.baseWavImageH
				? fsMetadata[1]
				: stateRef.current.baseWavImageH;

		stateRef.current.zoomOffsetX =
			Math.round((newW - stateRef.current.baseWavImageW) / 2) * -1;
		stateRef.current.zoomOffsetY =
			Math.round((newH - stateRef.current.baseWavImageH) / 2) * -1;
	};

	const imageChange = (meta, imgDimensions) => {
		if (
			!stateRef.current.selectedWavCamId &&
			selectedWavCam.id !== stateRef.current.selectedWavCamId
		) {
			return;
		}
		...
		if (
			...
			currentSeqNum > stateRef.current.activeSeqNum
		) {
			const horzExtent = parseInt(meta["x-wavcam-horizontal-extents"]);
			const vertExtent = parseInt(meta["x-wavcam-vertical-extents"]);
			const pixHPerPixW = vertExtent / horzExtent;
			...
			winH =
				winH < stateRef.current.baseWavImageH
					? stateRef.current.baseWavImageH
					: winH;

			const left =
				((winW - stateRef.current.baseWavImageW) / 2) * -1 +
				stateRef.current.factoredOffsetX;
			const top =
				((winH - stateRef.current.baseWavImageH) / 2) * -1 +
				stateRef.current.factoredOffsetY; //+ stateRef.current.panOffsetY;
	...
	const collapseEntered = function (elem, isAppearing) {
		if (!stateRef.current.selectedWavCamId)
			stateRef.current.selectedWavCamId =
				wavCamPersistedState?.selectedWavCam
					? wavCamPersistedState.selectedWavCam
					: selectedWavCam.id;
		const meta =
			wavCamPersistedState && wavCamPersistedState.wavCamMetadata
				? wavCamPersistedState.wavCamMetadata[
						stateRef.current.selectedWavCamId
				  ]
				: null;
		...
		wavOverlayContainerRef.current.style.width =
			wavImageContainerRef.current.style.width = `${w}px`;
		wavOverlayContainerRef.current.style.height =
			wavImageContainerRef.current.style.height = `${h}px`;
		wavOverlayContainerRef.current.style.left =
			wavImageContainerRef.current.style.left = `${x}px`;
		wavOverlayContainerRef.current.style.top =
			wavImageContainerRef.current.style.top = `${y}px`;

		dispatch(
			startFOVItemStream(stateRef.current.selectedWavCamId, "wavcam")
		);
	...
	return (
		...
		>
			<div>
				<Drawer
					PaperProps={{ style: { right: 48, top: 48, height: 240 } }}
				...
				>
					...
					>
						<h4>
							<Translate value="global.dock.wavCam.main.wavCamOptions" />
						</h4>
						<List>
							<ListItem
								style={
							...
							>
								<ListItemText
									primary={getTranslation(
										"global.dock.wavCam.main.wavCamLabels"
									)}
							...
							<ListItem>
								<SelectField
									...
									label={getTranslation(
										"global.dock.wavCam.main.selectWavCam"
									)}
									handleChange={(e) => {
										resetState();
										dispatch(
											setSelectedWavCam(e.target.value)
										);
										stateRef.current.selectedWavCamId =
											e.target.value;
			...
			<div style={styles.containerStyle}>
				...
				<div style={styles.wavCamControlsContainer}>
					<div style={styles.wavCamControlsContainer}>
						<IconButton
							style={{ paddingTop: 12 }}
							onClick={() => dispatch(toggleWavCam())}
						>
