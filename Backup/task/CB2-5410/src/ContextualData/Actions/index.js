 ...
 * @param subscriptionId: ID of feed and related subscription
 */
const shouldUnsubscribeFeed = (subscriptions, subscriptionId, subscriberRef) => {
	return (dispatch) => {
		...
		if (!subscription) {
			...
			(subscription.subscribers.length === 1 && includes(subscription.subscribers, subscriberRef))
		) {
			if (subscription.subscription) {
				...
				isArray(channel)
					? each(channel, (channel) => dispatch(unsubscribe(channel)))
 ...
 * @param subscriberRef: Reference to component accessing a feed
 */
export const unsubscribeFromFeed = (contextId, subscriptionId, subscriberRef, forReplay) => {
	return (dispatch, getState) => {
		...
		if (!(context.trackHistory && subscriptionId === "entity")) {
			...
			if (dispatch(shouldUnsubscribeFeed(subscriptions, subscriptionId, subscriberRef))) {
				...
				if (size(subscriptions) <= 1 && !forReplay) {
...
export const removeProfileSubscription = () => {
	return (dispatch, getState) => {
		...
		each(contextualData, (context, contextId) => {
			if (!context.trackHistory && Object.values(selectedContext).includes(contextId)) {
				const subscriptionIds = keys(context.subscriptions);
				each(subscriptionIds, (subscriptionId) =>
