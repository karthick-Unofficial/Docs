...
const StatusCardDialog = ({ open, closeDialog, card, dir }) => {
	...
	return (
		<Dialog
			open={open}
			confirm={{
				label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.save"),
				action: () => save()
			}}
			abort={{
				label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.cancel"),
				action: () => cancel()
			}}
			deletion={{
				label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.delete"),
				action: () => deleteCard()
			}}
			options={{
				maxWidth: "xs",
		...
		>
			<div style={{ minHeight: "500px" }}>
				...
				<FormControlLabel
					...
					control={<Checkbox checked={global} onChange={() => setGlobal(!global)} />}
