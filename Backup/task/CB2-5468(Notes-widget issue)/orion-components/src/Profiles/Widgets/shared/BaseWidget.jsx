...
const BaseWidget = ({
	...
	return enabled ? (
		 ...
		 * profile layout
		 */
		<div style={expanded && !_.size(children) ? {} : styles.wrapper}>
			{!expanded && (
				<div style={styles.header}>
					{Boolean(icon) && icon}
					<Typography
						style={dir == "rtl" ? { marginLeft: "auto", marginRight: icon ? "1rem" : 0 } : { marginRight: "auto", marginLeft: icon ? "1rem" : 0 }}
						variant="subtitle1"
					>
						{title}
					</Typography>
					<div style={dir == "rtl" ? styles.headerButtonsRTL : styles.headerButtons}>
						{expandable && (
							<IconButton
								disableTouchRipple
						...
						{launchable && (
							<IconButton
								disableTouchRipple
								style={dir == "rtl" ? { paddingLeft: 0, width: "auto", color: "white" } : { paddingRight: 0, width: "auto", color: "white" }}
