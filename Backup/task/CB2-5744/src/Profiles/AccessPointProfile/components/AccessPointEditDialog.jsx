...
const AccessPointEditDialog = ({ accessPoint, close, update, open, dir }) => {
	const dispatch = useDispatch();
	const type = accessPoint ? accessPoint.entityData.properties.type : "";
	const accessPointId = accessPoint ? accessPoint.id : "";

	const [name, setName] = useState(
		accessPoint ? accessPoint.entityData.properties.name : ""
	);
	const [description, setDescription] = useState(
		accessPoint ? accessPoint.entityData.properties.description : ""
	);
	...
	const handleNameChange = (event) => {
		if (event.target.value.length > 50) {
			setError({
				...error,
				name: getTranslation(
					"global.profiles.accessPointProfile.Dialog.errorText.lessThanFifty"
				)
			});
			return;
		}
	...
	const handleSave = () => {
		if (!name) {
			setError({
				...error,
				name: getTranslation(
					"global.profiles.accessPointProfile.Dialog.errorText.noName"
				)
			});
			return;
		}
	...
	const actions = [
		...
		>
			{getTranslation(
				"global.profiles.accessPointProfile.Dialog.confirm"
			)}
		</Button>
	];
	...
	const dialogTitle = [
		...
		>
			{getTranslation(
				"global.profiles.accessPointProfile.Dialog.editAcc"
			)}
		</Typography>
	];
			...
			<div>
				<TextField
					...
					label={getTranslation(
						"global.profiles.accessPointProfile.Dialog.accName"
					)}
					value={name}
				...
				<TextField
					...
					variant="standard"
					label={getTranslation(
						"global.profiles.accessPointProfile.Dialog.desc"
					)}
					value={description}
				...
				<TextField
					...
					variant="standard"
					label={getTranslation(
						"global.profiles.accessPointProfile.Dialog.type"
					)}
					disabled={true}
					value={type}
				...
				<TextField
					...
					variant="standard"
					label={getTranslation(
						"global.profiles.accessPointProfile.Dialog.accId"
					)}
					disabled={true}
					value={accessPointId}
					style={textFieldStyles}
...
const componentUpdate = (prevProps, nextProps) => {
	if (
		prevProps.accessPoint.id !== nextProps.accessPoint.id ||
