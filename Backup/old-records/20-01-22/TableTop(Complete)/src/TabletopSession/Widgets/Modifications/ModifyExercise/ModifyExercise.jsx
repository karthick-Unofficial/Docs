...
const propTypes = {
	...
	closeModificationWidget: PropTypes.func.isRequired,
...
const ModifyExercise = ( { isController, userInfo, users, sessionId, simulations, simId, simTime, simTimePrecision, baseSimName, agents, agentGroups, simulationData,
	modifications, pencilsDownData, modificationsConfig, externalMessage, createModification, cancelModifications, clearComponentMessage, closeModificationWidget,
	...
	setModificationExclusiveModeOn, setModificationExclusiveModeOff, floorPlans } ) => {
	...
	const displayNotification = () =>{
		...
		if (!isController){
			...
			if (pencilsDown.endModifications){
				msgToDisplay = <Translate value="tableopSession.widgets.modifications.modifyExercise.contEnded"/>;
				return true;
			} 

			if (!pencilsDown.endModifications && pencilsDown.restartModification){
				msgToDisplay = <Translate value="tableopSession.widgets.modifications.modifyExercise.contRestarted"/>;
				return true;
			} 
			...
			if (getMyModification().length< 1 && getMyPendingModification().length <1 && getAccepted_RejectedModification().length <1){
				msgToDisplay = <Translate value="tableopSession.widgets.modifications.modifyExercise.contPaused"/>;
				return true;
			}
		}
		msgToDisplay ="";
	...
	const displayModifications = () =>{
		...
		if (!isController){
			if (getMyModification().length< 1 && getMyPendingModification().length <1 && getAccepted_RejectedModification().length <1){
				infoMsg = <Translate value="tableopSession.widgets.modifications.modifyExercise.noModTxt"/>;
				return false;
			} else{
				infoMsg ="";
				return true;
			...
			if (getNonControllerModifications().length<1){
				infoMsg = <Translate value="tableopSession.widgets.modifications.modifyExercise.noPlayerTxt"/> ;
				return false;
			} else {
				infoMsg ="";
				return true;
		...
		<div>
			...
			}
			{
				...
				<div className="modificationActionsRight">
					<Button style={{ marginRight:0}} onClick={handleModifyClose}>Cancel</Button>
					<Button variant="contained" color="primary" disabled={disabledSubmit()} 
						style={{ minWidth: 222, marginLeft:10, marginRight:10 }} 
						onClick={isController? submitExercise : submitModification}>{isController? <Translate value="tableopSession.widgets.modifications.modifyExercise.generateNewExercise"/>: <Translate value="tableopSession.widgets.modifications.modifyExercise.submitMod"/>}</Button>
				</div>
			}
			{
				...
				<div>
					{
						...
						<div className="modificationflexColumn" style={{marginTop:20}}>
							<div style={{width: "100%"}}>
								<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{<Translate value="tableopSession.widgets.modifications.modifyExercise.myMod"/>}</Typography>
							</div>
									...
									agents && getMyModification().map((modObj, index) => {
										return (
											<ModificationEntry key={`MyMod${index}`} entities={getEntitiesForTarget(modObj)} 
												...
												onAccept={null} onReject={null} onUndo={null}/>
										);
									})
								}
							</div>
					...
					}
					{
						...
						<div className="modificationflexColumn" style={{marginTop:20}}>
							<div style={{width: "100%"}}>
								<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{"My Pending Modifications"}</Typography>
							</div>
							<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
								{
					...
					}
					{
						...
						<div className="modificationflexColumn" style={{marginTop:20}}>
							<div style={{width: "100%"}}>
								<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{<Translate value="tableopSession.widgets.modifications.modifyExercise.acceptedRejectedMod"/>}</Typography>
							</div>
							<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
								{
					...
					<div className="modificationflex">
						<div style={{flex:1, marginRight: 10}}>
							<TextField
								...
								label={<Translate value="tableopSession.widgets.modifications.modifyExercise.newExerciseName"/>}
						...
						<div style={{width: 120, marginLeft: 10, marginRight: 10}}>
							<TextField
								id="txtstartTime"
								formControlStyles={{ margin: "0px 0px 0px 0px" }}
								label={<Translate value="tableopSession.widgets.modifications.modifyExercise.startTime"/>}
			...
			}
			{
				...
				<div>
					<div className="modificationflexColumn" style={{marginTop:20}}>
						<div style={{width: "100%"}}>
							<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}><Translate value="tableopSession.widgets.modifications.modifyExercise.controller" count={users[userInfo.userId].name}/></Typography>
						</div>
						<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
							{
								agents && getMyModification().map((modObj, index) => {
							...
							&& _.map(getNonControllerModifications()[0], (ncmModObj, key) => {
								return (
									<div key={`ncm-${key}`} className="modificationflexColumn" style={{marginTop:20}}>
										<div style={{width: "100%"}}>
											<Typography variant="body1" className="b2-gray" style={{textAlign:"left"}}>{key}</Typography>
										</div>
										<div style={{width: "100%", backgroundColor: "transparent", marginTop:10, marginBottom:10, paddingRight:10}}>
											{
												agents && _.map( ncmModObj, (modObj, index) => {
													return (
														<ModificationEntry key={`cm-MyMod${index}`} entities={getEntitiesForTarget(modObj)} 
															...
															onAccept={(comment) => onAccept(modObj, comment)} 
															onReject={(comment) => onReject(modObj, comment)} onUndo={() => onUndo(modObj)} />
													);
												})
											}
										</div>
			...
			<div>
				{ showEditor &&  (editorData.objectType.toUpperCase() !== "OBJECTIVE") && <ModificationEditorContainer 
			...
			{confirmCancel && 
				<ConfirmationDialog
					open={true}
					title=""
					content={<Translate value="tableopSession.widgets.modifications.modifyExercise.dialogContent"/>}
