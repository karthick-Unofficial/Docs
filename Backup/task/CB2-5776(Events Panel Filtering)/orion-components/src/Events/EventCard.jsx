...
const EventCard = (props) => {
	const { event, selectFloorPlanOn, getPinnedItems, profileIconTemplate, filterType, targetingEnabled } =
		props;
	...
	if (isTemplate) {
		...
		const dateTimeFormat = `MMM D, YYYY ${timeFormatPreference === "24-hour" ? "H" : "h"}:mm ${timeFormatPreference === "24-hour" ? "" : "A "
			}z`;
	...
	const styles = {
		...
		listItemIcon: {
			...
			alignItems: "center",
			...(dir === "ltr" && (filterType ? { marginRight: 12 } : isTemplate && { paddingRight: 12 })),
			...(dir === "rtl" && (filterType ? { marginLeft: 12 } : isTemplate && { paddingRight: 12 }))
		}
	}
	...
	return (
		<Collection
			...
			iconStyles={styles.listItemIcon}
		>
			{pinnedItems &&
