...
const MapObjectSelector = React.forwardRef(({id, definitionDesc, itemSelectionFor, enableItemSelectionOnMap, map, 
	controlDataTypes, value, agents, simulationData, facilities, floorPlans, itemSelectionHandler, mapClickedHandler, headerText, required, modificationMode, dir},
	 ref) => {
	const radioButtonclasses = useRadioButtonStyles();
	
	const getSelectorLabel =() =>{

		let labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.chooseItemTarget"/>;
		
		switch (controlDataTypes.toUpperCase()) {
			case "AGENT":
				labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.choosePlayer"/>;
				break;
			case "POSITION":
				{
					if (!headerText || headerText === ""){
						labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.chooseLocation"/>;
					} else {
						labelTmp = headerText;
					}	
				}
			...
			case "INTERDICTIONSITE":
				labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.chooseIntSide"/>;
				break;
			case "OBJECTIVE":
				labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.chooseObjective"/>;
				break;
			default:
				labelTmp = <Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.chooseItem"/>;
				break;
		}
			...
			<div>
				{
					...
					&& <MapMarker style={dir == "rtl" ? {textAlign: "right", width:20, height:26, marginRight:10, marginTop:8} : {textAlign: "left", width:20, height:26, marginLeft:10, marginTop:8}} />
				}
				{
	...
	return (
		<div id={id} className="modificationflexColumn" style={{backgroundColor:"#3C4656", borderRadius: 5}}>
			<div className="modificationflex" style={{width:"100%", justifyContent:"flex-start", alignItems: "center"}}>
				<MapMarkerRadius style={dir == "rtl" ? {textAlign: "right", width:20, height:26, marginRight:10} : {textAlign: "left", width:20, height:26, marginLeft:10}} />
				...
				<Button color="primary" onClick={(e) => enableItemSelection(e, id)} 
					style={dir == "rtl" ? {visibility:(id === itemSelectionFor)? "hidden": "visible", marginRight:10, marginLeft:20} : {visibility:(id === itemSelectionFor)? "hidden": "visible", marginLeft:10, marginRight:20}}
					size="small"
				>
					<MapMarkerPlus 
						style={dir == "rtl" ? {cursor: "pointer", width:20, height:26, marginLeft: 5} : {cursor: "pointer", width:20, height:26, marginRight: 5}}/>
					<Translate value="tableopSession.panels.modifications.modificationEditor.mapObjectSelector.selectOnMap"/>
				</Button>
			</div>
			{
				mapEntities &&
							...
							getMapEntities(mapEntities).map((ent) => {
								return (
									<div key={`div-${ent.id}`} className="modificationItemContainer" style={{marginTop:5, marginBottom:5}}>
										...
										<div style={{width:"100%"}}>
											<Typography variant="body1" className="b1-white" 
												style={dir == "rtl" ? {margin:10, textAlign:"right", flex:1, fontSize:16} : {margin:10, textAlign:"left", flex:1, fontSize:16}}>{ent.displayName}</Typography>
										</div>
									</div>
								);
							})
