...
EventModel.prototype.escalateEvent = async function (userId, id, translations) {
	try {
		...
		if (notification.escalationEvent) {
			...
			newEvent = await this.create(userId, orgId, event, null, translations.create);
			...
			if (entity) {
				const activity = {
					...
					object: activityModel.generateObject(
						entity.entityType,
						entity.id,
						await feedModel.getDisplayNameByEntity(entity),
						entity.feedId
					),
...
EventModel.prototype.addComment = async function (
	...
	try {
		...
		try {
			const result = await this.mockUpdate(userId, eventId, permission);
		} catch (err) {
