...
import {
	...
	restoreCollection,
	deleteCollection
} from "orion-components/SharedActions/commonActions";
...
const EntityCollection = (props) => {
		...
		const filteredMembers = pickBy(entities, (member) => {
			...
			if (entityData) {
				...
				properties.profileIconTemplate =
					profileIconTemplates[member.feedId];
	...
	const childSelected =
		selectedEntity && Object.keys(members).indexOf(selectedEntity.id) >= 0;
			...
			<Collection
				...
				secondaryText={
					...
					<Fragment>
						...
						{size(members) === 1 ? (
							<Translate
								value="listPanel.entityCollection.item"
								count={size(members)}
							/>
						) : (
							<Translate
								value="listPanel.entityCollection.items"
								count={size(members)}
							/>
						)}
					</Fragment>
