...
const ContextMenu = ({
	...
	const setupEntities = (features) => {
		...
		features.forEach((feature) => {
			if (!includes(feature.layer.id, "-cluster-count")) {
				if (includes(feature.layer.id, "-clusters"))
	...
	const addEntity = (entity) => {
		...
		if (entityType && config[entityType] && entity.type !== "FOV") {
			...
			if (
				currentEntities[entityType].filter((e) => e.id === entity.id)
					.length === 0
			)
			...
			return (
				<NestedMenuItem
					...
					label={
						...
						config[entityType].label == "Shapes" ||
						"Tracks" ||
						"Cameras" ||
						"AccessPoints"
							? getTranslation(
									`global.map.contextMenu.main.${config[entityType].label}`
							  )
					...
					dir={dir}
				>
					{orderBy(entities[entityType], ["name"], ["asc"]).map(
						(entity) => {
							return (
								<ContextMenuItem
									key={entity.id}
									onClick={() => loadProfileForEntity(entity)}
								>
									<ListItemIcon
										style={{
											color: "#ffffff",
	...
	const getIcon = (entity, entityType) => {
		...
		if (hasOwn(entity, propertyName)) {
			...
			config[entityType].types.forEach((t) => {
				if (
					t.propertyValue.toString().toLowerCase() ===
