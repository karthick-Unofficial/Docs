...
const ViewRule = (props) => {
	const { removeContext, closeDialog, openDialog, deleteRule, updateRule, orgUsers, userId, user, canManage, timeFormatPreference, WavCamOpen, dir, collections, isOpen, params, rules, addContext } = props;
	...
	useEffect(() => {
		...
		const filteredRule = rules.filter(rule => {
			if (rule.id === ruleId) {
				return JSON.parse(JSON.stringify(rule));
			}
		})[0];
		console.log("!!!", prevRule, "|", filteredRule);
		if (filteredRule && prevRule && (!prevRule.id || prevRule.id !== filteredRule.id)) {
			addContext(filteredRule);
