...
const cameraDock = (state = initialState, action) => {
	switch (action.type) {
		...
		case "CAMERA_DOCKED": {
			...
			dockedCameras[action.payload.cameraPosition] =
			...
			const newState = {
				...
				cameraPriority: {
					dockOpen:
						dockOpen !== null
							? dockOpen
							: state.cameraPriority.dockOpen,
					modalOpen:
						modalOpen !== null
							? modalOpen
							: state.cameraPriority.modalOpen
				}
			};
		...
		case "TOGGLE_FIND_NEAREST": {
			...
			if (
				state.findNearestMode[action.payload.cameraPosition] === false
			) {
