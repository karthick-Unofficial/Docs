...
const mapStateToProps = state => {
	...
	const accessPointData = context ? { accessPoint: context.entity, fov: context.fov } : {};
	let canEdit = false;
	const dialog = state.appState.dialog.openDialog;
	if (accessPointData.accessPoint) {
		canEdit = user.integrations
			&& user.integrations.find(int => int.intId === accessPointData.accessPoint.feedId)
			&& user.integrations.find(int => int.intId === accessPointData.accessPoint.feedId).permissions
			&& user.integrations.find(int => int.intId === accessPointData.accessPoint.feedId).permissions.includes("manage");
	}
