...
const EquipmentSharing = ({classes, userInfo, selfAgent, agentInfo, agents, simulationData, sessionId, simId, simTimePrecision, simTime, shareEvents, communications, equipmentConfig, onClose}) => {
	...
	return (
		<Container className={ classes.flexContainer} style={{width:isFacilitator()? 632: 722, marginLeft:-20}}>
			<div className={ classes.flexContainerHorizontalStart}>
				<div style={{marginLeft:15}}><AgentIcon agent={agentInfo} group={false} size={30}/></div>
				<div style={{marginLeft:0}}>
					<Typography variant="body1" className="b1-bright-gray" style={{marginLeft:10, marginTop: 5, color:"#b5b9be", height:16}}>
						{<Translate value="tableopSession.widgets.roster.equipmentSharing.chooseWeapons" count={agentInfo.entityData.properties.displayName|| agentInfo.entityData.properties.name}/>}</Typography>
				</div>
			</div>
			
			{/*Choose Equipments to notify*/}
			<div className={ classes.flexContainerHorizontalWrap} style={{width:"100%", paddingTop:0, paddingLeft:10, marginTop:10}}>
					...
					return (
						<Tooltip key={equip.libraryId} title={equip.name}>
							<div   
								...
								<div className={ classes.flexContainer} style={{marginTop:5, marginBottom:5, marginLeft:5, backgroundColor:"transparent"}}>
									<div style={{width: 22, height: 16, alignSelf: "center"}}>
										<img style={{width: 22}} src={equip.icon} className="b1-white"></img>
									</div>
									<Typography variant="body1" className="b1-white" style={{marginRight:5, width: 75, textAlign:"center"}}>{equip.category}</Typography>
								</div>
							</div>
						</Tooltip>
					);
			...
			<div className={ classes.flexContainer} style={{width:"100%", paddingTop:20, paddingLeft:10, marginTop:0}}>
				{
					!isFacilitator() && 
					<Typography variant="body1" className="b1-bright-gray" style={{marginLeft:10, color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.chooseCommDevices"/></Typography>
				}
				{
					isFacilitator() && <Typography variant="body1" className="b1-bright-gray" style={{marginLeft:10, color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.choosePlayers"/></Typography>
				}
							...
							<Table size="small" aria-label="a dense table">
								<TableHead>
									<TableRow>
										...
										<TableCell align="left" width="80" ><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.team"/></div></TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.player"/></div></TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.time"/></div></TableCell>
									</TableRow>
								</TableHead>
								<TableBody>
										...
										return (<TableRow key={`tr ${agnt.id}`}>
											...
											<TableCell align="left" className="viewTimeLink">
												{lastCommunicatedTime != null &&
													<div>
														<a className="viewTimes" style={{color: "#4db5f4", cursor: "pointer"}} onClick={(event) => handleToggleFacilitator(event, agnt.id)}><Translate value="tableopSession.widgets.roster.equipmentSharing.viewTimes"/></a>
						...
						<TableContainer className={classes.tableContainer}>
							<Table size="small" aria-label="comm sharing table">
								<TableHead>
									<TableRow>
										<TableCell> </TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.comm"/>&nbsp;<Translate value="tableopSession.widgets.roster.equipmentSharing.device"/></div></TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be", width:150}}><Translate value="tableopSession.widgets.roster.equipmentSharing.owner"/></div></TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.recipients"/></div></TableCell>
										<TableCell align="left"><div style={{color:"#b5b9be"}}><Translate value="tableopSession.widgets.roster.equipmentSharing.time"/></div></TableCell>
									</TableRow>
								</TableHead>
								<TableBody>
									{ownerCommDevices.map(comm => {
										...
										return (<TableRow key={`tr ${comm.id}`}>
											...
											<TableCell align="left">{
												<div style={{color: "#414449"}}>
													<List key={comm.id} component="nav" aria-label={"View recipients"}>
														...
														>
															<ListItemText primary={<span style={{color:"#4DB5F4", marginLeft:-12}}><Translate value="tableopSession.widgets.roster.equipmentSharing.viewRecipients"/></span>}/>
											...
											<TableCell align="left" className="viewTimeLink">
												{lastCommunicatedTime != null &&
													<div>
														<a className="viewTimes" style={{color: "#4db5f4", cursor: "pointer"}} onClick={(event) => handleToggleNonFacilitator(event, comm.id)}><Translate value="tableopSession.widgets.roster.equipmentSharing.viewTimes"/></a>
			...
			<Container className={classes.dialogActions}>
				<Button style={{ width: 74, height:41, marginLeft:10, marginRight:20}} onClick={onClose}><Translate value="tableopSession.widgets.roster.equipmentSharing.cancel"/></Button>
				<Button variant="contained" color="primary" disabled={disabledNotify()} style={{ width: 74, height:41, marginLeft:0, marginRight:10 }} onClick={handleNotify}><Translate value="tableopSession.widgets.roster.equipmentSharing.notify"/></Button>
			</Container>
