...
const ListDialogBody = props => {
	if (props.availableLists) {
		...
		const convertedCategories = reduce(props.availableLists, (result, list) => {
				...
				<List key="add-lists-category-section">
					{
						Object.values(occupiedCategories).map(category => {
							return (
								<Fragment>
									<p key={`add-lists-category-${category.id}-label`} style={{ color: "#fff" }}>{category.name}</p>
					...
					<p key={"add-lists-category-uncategorized-label"} style={{ color: "#fff" }}>{getTranslation("global.profiles.widgets.list.main.uncategorizedLists")}</p>
...
const ListWidget = forwardRef((props, ref) => {
	...
	const organizeSortedLists = () => {
		...
		let sortedLists = sortBy(Object.values(lists).filter(list => list.hasOwnProperty("index")), list => list.index);
