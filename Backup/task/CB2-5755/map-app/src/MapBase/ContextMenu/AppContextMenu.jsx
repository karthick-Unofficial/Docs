...
const AppContextMenu = ({ map }) => {
	...
	const trackHistoryDuration = useSelector((state) =>
		trackHistoryDurationSelector(state)
	);
	...
	const contextMenuOpening = (features, lonLat) => {
		...
		const track = features.find(
			(feature) => feature.properties.entityType === "track"
		);
		if (track) {
			const hasHistory =
				trackHistoryContextsRef.current.indexOf(track.properties.id) >
				-1;
			setTrack(track);
	...
	const spotlightsEnabled = useMemo(() => {
		return (
			!!spotlights &&
			Object.values(spotlights).filter((s) => !!s).length < 3
		);
	}, [spotlights]);
	...
	return (
		<Fragment>
			...
			>
				{apps["events-app"] && (
					<ContextMenuItem
						onClick={() => handleOpenDialog("new-event")}
					>
				...
				{track && (
					<ContextMenuItem
						...
						style={
							trackHistoryOn ? { color: "rgb(53, 183, 243)" } : {}
						}
					>
