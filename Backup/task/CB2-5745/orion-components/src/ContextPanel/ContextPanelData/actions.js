...
export const viewPrevious = (history) => {
	return (dispatch) => {
		...
		if (type === "gis") {
			dispatch(
				loadGISProfile(id, name, layerId, type, profileRef, context)
			);
		} else {
...
export const updateSelectedContext = (entityId, context = "secondary") => {
	return (dispatch, getState) => {
		const selectedContext =
			getState().appState.contextPanel.contextPanelData.selectedContext;

		// Get contexts that have already been set
		const setContexts = _.pickBy(
			selectedContext,
					...
					_.each(
						...
						(contextId) => {
							...
							if (getState().contextualData[contextId]) {
								_.each(
									_.keys(
										getState().contextualData[contextId]
											.subscriptions
									),
									(subscriptionId) =>
										dispatch(
...
export const loadProfile = (
	...
	return (dispatch, getState) => {
		...
		dispatch(clearSelectedEntity());
		if (
			shouldUpdateViewingHistory(
				contextPanelData.viewingHistory,
				entityId
			)
		) {
			dispatch(
				...
				if (err) console.log(err);
					...
					switch (response.type) {
						case "initial":
							...
							dispatch(
								setSelectedEntity(response.new_val, entityType)
							);
							break;
						case "change":
