...
const DistanceToolWrapper = ({ activePath, before }) => {
	const track = useSelector(
		(state) => feedEntitiesGeoByTypeSelector("track")(state),
		shallowEqual
	);
	const landUnitSystem = useSelector(
		(state) => state.appState.global.unitsOfMeasurement.landUnitSystem
	);
	const { baseMap, distanceTool, mapTools } = useSelector(
...
const DistanceTool = (props) => {
		...
		if (stateRef.current.editing === true) {
			...
			const features = _.filter(
				...
				(feature) =>
					...
					feature.properties.entityType === "track"
			);
			...
			const path = {
				...
				distance:
					currentPath.coordinates.length > 1
						? getDistance(currentPath.coordinates, unit)
						: 0,
				eta: getETA(segments),
				unit
			};
			...
			const newPath = {
				...
				tracks:
					features.length > 0
						? [
								...
								...stateRef.current.currentPath.tracks
						  ]
				...
				name: currentPath.name
					? currentPath.name
					: getTranslation("global.map.tools.distanceTool.path") +
					  (pathArray.length + 1),
				zoomLevel: isAdding
					? currentPath.zoomLevel
					: props.map.getZoom(),
				unit
			};
			stateRef.current = {
	...
	const updatePathEvent = (path) => {
		...
		const coordinates = path.coordinates.map((coord, index) => {
			const track = _.find(
				trackArr,
				(track) => track.id === path.tracks[index]
			);
			if (track) {
				update = true;
			...
			if (newPath) {
				...
				newPath.distance =
					coordinates.length > 1
						? getDistance(coordinates, unit || newPath.unit)
						: null;
	...
	const getSegments = (path, unit, trackData) => {
		if (path.length > 0) {
			const lineString =
				path.length > 1 ? tLineString(path) : tPoint(path[0]);
			let segmentsArr =
			...
			const trackSpeedMPH = trackData[0]
				? trackData[0].speed * 1.151
				: null;
			const segments =
				segmentsArr.length > 0
					? segmentsArr.map((feature, index) => {
							...
							const eta = trackSpeedMPH
								? (distance / trackSpeedMPH) * 60
								: null;
							const id = index;
							const segment = {
								id: id,
						...
						.map((path) => {
							const trackArr =
								_.compact(path.tracks).length > 0
									? _.compact(_.values(tracks)).filter(
											(track) => {
												return path.tracks.includes(
													track.entityData.properties
														.id
												);
											}
									  )
							...
							const pathVisible =
								path.zoomLevel - map.getZoom() > 4 ? 0 : 1;
							...
							const coordinateData = path.coordinates.map(
								(coord, index) => {
									...
									if (
										...
										track[0] &&
										path.tracks[index] ===
											track[0].entityData.properties.id
									) {
										const newCoords =
											track[0].entityData.geometry
												.coordinates;
										return {
											coords: newCoords,
											name:
												track[0].entityData &&
												track[0].entityData.properties
													? track[0].entityData
															.properties.name
													: track[0].id || null
										};
							...
							const points = coordinateData.map(
								({ coords, name }, index) => {
									...
									if (name)
										pointFeatures.properties["name"] = name;
									return pointFeatures;
								}
							);

							// Get all the line segments (for displaying distances)
							const pathSegments = getSegments(
								coordinates,
								path.unit,
								path.trackData
							);
									...
									? _.flatten(
											pathSegments.map((segment) => {
												...
												const bearing = segment.bearing
													? segment.bearing.toFixed(
															2
													  ) + "\xB0"
													: null;
												const etaString = segment.eta
													? `\nETA: ${segment.eta.toFixed(
															2
													  )} min`
													: "";
															...
															segment.id !== null
																? {
																		...
																		subtype:
																			segment.eta
																				? "EtaSegment"
																				: "Segment", // differentiate Paths from Segments when rendering layers
																		labelVisibility:
																			labelsVisible,
																		labelText: `${distance}\n${bearing}${etaString}`
																  }
			...
			{pathData && pathData.data.features && (
				<Fragment>
					...
					{!editing &&
						tooltip &&
						tooltipParent &&
						ReactDOM.createPortal(tooltip, tooltipParent)}
				</Fragment>
			)}
			{!toolType && (
				<UnitSelect
...
const MemoedDistanceTool = memo(DistanceTool, (prevProps, nextProps) =>
