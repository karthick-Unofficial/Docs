...
import { entityWidgetConfig, getCustomWidgetConfig } from "orion-components/Profiles/Selectors/EntityProfileSelectors";
...
const EntityProfile = ({ forReplay, appData, readOnly, endDate, selectFloorPlanOn, floorPlansWithFacFeed }) => {
	...
	const checkDrone = getCustomWidgetConfig("drone-association", context, integrations);
	...
	let disableEditAndDeleteSummaryAction = false;

	if (rules?.length > 0 && entityType === "shapes") {
		disableEditAndDeleteSummaryAction = true;
	}
	...
	const widgetState = useSelector(
		(state) => entity && entityWidgetConfig(state)(context, marineTrafficVesselData, marineTrafficVisible)
	);
		...
		if (canManage) {
			actions = [
				...actions,
				{
					...
					disabled: disableEditAndDeleteSummaryAction
				},
				{
	...
	const widgetRenderFilters = {
		details: !feedDisplayProps && entityType === "shapes",
		marineTrafficParticulars: !marineTrafficVesselData || !marineTrafficVisible,
		alerts: !notifications,
		rules: !canViewRules && isMarker
	};

	widgetRenderFilters["drone-association"] = checkDrone.length === 0;
			...
			case "details":
				return (
					<DetailsWidget
						...
						enabled={!widgetRenderFilters.details}
					/>
				);
			case "marineTrafficParticulars":
				return <MarineTrafficParticularsWidget id={widgetName} data={marineTrafficVesselData} enabled={!widgetRenderFilters.marineTrafficParticulars} />;
			case "cameras":
				return (
					<CamerasWidget
						id={widgetName}
			...
			case "drone-association":
				return (
					<DroneAssociation
						...
						contextId={contextId}
						enabled={!widgetRenderFilters["drone-association"]}
					/>
				);
			default:
				return null;
				...
				<div style={{ display: layoutControlsOpen ? "block" : "none" }}>
					<ErrorBoundary>
						<LayoutControls
							...
							profile="entity"
							layoutControlsOpen={layoutControlsOpen}
						/>
					</ErrorBoundary>
				</div>
