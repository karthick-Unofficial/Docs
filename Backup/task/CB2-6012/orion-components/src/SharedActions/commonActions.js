...
export const setWidgetOrder = (profile, widgets, autoExpand) => {
	return (dispatch, getState) => {
		const appId = getState().application.appId;
		const keyVal = { [profile]: { widgetOrder: widgets, layoutControls: { autoExpand } } };
		if (appId === "cameras-app") {
			dispatch(updatePersistedState("global", "profiles", keyVal, false));
		} else {
			dispatch(updatePersistedState("global", "profiles", keyVal));
		}
	};
};
					...
					if (!undoing) {
						...
						if (added[0]) {
							added.forEach((collection, index) => {
								addedMessage +=
									index !== added.length - 1 ? `${collection.name}, ` : `${collection.name}`;
							});
						}
						if (removed[0]) {
							removed.forEach((collection, index) => {
								removedMessage +=
									index !== removed.length - 1 ? `${collection.name}, ` : `${collection.name}`;
							});
						}
						...
						dispatch(createUserFeedback(completeMessage, undoFunc));
					}
				}
			}
		);
...
export const addRemoveFromEvents = (entityId, entityType, feedId, added, removed) => {
	return () => {
		eventService.updatePinnedEntities(entityId, entityType, feedId, added, removed, (err, response) => {
			if (err) {
				console.log(err, response);
			}
		});
		...
		entityCollection.deleteCollection(collectionId, (err, response) => {
			if (err) {
				...
				if (!undoing) {
					...
					dispatch(createUserFeedback(collectionName + " has been deleted.", undoFunc));
				}
			}
		});
	};
...
export const setWidgetState = (widgetName, widgetState) => {
	return (dispatch) => {
		const keyVal = { [widgetName]: widgetState };
		dispatch(updatePersistedState("global", "widgets", keyVal));
	};
};
