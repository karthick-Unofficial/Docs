					...
					entities.forEach(ent => {
						...
						appDataObj.appSpecifics.forEach(batch => {
							...
							if (entityType === "event") {
								...
								dispatch(dataReceived(ent, ent.feedId || "events", batch));
								if (entityType === "camera" && ent.entityData && ent.entityData.displayType === "facility") {
									const floorPlan = state.globalData && state.globalData.floorPlans ? state.globalData.floorPlans[ent.entityData.displayTargetId] : null;
									if (floorPlan) {
										dispatch(subscriptionDataBatchReceived(floorPlan.facilityId, [ent], "floorPlanCameras"));
									}
