...
const RobotCamerasWidget = ({
	...
	const getRobotLayoutCameras = (hideInVisibleCameras) => {
		...
		if (cameras && cameras.length > 0) {
			cameras.forEach((cam) => {
				...
				const robotCamera = robotCameras.find(
					(robotCamera) => robotCamera.id === camTmp.id
				);
				if (robotCamera) {
			...
			if (hideInVisibleCameras) {
				camerasInitial = camerasInitial.filter(
					(c) => c.isVisible === true
				);
			}
	...
	useEffect(() => {
		if (
			...
			(!robotCameras || robotCameras.length < 1)
		) {
			setExpandedCameras([]);
			setHideCameras(true);
		}
	...
	const blueprintCameraClick = (cameraId, cameraState) => {
		...
		const cameraStates = robotCameras.find(
			(robotCamera) => robotCamera.id === cameraId
		).states;
		if (
			(expandedCameras.includes(cameraId) &&
				cameraState === cameraStates[cameraStates.length - 1]) ||
			(!expandedCameras.includes(cameraId) &&
				cameraState !== cameraStates[cameraStates.length - 1])
		) {
	...
	const toggleCardExpand = (id) => {
		...
		if (cameraIndex < 0) {
			...
			if (robotCamera && robotCamera.states) {
				toggleCameraState(
					id,
					robotCamera.states[robotCamera.states.length - 1]
				);
			}
		}
	};
	...
	const getCameraCone = (
		cameraIndex,
		cameraState,
		frontLeftCameraState,
		frontRightCameraState
	) => {
		if (
			cameraIndex <= 1 &&
			frontLeftCameraState === frontRightCameraState
		) {
	...
	const setFrontLeftRightCameraState = () => {
		const camFrontLeft = layoutCameras.find(
			(c) => c.positionLookup === "frontLeft"
		);
		const camFrontRight = layoutCameras.find(
			(c) => c.positionLookup === "frontRight"
		);

		frontLeftCameraState = getCameraState(
			camFrontLeft ? camFrontLeft.id : null
		);
		frontRightCameraState = getCameraState(
			camFrontRight ? camFrontRight.id : null
		);
	};
	...
	const displaySensorsButton = useCallback(() => {
		return (
			<div
				...
				style={{ marginTop: hideBlueprint ? 10 : -5 }}
			>
				<div
					className={`icon-container ${
						hideCameras ? "b2-dark-gray" : "b2-white"
					}`}
					onClick={toggleCameraList}
				>
					<div
						className="icon"
						style={{ color: hideCameras ? "#828283" : "#4DB5F4" }}
					>
						<Video />
					</div>
			...
			!hideBlueprint && (
				<div className="camera-layout-container">
					...
					{getRobotCameras(true).map((camera) => {
						...
						const isOutside = isOutsideCamera(
							index,
							frontLeftCameraState,
							frontRightCameraState
						);
						...
						return (
							<Fragment key={camera.id}>
								<div
									className={`blueprint-camera ${
										CAMERA_POSITION_CLASS[camera.position]
									} ${isOutside && "outside"}`}
									style={{ zIndex: 2 }}
								>
									{getCameraCone(
									...
									<Videocam
										className={`camera-icon ${cameraState}`}
										fontSize="large"
									/>
									<p>{camera.label}</p>
								</div>
								...
								<div
									className={`blueprint-camera click-handler ${
										CAMERA_POSITION_CLASS[camera.position]
									} ${isOutside && "outside"}`}
									style={{ zIndex: 3 }}
									onClick={() =>
										blueprintCameraClick(
											camera.id,
											cameraState
										)
									}
				...
				<List id="camera-list">
					{camerasVisible &&
						...
						camerasVisible.map((camera) => {
							...
							const cameraIcon = CAMERA_ICONS[index] || (
								<Videocam fontSize="large" />
							);
							const isOpen = expandedCameras.includes(camera.id);
							const showCamera =
								camera &&
								(!hideCameras || (hideCameras && isOpen));
