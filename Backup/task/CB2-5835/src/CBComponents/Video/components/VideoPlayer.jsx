...
import { WSImagePlayer, WowzaWebRTCPlayer, WebRTCPlayer } from "orion-components/CBComponents";
...
const VideoPlayer = (props) => {
	...
	const cameraDock = useSelector((state) => state.appState?.dock?.cameraDock);

	const iscameraInDock =
		dockData.isOpen && dockData.tab === "Cameras" && cameraDock.dockedCameras.includes(camera.id);
	...
	const getPlayer = () => {
		...
		switch (type) {
			...
			case "flashphoner":
				return <WebRTCPlayer url={player.url} streamUrl={streamUrl} streamUrlError={streamUrlError} />;
			case "ws-jpeg":
				return (
					<WSImagePlayer
						cameraId={id}
