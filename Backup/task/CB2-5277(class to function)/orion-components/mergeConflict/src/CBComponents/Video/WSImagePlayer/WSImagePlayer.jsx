...
const WSImagePlayer = ({
	...
	useEffect(() => {
		...
		if (prevPropCameraId !== cameraId) {
			...
			if (playbackVideoId && prevPropPlaybackPlaying !== playbackPlaying) {
				if (playbackPlaying) {
		...
		if (prevPropCurrentReplayMedia !== currentReplayMedia && currentReplayMedia.length > 0 && playbackStartTime) {
			// -- restart playback video if the new media is not this camera
			const newMediaId = currentReplayMedia.filter(media => !prevPropCurrentReplayMedia.includes(media))[0];
