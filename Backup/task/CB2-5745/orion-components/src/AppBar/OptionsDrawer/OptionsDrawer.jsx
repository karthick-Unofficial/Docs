...
const OptionsDrawer = ({
	...
	const settings = useSelector(
		(state) => mapSettingsSelector(state),
		shallowEqual
	);
	const globalState = useSelector((state) => state.appState.global);
	const clientConfig = useSelector((state) => state.clientConfig);
	const { nauticalChartsEnabled, weatherEnabled } =
		size(clientConfig) && clientConfig.mapSettings;
	...
	const showAllFOVs = useSelector(
		(state) => persistedState(state).showAllFOVs || false
	);
	...
	const handleSliderChange = (event, value, layer) => {
		if (layer === "nauticalCharts") {
			dispatch(setLocalAppState("nauticalChartLayerOpacity", value));
		} else if (layer === "roadAndLabels") {
			dispatch(setLocalAppState("roadAndLabelLayerOpacity", value));
		} else if (layer === "weatherRadar") {
			dispatch(setLocalAppState("weatherRadarLayerOpacity", value));
	...
	const handleSliderChangeCommitted = (event, value, layer) => {
		if (layer === "nauticalCharts") {
			dispatch(
				updatePersistedState(appId, "nauticalChartLayerOpacity", {
			...
			dispatch(
				updatePersistedState(appId, "weatherRadarLayerOpacity", {
		...
		>
			...
			<List>
				{settingsMenu ? (
					<LayerToggle
						label={getTranslation(
							"global.appBar.optionsDrawer.mapLabels"
						)}
					...
					<ListItem sx={styles.listItem}>
						...
						<Switch
							...
							classes={{
								thumb: entityLabelsVisible
									? classes.thumbSwitched
									: classes.thumbOff,
								track: entityLabelsVisible
				...
				{cameraFOV && (
					<ListItem style={styles.textAlignRight}>
						<ListItemText
							primary={getTranslation(
								"mapAppBar.listItem.cameraFOV"
							)}
							primaryTypographyProps={{
								style: { fontSize: 16 },
								className: `option-label ${
									showAllFOVs ? "toggle-on" : "toggle-off"
								}`
							}}
						/>
						<Switch
			...
			<List>
				...
				{!nauticalChartsEnabled ? null : settingsMenu ? (
					<LayerToggle
						label={getTranslation(
							"global.appBar.optionsDrawer.nauticalCharts"
						)}
					...
					<>
						<ListItem sx={styles.listItem}>
							...
							<Switch
								...
								classes={{
									thumb: nauticalChartsVisible
										? classes.thumbSwitched
										: classes.thumbOff,
									track: nauticalChartsVisible
										? classes.trackSwitched
										: classes.trackOff
								}}
							/>
				...
				{settingsMenu ? (
					<LayerToggle
						label={getTranslation(
							"global.appBar.optionsDrawer.roadsLabels"
						)}
				...
				{!weatherEnabled ? null : settingsMenu ? (
					<LayerToggle
						label={getTranslation(
							"global.appBar.optionsDrawer.weatherRadar"
						)}
						updateKey="weather"
						withOpacity={true}
						dir={dir}
					...
					<>
						<ListItem sx={styles.listItem}>
							...
							<Switch
								...
								classes={{
									thumb: weatherVisible
										? classes.thumbSwitched
										: classes.thumbOff,
									track: weatherVisible
										? classes.trackSwitched
										: classes.trackOff
								}}
							/>
				...
				{ssrRadarOverlayEnabled && (
					<ListItem style={styles.textAlignRight}>
						<ListItemText
							primary={getTranslation(
								"mapAppBar.listItem.ssrRadar"
							)}
							primaryTypographyProps={{
								style: { fontSize: 16 },
								className: `option-label ${
									ssrRadarVisible ? "toggle-on" : "toggle-off"
								}`
							}}
						/>
						<Switch
							...
							classes={{
								thumb: ssrRadarVisible
									? classes.thumbSwitched
									: classes.thumbOff,
								track: ssrRadarVisible
									? classes.trackSwitched
									: classes.trackOff
							}}
						/>
				...
				<Collapse style={{ padding: "0px 16px" }} in={ssrRadarVisible}>
					<CBSlider
						...
						step={0.1}
						onChange={(e, value) =>
							handleSliderChange(e, value, "ssrRadar")
						}
						onChangeCommitted={(e, value) =>
							handleSliderChangeCommitted(e, value, "ssrRadar")
						}
					/>
				</Collapse>
			</List>
