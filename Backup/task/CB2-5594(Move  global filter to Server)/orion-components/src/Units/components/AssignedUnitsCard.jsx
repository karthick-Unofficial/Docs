...
import {
	Checkbox, FormControlLabel, Grid, Typography, TextField,
	MenuItem, IconButton, List, ListItem, SvgIcon, ListItemText,
	Popover, Divider
} from "@mui/material";
...
import { unitMemberSelector } from "../../GlobalData/Selectors";
...
const AssignedUnitsCard = ({
	...
	const unitMembers = useSelector(state => unitMemberSelector(state, unitData.id, feedSettings));
	const prevUnitMembers = usePrevious(unitMembers);
	const map = useSelector(state => state.replayMapState ? state.replayMapState.replayBaseMap.mapRef : state.mapState ? state.mapState.baseMap.mapRef : null);
	...
	const getUnitgeoFromMembers = useCallback(() => {
		...
		const ac2feedLayers = layers.filter((layer) => layer.source && layer.source.includes("ac2"));
		unitMembers.forEach((member) => {
			const layerByFeedId = ac2feedLayers.filter((layer) => layer.source && layer.source.includes(member.feedId));
			if (size(layerByFeedId)) geoArr.push(member.geometry);
	...
	const handleUnitStatusType = (e) => {
		...
		unitService.setUnitStatus(unit.id,
			status,
			(err, response) => {
				if (err) {
					console.log("ERROR:", err);
	...
	return (
		...
		<div className="unitsCardContainer"
			...
			<div className="unitsCardHeader">
				<Grid container>
					...
					<Grid item xs={2} sm={2} md={2} lg={2} style={styles.targetingICon}>
					...
					<Grid item xs={8} sm={8} md={8} lg={8} style={{ padding: "10px" }}>
						...
						<div style={styles.statusTypeContainer}>
							...
							>
								{unitStatusTypes && unitStatusTypes.map((type) => {
									return (
			...
			<div className="unitsMember" style={{ color: "#fff", paddingRight: "16px" }}>
				<UnitMemberRows unitMembers={unitMembers} memberRoleTypes={memberRoleTypes} />
			</div>
			<Popover
				open={!!anchorEl}
				anchorEl={anchorEl}
				...
				<List style={{ background: "#4A4D52" }}>
					<Fragment>
						...
						>
							...
							<ListItemText
								primary={getTranslation("global.units.components.assignedUnitsCard.edit")}
						...
						>
							...
							<ListItemText
								primary={getTranslation("global.units.components.assignedUnitsCard.delete")}
								style={styles.textAlignRight} />
						</ListItem>
					</Fragment>
				</List>
			...
			<Dialog
				...
				title={getTranslation("global.units.components.assignedUnitsCard.deleteUnit")}
				textContent={getTranslation("global.units.components.assignedUnitsCard.deleteConfirmationText")}
				confirm={{
					action: () => {
					...
					label: <Translate value="global.units.components.assignedUnitsCard.confirm" />
				}}
				abort={{
					action: () => {
						dispatch(closeDialog("deleteUnitDialog"));
					...
					label: <Translate value="global.units.components.assignedUnitsCard.cancel" />
				}}
			/>
		</div>
