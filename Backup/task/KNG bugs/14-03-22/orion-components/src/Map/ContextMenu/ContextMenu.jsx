...
import { Popover, Typography, ListItemIcon, ListItemText } from "@material-ui/core";
...
const ContextMenu = ({ 
	...
	const getProfileItems = () => {
		return [...Object.keys(entities)].sort().map( entityType => {
			return (
				<NestedMenuItem key={entityType} label={config[entityType].label == "Shapes" || "Tracks" || "Cameras" ? getTranslation(`global.map.contextMenu.main.${config[entityType].label}`)  : config[entityType].label} parentMenuOpen={!!anchorPosition} dir={dir}>
