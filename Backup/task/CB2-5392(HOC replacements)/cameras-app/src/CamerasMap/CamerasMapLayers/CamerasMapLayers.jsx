...
import { mapSettingsSelector, layerOpacitySelector, nauticalChartLayerOpacitySelector, roadAndLabelLayerOpacitySelector, weatherRadarLayerOpacitySelector } from "orion-components/AppState/Selectors";
import { primaryContextSelector, contextPanelState, selectedContextSelector } from "orion-components/ContextPanel/Selectors";
...
const MapLayerWrapper = ({ map, feedId }) => {
	...
	const mapLabel = _.capitalize(mapStyle);
...
const CamerasMapLayers = memo(props => {
	...
	useEffect(() => {
		...
		if (prevProps) {
			...
			if (prevProps.roadAndLabelLayerOpacity !== roadAndLabelLayerOpacity) {
				console.log("#roadsandprevProps", prevProps.roadAndLabelLayerOpacity);
	...
	const getGeoJSON = entityType => {
		...
		console.log("#geoJSONsource", source);
		return source;
	};
	...
	return (
		<div>
			...
			{/* Oceanographic  */}
			{!nauticalChartsEnabled ? null : nauticalChartsVisible && (
				<Source id="nauticalCharts" tileJsonSource={nauticalSource} />
			)}
			{!nauticalChartsEnabled ? null : (nauticalChartsVisible && currentZoomLevel >= 7) && (
				<Layer
					type="raster"
					id="nautical"
