...
import { ListItem, ListItemText, ListItemButton, FormControlLabel, Checkbox, Popover, MenuItem, Menu, Dialog, DialogTitle, DialogActions, Button, Fab, IconButton, Collapse } from "@mui/material";
...
const Main = ({ navigate }) => {
	...
	const styles = {
		...
		listItemText: {
			...(dir === "rtl" ? { padding: "16px 0px 16px 56px", textAlign: "right" } : { padding: "16px 56px 16px 0px" }),
			fontSize: 12,
			lineHeight: "16px",
			letterSpacing: 0
		},
		listItemButton: {
			...(dir === "rtl" ? { padding: "0 0 0 20px" } : { padding: "0 20px 0 0" })
		},
		checkBoxRoot: {
			padding: "0px",
			...(dir === "rtl" ? { marginLeft: "16px" } : { marginRight: "16px" })
		},
		paperProps: {
			width: "100%",
			borderRadius: "2px"
		...
		<div className="rules-wrapper-home">
			<div className="rules-filters" style={styles.ruleFilters}>
				...
				>
					...
					<Collapse in={collapse.userCheck}>
						{userChecklist}
					</Collapse>
				</ListItem>
			</div>
