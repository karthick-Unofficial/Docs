...
const SetScheduleDialog = ({
	...
	const [conditionIndefiniteValue, setConditionIndefiniteValue] =
		useState(true);
	...
	const [beforeTimeValue, setBeforeTimeValue] = useState(
		timeFormatPreference === "24-hour" ? "6:00" : "6:00 AM"
	);
	const [afterTimeValue, setAfterTimeValue] = useState(
		timeFormatPreference === "24-hour" ? "18:00" : "6:00 PM"
	);
		...
		if (editing) {
			if (priorityScheduleRow) {
				...
				if (schedule) {
					...
					const timeFormat =
						timeFormatPreference === "24-hour" ? "H:mm" : "h:mm A";
					if (!schedule.anyTimeOfDay) {
						if (schedule.startTime) {
							setBeforeTimeValue(
								MomentTZ.utc(schedule.startTime, "h:mm A")
						...
						if (schedule.endTime) {
							setAfterTimeValue(
								MomentTZ.utc(schedule.endTime, "h:mm A")
									.tz(MomentTZ.tz.guess())
									.format(timeFormat)
							);
						}
	...
	const handleSchedule = () => {
		const timeFormat =
			timeFormatPreference === "24-hour" ? "H:mm" : "h:mm A";
		const beforeTime = MomentTZ.tz(
			beforeTimeValue,
			timeFormat,
			MomentTZ.tz.guess()
		);
		const afterTime = MomentTZ.tz(
			afterTimeValue,
			timeFormat,
			MomentTZ.tz.guess()
		);
		if (
			beforeTimeValue.charAt(0) === ":" ||
			afterTimeValue.charAt(0) === ":"
		) {
			setDialogError(
				getTranslation(
					"global.profiles.widgets.facilityCondition.setSchedule.dialogError.validTimeValue"
				)
			);
			...
			setDialogError(
				getTranslation(
					"global.profiles.widgets.facilityCondition.setSchedule.dialogError.startEndTimeDiff"
				)
			);
		} else if (
			weekdaysValue.length === 1 &&
			beforeTime.isAfter(afterTime)
		) {
			//Check to see if start time is before the end time value, if there is only 1 day selected.
			setDialogError(
				getTranslation(
					"global.profiles.widgets.facilityCondition.setSchedule.dialogError.startBeforeEndTime"
				)
			);
		} else if (
			...
			setDialogError(
				getTranslation(
					"global.profiles.widgets.facilityCondition.setSchedule.dialogError.fallAfterStart"
				)
			);
		} else if (weekdaysValue.length === 0) {
			setDialogError(
				getTranslation(
					"global.profiles.widgets.facilityCondition.setSchedule.dialogError.atLeastOneDay"
				)
			);
		}
		//else if (
	...
	const dialogActions = [
		...
		>
			{getTranslation(
				"global.profiles.widgets.facilityCondition.setSchedule.cancel"
			)}
		</Button>,
		<Button
		...
		>
			{editing
				? getTranslation(
						"global.profiles.widgets.facilityCondition.setSchedule.update"
				  )
				: getTranslation(
						"global.profiles.widgets.facilityCondition.setSchedule.save"
				  )}
		</Button>
	];
	...
	return (
		...
		>
					...
					>
						<FormControlLabel
							...
							label={getTranslation(
								"global.profiles.widgets.facilityCondition.setSchedule.indefinite"
							)}
							sx={{ minWidth: "150px" }}
						/>
						<FormControlLabel
							...
							control={<Radio />}
							label={getTranslation(
								"global.profiles.widgets.facilityCondition.setSchedule.setRange"
							)}
							sx={{ minWidth: "150px" }}
						/>
					</RadioGroup>
					<div className="date-pickers-wrapper">
						...
						>
							<FormControlLabel
								...
								control={<Radio />}
								label={getTranslation(
									"global.profiles.widgets.facilityCondition.setSchedule.allDay"
								)}
								sx={{ minWidth: "150px" }}
							/>
							<FormControlLabel
			...
			<DialogActions sx={overrides.dialogActions}>
				{dialogActions}
			</DialogActions>
		</Dialog>
	);
