...
import { BatteryCharging50, ExpandMore } from "@mui/icons-material";
...
const STATE_STRINGS = {
	...
	stationary: getTranslation(
		"global.profiles.widgets.missionControl.stationary"
	),
	standby: getTranslation("global.profiles.widgets.missionControl.standby"),
	"in-motion": getTranslation(
		"global.profiles.widgets.missionControl.inMotion"
	),
	"on-mission": getTranslation(
		"global.profiles.widgets.missionControl.onMission"
	),
...
const MissionControlWidget = ({ order, enabled, details, dir }) => {
	useEffect(() => {
		// ***** TODO: anything we want to hook into on loading?
	}, []);

	const getStatusPanel = useCallback(() => {
		const { speed, grade, battery, batteryStatus, assignedMission } =
			details;

		const state =
			assignedMission && Object.keys(assignedMission).length > 0
				? "on-mission"
		...
		const slopeIcon =
			grade >= 0 ? (
				<SlopeUphill className={"detail-icon"} />
			) : (
				<SlopeDownhill className={"detail-icon"} />
			);
		const panelDetails =
			state !== "charging" ? (
				<div className="details-container">
					...
					<div className="detail">
						...
						>
							<Translate value="global.profiles.widgets.missionControl.ftPerSec" />
						</p>
					</div>
					<div className="detail">
		...
		return assignedMission ? (
			<div style={{ paddingTop: "10px" }}>
				<Accordion style={{ backgroundColor: "#34383C" }}>
					...
					<MCWAccordionDetails>
						<List>
							{assignedMission.waypoints.map(
								(waypoint, index) => {
									const waypointIcon = waypoint.reached ? (
										<WaypointGreen />
									) : (
										<WaypointGray />
									);
									const nextWaypoint =
										assignedMission.waypoints[index + 1];
									const nextWaypointReached = nextWaypoint
										? nextWaypoint.reached
										: false;
									const connectingLine = nextWaypoint
										? nextWaypointReached
											? "solid"
											: "dashed"
										: null;
										...
										<MCWListItem key={index}>
											...
											<ListItemText
												...
												primary={
													index === 0
														? getTranslation(
																"global.profiles.widgets.missionControl.waypointStatic"
														  )
														: getTranslation(
																"global.profiles.widgets.missionControl.waypointDynamic",
																index + 1
														  )
												}
											/>
	...
	return (
		<BaseWidget
			...
			title={getTranslation(
				"global.profiles.widgets.missionControl.missionControl"
			)}
			dir={dir}
		>
			{/* STATUS PANEL */}
