...
export const removeData = (id, feedId, batch, deleting = true) => {
	return (dispatch, getState) => {
		...
		const selectedEntity = appState && appState.contextPanel && appState.contextPanel.profile ? appState.contextPanel.profile.selectedEntity : null;
 ...
 */
export const subscribeData = (feedId, source) => {
	return (dispatch, getState) => {
		...
		feedService
			.subscribeGlobalFeed(feedId, (err, response) => {
				...
				if (response.type) {
					...
					if (removals.length) {
						_.each(removals, id => dispatch(removeData(id, feedId, batch)));
					}
					if (additions.length) {
