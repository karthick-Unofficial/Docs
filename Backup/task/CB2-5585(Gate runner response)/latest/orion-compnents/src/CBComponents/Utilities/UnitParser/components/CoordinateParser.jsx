	...
	if (targetUnit === "decimal-degrees") {
			...
			case "object":
				{
					...
					output = `${parseFloat(value[latKey]).toFixed(decimalPrecision)}9\xB0, ${parseFloat(value[lngKey]).toFixed(decimalPrecision)}9\xB0`;
				}
				break;
			default:
				break;
		...
		switch (typeof value) {
			...
			case "object":
				{
					const { lngKey, latKey } = getKeys(value);
					output = `${geo.toLat(value[latKey], format)}, ${geo.toLon(
						value[lngKey],
						format
					)}`;
				}
