...
export { collectionsSelector, makeGetCollection, makeGetCollectionMembers } from "../Collections/selectors";
...
export {
	...
	getEventsWithoutFiltersSelector,
	...
	sortedStarredTemplatesSelector
} from "../Events/selectors";
export { userExclusionSelector } from "../Exclusions/selectors";
export { getUnassignedMembers } from "../UnitMembers/selectors";
export { unitMemberSelector, getAllUnits, getUnits, unitMemberMemoized } from "../Units/selectors";
...
const userIdSelector = (state) => (state.session ? state.session.user.profile.id : null);
...
export const priorityNotificationSelector = createSelector(notifications, (notifications) => {
...
export const activeAlertsSelector = createSelector(notifications, (notifications) => {
	const { activeItemsById } = notifications;
	const alerts = _.map(
...
export const userIntegrations = createSelector(userFeedsSelector, disabledFeedsSelector, (userFeeds, disabled) => {
	const integrations = _.pickBy(userFeeds, (integration) => integration.canView && !disabled.includes(integration));
	return integrations;
});
...
export const feedEntitiesByTypeSelector = (entityType) =>
	createDeepEqualSelector(globalData, userIntegrations, (globalData, integrations) => {
		let entities = {};
		_.each(
			_.pickBy(
				integrations,
...
export const feedEntitiesGeoByTypeSelector = (entityType) =>
	createDeepEqualSelector(globalGeo, userIntegrations, (globalGeo, integrations) => {
		let entities = {};
		_.each(
			_.pickBy(
				integrations,
				(integration) => integration.entityType === entityType && globalGeo[integration.feedId]
			),
			(integration) => {
				entities = {
					...entities,
...
export const feedEntitiesWithGeoByTypeSelector = (entityType) =>
	createDeepEqualSelector(globalData, globalGeo, userIntegrations, (globalData, globalGeo, integrations) => {
		let entities = {};
		_.each(
			_.pickBy(
				integrations,
...
export const collectionMemberSelector = (members) => {
	return createDeepEqualSelector(feedEntitiesSelector, (entities) => {
		const filteredMembers = _.pickBy(_.cloneDeep(entities), (entity) => _.includes(members, entity.id));
		return filteredMembers;
	});
};

export const sharedEntitiesSelector = createSelector(userIdSelector, feedEntitiesSelector, (userId, entities) => {
...
// Data Type: Object
export const feedEntityGeoSelector = createSelector(globalGeo, userIntegrations, (globalGeo, integrations) => {
	let geoObj = {};
	_.each(
...
export const getEntityId = (state, props) => (props.entity ? props.entity.id : null);

export const getGeo = createSelector(feedEntityGeoSelector, getEntityId, (geos, id) => {
	// cSpell:ignore geos
	const geo = geos[id];
...
const globalDataSelector = createDeepEqualSelector(globalData, (globalData) => globalData);

export const feedDataSelector = createDeepEqualSelector(globalDataSelector, (global) => {
...
export const alertSelector = createDeepEqualSelector(notificationSelector, (notifications) => {
	return _.pickBy(notifications, (notification) => notification.isPriority && notification.viewed === false);
});
...
const getEntityById = (state, props) => {
	...
	const entity = state.appState["mapRef"].entities[feedId] ? state.appState["mapRef"].entities[feedId][id] : null;
	return entity;
};
...
export const userFeedsByTypeSelector = (entityType) =>
	createSelector(userFeedState, (userFeed) => {
		const items = _.filter(userFeed, (feed) => feed && feed.entityType === entityType);
		return items;
	});
...
export const getFeedEntitiesByProperty = (state, feedId, property, value) => {
	if (state.globalData[feedId] && state.globalData[feedId].data) {
		...
		const items = _.filter(globalData, (feed) => feed && feed.entityData.properties[property] === value);
		return items;
	}
};
