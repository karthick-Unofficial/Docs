...
class ListWidget extends Component {
	...
	onSortEnd = (oldIndex, newIndex) => {
		// Get new sorted list
		const sortedLists = arrayMove(this.state.sortedLists, oldIndex, newIndex);
	...
	getListCardProps = (item, cardStyles, forDialog, dialogProps) => {
		...
		return selected || !enabled ? (
			...
			>
				...
				<div className="widget-content">
					...
					{this.state.sortedLists && this.state.sortedLists.length > 0 ? (
						<SortableList
							...
							children={this.state.sortedLists.map((list, index) => {
								return <SortableItem index={index} key={list.id} passedProps={{ ...this.getListCardProps(list, cardStyles) }} dir={dir} locale={locale} />;
							})}
						/>
					) : null}
					...
					>
						...
						<ListDialogBody
							...
							getLookupValues={getLookupValues}
							cardStyles={cardStyles}
							getListCardProps={this.getListCardProps}
							user={user}
							lookupData={lookupData}
							expanded={expanded}
