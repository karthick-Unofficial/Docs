...
import TrackHistoryLayer from "./Layers/TrackHistoryLayer";
import RoadsAndLabels from "./Layers/RoadsAndLabels";
...
const MapWrapper = () => {
	...
	const settings = useSelector(
		(state) => !isEmpty(appState.persisted) && mapSettingsSelector(state)
	);
	const zoom = !isEmpty(appState.persisted)
		? settings.mapZoom
		: mapSettings.zoom;
	const center = !isEmpty(appState.persisted)
		? settings.mapCenter
		: mapSettings.center;
	const maxZoom = mapSettings.maxZoom || 18;
	const minZoom = mapSettings.minZoom || 2;
	const style = !isEmpty(appState.persisted)
		? settings.mapStyle
		: "satellite";
...
const Map = ReactMapboxGl({
	accessToken:
		"pk.eyJ1IjoibGNsYXJrc29uIiwiYSI6ImNpaWh2dzlnaDAwZmp1ZGtueXdpa2loNXcifQ.gcbR_GT1AD2kUmIxkSxseA" // cSpell:disable-line
});

const MapBaseMapboxGL = memo(
	(props) => {
		const {
			setMapReference,
			zoom,
		...
		const getStyle = useCallback(() => {
			if (baseMaps.length > 0) {
				const baseMapSelection = baseMaps.filter(
