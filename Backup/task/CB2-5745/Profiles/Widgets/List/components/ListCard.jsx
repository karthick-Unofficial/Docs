...
const ListCard = ({
	...
	const handleLookupDataRetrieval = () => {
		...
		for (let i = 0; i < list.columns.length; i++) {
			if (
				list.columns[i].type === "lookup" &&
				!lookupData[list.columns[i].lookupType] &&
				!lookingUp[list.columns[i].lookupType]
			) {
	...
	const handleCheckboxToggle = (order, id) => {
		...
		if (`${id}_user-completed` in rowData) {
			if (toggled) {
				_.find(newRows, (row) => row.order === order).data[
					id + "_user-completed"
				] = "";
			} else {
				_.find(newRows, (row) => row.order === order).data[
	...
	const handleNotesChange = _.debounce((text, row, column) => {
		const newRows = [...list.rows];
		_.find(newRows, (newRow) => newRow.order === row.order).data[
			column.id
		] = text;
		if (!list.targetId) {
			listService.updateList(list.id, { rows: newRows }, (err, res) => {
	...
	const getAvatar = () => {
		...
		if (
			...
			list.rows &&
			list.rows.length > 0 &&
			list.columns.sort((col) => col.order)[0].type === "checkbox"
		) {
			...
			if (firstColumn.type === "checkbox") {
				...
				if (numberIncomplete > 0) {
					return (
						<Avatar style={avatarStyle}>{numberIncomplete}</Avatar>
					);
				} else {
	...
	return list ? (
		<Card style={{ marginBottom: ".5rem" }} key={`list-card-${list.id}`}>
			{header}
			<Collapse
				in={expandedState}
				unmountOnExit
					...
					<Dialog
						...
						confirm={{
							label: getTranslation(
								"global.profiles.widgets.list.listCard.delete"
							),
							action: deleteList
						}}
						abort={{
							label: getTranslation(
								"global.profiles.widgets.list.listCard.cancel"
							),
