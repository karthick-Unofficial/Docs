...
import {
	cameraMapFeatures,
	accessPointMapFeatures
} from "orion-components/Map/Selectors";
import {
	mapSettingsSelector,
...
const MapLayerWrapper = ({ map, feedId }) => {
	let { entityLabelsVisible, roadsVisible, mapStyle } = useSelector((state) =>
	...
	const cameraFeatures = useSelector((state) =>
		cameraMapFeatures(contextId)(state)
	);
	const accessPointFeatures = useSelector((state) =>
		accessPointMapFeatures(contextId)(state)
	);
	...
	const roadAndLabelLayerOpacity = useSelector((state) =>
		roadAndLabelLayerOpacitySelector(state)
	);
	...
	(props) => {
		...
		Object.values(userFeedsSelector).forEach((feed) => {
			profileIconTemplates[feed.feedId] = jsonata(
				feed.mapIconTemplate ||
				"properties.(iconType & '_' & disposition)"
			);
		});
		...
		const addClickHandlers = () => {
			...
			const handlePolygonClick = (e, context = "secondary") => {
				if (mapState.mapTools.type !== "drawing") {
					...
					if (features.length) {
						...
						// Prioritize smallest
						const smallestArea = features.reduce(
							(previous, current) => {
								return previous.properties.area <
									current.properties.area
									? previous
									: current;
							}
						);
			...
			map.on("click", "ac2-unclustered-point", (e) =>
				handleEntityClick(e)
			);
			map.on("touchend", "ac2-unclustered-point", (e) =>
				handleEntityClick(e)
			);
			// Loads details for markers
			map.on("click", "ac2-unclustered-markers", (e) =>
				handleEntityClick(e)
			);
			map.on("touchend", "ac2-unclustered-markers", (e) =>
				handleEntityClick(e)
			);
			// Loads details for lines (solid, dashed, and dotted)
			map.on("click", "ac2-unclustered-lines", (e) =>
				handleEntityClick(e)
			);
			map.on("touchend", "ac2-unclustered-lines", (e) =>
				handleEntityClick(e)
			);
			map.on("click", "ac2-unclustered-lines-dashed", (e) =>
				handleEntityClick(e)
			);
			map.on("touchend", "ac2-unclustered-lines-dashed", (e) =>
				handleEntityClick(e)
			);
			map.on("click", "ac2-unclustered-lines-dotted", (e) =>
				handleEntityClick(e)
			);
			map.on("touchend", "ac2-unclustered-lines-dotted", (e) =>
				handleEntityClick(e)
			);
			// Loads details for cameras
			map.on("click", "ac2-unclustered-camera", (e) =>
				handleEntityClick(e, "primary")
			);
			map.on("touchend", "ac2-unclustered-camera", (e) =>
				handleEntityClick(e, "primary")
			);
			// Loads details for accessPoints
			map.on("click", "ac2-unclustered-accessPoint", (e) =>
				handleEntityClick(e, "primary")
			);
			map.on("touchend", "ac2-unclustered-accessPoint", (e) =>
				handleEntityClick(e, "primary")
			);
			// Loads details for polygons
			map.on("click", "ac2-unclustered-polygons", (e) =>
				handlePolygonClick(e)
			);
			map.on("touchend", "ac2-unclustered-polygons", (e) =>
				handlePolygonClick(e)
			);
			map.on("click", "ac2-unclustered-events", (e) =>
				handleEntityClick(e, "secondary")
			);
			map.on("touchend", "ac2-unclustered-events", (e) =>
				handleEntityClick(e, "secondary")
			);
		};
		...
		const setMapEntity = (item) => {
			...
			_.each(_.keys(byFeed), (feed) =>
				dispatch(setMapEntities({ [feed]: byFeed[feed] }))
			);
		};
			...
			if (prevProps) {
				if (mapName !== prevProps.mapName) {
					if (baseMaps.length > 0) {
						const selectedMap = baseMaps.filter(
							(element) => element.name === mapName
						)[0];
			...
			if (!mapState.mapTools.type) {
				_.each(sources, (id) => {
					const pointLayers = _.filter(
						layers,
						(layer) =>
							_.includes(layer, feedId) &&
							_.includes(layer, "-symbol")
					);
					const circleLayers = _.filter(
						layers,
						(layer) =>
							_.includes(layer, feedId) &&
							_.includes(layer, "-circle")
					);
					const lineLayers = _.filter(
						layers,
						(layer) =>
							_.includes(layer, feedId) &&
							_.includes(layer, "-line")
					);
			...
			const filteredItems = Object.values(items)
						...
						const type =
							item.entityType === "accessPoint"
								...
								: item &&
									...
									: item.entityType;
						return (
							type.toLowerCase() === entityType.toLowerCase() &&
							!item.parentEntity
						);
					}
				})
				...
				.map((item) => {
					...
					if (
						...
						item.entityData.properties.mapIcon =
							profileIconTemplates[item.feedId].evaluate(
								item.entityData
							);
					}
					...
					<div>
						...
						<Layer
							...
							layout={{
								...
								"text-field": entityLabelsVisible
									? "{name}"
									: "",
								"text-font": [
									"Open Sans Bold",
