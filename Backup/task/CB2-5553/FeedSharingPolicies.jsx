...
const FeedSharingPolicies = () => {
	...
	useEffect(() => {
		if (chosenIntegration && ecosystem && policies) {
			...
			let items = {};
			const getSelectedPolicy = getPolicies();
			ecosystem.forEach(org => {
				const policy = getSelectedPolicy[org.orgId];
				if (!policy) {
	...
	const styles = {
		...
		heading: {
			...(dir == "rtl" ? { marginRight: 18 } : { marginLeft: 18 })
		},
		formControlLabel: {
			...(dir == "rtl" ? { marginLeft: 50 } : { marginRight: 50 })
		},
		cb2FontPolicy: {
			...(dir == "rtl" && { flexDirection: "row-reverse" })
		},
		cb2FontPerm: {
			...(dir == "rtl" ? { marginTop: 35, textAlign: "right" } : { marginTop: 35 })
		},
		policyTerm: {
			...(dir == "rtl" ? { marginLeft: 45 } : { marginRight: 45 })
		},
		policyFormControlLabel: {
			...(dir == "rtl" ? { marginRight: -11, marginLeft: 16 } : {})
		}

	};

	const handlePolicyUpdate = (selectedIntegration, orgId) => e => {
		const policies = cloneDeep(newPolicies);
		...
		dispatch(savePolicies(
			...
			(err) => {
				if (err) {
					setPolicyState(newPolicies);
				}
			}
		));
	};

	const handleCheckUnlimited = (orgId, feedId) => {
		const policies = cloneDeep(newPolicies);
		policies[orgId].type === "always" ? policies[orgId].alwaysShared.unlimited = !policies[orgId].alwaysShared.unlimited
			: policies[orgId].eventSharingOnly.unlimited = !policies[orgId].eventSharingOnly.unlimited;
		setPolicyState(policies);
		dispatch(savePolicies(
			feedId,
			policies,
	...
	const handleAllowedPermissionsUpdate = (permission, checked, orgId, feedId) => {
		const policies = cloneDeep(newPolicies);

		if (!policies[orgId].allowedPermissions) policies[orgId].allowedPermissions = [];
		const permissionIndex = policies[orgId].allowedPermissions.indexOf(permission);
		if (checked) {
			if (permissionIndex === -1) policies[orgId].allowedPermissions.push(permission);
		}
		else {
			if (permissionIndex > -1) policies[orgId].allowedPermissions.splice(permissionIndex, 1);
		}
	...
	const handleDeactivateAll = (selectedIntegration) => {
		const policies = cloneDeep(newPolicies);
	...
	const handleStartDateChange = (date, orgId, feedId, updateNewPolicies, updateErrorState) => {
		const policies = cloneDeep(newPolicies);
	...
	const handleEndDateChange = (date, orgId, feedId, updateNewPolicies, updateErrorState) => {
		const policies = cloneDeep(newPolicies);
	...
	console.log("@@@", policyState, newPolicies);
				...
				<header style={{ marginBottom: 24 }}>
					<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
						...
						<Button style={{ textTransform: "none", fontSize: 12, color: "#4eb5f3" }}
							onClick={() => handleDeactivateAll(chosenIntegration)}
						>
							...
							return (
								<Fragment key={index}>
									<ListItem button={selected !== index ? true : false} onClick={selected !== index ? () => setSelected(index) : null}
										style={selected === index ? { ...styles.listItemExpanded, ...(index !== 0 ? { marginTop: 13 } : {}) } : { ...styles.listItem, ...(index !== 0 ? { marginTop: 24 } : {}) }}
									>
										<ListItemText
												...
												<div style={{
													...
													<FormControl component="fieldset" style={styles.formControl}>
														<RadioGroup
															...
															name="alertAudio"
															value={policyState[org.orgId] ? policyState[org.orgId].type : "never-shared"}
															style={{ flexDirection: "row" }}
															onChange={handlePolicyUpdate(chosenIntegration, org.orgId)}
														>
															<FormControlLabel
															...
															/>
															{chosenIntegration.sharingOptions.includes("always") &&
																<FormControlLabel
																	style={styles.formControlLabel}
																	control={
															...
															{chosenIntegration.sharingOptions.includes("event") &&
																<FormControlLabel
																	style={styles.formControlLabel}
																	control={
													...
													{policyState[org.orgId] && policyState[org.orgId].type !== "never-shared" && (
														<Fragment>
															<div style={styles.cb2FontPolicy} className="cb-font-b2"><Translate value="mainContent.manageOrganization.orgSettings.manageFeedSharing.radioLabel.policyTerm" /></div>
															<div style={{ display: "flex", alignItems: "baseline" }}>
																<div style={styles.policyTerm}>
																	<FormControlLabel
																		control={
																			<Checkbox
																				color="primary"
																				onChange={() => handleCheckUnlimited(org.orgId, chosenIntegration.feedId)}
																				checked={
																					policyState[org.orgId].type === "always" ?
																						policyState[org.orgId].alwaysShared.unlimited
																						:
																						policyState[org.orgId].eventSharingOnly.unlimited
																				}
																			/>
																		}
																		label={<Translate value="mainContent.manageOrganization.orgSettings.manageFeedSharing.radioLabel.unlimited" />}
																		style={styles.policyFormControlLabel}
																	/>
																</div>
																<div style={{ display: "flex", alignItems: "baseline" }}>
																	<DatePicker
																		...
																		fullWidth={false}
																		handleChange={(date) => {
																			handleStartDateChange(date, org.orgId, chosenIntegration.feedId, updateNewPolicies, updateErrorState);
																		}}
																		options={{
																	...
																	<DatePicker
																		...
																		fullWidth={false}
																		handleChange={(date) => {
																			handleEndDateChange(date, org.orgId, chosenIntegration.feedId, updateNewPolicies, updateErrorState);
																		}}
																		options={{
															...
															{(org._ecosystem ? chosenIntegration.externalEcoPermissionOptions : chosenIntegration.permissionOptions)
																&& (org._ecosystem ? chosenIntegration.externalEcoPermissionOptions : chosenIntegration.permissionOptions).length > 0 && (
																	<Fragment>
																		<div style={styles.cb2FontPerm} className="cb-font-b2"><Translate value="mainContent.manageOrganization.orgSettings.manageFeedSharing.radioLabel.allowedPerm" /></div>
																		<div style={{ display: "flex", alignItems: "baseline" }}>
																			{(org._ecosystem ? chosenIntegration.externalEcoPermissionOptions : chosenIntegration.permissionOptions).map(permission => {
																				const includePermission = policyState[org.orgId].allowedPermissions && policyState[org.orgId].allowedPermissions.includes(permission);
																				return (
																					<FormControlLabel
																						...
																						control={
																							<Checkbox
																								...
																								color="primary"
																								onChange={(e, checked) => {
																									handleAllowedPermissionsUpdate(permission, checked, org.orgId, chosenIntegration.feedId);
																								}}
																							/>
																						}
