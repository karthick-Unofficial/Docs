...
import { Card, CardContent, Collapse, ListItem, ListItemText, Button } from "@mui/material";
	...
	return (
		...
		>
			<ListItem style={{ backgroundColor: "#494d53" }}>
				<div style={{ padding: "12px" }}>{hasError ? <Alert /> : <Check />}</div>
				<ListItemText
					style={{
						fontSize: "14px",
						color: "#FFF",
				...
				{healthSystems.length ? (
					...
					>
						{expanded ? (
							<Translate value="global.dock.systemHealth.systemHealthCard.hide" />
						) : (
							<Translate value="global.dock.systemHealth.systemHealthCard.more" />
						)}
					</Button>
				) : null}
			</ListItem>
					...
					{healthSystems.map((system, index) => {
						return (
							...
							>
								...
								>
									{getTranslation(system.label)}
								</p>
								{system.checks.map((check) => {
									return <SystemHealthItem key={check.id} check={check} locale={locale} />;
								})}
