...
import { AccessPointProfile, CameraProfile, EntityProfile, EventProfile } from "orion-components/Profiles";
...
import { clearMapFilters, loadProfile, setWidgetLaunchData, selectWidget, updateSearchValue } from "./camerasListPanelActions";
import { getDir } from "orion-components/i18n/Config/selector";
import { selectedContextSelector, selectedEntityState, listPanelState, searchValueSelector, viewingHistorySelector } from "orion-components/ContextPanel/Selectors";
import { collectionsSelector, feedEntitiesByTypeSelector } from "orion-components/GlobalData/Selectors";
...
const CamerasListPanel = () => {
	...
	const cameras = useSelector(state => feedEntitiesByTypeSelector("camera")(state));
	const accessPoints = useSelector(state => feedEntitiesByTypeSelector("accessPoint")(state));
	...
	const profileLoaded = useSelector(state => !state.appState.loading.profileLoading && !!selectedContextSelector(state));
	...
	const renderProfile = mode => {
		switch (mode) {
			case "camera":
				return (
					<ErrorBoundary>
						<CameraProfile {...camerasActions} collections={collections} facilityOption={true} widgetsExpandable={true} widgetsLaunchable={false} disabledLinkedItemTypes={["facility"]} />
					</ErrorBoundary>
				);
			...
			case "track":
				return (
					<ErrorBoundary>
						<EntityProfile {...entityActions} collections={collections} widgetsLaunchable={true} />
					</ErrorBoundary>
				);
			case "event":
				return (
					<ErrorBoundary>
						<EventProfile {...eventActions} widgetsLaunchable={true} />
					</ErrorBoundary>
				);
			case "accessPoint":
				return (
					<ErrorBoundary>
						<AccessPointProfile facilityOption={true} widgetsExpandable={false} widgetsLaunchable={true} />
					</ErrorBoundary>
				);
				...
				<div id="cameras-list-wrapper" style={{
					...
					<div id="cameras-list" style={{
						...
						<SearchField
							...
							value={searchValue}
							filters={filterCount}
							handleClearFilters={() => dispatch(clearMapFilters())}
							dir={dir}
						/>
						{size(cameras) < 1 ? (
								...
								<section>
									<List>
										...
										{collections.map(collection => {
											...
											const cams = filter(members, member =>
												includes(Object.keys(cameras), member)
											);
