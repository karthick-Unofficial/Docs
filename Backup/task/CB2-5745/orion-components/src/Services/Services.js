...
const Services = ({ exclude, children }) => {
	...
	const [configReady, setConfigReady] = useState(
		exclude.indexOf("config") >= 0
	);
	const [applicationProfileReady, setApplicationProfileReady] = useState(
		...
		<Fragment>
			...
			{exclude.indexOf("applicationProfile") < 0 && (
				<ApplicationProfileService
					setReady={() => setApplicationProfileReady(true)}
				/>
			)}
			{exclude.indexOf("systemNotification") < 0 && (
