		...
		<Fragment>
			...
			>
				<Typography variant="h6">
					{selectedFloorPlan && !creating ? (
						<Translate value="drawingPanel.floorPlanForm.editFloorPlan" />
					) : (
						<Translate value="drawingPanel.floorPlanForm.createFloorPlan" />
					)}
				</Typography>
			...
			/>
			{!file && (
				<Dropzone accept="image/*" onDrop={(acceptedFiles) => handleUpload(acceptedFiles)}>
					{({ getRootProps, getInputProps }) => (
						<div {...getRootProps()}>
							<input {...getInputProps()} />
							<Button color="primary">
								<Translate value="drawingPanel.floorPlanForm.uploadFloorPlan" />
							</Button>
						</div>
					)}
				</Dropzone>
				...
				>
					...
					{selectedFloorPlan && !creating ? (
						<FloorPlan
							...
							coordinates={coordinates || selectedFloorPlan.geometry.coordinates[0]}
						/>
					) : (
			...
			{!creating && selectedFloorPlan && (
				<Button
					style={{ margin: "auto", width: "100%", color: "#F50057" }}
			...
			<Dialog
				...
				textContent={getTranslation("drawingPanel.floorPlanForm.dialog.confirmationText")}
				confirm={{
					action: () => {
