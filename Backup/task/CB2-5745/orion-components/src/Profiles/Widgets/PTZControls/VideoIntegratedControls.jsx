...
const VideoIntegratedControls = ({ instanceId, camera }) => {
	...
	const _onMouseWheel = (e) => {
		...
		const distance = Math.abs(
			Math.round(e.nativeEvent.deltaY / 100) / zoomFactor
		);
		if (distance !== 0) {
			const direction =
				(e.nativeEvent.deltaY < 0 ? -1 : 1) * zoomDirFactor;
			setZoom(distance);
