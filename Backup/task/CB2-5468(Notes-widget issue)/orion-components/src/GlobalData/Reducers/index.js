				...
				if (feedId === payload.feedId) {
					switch (type) {
						case "DATA_BATCH_RECEIVED": {
							...
							const newBatch = _.keyBy(data, key);
							...
							if (!newData) {
								...
								if (useQueue) {
									...
									return {
										...state,
										dataQueue: { ...update },
										dataByIdQueue: _.uniq([...state.dataByIdQueue, ...newBatchIds]),
									...
									return {
										...state,
										data: { ...update },
										dataById: _.uniq([...state.dataById, ...newBatchIds])
									};
								}
							}
						}
							...
							if (!newData) {
								...
								if (useQueue) {
									...
									return {
										...
										dataByIdQueue: _.uniq([...state.dataByIdQueue, data.id]),
										dataRemoveQueue: newDataRemoveQueue
									};
								} else {
									return {
										...
										dataById: _.uniq([...state.dataById, data.id])
									};
								}
							}
						}
						case "DATA_REMOVED": {
							...
							if (useQueue) {
								return {
									...state,
									dataRemoveQueue: _.uniq([...state.dataRemoveQueue, dataId])
								};
							} else {
								const newData = { ...state.data, ...state.dataQueue };
								const newDataById = _.filter([...state.dataById, ...state.dataByIdQueue], id => {
									return id !== dataId;
								});
						...
						case "RUN_QUEUE": {
							if (useQueue) {
								...
								const newData = { ...state.data, ...state.dataQueue };
								const newDataById = _.uniq([...state.dataById, ...state.dataByIdQueue]);
