...
const cameraProps = (state) => {
	const globalDataIds = Object.keys(
		state && state.globalGeo ? state.globalGeo : {}
	);
	let cameras = {};
...
const cameraGeo = (state) => {
	const globalGeoIds = Object.keys(
		state && state.globalGeo ? state.globalGeo : {}
	);
	let cameras = {};
...
export const floorPlanSelector = createSelector(floorPlan, (floorPlan) => {
	const { selectedFloor, coordinates, creating, preLoaded, image } =
		floorPlan || {};
	...
	return createDeepEqualSelector(
		...
		(context, cameraGeo, cameraProps, filters) => {
			...
			if (context && context.entity) {
				const { fov, fovItems, entity, fovEvents, linkedEntities } =
					context;
				mapFeatures = {
					[entity.id]: entity
				};
