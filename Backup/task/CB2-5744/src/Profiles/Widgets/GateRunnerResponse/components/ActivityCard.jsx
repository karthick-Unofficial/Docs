...
const ActivityCard = ({
	...
	const styles = {
		...
		lprCard: {
			textAlign: "center",
			...(selectedDetection
				? { width: "55%", background: "#1f1f21", padding: "5px 15px" }
				: { width: "45%" })
		},
		gate: {
			...
			<ListItem style={styles.activityCardWrapper}>
				...
				<div style={styles.selectedGateWrapper}>
					...
					>
						<Translate
							value={
								"global.profiles.widgets.gateRunnerWidget.activityCard.elapsedTime"
							}
						/>
					</Typography>
					{initialActivityDate ? (
			...
			>
				<div style={styles.lprCard}>
					...
					<Typography style={styles.timeAgo}>
						{moment(detection.CaptureDateTime)
							.locale(locale)
							.fromNow()}
					</Typography>
				</div>
