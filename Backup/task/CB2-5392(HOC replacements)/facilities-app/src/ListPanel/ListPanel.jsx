...
const renderProfile = mode => {
	switch (mode) {
		...
		case "accessPoint":
			return (
				<AccessPointProfile {...accessPointProfileActions} widgetsExpandable={false} widgetsLaunchable={true} />
			);
...
const ListPanel = ({ WavCamOpen }) => {
	...
	const { mapState, appState, session } = useSelector(state => state);
	...
	const { widgetLaunchData } = useSelector(state => state.userAppState);
	const allowImport = State.clientConfig ? State.clientConfig.allowImport : false;
	const floorPlans = useSelector(state => state.globalData && state.globalGeo ? state.globalData.floorPlans : undefined)
