...
const mapStateToProps = state => {
	...
	const { accessPoint } = context;
	const isLoaded = _.isObject(context) && accessPoint;
	...
	if (isLoaded) {
		const { orgId } = user;
		const { ownerOrg, entityType, feedId } = accessPoint;
		...
		return {
			accessPointCollections: collectionsSelector(state),
			user,
