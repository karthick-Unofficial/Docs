...
const getTransactions = async (
	...
	try {
		if (result && result.items) {
			...
			if (result.total === 0) {
				dispatch(transactionError(getTranslation("replay.replayScreen.transactionErrorMessage")));
			} else {
				dispatch(transactionError(null));
			}
		...
		try {
			...
			if (result && result.items) {
				...
				initialSnapshot.forEach((ent) => {
					ent["transactionTime"] =
						ent.entityType === "track"
							? ent.acquisitionTime
							: ent.lastModifiedDate
							? ent.lastModifiedDate
							: ent.lastModified;
				});
				const initialTimeGroups = groupBy(initialSnapshot, function (txn) {
...
export const getAlertTransactions = async (
	startDate,
	endDate,
	filter,
	alertTransactions,
