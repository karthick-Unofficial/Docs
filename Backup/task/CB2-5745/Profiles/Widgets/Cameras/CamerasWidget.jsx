...
const CamerasWidget = ({
	...
	const handleLaunch = () => {
		...
		if (et === "floorplan") {
			...
			const type = edProps
				? edProps.subtype || edProps.type || backupType
				: backupType;
			const displayName = (
				name ? name.replace("/", "%2F") : contextId
			).toUpperCase();
		...
		cameras.forEach((camera) => {
			...
			if (
				!(
					...
					camera.entityData.properties.features.includes("ribbon")
				)
			) {
				cameraService.moveGeo(
					camera.id,
					coordinates[1],
					coordinates[0],
					0,
		...
		<div className={`widget-wrapper collapsed ${"index-" + order} `}>
			{!isAlertProfile && (
				<div className="widget-header">
					...
					{["camera", "shapes", "track", "accessPoint"].includes(
						entityType
					) &&
						canLink && (
							<div
			...
			<div className="widget-content" style={{ display: "flex" }}>
				{cameras && cameras.length > 0 ? (
					...
					<Typography
						style={{ margin: "12px auto", color: "#fff" }}
						align="center"
						variant="caption"
					>
			...
			{["camera", "shapes", "track", "accessPoint"].includes(
				...
				<LinkDialog
					...
					title={getTranslation(
						"global.profiles.widgets.cameras.main.linkCams"
					)}
					closeDialog={closeDialog}
					entity={entity}
