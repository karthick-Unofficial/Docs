...
const FloorPlan = (props) => {
	...
	const { setCoordinates } = actionCreators;

	const { image, id, editing, before } = props;

	const floorPlan = useSelector((state) => state.floorPlan);
	const mapState = useSelector((state) => state.mapState);
	const map = mapState.baseMap.mapRef;
	const coordinates = props.coordinates ? props.coordinates : floorPlan ? floorPlan.coordinates : null;
	const imgSrc = image
		? image
		...
		const addFloorPlan = () => {
			...
			img.onload = () => {
				...
				if (
					...
					area(bBoxPoly([...Object.values(_ne), ...Object.values(_sw)])) < area(bBoxPoly([...dimA, ...dimC]))
				) {
				...
				if (!coordinates) {
					...
					dispatch(setCoordinates(coordinates.length < 5 ? [...coordinates, coordinates[0]] : coordinates));
				}
				const sourceData = {
					type: "image",
					url: imgSrc,
