...
const trackMovementBuilder = (rule, collections, targetAction, hasLinks, dispatch) => {
	...
	return (
		<div id="rule-statement" className="cb-font-b9">
			<Translate value="global.ruleBuilder.trackMovementBuilder.alertMe" />{_getSubjects(rule, collections, targetAction, hasLinks, dispatch)}
			<span>
				{" " + rule.trigger}
				{rule.trigger === "cross" ? "es " : "s "}
			</span>
			{_getTargets(rule, collections, targetAction, hasLinks, dispatch)}
			{_getConditions(rule, collections, targetAction, hasLinks)}
		</div>
	);
};

const _getSubjects = (rule, collections, targetAction, hasLinks, dispatch) => {
