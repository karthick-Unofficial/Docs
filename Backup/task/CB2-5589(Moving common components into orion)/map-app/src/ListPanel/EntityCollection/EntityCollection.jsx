...
import { activeAlertsSelector, sharedEntitiesSelector, makeGetCollection, makeGetCollectionMembers, userFeedsSelector } from "orion-components/GlobalData/Selectors";
...
const EntityCollection = memo((props) => {
	...
	const entities = useSelector(state => props.id === "shared" ? sharedEntitiesSelector(state) : getCollectionMember(state, props));
	const manageable = useSelector(state =>
		props.id !== "shared" && state.session.user.profile.applications &&
			...
			<Collection
				...
				secondaryText={
					...
					<Fragment>
						...
						{size(members) === 1 ? <Translate value="listPanel.entityCollection.item" count={size(members)} /> : <Translate value="listPanel.entityCollection.items" count={size(members)} />}
					</Fragment>
				}
