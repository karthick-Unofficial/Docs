...
const NewNotificationItem = ({ clearNotification, notification, timeFormatPreference, firstPriority }) => {
	...
	useEffect(() => {
		...
		clearItem = setTimeout(() => dispatch(clearNotification(notification.id)), 5000);
	}, []);
	...
	const handleUndo = () => {
		...
		setTimeout(() => dispatch(clearNotification(notification.id)), 5000);
	};
	...
	const { createdDate, summary, isPriority, feedback, undoFunc } = notification;
		...
		>
				...
				<p className="time">
					{isMoreThanTwoDaysAgo
						? timeConversion.convertToUserTime(
								createdDate,
								`full_${timeFormatPreference ? timeFormatPreference : "12-hour"}`
						  )
			...
			{!!undoFunc && (
				<div className="undo-button">
					<Button onClick={undone ? null : handleUndo} variant="text" style={{ color: "#fff" }}>
						{undone
							? getTranslation("global.dock.shared.newNotificationItem.cancelled")
							: getTranslation("global.dock.shared.newNotificationItem.undo")}
					</Button>
				</div>
			)}
		</div>
