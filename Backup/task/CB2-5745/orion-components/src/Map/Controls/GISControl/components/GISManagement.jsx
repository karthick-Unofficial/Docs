...
const GISManagement = ({
	...
	const handleChange = (name, field, dynamicSetState) => (event) => {
		...
		if (dynamicSetState) {
			...
			setState((prevState) => ({
				...prevState,
				[name]: { ...state[name], [field]: event.target.value }
			}));
		}
	};
		...
		<Dialog
			confirm={{
				label: getTranslation(
					"global.map.controls.gisControl.gisManagement.save"
				),
				action: handleSave,
				disabled:
			...
			abort={{
				label: getTranslation(
					"global.map.controls.gisControl.gisManagement.cancel"
				),
				action: handleCancel
			}}
			deletion={{
				label: getTranslation(
					"global.map.controls.gisControl.gisManagement.delete"
				),
				action: handleDelete
			}}
			open={open}
		...
		>
			<div style={{ width: isXS ? "auto" : 350 }}>
				<TextField
					...
					label={getTranslation(
						"global.map.controls.gisControl.gisManagement.fieldLabel.serviceName"
					)}
				...
				<TextField
					id="url"
					label={getTranslation(
						"global.map.controls.gisControl.gisManagement.fieldLabel.esriServiceEndpoint"
					)}
					...
					helperText={getTranslation(
						"global.map.controls.gisControl.gisManagement.helperText.exampleURL",
						"http://services.arcgis.com/ArcGIS/rest/services/Port/MapServer"
					)}
					dir={dir}
				/>
				<SelectField
					id="auth-select"
					label={getTranslation(
						"global.map.controls.gisControl.gisManagement.fieldLabel.authType"
					)}
				...
				{authenticationState.type === "login" && (
					<Fragment>
						<TextField
							id="username"
							label={getTranslation(
								"global.map.controls.gisControl.gisManagement.fieldLabel.username"
							)}
						...
						<TextField
							id="password"
							label={getTranslation(
								"global.map.controls.gisControl.gisManagement.fieldLabel.password"
							)}
				...
				{authenticationState.type === "token" && (
					<TextField
						id="token"
						label={getTranslation(
							"global.map.controls.gisControl.gisManagement.fieldLabel.token"
						)}
