...
const Reports = () => {
	...
	const categories = useSelector((state) =>
		state.globalData.reportCategories.filter((cat) => {
	...
	const generateList = (category, index) => {
		...
		const header =
			locale === "en"
				? getTranslation(`mainpage.list.${category}`).toUpperCase()
				: getTranslation(`mainpage.list.${category}`);
		const filteredTypes = reportTypes
			.filter((report) => {
				if (!typeAheadFilter) return report;
				else return report.name.toLowerCase().includes(typeAheadFilter.toLowerCase());
			})
			.filter((report) => {
