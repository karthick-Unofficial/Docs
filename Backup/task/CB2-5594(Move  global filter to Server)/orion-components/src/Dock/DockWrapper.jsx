...
const DockWrapper = (props) => {
	...
	const cameraIntegrations = useSelector(state => userIntegrationsOfEntityTypeSelector("camera")(state), shallowEqual);
	const userCameras = useSelector(state => userCamerasSelector(state));
	const appId = useSelector(state => state.appId);
	const user = useSelector(state => state.session.user);
	const { profile, firstUseText, sessionEnded } = user;
	const { firstUseAck, id } = profile;
	const hasSysHealthError = useSelector(state => state.systemHealth.hasHealthError);
	const externalSystems = useSelector(state => (state.session && state.session.organization && state.session.organization.externalSystems) || []);
	...
	const getUnitPanel = (appName) => {
		switch (appName) {
			...
			case "facilities-app":
			case "reports-app":
			case "status-board-app":
			case "lists-app":
			case "cameras-app":
			case "camera-wall-app":
			case "replay-app":
				return true;
	...
	const alerts = useMemo(() => openNotifications.filter(item => {
		return item.isPriority;
	}), [openNotifications]);

	const nonPriority = useMemo(() => openNotifications.filter(item => {
		return !item.isPriority;
	}), [openNotifications]);

	const userHasWavCameras = useMemo(() => userCameras.some(cam =>
			...
			<ThemeProvider theme={createTheme(customTheme)}>
				<div className={dir && dir === "rtl" ? "alert-sidebar-trayRTL" : "alert-sidebar-tray"}>
					<Dialog
						...
						confirm={{ label: getTranslation("global.dock.dockWrapper.ok"), action: () => dispatch(logOut()) }}
						dir={dir}
						dialogContentStyles={{
							color: "#fff"
						}}
					...
					<Dialog
						...
						abort={{ label: getTranslation("global.dock.dockWrapper.decline"), action: () => dispatch(logOut()) }}
						dir={dir}
					/>
					...
					{getUnitPanel(appId) && unitsApp
						&&
						unitsApp.permissions.includes("manage")
						&&
						<a
					...
					{userHasCameras && (
						...
						>
							<Videocam style={{ width: "24px", height: "24px", color: "#FFF" }} />
						</a>
					)}
