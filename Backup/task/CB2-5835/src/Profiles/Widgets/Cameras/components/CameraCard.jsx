...
const propTypes = {
	...
	handleCardExpand: PropTypes.func,
...
const CameraCard = ({
	...
	const dockedCameras = useSelector((state) => entity ? state.appState.dock?.cameraDock?.dockedCameras : []);
	...
	const handleHeaderClick = (e, camera) => {
		...
		if (secondaryLoadProfile) {
			dispatch(loadProfile(id, entityData.properties.name, "camera", "profile", "secondary"));
		} else {
	...
	const handleSlewLock = (e, locked) => {
		...
		locked
			...
			: cameraService.setSlewLock(id, contextId, entityType, geometry, (err, response) => {
				if (err) {
					console.log(err, response);
				} else {
					// -- force rerender to pick up latest camera value
	...
	const slewControls = !disableSlew ? (
		<div style={styles.marginAuto}>
			{!slewLock && (
				<Button onClick={(e) => handleSlew(e)} color="primary" style={{ textTransform: "none" }}>
					<Translate value="global.profiles.widgets.cameras.camCard.slew" />
				</Button>
			)}
			<IconButton style={{ padding: 0 }} onClick={(e) => handleSlewLock(e, slewLock)}>
	...
	return (
		<Card style={{ borderRadius: 0, marginBottom: 12 }}>
			<ListItem button={canExpand} onClick={() => handleCardExpand(id)} style={styles.cardExpand}>
				{canTarget && (
					<TargetingIcon
								...
								{!presetsOpen ? (
									...
									<Fragment>
										...
										>
											...
											<ListItemText
												primary={getTranslation("global.profiles.widgets.cameras.camCard.back")}
												style={styles.textAlignRight}
											/>
											<ListItemSecondaryAction
												style={styles.listItemSecondaryAction}
