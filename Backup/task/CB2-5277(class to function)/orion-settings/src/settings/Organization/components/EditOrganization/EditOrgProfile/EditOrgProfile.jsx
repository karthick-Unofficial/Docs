...
const handleSubmit = async (values, stagedFile, org, updateOrg, updateDisplayErrors) => {
	// clear old errors and recheck if they are still valid errors
	const newDisplayErrors = {};
	let errors = false;
	if (!values.organizationName) {
		newDisplayErrors.organizationName = getTranslation("mainContent.manageOrganization.orgProfile.editOrganization.errorMessage.emptyName");
		errors = true;
	}
	if (values.organizationDescription.length > 50) {
		newDisplayErrors.organizationDescription = getTranslation("mainContent.manageOrganization.orgProfile.editOrganization.errorMessage.descTooLong");
	}
	if (errors) {
	...
	if (org.orgId) {
		...
		updateDisplayErrors({});
		updateOrg(org.orgId, data, stagedFile);
		...
		<EditPageTemplate title={getTranslation("mainContent.manageOrganization.title")} subTitle={getTranslation("mainContent.manageOrganization.orgProfile.editOrganization.titleDesc")} dir={dir}>
			<ArticleContainer headerTitle={getTranslation("mainContent.manageOrganization.orgProfile.editOrganization.formTitle")} headerDescription={""} editing={true} >
				<Fragment>
					...
					<Container style={dir == "rtl" ? { textAlign: "left", paddingLeft: 0, paddingRight: 0 } : {
						textAlign: "right",
						paddingLeft: 0,
						paddingRight: 0
					}}>
						...
						<Button
							...
							onClick={() => {
								updateDisplayErrors({});
								handleSubmit(values, stagedFile, org, updateOrg, updateDisplayErrors);
							}}
						>
