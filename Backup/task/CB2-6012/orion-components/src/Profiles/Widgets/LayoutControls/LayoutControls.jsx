...
const SortableList = SortableContainer(({ items, enable, disable, expandedWidget, disabled = false }) => {
	const isWidgetExpanded = (widget) => {
		if (expandedWidget === "map-view") {
			...
			return expandedWidget === widget.id;
		}
	};
...
//TODO: Move all the translation data to its required parent object. Specifically, every translation for the below function should be provided in the LayoutControls object of profile in the global.json file of translation-app. Please refer to the activities case as an example. Similarly, for every other case, the translations should be moved accordingly to the layoutControls component.
const getWidgetName = (id) => {
	switch (id) {
		case "linked_items":
			return getTranslation("global.profiles.cameraProfile.linkedItems");
		case "activities":
			return getTranslation("global.profiles.eventProfile.main.activities");
		case "live_camera":
...
const LayoutControls = ({ widgetOrder, close, expandedWidget, profile, layoutControlsOpen }) => {// using layoutControlsOpen prop to rerender component on view change
	const dispatch = useDispatch();
	const dir = useSelector((state) => getDir(state));
	const widgetState = useSelector((state) => widgetStateSelector(state)?.layoutControls) || {};
	...
	const handleSaveOrder = () => {
		...
		dispatch(setWidgetOrder(profile, widgets, autoExpand));
		close();
	};
	...
	const styles = {
		...
		formControlLabel: {
			color: "#fff",
			...(dir === "ltr" && { margin: "10px 16px 0px -11px" }),
			...(dir === "rtl" && { margin: "10px -11px 0px 16px" }),
			fontSize: 11
		}
	};
			...
			<div className="layout-button-wrapper" style={styles.layoutButtonWrapper}>
				<div style={styles.titleWrapper}>
					<Typography style={styles.title}>
						<Translate
							value="global.profiles.widgets.layoutControls.title"
							count={getTranslation(`global.profiles.widgets.layoutControls.${profile}`)}
						/>
					</Typography>

					<div style={styles.explanation}>
						<Translate value="global.profiles.widgets.layoutControls.explanation" />
						<br />
						<Translate
							value="global.profiles.widgets.layoutControls.note"
							count={`"${getTranslation(`global.profiles.widgets.layoutControls.${profile}`)}"`}
						/>
					</div>
...
const componentUpdate = (prevProps, nextProps) => {
	if (!isEqual(prevProps.widgetOrder, nextProps.widgetOrder)) {
		...
		return false;
	} else if (!isEqual(prevProps.layoutControlsOpen, nextProps.layoutControlsOpen)) {
		return false;
	} else {
		return true;
	}
