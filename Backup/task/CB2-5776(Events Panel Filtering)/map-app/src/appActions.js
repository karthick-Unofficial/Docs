...
export { getAppState, toggleMapVisible, getGlobalAppState } from "orion-components/AppState/Actions";
export {
	subscribeFeed,
		...
		spotlightService.getAllActive((err, result) => {
			if (err) {
				...
				if (result.spotlights.length) {
					dispatch(
						openDialog("resumeSpotlightDialog", {
							activeSpotlights: result.spotlights
						})
					);
				}
			}
