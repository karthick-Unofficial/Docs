...
const SpotlightChip = ({ spotlight, index, handleClick, cameras, setSpotlight, user, dir }) => {
	...
	if (mounted) {
		...
		const windowReference = new NewWindow({
			...
			properties: `width=${window.innerWidth - 200},height=${window.innerHeight - 100},left=0,top=0`
		});
		...
		if (windowRef) {
			...
			spotlightService.update(id, spotlight, (err) => {
				if (err) {
					...
					windowRef.sendMessage({ cameras: newCameras }, `${window.location.origin}/map-app/#/spotlight`);
				}
			});
		}
		return () => {};
