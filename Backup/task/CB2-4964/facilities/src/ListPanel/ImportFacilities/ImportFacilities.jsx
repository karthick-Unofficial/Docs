...
const propTypes = {
	...
	facilitiesImportData: PropTypes.object,
	close: PropTypes.func.isRequired,
	createFacility: PropTypes.func.isRequired,
	createFloorPlan: PropTypes.func.isRequired,
	facilitiesImportDone: PropTypes.func.isRequired
};
...
const ImportFacilities = ({
	...
	const validateFacility = (facility) => {
		...
		if (!facility.entityData || !facility.entityData.properties) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.malformed");
		} else if (!facility.entityData.geometry || !facility.entityData.geometry.coordinates) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingGeo");
		} else if (!facility.entityData.properties.name) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingName");
		}
		return errorMessage;
	};
	...
	const validateFloorPlan = (floorplan) => {
		let errorMessage;
		if (!floorplan.name) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingFloorPlanName");
		} else if (!floorplan.geometry || !floorplan.geometry.coordinates) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingFloorPlanGeo");
		} else if (!floorplan.order) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingFloorPlanOrder");
		} else if (!floorplan.image) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.missingFloorPlanImgName");
		} else if (!filesRef.current.find(containedFile =>
			containedFile.name.toLowerCase() === `images/${floorplan.image.toLowerCase()}`)) {
			errorMessage = textConverter("listPanel.importFac.errorMessage.imageNotFound");
		}
		return errorMessage;
	};
	...
	const processFacility = () => {
		...
		if (_.values(facilities).find(existingFacility =>
	...
	const importFloorPlans = () => {
		...
		const orderedFloorplans = _.orderBy(currentState.facility.floorPlans, "order", "asc");
		...
		if (extractedFile) {
			...
			createFloorPlan(currentState.facilityId, floorPlan, file);
		}
	};
	...
	useEffect(() => {
		...
		if (facilities && currentState.facility && !currentState.facilityId) {
			const facility = _.values(facilities).find(existingFacility =>
				existingFacility.entityData.properties.name.toLowerCase()
	...
	return (
		<Dialog
			open={true}
			title={<Translate value="listPanel.importFac.dialog.title" />}
