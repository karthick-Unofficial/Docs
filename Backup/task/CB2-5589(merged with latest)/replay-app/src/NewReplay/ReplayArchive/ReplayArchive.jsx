...
//material-ui
import { Button, SvgIcon, List, ListItem, ListItemIcon, ListItemText, Checkbox, Divider } from "@mui/material";
import { mdiMapClockOutline, mdiMapMarkerPath, mdiCalendarClock } from "@mdi/js";
...
const getIcon = (type, styles) => {
	switch (type) {
		case "track":
			return <img src={require("../images/Replay_Track_Icon_sm.png")} style={{ height: 20, width: 31 }} />;
		case "event": {
			return (
...
const NewReplay = () => {
	...
	const { openDialog, closeDialog } = actionCreators;
		...
		<Fragment>
								...
								<section style={styles.archiveTable}>
									...
									>
										{checkForDeletion ? (
											...
											// }}
											<Button
												onClick={() => dispatch(openDialog("deleteReplay"))}
												color="primary"
												style={{
													textTransform: "none",
													marginTop: 16
										...
										<div>
											<SearchField
												dir={dir}
												value={searchFilter}
												handleChange={(e) => updateSearchFilter(e.target.value)}
												handleClear={(e) => updateSearchFilter("")}
												id={"archive-search"}
												placeholder={getTranslation("newReplay.newEventReplay.search")}
											/>
										</div>
									</div>
									<List>
										<ListItem style={styles.textAlignRight}>
											...
											<ListItemText
												...
												primary={getTranslation("newReplay.replayArchive.tableHeaders.type")}
											/>
											<ListItemText
												primaryTypographyProps={{
													style: { color: "#b5b9be" }
												...
												style={{
													...styles.archiveColumns,
													color: "#b5b9be"
												}}
												primary={getTranslation("newReplay.replayArchive.tableHeaders.name")}
											/>
											<ListItemText
												primaryTypographyProps={{
													style: { color: "#b5b9be" }
												...
												primary={getTranslation("newReplay.replayArchive.tableHeaders.created")}
											/>
											<ListItemText
												primaryTypographyProps={{
													style: { color: "#b5b9be" }
												...
												primary={getTranslation(
													"newReplay.replayArchive.tableHeaders.createdBy"
												)}
											/>
											<div
												style={{
										...
										{gettingReplays ? (
											...
											>
												<p>
													<Translate value="newReplay.replayArchive.saved" />
												</p>
											</div>
										) : (
												...
												return (
													<div key={`data-${index}`}>
														<ListItem style={styles.textAlignRight}>
															...
															/>
															<ListItemIcon>{getIcon(data.type, styles)}</ListItemIcon>
															<ListItemText
																style={styles.archiveType}
																primary={capitalizeFirstLetter(data.type)}
															/>
															<ListItemText
																style={styles.archiveColumns}
																primary={data.name}
															/>
															<ListItemText
																style={styles.archiveColumns}
															...
															<Button
																...
																onClick={() => {
																	restClient.exec_get(
																		`/replay-app/api/portable/download/${data.id}`
																	);
			...
			<Dialog
				...
				title={`${getTranslation(
					"newReplay.replayArchive.deleteReplay.title.areYouSure"
				)} ${selectedForDeletionCount} ${getTranslation(
					"newReplay.replayArchive.deleteReplay.title.selectedReplay"
				)}${
					selectedForDeletionCount > 1
						? `${getTranslation("newReplay.replayArchive.deleteReplay.title.s")}`
						: ""
				}?`}
				abort={{
