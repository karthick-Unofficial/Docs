...
import { Translate, getTranslation, getLocalize } from "orion-components/i18n";
...
let CBTableRow = ({
	...
	return (
		<TableRow>
			{removing && (
				<TableCell className={classes.body} padding="none" style={CBTableRowStyles.tableCell}>
					<Checkbox
						color="primary"
						classes={{
							root: classes.root,
				...
				switch (column.type) {
					case "date-time":
						if (!row.data[column.id]) value = "";
							...
							value = timeConversion.convertToUserTime(
								moment(row.data[column.id]).locale(locale),
						...
						{_.isBoolean(value) ? (
							...
							<Button
								...
								className={classNames(
									dir == "rtl" ? classes.textRTL : classes.text,
									dir && classes.label
								)}
...
const paginationPropTypes = {
	...
	rowsPerPageOptions: PropTypes.oneOfType([PropTypes.array, PropTypes.object])
};
...
const tablePropTypes = {
	listPaginationOptions: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
	defaultListPagination: PropTypes.number,
	canViewNotes: PropTypes.bool,
	classes: PropTypes.object.isRequired,
...
const CBTable = ({
	...
	const [rowsPerPage, setRowsPerPage] = useState(
		(defaultListPagination || defaultListPagination === 0) &&
			listPaginationOptions &&
			listPaginationOptions.includes(defaultListPagination)
			? defaultListPagination
			: listPaginationOptions && listPaginationOptions.length
				? listPaginationOptions[0]
				: 10
	);
	...
	return (
		<div className={classes.tableWrapper}>
			<Table className={classes.root}>
				<CBTableHead columns={columns} removing={removing} hasAttachments={hasAttachments} dir={dir} />
				{_.size(rows) ? (
					<TableBody>
						{_.map(
							noPagination ? rows : _.slice(rows, page * rowsPerPage, page * rowsPerPage + rowsPerPage),
							(row, index) => (
								<CBTableRow
									key={index}
									columns={columns}
