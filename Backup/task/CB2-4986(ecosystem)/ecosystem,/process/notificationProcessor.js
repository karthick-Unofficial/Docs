...
function startQueueMonitor() {
	notificationQueue.subscribe(async function (job) {
		try {
			...
			const translations = await geti18n(global.appRequest, locale);
			const { systemNotification } = translations.ecosystem.email;
			...
			if (to && to.constructor === Array && to.length > 0) {
				...
				logger.info("subscribe-callback", `Authorized users for activity (${job.notification.activityId}): ${JSON.stringify(activityAuthUsers)}`);
				...
				for (let i = 0; i < to.length; i++) {
					...
					switch (keyVal[0].toLowerCase()) {
						case "organization": {
							...
							if (recipient.system) {
								recipients = recipients.concat(authOrgUsers.map(function (user) {
									recipientPriority[user.id] = recipient.isPriority || false;
									return user.id;
								}));
							}
							if (recipient.email) {
								emailRecipients = emailRecipients.concat(authOrgUsers.map(function (user) {
									return user.email;
								}));
							}
							if (recipient.pushNotification) {
								authOrgUsers.forEach((user) => {
									const deviceKeys = userDeviceCache[user.id] ? Object.keys(userDeviceCache[user.id]) : [];
				...
				// remove actor if user ( receive no notifications for things you do )
				if (actor.type === "user") {
				...
				if (emailRecipients.length > 0) {
					const templateData = {
						...
						translations: systemNotification
					};
					emailRecipients.forEach(recipient => {
						logger.info("subscribe-callback", `Queueing email to ${recipient} for notification: ${JSON.stringify(job.notification)}`);
						emailProvider.queueEmail("systemNotification", config.environment.fromEmail, recipient, templateData);
				...
				if (pushNotificationRecipients.length > 0) {
					...
					logger.info("subscribe-callback", `Queueing push notification to ${JSON.stringify(pushNotificationRecipients)} for notification: ${JSON.stringify(job.notification)}`);
					...
					processResult["queuepushNotificationNotifications"] = "No pushNotification recipients for this notification";
				}

				if (notifications.length > 0) {
					logger.info("subscribe-callback", `Inserting system notifications: ${JSON.stringify(notifications)}`);
					...
					processResult["insertSystemNotifications"] = { "result": "no intended recipients" };
				}
			}
			else {
				processResult = { "result": "to field empty, no intended recipients" };
			}
			...
			logger.error("subscribe-callback", `Error processing notification job for activity (${job.notification.activityId}).`, { err: { message: err.message, stack: err.stack } });
		}
