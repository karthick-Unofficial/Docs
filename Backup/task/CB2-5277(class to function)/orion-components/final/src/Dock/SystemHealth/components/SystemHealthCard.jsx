...
const SystemHealthCard = ({ title, hasError, healthSystems, dir, locale }) => {
	...
	const ListItemTranslate = (systemName) => {
		switch (systemName) {
			case "My Apps":
				return "global.dock.systemHealth.systemHealthCard.myapps";
			case "Core System":
				return "global.dock.systemHealth.systemHealthCard.coresystem";
			case "My Feeds":
				return "global.dock.systemHealth.systemHealthCard.myfeed";
			case "Linked Ecosystems":
				return "global.dock.systemHealth.systemHealthCard.linkedecosystems";
			case "Server Health":
				return "global.dock.systemHealth.systemHealthCard.serverhealth";
			default:
				return systemName;
		}
	};
		...
		>
			<ListItem style={{ backgroundColor: "#494d53" }}>
				...
				<ListItemText
					...
					primary={getTranslation(ListItemTranslate(title))}
				...
				<CardContent style={{ padding: dir && dir == "rtl" ? "0 50px 0 0" : "0 0 0 50px", backgroundColor: "#494d53" }}>
					{healthSystems.map((system, index) => {
						return (
							...
							>
								<p style={{ color: "white", fontWeight: "bold" }}>{system.label}</p>
