...
const ElapsedTimer = ({ initialActivityDate, eventEndDate }) => {
	...
	const updateTimer = () => {
		const formattedDateTime = moment(initialActivityDate).format(
			"YYYY/MM/DD HH:mm:ss"
		);
		const endTime = eventEndDate ? moment(eventEndDate) : moment();
		const timeDiff = endTime.diff(
			moment(formattedDateTime, "YYYY/MM/DD HH:mm:ss")
		);
	...
	return (
		<Typography
			style={{ color: "#fff", fontSize: 22, lineHeight: "unset" }}
		>
