...
const UserIntegrations = ({
	...
	const orgs = useSelector(state => keyBy(state.globalData.orgs, "orgId"));
	...
	useEffect(() => {
		...
		if (Object.keys(toggleValues).length) {
			if (user && user.integrations && ecoIntegrations && ecoIntegrations.length) {
				const ints = unionBy(ecoIntegrations, user.integrations, "orgIntId");
				...
				each(ints, int => {
					if (some(user.integrations, { orgIntId: int.orgIntId })) {
						// If user already has an object for that feed, set to config.canView
						const config = find(ecoIntegrations, {
							orgIntId: int.orgIntId
						}).config.canView;

						toggleValues[int.orgIntId] = !isEmpty(config)
							? config
