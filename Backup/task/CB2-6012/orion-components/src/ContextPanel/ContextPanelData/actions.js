...
import { addContext, updateContext, subscriptionReceived } from "../../ContextualData/Actions";
import { setSelectedEntity, clearSelectedEntity } from "../../Profiles/ProfileState/Actions/index";

import { unsubscribeFromFeed, removeProfileSubscription } from "../../ContextualData/Actions/index";
...
export const updateViewingHistory = (id, name, type, profileRef, context, options) => {
	return {
...
export const updateSelectedContext = (entityId, context = "secondary") => {
	return (dispatch, getState) => {
		const selectedContext = getState().appState.contextPanel.contextPanelData.selectedContext;
		...
		switch (context) {
			case "primary":
				...
				if (!_.isEmpty(setContexts)) {
					_.each(
						_.filter(_.values(setContexts), (contextId) => contextId !== entityId),
						(contextId) => {
							// Unsubscribe profile from old context
							if (getState().contextualData[contextId]) {
								_.each(_.keys(getState().contextualData[contextId].subscriptions), (subscriptionId) =>
			...
			case "secondary":
				...
				if (_.includes(_.keys(setContexts), "secondary") && setContexts["secondary"] !== entityId) {
					dispatch(
						// Unsubscribe profile from old context
...
export const loadProfile = (entityId, entityName, entityType, profileRef, context = "primary") => {
	return (dispatch, getState) => {
		...
		feedService
			...
			.then((subscription) => {
				...
				dispatch(subscriptionReceived(entityId, subscription.channel, "entity", profileRef));
...
export const loadProfileOffline = (entityId, entityName, entityType, entity, profileRef, context = "primary") => {
	return (dispatch, getState) => {
		// If another profile load was attempted within 1 sec, bail out
		if (loadingId) {
			return;
...
export const loadGISProfile = (id, name, layerId, type, profileRef, context = "primary") => {
	return (dispatch, getState) => {
		// If another profile load was attempted within 1 sec, bail out
		if (loadingId) {
			return;
		...
		if (shouldUpdateViewingHistory(contextPanelData.viewingHistory, id)) {
			dispatch(
				updateViewingHistory(id, name, type, profileRef, context, {
					layerId
				})
			);
		}
