...
const ProximityWidget = ({
	...
	useEffect(() => {
		if (!forReplay) {
			...
			if (
				...
				context.proximityEntities.length > 0
			) {
				context.proximityEntities = [];
			}
	...
	const handleEditProximity = (proximityId) => {
		const proximity = event.proximities.find(
			(x) => x.id === Number(proximityId)
		);
		setIsEditing(true);
			...
			{!expanded && !readOnly && (
				<div className="widget-header">
					...
					{canCreateProximityArea && !eventEnded && (
						...
						>
							{getTranslation(
								"global.profiles.widgets.proximity.main.createProxZone"
							)}
						</Button>
					)}
					{widgetsExpandable ? (
					...
					event.proximities.map((proximity) => {
						return (
							<ProximityCard
								...
								handleEdit={() =>
									handleEditProximity(proximity.id)
								}
								handleRemove={() =>
									handleRemoveProximity(proximity.id)
								}
								...
								entities={
									context && context.proximityEntities
										? context.proximityEntities
										: []
								}
								dir={dir}
