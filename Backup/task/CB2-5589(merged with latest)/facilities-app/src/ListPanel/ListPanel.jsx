...
import { selectedEntityState, selectedContextSelector } from "orion-components/ContextPanel/Selectors";
...
const renderProfile = (mode) => {
	switch (mode) {
		case "camera":
			return <CameraProfile {...cameraProfileActions} widgetsExpandable={false} widgetsLaunchable={true} />;
...
const ListPanel = ({ WavCamOpen, location }) => {
	...
	const canCreate = userFeeds.some((feed) => {
		return (
			...
			feed.ownerOrg === session.user.profile.orgId &&
	...
	const facilities = useSelector((state) => {
		...
		if (globalData && globalGeo) {
			facilityFeeds.map((feed) => {
				retVal = merge(retVal, cloneDeep(layerSourcesSelector(state, { feedId: feed })) || {});
			});
		}
		return retVal;
	});
	...
	}, [widgetLaunchData, facilities, dispatch]);

	useEffect(() => {
		const params = location ? location.search.split("?")[1] : null;

		if (params && !preLoaded) {
			facilityService.getFloorPlan(params, (err, res) => {
				if (err) {
					console.log(err);
					...
					if (facilities[floorPlan.facilityId] && !preLoaded) {
						dispatch(
							loadProfile(
								facilities[floorPlan.facilityId].id,
								facilities[floorPlan.facilityId].entityData.properties.name,
								"facility",
								"profile"
							)
						);
						...
						window.history.replaceState(null, null, urlWithoutHash); // clearing url params
					}
				}
			});
		}
	}, [preLoaded, facilities, dispatch, location]);
		...
		>
			<div>
				...
				{allowImport && (
					<div style={styles.controls}>
						<Fab
							onClick={() => setImportFacilities(true)}
							disabled={!canCreate}
							color="primary"
							size="small"
						>
							...
							<List>
								{facilities &&
									Object.values(facilities)
										.sort((a, b) => {
											const aName =
												a.entityData && a.entityData.properties
													? a.entityData.properties.name
													: a.id;
											const bName =
												b.entityData && b.entityData.properties
										...
										.filter((facility) => {
											const facilityName =
												facility.entityData && facility.entityData.properties
													? facility.entityData.properties.name
													: facility.id;
											const facilityDescription =
												facility.entityData && facility.entityData.properties
													? facility.entityData.properties.description
													: "";
