...
const EntityAddToCollection = ({
	...
	return (
		...
		>
			<DialogTitle sx={{ padding: "24px 24px 20px", fontSize: "22px", fontWeight: 400, lineHeight: "32px", background: "#1f1f21" }}>{getTranslation("global.sharedComponents.entityAddToColl.title")}</DialogTitle>
			{errorMsg && <p className="dialog-error">{errorMsg}</p>}
			<div style={{ color: "rgba(255, 255, 255, 0.6)", padding: "0px 24px 24px" }}>				
				<TextField
					label={getTranslation("global.sharedComponents.entityAddToColl.newColl")}
					placeholder={getTranslation("global.sharedComponents.entityAddToColl.enterCollTitle")}
			...
			<DialogActions sx={{ flexDirection: "row!important" }} disableSpacing={true}>{actions}</DialogActions>
		</Dialog>
	);
};
