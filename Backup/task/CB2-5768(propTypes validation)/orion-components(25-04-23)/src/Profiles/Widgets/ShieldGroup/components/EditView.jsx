...
import { SelectField, SearchSelectField, Portal } from "orion-components/CBComponents";
import {
	List,
	ListItem,
	ListItemText,
		...
		<Fragment>
			...
			{threadId && (
				<Typography variant="h6">
					<Translate value="global.profiles.widgets.shieldGroup.editView.threadId" count={threadId} />
				</Typography>
			)}
							...
							<List>
								{map(groups, (group) => (
									<ListItem key={group.id}>
										...
										<ListItemSecondaryAction style={inlineStyles.listItemSecondaryAction}>
											<Switch
												onClick={() => handleSelect("audience_groups", group.id)}
												checked={includes(audience_groups, group.id)}
												color="primary"
											/>
										</ListItemSecondaryAction>
									</ListItem>
							...
							<List>
								{map(departments, (department) => (
									<ListItem key={department.id}>
										...
										<ListItemSecondaryAction style={inlineStyles.listItemSecondaryAction}>
											<Switch
												onClick={() => handleSelect("audience_departments", department.id)}
												checked={includes(audience_departments, department.id)}
												color="primary"
											/>
										</ListItemSecondaryAction>
									</ListItem>
							...
							<List>
								{map(districts, (district) => (
									<ListItem key={district.id}>
										...
										<ListItemSecondaryAction style={inlineStyles.listItemSecondaryAction}>
											<Switch
												onClick={() => handleSelect("audience_districts", district.id)}
												checked={includes(audience_districts, district.id)}
												color="primary"
											/>
										</ListItemSecondaryAction>
									</ListItem>
							...
							<List>
								...
								>
									<SearchSelectField
										...
										handleSelect={(id) => handleSelect("audience_individuals", id)}
								...
								{map(
									filter(users, (individual) =>
										includes(settingsState["audience_individuals"], individual.id)
									),
									(individual) => (
										<ListItem key={individual.id}>
											...
											<ListItemSecondaryAction style={inlineStyles.listItemSecondaryAction}>
												<IconButton
													onClick={() => handleRemove("audience_individuals", individual.id)}
												>
													<Cancel />
												</IconButton>
											</ListItemSecondaryAction>
