...
const Field = ({
	...
	const handleSelection = (values) => {
		...
		const selectAll = valuesArr.some(el => {
			return el.value === "select_all";
		});
		if (selectAll) {
			newValues = [{ value: "select_all", label: getTranslation("reportBuilder.components.field.selectAll") }];
		...
		const payload = selectAll
			? "SELECT_ALL"
			: newValues.map(value => {
				return { id: value.value.id ? value.value.id : value.value, entityType: value.value.entityType, feedId: value.value.feedId, name: value.label };
			});
		const wipeCheck = closing
			? true
			: field.typeAheadClear
				? queryString
					? false
					: true
				: false;
	...
	const generateField = field => {
		...
		switch (field.type) {
			...
			case "drop-down-menu":
				{
					let menuItems = [];
					// Dont render other menu options if Select All is chosen
					if (
						!selectAll ||
						field.noMultiple ||
					...
					generatedField = (
						...
						<SuperSelectField
							...
							multiple={
								field.noMultiple || field.singleSelection ? false : true
							}
