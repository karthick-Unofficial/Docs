...
const MapLayer = (props) => {
	...
	const feed = useSelector((state) =>
		_.find(userFeedsSelector(state), (feed) => feed.feedId === props.feedId)
	);
	...
	const userFeed = useSelector((state) =>
		userIntegrationByFeedIdSelector(props.feedId)(state)
	);
	...
	const clusterMaxZoomLevel = useSelector(
		(state) => state.clientConfig.mapSettings.clusterMaxZoomLevel || 11
	);
	const silhouetteMinZoomLevel = useSelector(
		(state) => state.clientConfig.mapSettings.silhouetteMinZoomLevel || 11
	);
	...
	const handleFeatureClick = (e) => {
		...
		if (
			_.find(features, (feature) =>
				_.includes(feature.layer.id, "cluster")
			)
		) {
			return;
		}
		...
		if (types.length > 1 && includesFill && includesOther) {
			features = features.filter(
				(feature) => feature.layer.type !== "fill"
			);
		}
	...
	useEffect(() => {
		if (!handlersSet) {
			...
			map.on("touchend", feedId + "-clusters", (e) =>
				...
				if (selectedFloors) {
					Object.keys(selectedFloors).some((facilityId) => {
						if (
							...
							entity.entityData.displayTargetId
						) {
							activated = true;
							return true;
						}
	...
	useEffect(() => {
				...
				_.filter(entities, (entity) => {
					if (entity.entityData) {
						if (
							...
							let activated = false;
							if (selectedFloors) {
								Object.keys(selectedFloors).some(
									(facilityId) => {
										if (
											selectedFloors[facilityId] &&
						...
						entity.entityData.properties.entityType =
							entity.entityType;
						...
						const hasAlert = _.find(
							...
							(alert) =>
								_.size(alert.object) &&
								alert.object.id ===
								entity.entityData.properties.id
						);
						...
						if (hasOwn(entity, "controls"))
							entity.entityData.properties.controls =
								entity.controls;
					}
		...
		if (layerSources !== sources) {
			setLayerSources(sources);
		}
	}, [disabledFeeds, selectedFloors, data, alerts, filters, mapTools]);
	...
	const renderSilhouettes = (index, source, before) => {
		...
		source.data.features.forEach((feature) => {
			if (
				[
					...
					"dimC",
					"dimD"
				].every((key) => hasOwn(feature.properties, key))
			) {
		...
		return (
			<Fragment key={feedId + index + "-silhouettes"}>
				<VesselPolygons
					vessels={vessels}
					minZoom={silhouetteMinZoomLevel}
					before={before}
				/>
			</Fragment>
		);
	...
	const renderClusters = () => {
		...
		const clusters = _.map(
			_.filter(
				sources,
				(source) => source.type && source.type === "Point"
			),
			(source, index) => {
				return (
	...
	return data && canView ? (
		<Fragment>
			{layerSources && layerSources.length && _.map(
				_.filter(layerSources, (source) => source.type),
				(source, index) => {
					const isShapeOrLine = !!(
						source.type === "Polygon" || source.type === "LineString"
					);
					...
					const linePaintProp = linePaint
						? linePaint[
						_.filter(_.keys(linePaint), (key) =>
							_.includes(feedEntType, key)
						)[0]
						]
						: {};

					const layerBefore = `---ac2-${isShapeOrLine
						? source.type.toLowerCase() + "s"
						: "feed-entities"
						}-position-end`;
					return (
