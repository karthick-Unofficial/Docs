...
import { mapSettingsSelector, replayMapObject } from "orion-components/AppState/Selectors";
...
const EventsWrapper = () => {
	...
	return <Events events={events} labelsVisible={labelsVisible} map={map} selectedEvent={selectedEvent} />;
};

const Events = memo(
	({ events, labelsVisible, map, selectedEvent }) => {
		const dispatch = useDispatch();

		const [selectedEventId, setSelectedEventId] = useState(null);

		const { loadProfileOffline, setMapEntities } = actionCreators;
		...
		useEffect(() => {
			if (events.length) {
				...
				each(keys(byFeed), (feed) => (byFeed[feed] = keyBy(byFeed[feed], "id")));
		...
		const handleClick = useCallback(
			(eventId, eventName) => {
				...
				dispatch(loadProfileOffline(eventId, eventName, "event", currentEvent, "profile", "primary"));
			},
			[loadProfileOffline]
		);
