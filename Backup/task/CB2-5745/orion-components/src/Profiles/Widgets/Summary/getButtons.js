...
const getButtons = (user, context, appId, actions, readOnly) => {
	...
	const { isPublic, entityType, fov, activeFOV, entityData, sharedWith } =
		entity;
	...
	switch (entityType) {
		case "track":
			return [
				...
				{
					name: getTranslation(
						"global.profiles.entityProfile.main.hide"
					),
					icon: "delete",
					toggled: false,
					viewable: !readOnly
		...
		case "camera": {
			...
			if (appId && appId.toLowerCase() === "cameras-app") {
				...
				buttons.push({
					name: getTranslation(
						"global.profiles.cameraProfile.facility"
					),
					viewable:
						!readOnly &&
			...
			return [
				...buttons,
				{
					name: activeFOV
						? getTranslation(
								"global.profiles.cameraProfile.hideFOV"
						  )
						: getTranslation(
								"global.profiles.cameraProfile.showFOV"
						  ),
					icon: "network_wifi",
					toggled: activeFOV,
		...
		case "event": {
			...
			return [
				{
					name: getTranslation(
						"global.profiles.eventProfile.main.report"
					),
					icon: "description",
					toggled: false,
				...
				{
					name:
						isPublic && !canShareEvents
							? getTranslation(
									"global.profiles.eventProfile.main.shared"
							  )
							: getTranslation(
									"global.profiles.eventProfile.main.share"
							  ),
					icon: "share",
