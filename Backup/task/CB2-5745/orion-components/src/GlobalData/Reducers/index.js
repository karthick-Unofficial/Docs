...
export { default as unitMembers } from "../UnitMembers/reducer";
export { default as units } from "../Units/reducer";
export { default as webrtcPlay } from "../WowzaWebRTC/WebRTCPlay/reducer";
export { default as playSettings } from "../WowzaWebRTC/PlaySettings/reducer";
				...
				if (feedId === payload.feedId) {
					switch (type) {
						case "DATA_BATCH_RECEIVED": {
							...
							const newData = !useQueue ? deepMerge({}, state.data) : deepMerge({}, state.dataQueue);
							...
							if (!newData) {
								...
								if (useQueue) {
									...
									const newDataRemoveQueue = [
										...state.dataRemoveQueue
									];
									newBatchIds.forEach((id) => {
										const index =
											newDataRemoveQueue.indexOf(id);
										if (index !== -1) {
						...
						case "DATA_RECEIVED": {
							...
							if (!newData) {
								...
								if (useQueue) {
									...
									const index = newDataRemoveQueue.indexOf(
										data.id
									);
									if (index !== -1) {
										newDataRemoveQueue.splice(index, 1);
						...
						case "DATA_REMOVED": {
							...
							if (useQueue) {
								...
								const newDataById = filter(
									[...state.dataById, ...state.dataByIdQueue],
									(id) => {
										return id !== dataId;
									}
								);
