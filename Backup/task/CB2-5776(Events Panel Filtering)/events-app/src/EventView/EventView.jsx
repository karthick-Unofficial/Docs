...
const EventView = () => {
	...
	useEffect(() => {
		if (event) {
			...
			if (prevEvent) {
				if ((id !== prevEvent.id && notes) || (!prevEvent.notes && notes) || prevEvent.notes !== notes) {
					retrieveHTML(notes);
				}
			}
					...
					return (
						<ErrorBoundary>
							<NotesWidget
								...
								hasNotes={notes && notes.html ? true : false}
								notes={
									notes && notes.html
										? notes
		...
		switch (view) {
			case "Pinned Items": {
				return canManage ? (
					<div className="widget-option-button">
						{!eventEnded && (
							<Button variant="text" onClick={() => dispatch(openDialog("pinnedItemDialog"))}>
			...
			case "Event Lists": {
				return (
					<div className="widget-option-button">
						{canManage ? (
							<Button variant="text" onClick={() => dynamicWidget.current.openListDialog()}>
			...
			case "Notes": {
				return (
					<div>
						{notesChanged && canManage ? (
							<div className={"widget-option-button"}>
								<Button variant="text" onClick={() => dynamicWidget.current.cancelEdit()}>
									{getTranslation("eventView.cancel")}
								</Button>
								<Button variant="text" onClick={() => dynamicWidget.current.saveEdit()}>
			...
			case "Proximity": {
				return canManage ? (
					<div className="widget-option-button">
						{!eventEnded && (
							<Button variant="text" onClick={() => dispatch(openDialog("proximityDialog"))}>
		...
		<div style={{ height: `calc(100vh - ${WavCamOpen ? "288px" : "48px"})` }} className="view-wrapper">
			...
			>
				{event && view && showHeader && (
					<div
						className={`view-header cb-font-b1 ${viewWidth} ${
							view === "map-view" ? "map-view" : "normal-view"
						} ${dir}`}
					>
