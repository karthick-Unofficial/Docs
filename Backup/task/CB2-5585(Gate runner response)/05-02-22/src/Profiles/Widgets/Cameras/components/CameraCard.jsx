...
import { ArrowBackIos, ArrowForwardIos, Close, Lock, LockOpen, MoreVert } from "@mui/icons-material";
...
const CameraCard = ({
	...
	const unlinkControls = canUnlink && camera.linkedWith && camera.linkedWith === contextId ? (
		<div style={styles.marginAuto}>
			<Button
				onClick={() => dispatch(unlinkCameras([{ id: contextId, type: entityType }, { id: camera.id, type: "camera" }], "manually-assigned-camera"))}
				color="primary"
								...
								{!presetsOpen ? (
									<Fragment>
										<ListItem style={{ paddingLeft: 16, paddingRight: 16, height: 30 }}>
											...
											<ListItemSecondaryAction style={styles.listItemSecondaryAction} onClick={handleCloseMenu}>
										...
										{sidebarOpen && dockedCameras.includes(id) ? (
											...
											>
												<ListItemText primary={getTranslation("global.profiles.widgets.cameras.camCard.removeFromDock")}
											...
											>
												<ListItemText
													primary={getTranslation("global.profiles.widgets.cameras.camCard.addToDock")}
													style={styles.textAlignRight} />
											</ListItem>
										)}
										...
										{(hasCapability("control") && canControl) && (
											...
											>
												<ListItemText
													primary={getTranslation("global.profiles.widgets.cameras.camCard.presets")}
													style={styles.textAlignRight} />
											...
											<Fragment>
												...
												{buttonFeatures.map(feat => (
													<ListItem button style={{ paddingTop: 8, paddingBottom: 8, paddingLeft: 16, paddingRight: 16 }}>
														<ListItemText primary={feat.label} style={styles.textAlignRight} />
														<ListItemSecondaryAction style={styles.listItemSecondaryAction} onClick={e => execFeatureCommand(feat.auxCmd)}>
									...
									<Fragment>
										...
										>
											...
											<ListItemText primary={getTranslation("global.profiles.widgets.cameras.camCard.back")}
												style={styles.textAlignRight} />
											<ListItemSecondaryAction style={styles.listItemSecondaryAction} onClick={handleCloseMenu}>
												<IconButton edge={dir == "rtl" ? "start" : "end"} aria-label="close">
													<Close style={{ color: "#FFF" }} />
										...
										{presets.length > 0 ? presets.map(preset => (
											...
											>
												<ListItemText primary={preset.label} style={styles.textAlignRight} />
											</ListItem>
										)) : (
											<ListItem
												key={"none"}
											...
											>
												<ListItemText
													primary={getTranslation("global.profiles.widgets.cameras.camCard.noPresets")}
													style={styles.textAlignRight} />
											</ListItem>
										)}
									</Fragment>
