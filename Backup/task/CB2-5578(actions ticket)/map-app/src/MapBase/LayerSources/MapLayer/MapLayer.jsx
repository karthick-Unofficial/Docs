...
const MapLayer = (props) => {
	...
	const feed = useSelector(state => _.find(
		userFeedsSelector(state),
		feed => feed.feedId === props.feedId
	));

	const { selectedFloors } = useSelector(state => persistedState(state));
	const userFacilitiesFloors = useSelector(state => state.globalData.floorPlanWithFacilityFeedId.floorPlans);
	...
	const clusteringEnabled = useSelector(state =>
		typeof state.clientConfig.mapSettings.clusteringEnabled !== "undefined" ? state.clientConfig.mapSettings.clusteringEnabled
			: true);
	...
	const [mapIconTemplate, setMapIconTempalte] = useState(null);
	...
	useEffect(() => {
		...
		setMapIconTempalte(feed ? feed.mapIconTemplate : null);
	...
	useEffect(() => {
		return () => { dispatch(setMapEntities({ [feedId]: {} })); };
	}, []);
