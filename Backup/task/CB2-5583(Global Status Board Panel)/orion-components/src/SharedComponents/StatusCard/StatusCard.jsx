...
const StatusCard = ({ card, setEditingMode, disableControls, disableMenu, userCanEdit, userCanShare, dir, locale }) => {
	...
	const getControlComponent = (type, statusControl, index, dir) => {
		switch (type) {
			...
			case "slides":
				return <Slides key={`${card.id}-${index}`} control={statusControl} updateSelected={updateSelected(index)} disableControls={disableControls || !userCanEdit} dir={dir} forceUpdate={forceUpdate}/>;
			case "text":
			...
			<section style={styles.contentArea}>
				...
				{<div style={styles.bottomBar}>
					{disableControls ? <Typography variant="body1" style={styles.typography}>
						<Translate value="global.sharedComponents.statusCard.main.sharedFrom" count={card.ownerOrgName} />
					</Typography> : <div></div>}
					{card.lastUpdatedBy ? <p style={styles.updatedText}><Translate value="global.sharedComponents.statusCard.main.changed" count={moment(card.lastUpdateDate).locale(locale).fromNow()} /><Translate value="global.sharedComponents.statusCard.main.by" count={card.lastUpdatedBy} /></p> : null}
				</div>}
			</section>
			<Popover
				open={!!anchorEl}
					...
					<Fragment>
						{userCanEdit && !disableControls && (
							...
							>
								...
								<ListItemText primary={getTranslation("global.sharedComponents.statusCard.main.editCard")}
						...
						{!disableControls && userCanShare && (
							...
							>
								<Share
									style={{ color: card.sharedWith.length ? "#4eb5f3" : "#fff" }}
								/>
								<ListItemText primary={getTranslation("global.sharedComponents.statusCard.main.share")}
									style={styles.textAlignRight} />
							</ListItem>)}
					</Fragment>
				</List>
