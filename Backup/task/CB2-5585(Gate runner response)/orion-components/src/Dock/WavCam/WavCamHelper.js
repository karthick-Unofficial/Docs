...
const WavCamHelper = (metadata) => {
	let CAMERA_AZ, CAMERA_EL, CAMERA_HFOV, CAMERA_VFOV;
	let CAMERA_LAT = parseFloat(metadata["x-wavcam-lat"]);
	let CAMERA_LON = parseFloat(metadata["x-wavcam-lon"]);
	let CurrentAz = CAMERA_AZ = parseFloat(metadata["x-wavcam-camera-az"]);
	let CurrentEl = CAMERA_EL = parseFloat(metadata["x-wavcam-camera-el"]);
	let currentVFOV = CAMERA_VFOV = parseFloat(metadata["x-wavcam-vfov"]);
	let currentHFOV = CAMERA_HFOV = parseFloat(metadata["x-wavcam-hfov"]);
	let CAMERA_ALT = parseFloat(metadata["x-wavcam-alt"]);
	...
	const updateParams = (metadata) => {
		...
		let VIEWABLE_AREA_HEIGHT = parseFloat(metadata["x-wavcam-viewable-area-height"]);
		let VIEWABLE_AREA_WIDTH = parseFloat(metadata["x-wavcam-viewable-area-width"]);
		let VIEWABLE_AREA_X = parseFloat(metadata["x-wavcam-viewable-area-x"]);
		let VIEWABLE_AREA_Y = parseFloat(metadata["x-wavcam-viewable-area-y"]);
