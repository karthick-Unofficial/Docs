...
const GISCollection = ({
	...
	const renderListItem = layer => {
		...
		let listItem;
		const isSubLayer = includes(
			flattenDeep(map(layers, layer => layer.subLayerIds)),
			id
		);
		if (subLayerIds) {
			listItem = (
				<Fragment key={id}>
					<ListItem disableGutters>
						<ListItemText
							...
							primary={name}
							secondary={size(subLayerIds) === 1 ? getTranslation("global.map.controls.gisControl.gisCollection.layer", size(subLayerIds)) : getTranslation("global.map.controls.gisControl.gisCollection.layers", size(subLayerIds))}
