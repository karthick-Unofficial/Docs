...
import {
	TextField, Radio, RadioGroup, FormControlLabel, Grid, Typography, MenuItem, Divider
} from "@mui/material";
...
const Settings = ({ open, closeDialog, unitMember }) => {
	...
	const { memberRoleTypes } = unitsPanel;



	useEffect(() => {
		if (unitMember) {
			const { phone, name, unitId, roles, memberType } = unitMember;
			const CountryCode = countryCodesArray.find((code) => _.includes(phone, code.value));
			setUnitMemberName(name);
			setUnit(unitId);
			setSelectedRoles(roles);
			setMemberType(memberType);
			if (size(CountryCode)) {
				setCountryCode(CountryCode.value);
				setPhoneNumber(formatPhone(phone.replace(CountryCode.value, "")));
			} else {
	...
	const unassignUnitFromUnitMember = () => {
		unitService.unassignUnitFromUnitMember(
			unitMember.id,
			(err, response) => {
				if (err) {
			...
			>
				<div
					...
					<div style={{ marginBottom: "25px", display: "flex" }}>
						<Grid container columnSpacing={3}>
							<Grid item xs={4} sm={4} md={4} lg={4} >
								...
								>
									{countryCodesArray.map((countryCodes) => <MenuItem value={countryCodes.value}>{countryCodes.name}</MenuItem>)}
								</TextField>
							</Grid>
					...
					<div style={{ marginTop: "10px", marginBottom: "25px" }}>
						<Typography fontSize="12px" style={{ paddingTop: "20px", fontWeight: "300px", marginBottom: "25px" }}>
						...
						<div>
							...
							>
								...
								{memberRoleTypes.length > 0 && memberRoleTypes.map((type) => {
									return (
										<MenuItem value={type.id}>{type.name}</MenuItem>
									)
								})}
