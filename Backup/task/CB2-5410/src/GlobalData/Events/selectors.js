...
import { eventSearchSelector, eventTemplateSearchSelector } from "../../ContextPanel/Selectors";
...
import { StarredTemplateSort } from "../../SharedComponents/EventTemplate/StarredTemplateSort";
...
export const eventTypesSelector = createSelector(eventTypesState, (types) => types);

export const templatesSelector = createSelector(templatesState, (templates) => {
	return templates;
});

export const currentEventsSelector = createSelector(eventsSelector, (events) => {
	return events
		.map((event) => {
			const startDate = moment(event.startDate);
			const endDate = moment(event.endDate);

			if ((endDate > moment() && startDate < moment()) || (event.endDate === undefined && startDate < moment())) {
				event.status = "active";
...
const filterEvent = (event, filters) => {
	...
	const active = moment().isBetween(eventStart, eventEnd) || (endDate === undefined && moment().isAfter(eventStart));
	...
	each(filters, (filter, key) => {
		...
		switch (key) {
			case "status":
				if (active) {
					filtered["status"] = activeFilter && !filter.includes("active");
				} else if (closed) {
					filtered["status"] = activeFilter && !filter.includes("closed");
				} else if (upcoming) {
					filtered["status"] = activeFilter && !filter.includes("upcoming");
				}
				break;
			case "template":
				filtered["template"] =
					activeFilter && (!eventTemplate || (eventTemplate && !filter.includes(eventTemplate)));
				break;
			default:
				break;
		}
...
export const currentOwnedEventsSelector = createDeepEqualSelector(
	...
	(events, userId, filters, search) => {
		const filteredEvents = pickBy(events, (event) => {
			...
			return owner === userId && name.toLowerCase().includes(search) && filterEvent(event, filters);
		});
		return filteredEvents;
	}
);

export const currentOwnedEventsWithoutSearchAndFilter = createDeepEqualSelector(
	eventsSelector,
	userIdSelector,
	(events, userId) => {
		const filteredEvents = pickBy(events, (event) => {
...
export const activeOwnedEventsSelector = createDeepEqualSelector(
	...
	(events, userId, filters, search) => {
		const filteredEvents = pickBy(events, (event) => {
			...
			const eventEnd = moment(endDate);
			const active =
				moment().isBetween(eventStart, eventEnd) || (endDate === undefined && moment().isAfter(eventStart));
...
export const closedEventsSelector = createDeepEqualSelector(
	eventsSelector,
	eventFiltersSelector,
	eventSearchSelector,
	(events, filters, search) => {
		const filteredEvents = pickBy(events, (event) => {
			const { name, endDate } = event;

			const eventEnd = moment(endDate);
			const closed = eventEnd && moment().isAfter(eventEnd);

			return name.toLowerCase().includes(search ? search : "") && closed && filterEvent(event, filters);
		});
		return filteredEvents;
	}
);
...
export const scheduledEventsSelector = createDeepEqualSelector(
	eventsSelector,
	eventFiltersSelector,
	eventSearchSelector,
	(events, filters, search) => {
		const filteredEvents = pickBy(events, (event) => {
			...
			const eventStart = moment(startDate);
			const scheduled = eventStart && moment().isBefore(eventStart);

			return name.toLowerCase().includes(search ? search : "") && scheduled && filterEvent(event, filters);
		});
		return filteredEvents;
	}
);
...
export const sharedEventsSelector = createDeepEqualSelector(
	...
	(fromEco, fromOrg, filters, search) => {
		...
		const filteredEvents = pickBy(events, (event) => {
			const { name } = event;
			return name.toLowerCase().includes(search) && filterEvent(event, filters);
		});
		return filteredEvents;
	}
);
...
export const activeSharedEventsSelector = createDeepEqualSelector(
	...
	(fromEco, fromOrg, filters, search) => {
		...
		const filteredEvents = pickBy(events, (event) => {
			...
			return name.toLowerCase().includes(search) && active && filterEvent(event, filters);
		});
		return filteredEvents;
	}
);
...
export const getEventsWithoutFiltersSelector = createDeepEqualSelector(
	sharedEventsSelector,
	currentOwnedEventsWithoutSearchAndFilter,
	(shared, owned) => {
		const events = { ...shared, ...owned };
		return events;
	}
...
export const makeGetPinnedItems = () => {
	return createDeepEqualSelector(getPinnedItems, feedEntitiesSelector, (pinnedItems, entities) => {
		const fullItems = pickBy(entities, (entity) => pinnedItems.includes(entity.id));
...
export const sortedStarredTemplatesSelector = createDeepEqualSelector(
	sharedTemplatesSelector,
	ownedTemplatesSelector,
	(shared, owned) => (favTemplates) => {
		const templatesArray = Object.values({ ...shared, ...owned });
		const templates = StarredTemplateSort(templatesArray, ["name"], ["asc"], favTemplates);
		return templates;
	}
);
