	...
	switch (type) {
		...
		case "UPDATE_CONTEXT": {
			...
			if (newContext.trackHistory) {
				...
				let lastKnownPosition = JSON.stringify(state[contextId].entity.entityData.geometry.coordinates);
				if (trackHistory[trackHistory.length - 1]) {
					...
					lastKnownPosition = JSON.stringify(trackHistory[trackHistory.length - 1].entityData.geometry.coordinates);
				} else {
				...
				if (lastKnownPosition !== JSON.stringify(update.entityData.geometry.coordinates && update.entityData && update.entityData.properties && update.entityData.geometry)) {
					const { name, mmsi: mmsid, imo, callsign, timestamp: acquisitionTime, speed, hdg, course } = update.entityData.properties;
					newContext.trackHistory.push({ entityData: {
		...
		case "SET_SUBSCRIPTION":
			{
				...
				if (!context || _.isEmpty(context)) {
					...
					const extantSub = { ...context.subscriptions[subscriptionId] };
					return {
						...state,
						[contextId]: {
							...context,
							subscriptions: {
								...
								[subscriptionId]: {
									...
									// concatentate into an array
									subscription: extantSub.subscription
										? _.concat(extantSub.subscription, subscription)
										: context.subscriptions[subscriptionId].susbcription,
				...
				return {
					...
					[contextId]: {
						...
						[subscriptionId]: iterable
							? _.uniqBy([data, ...context[subscriptionId]], "id") // Prevent duplicates on resubscriptions
							: data
					}
				};
		...
		case "SUBSCRIPTION_DATA_REMOVED": {
			...
			let newData;
			if (context) {
				newData = context[subscriptionId].filter(item => item.id !== dataId);
			}
				...
				updatedData = oldData.map(item => {
					if (item.id === data.id) {
						if (data.entityType === "list") {
							if (item.rows && data.rows) {
								item.name = data.name;
								item.rows = _.cloneDeep(data.rows);
							}
						}
						else {
							if(data.entityData.geometry) item.entityData.geometry = { ...item.entityData.geometry, ...data.entityData.geometry };
							if(data.entityData.properties) item.entityData.properties = { ...item.entityData.properties, ...data.entityData.properties };
							if (item.entityType === "camera") item.slewLock = data.slewLock;
						}
					}
					return item;
				});
