...
const StatusCardDialog = ({ open, closeDialog, card, dir }) => {
	...
	const getComponentByType = (dir) => {
		...
		switch (control.type) {
			...
			case "text":
				return <EditText control={control} setData={setChildData} global={global} dir={dir} />;
			case "selector":
				return;
			default:
				break;
	...
	return (
		<Dialog
			open={open}
			confirm={{ label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.save"), action: () => save() }}
			abort={{ label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.cancel"), action: () => cancel() }}
			deletion={{ label: getTranslation("global.sharedComponents.statusCard.StatusCardDialog.delete"), action: () => deleteCard() }}
			options={{
				maxWidth: "xs",
				fullWidth: true
			}}
		>
			<div style={{ minHeight: "500px" }}>
				<TextField
					label={getTranslation("global.sharedComponents.statusCard.StatusCardDialog.fieldLabel.cardName")}
					fullWidth={true}
					margin="normal"
					value={cardName}
					variant="standard"
				...
				<FormControlLabel
					...
					/>}
					label={getTranslation("global.sharedComponents.statusCard.StatusCardDialog.global")}
