...
import { useMediaQuery, MenuItem, Button, FormLabel } from "@mui/material";
...
const RowEdit = ({
				...
				if (response) {
					switch (response.type) {
						...
						case "add":
							...
							setAttachments(_.uniqBy(_.compact([...attachments, response.new_val]), "id"));
							break;
						case "remove":
							...
							setAttachments(
								_.filter(attachments, (attachment) => attachment.fileId !== response.old_val.fileId)
							);
							break;
						default:
							break;
	...
	const handleChange = (name) => (event) => {
		...
		const value = lookupName
			? { id: lookupName.id, name: lookupName.name }
			: event && event.target && hasOwn(event.target, "value")
			? event.target.value
	...
	const handleCancel = () => {
		if (_.size(newFiles) > 0) {
			const removals = _.filter(attachmentRef.current, (attachment) => _.includes(newFiles, attachment.fileId));
	...
	const handleConfirm = () => {
		...
		if (_.size(deleted) > 0) {
			const removals = _.filter(attachmentRef.current, (attachment) => _.includes(deleted, attachment.fileId));
			_.each(removals, (file) => handleDeleteFile(file.handle, file.fileId));
		}
		...
		_.each(list.columns, (column) => {
			...
			if (column.type === "date-time" && !column.forCheckBox) {
				setState((prevState) => ({
					...prevState,
					[column.id]: moment(state[column.id])
				}));
		...
		if (adding) {
			...
			const updateRow = _.find(rows, (updateRow) => updateRow.order === row.order);
		...
		attachmentService.uploadFiles(list.id, "list", acceptedFiles).then(
			(result) => {
				...
				if (!adding) {
					...
					const updateRow = _.find(newRows, (updateRow) => updateRow.order === row.order);
					updateRow.attachments = updateRow.attachments ? [...updateRow.attachments, ...fileIds] : fileIds;
			...
			? _.map(filteredColumns, (column) => {
					...
					if (value !== undefined) {
						switch (type) {
							...
							case "choice": {
								const choice = _.find(options, (option) => option.value === value);
								return (
									<SelectField
										id={column.id}
										key={column.id}
									...
									>
										<MenuItem value="">
											<Translate value="global.sharedComponents.rowEdit.none" />
										</MenuItem>
									</SelectField>
								);
							}
	...
	const handleDeleteFile = (handle, fileId) => {
		attachmentService.removeAttachment(list.id, "list", fileId, (err, response) => {
			if (err) console.log(err, response);
				...
				const newRows = list.rows;
				const updateRow = _.find(newRows, (updateRow) => updateRow.order === row.order);
				updateRow.attachments = _.filter(updateRow.attachments, (attachmentId) => attachmentId !== fileId);
				updateList(list.id, { rows: newRows });
			}
		});
	};
	...
	const rowFileIds = row && row.attachments ? [...row.attachments, ...newFiles] : [...newFiles];
	const rowAttachments = _.filter(attachmentRef.current, (attachment) => _.includes(rowFileIds, attachment.fileId));
	...
	return dialog === dialogRef ? (
		<Dialog
			...
			abort={{
				label: getTranslation("global.sharedComponents.rowEdit.cancel"),
				action: handleCancel
			}}
			dir={dir}
		>
			<div style={{ width: isXS ? "auto" : 350 }}>
				...
				<div style={{ paddingTop: 24 }}>
					<Dropzone ref={dropzone} onDrop={onDrop} style={{ display: "none" }} multiple={false}>
					...
					{_.map(
						_.filter(rowAttachments, (attachment) => !_.includes(deleted, attachment.fileId)),
						(attachment) => (
							<FileLink
