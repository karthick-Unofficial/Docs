...
const WSImagePlayer = (props) => {
	...
	const [recoverPlaybackStreamState, setRecoverPlaybackStreamState] =
		useState();
	...
	useEffect(() => {
		...
		if (
			...
			currentReplayMedia.length > 0 &&
			playbackStartTime
		) {
			prevPropCurrentReplayMedia.current = currentReplayMedia;
			// -- restart playback video if the new media is not this camera
			const newMediaId = currentReplayMedia.filter(
				(media) => !prevPropCurrentReplayMedia.current.includes(media)
			)[0];
	...
	const setupPlaybackStream = (cameraId, videoProfile, playbackStartTime) => {
		// -- Playback video
		cameraService.setupPlaybackStream(
			cameraId,
			videoProfile,
			playbackStartTime,
			(err, response) => {
		...
		cameraService.setupPlaybackStream(
			...
			(err, response) => {
				...
				if (!response || response.result === false) {
					setError(true);
					return console.log(
						"Error restarting stream. Check camera-app logs for more details."
					);
				}

				const t1 = performance.now();
				console.log(
					`Time to restart playback stream took ${
						t1 - t0
					} milliseconds.`
				);
	...
	const startPlaybackStream = (cameraId, videoId) => {
		...
		cameraService.startPlaybackStream(
			cameraId,
			videoId,
			(err, response) => {
				if (err) console.log(err);
				if (!response || response.result === false) {
					return console.log(
						"Error starting stream. Check camera-app logs for more details."
					);
				}
	...
	const pausePlaybackStream = (cameraId, videoId) => {
		...
		cameraService.pausePlaybackStream(
			cameraId,
			videoId,
			(err, response) => {
				if (err) console.log(err);
	...
	return (
		<div style={styles.wrapper} ref={playerDiv}>
			{displayProgress &&
				!error &&
				(subscription || replaySetupStream) &&
				!videoData && <CircularProgress />}
			{error && (
