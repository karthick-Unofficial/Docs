...
const FacilityCondition = ({ enabled, order, dir, facilityId, context, locale, priorityOptions, initialDefaultPriority }) => {
	...
	let defPriororVal = defaultPriority;
	...
	const toggleConditions = (control) => {
		const data = {
			"critical": control === "defaultPriority" ? criticalTarget : !criticalTarget,
			"defaultPriority": defPriororVal
		}
		facilityService.toggleConditions(
			facilityId,
			data,
			(err, response) => {
				if (err) {
	...
	return !enabled ? (
		...
		<div>
			<div className={`widget-wrapper collapsed ${"index-" + order} `}>
				...
				<div className="widget-content" style={{ display: "flex", padding: "0px 6px", direction: dir }}>
					<FormControlLabel
						control={
							<Checkbox
								style={{
							...
							<div>
								< div style={{ marginTop: "15px", display: "flex" }}>
									...
									<Button
										variant="text"
										size="small"
										startIcon={dir === "ltr" ? <AddIcon fontSize="11px" style={{ marginRight: "-1px !important" }} /> : null}
										endIcon={dir === "rtl" ? <AddIcon fontSize="11px" style={{ marginLeft: "-1px !important" }} /> : null}
										style={{
											fontSize: "11px",
											textTransform: "none"
										}}
									...
									<Grid item xs={4} sm={4} md={4} lg={4}>
										<TextField
											...
											onChange={(e) => {
												...
												defPriororVal = e.target.value;
												toggleConditions("defaultPriority");
											}}
											variant="standard"
											label={getTranslation("global.profiles.widgets.facilityCondition.priority")}
										>
											{priorityValues && priorityValues.map(priorityValue => {
												return (
													<MenuItem value={priorityValue.value}>{priorityValue.label}</MenuItem>)
											}
											)}
										</TextField>
									</Grid>
								...
								{schedule && schedule.length > 0 ?
									<Divider style={{ background: "#626466", marginTop: "20px" }} />
									: null}
								{/*schedule list*/}
											...
											< Grid container style={{ marginTop: "15px" }}>
												...
												<Grid item xs={6} sm={6} md={6} lg={6}>
													<div>
														...
														>
															{getTranslation("global.profiles.widgets.facilityCondition.setScheduleLabel")}
														</Button>
														...
														<div style={{ marginTop: "-12px" }}>
															{element.schedule.anyTimeOfDay ?
																(
																	<Typography>
																		{getTranslation("global.profiles.widgets.facilityCondition.allDay")}
																	</Typography>
																) :
																(
																	<Typography fontSize={11} >
																		{convertUtcTime(element.schedule.startTime)} {timeZoneName} to {convertUtcTime(element.schedule.endTime)} {timeZoneName}
																	</Typography>
																)
															}
				...
				<Dialog
					...
					textContent={
						getTranslation("global.profiles.widgets.facilityCondition.removeDialog.confirmationText")
					}
					confirm={{
						action: () => {
