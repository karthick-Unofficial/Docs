	...
	switch (type) {
		...
		case "UPDATE_VIEWING_HISTORY": {
			...
			if (!id) {
				...
				newHistory = [{ id, name, type, profileRef, context, ...options }, ...state.viewingHistory];
			}
		...
		case "SUBSCRIPTION_DATA_BATCH_RECEIVED": {
			...
			if (_.size(_.intersection(_.map(data, "id"), _.map(state.viewingHistory, "id")))) {
				const newIds = _.intersection(_.map(data, "id"), _.map(state.viewingHistory, "id"));
				const newHistory = [...state.viewingHistory];
