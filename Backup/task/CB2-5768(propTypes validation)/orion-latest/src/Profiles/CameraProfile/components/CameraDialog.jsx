...
const CameraDialog = ({ camera, close, update, dir }) => {
	...
	const dialog = camera && useSelector((state) => state.appState?.dialog?.openDialog || "");
	...
	const handleNameChange = (event) => {
		if (event.target.value.length > 50) {
			setError({
				...error,
				name: getTranslation("global.profiles.cameraProfile.cameraDialog.errorText.lessThanFifty")
			});
			return;
		}
	...
	const handleSave = () => {
		if (!name) {
			setError({
				...error,
				name: getTranslation("global.profiles.cameraProfile.cameraDialog.errorText.noName")
			});
			return;
		}
	...
	const actions = [
		<Button variant="text" style={{ color: "#B2B6BB" }} onClick={handleCancel} key="cancel-action-button">
			{getTranslation("global.profiles.cameraProfile.cameraDialog.cancel")}
		</Button>,
		<Button variant="text" color="primary" onClick={handleSave} key="confirm-action-button">
			{getTranslation("global.profiles.cameraProfile.cameraDialog.confirm")}
		</Button>
	];

	return (
		<Dialog open={dialog === "cameraEditDialog"} onRequestClose={handleCancel} PaperProps={{ sx: { ...dialogStyles.dialog } }}>
