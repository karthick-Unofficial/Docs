...
import { Dialog, SelectField, TextField, DatePicker, TimePicker } from "orion-components/CBComponents";
import { useMediaQuery, MenuItem, Button } from "@mui/material";
...
import EventReOpenAndClose from "./EventReOpenAndClose";
import { eventUpdated } from "orion-components/GlobalData/Actions";
...
const eventOptimisticStateReducer = (state, action) => {
	switch (action.type) {
		case "GET_PREV_EVENT":
			return { ...state, prevEvent: action.payload };
		case "GET_UPDATED_EVENT":
...
const EventDialog = (props) => {
	const { editing, closeDialog, open, allTemplates, selectedEvent, dir, isTemplate } = props;

	const [state, stateDispatch] = useReducer(eventOptimisticStateReducer, initialState);
	...
	const [hasManuallyModifiedFields, setManuallyModifiedFields] = useState(false);

	const theme = useTheme();
	const isXS = useMediaQuery(theme.breakpoints.only("xs"));

	useEffect(() => {
		stateDispatch({ type: "GET_PREV_EVENT", payload: selectedEvent });
		stateDispatch({ type: "GET_UPDATED_EVENT", payload: selectedEvent });
	}, [selectedEvent]);

	const handleChange = (name, setState) => (event) => {
		// Date Picker event does not have target prop, returns a moment date object
		setManuallyModifiedFields(true);
		const value = event && event.target && !isNil(event.target.value) ? event.target.value : event;
		if (name === "startDate") {
	...
	const handleSave = (type = null, currentDateTime = null) => {
		const event = {
			...selectedEvent,
			name,
			desc: description,
		...
		if (editing) {
			//Implementing optimistic event updates helps mitigate delayed updates on the list panel, which can occur due to a slow network or low-performance system.
			stateDispatch({ type: "GET_UPDATED_EVENT", payload: event });
			dispatch(eventUpdated(selectedEvent.id, event));
			eventService.updateEvent(selectedEvent.id, event, (err, response) => {
				if (err || !response.success) {
					dispatch(eventUpdated(selectedEvent.id, state.prevEvent));
					stateDispatch({ type: "RESET_UPDATED_EVENT" });
	...
	const handleClose = () => {
		...
		setTemplate(-1);
		setManuallyModifiedFields(false);
	};
	...
	const disabled = Boolean((!name && checkTemplate) || ((!name || !startDate) && !checkTemplate) || !validEndDate);
				...
				<div>
					{editing && (
						<EventReOpenAndClose
							...
							endDate={endDate}
							onSave={handleSave}
							hasManuallyModifiedFields={hasManuallyModifiedFields}
						/>
					)}
				</div>
				<div>
					<Button style={{ color: "#B2B6BB" }} variant="text" onClick={handleClose} key="close-action-button">
						{getTranslation("global.profiles.eventProfile.eventDialog.cancel")}
					</Button>
					<Button
						color="primary"
						variant="text"
					...
					>
						{editing
							? getTranslation("global.profiles.eventProfile.eventDialog.update")
							: getTranslation("global.profiles.eventProfile.eventDialog.create")}
					</Button>
				</div>
			</div>
		);
		...
		>
			<div style={{ width: isXS ? "auto" : 500 }}>
				<TextField
					...
					label={
						checkTemplate
							? getTranslation("global.profiles.eventProfile.eventDialog.fieldLabel.templateName")
							: getTranslation("global.profiles.eventProfile.eventDialog.fieldLabel.eventName")
					}
					value={name}
				...
				{!selectedEvent && !checkTemplate ? (
					<SelectField
						...
						dir={dir}
					>
						<MenuItem key={"0"} value={-1} style={{ display: "flex" }}>
						...
						{map(allTemplates, (template) => {
							return (
								<MenuItem key={template.id} value={template.id} style={{ display: "flex" }}>
									{template.name}
								</MenuItem>
							);
						})}
