...
const Spotlight = ({ addMessageHandler, sendMessage }) => {
	...
	// Set window title
	window.document.title = window.cbProps ? window.cbProps.windowTitle : "New Window";
		...
		cameras.forEach(cam => {
			if (pc.indexOf(cam.id) < 0) {
				cameraService.moveHome(cam.id, (err, response) => {
					if (err) {
						console.log(`Error sending camera with ID (${cam.id}) home.`, err);
					} else {
						console.log(`Successfully sent camera with ID (${cam.id}) home.`);
					}
				});
			}
		});
	...
	return (
		<div style={{ backgroundColor: "#2c2d2f", height: "100vh" }}>
			<div style={{ display: "flex", backgroundColor: window.cbProps.spotlightColor || "#2C2D2F" }}>
				<h1 style={{
					...
					marginLeft: "10px",
					fontFamily: "roboto"
				}}><Translate value="spotLight.spotLightWindow.spotlight"/></h1>
			</div>
			<CameraGrid user={user} cameras={cameras} sendMessage={sendMessage} />
		</div>
	);
};
