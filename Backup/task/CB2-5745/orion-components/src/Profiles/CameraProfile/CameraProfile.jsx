...
} from "../../SharedActions/cameraProfileActions";
import { createCollection } from "../../SharedActions/commonActions";
...
const CameraProfile = ({
	...
	const activeFOV =
		isLoaded && (appId === "map-app" || appId === "events-app")
			? includes(
					keys(useSelector((state) => state.globalData.fovs.data)),
					contextId
			  )
			: undefined;
		...
		DEFAULT_WIDGET_CONFIG = [
			{
				...
				id: "linked_items",
				name: getTranslation(
					"global.profiles.cameraProfile.linkedItems"
				)
			},
			{
				enabled: true,
				...
				<ErrorBoundary>
					<SummaryWidget
						...
						actions={[
							...
							{
								name: getTranslation(
									"global.profiles.cameraProfile.hide"
								),
								nameText: "Hide",
								action: () => {
									setHiding(true);
					...
					{$(window).width() > 1023 && getWidgetStatus("map") && (
						<MapWidget
							...
							title={getTranslation(
								"global.profiles.cameraProfile.mapLocationFOV"
							)}
							expanded={mapVisible}
							dir={dir}
						/>
