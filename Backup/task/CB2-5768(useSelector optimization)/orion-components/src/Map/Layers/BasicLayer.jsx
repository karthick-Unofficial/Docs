...
const BasicLayer = ({ map, layer, labelsVisible, layerTypes, handleClick, before }) => {
	...
	useEffect(() => {
		...
		const mapboxType =
			features && features[0] && features[0].geometry.type ? features[0].geometry.type : "-unknown";
		// Remove labels if labelsVisible is false
		if (!labelsVisible) {
			map.setLayoutProperty(`ac2-${name}-${mapboxType}-symbol`, "text-field", "");
		}
		if (includes(layerTypes, "symbol")) {
			handleLoadImages();
		}
	...
	const getPrimaryLayerType = (layerTypes) => {
		if (includes(layerTypes, "fill")) {
			return "fill";
		} else if (includes(layerTypes, "line")) {
			return "line";
		} else if (includes(layerTypes, "circle")) {
			return "circle";
		} else {
			return "symbol";
		}
