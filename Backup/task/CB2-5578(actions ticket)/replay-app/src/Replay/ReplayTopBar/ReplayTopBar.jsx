...
const ReplayTopBar = memo(({ location, map, componentState, notifications, currentMedia, dir, locale }) => {
	...
	useEffect(() => {
		if (location && location.query) {
			if (location.query.name) {
				...
				setReplayName(`${capitalizedTitle}  ${location.query.startDate ? moment(location.query.startDate).locale(locale).format("MMMM D, YYYY") : ""}`);
			}
		}
	...
	if (location.query && location.query.endDate && location.query.startDate && locale) {
		...
		if (timeFormatPreference === "24-hour") {
			dateString = `${startDate.format("MMMM D, YYYY HH:mm")} ${getTranslation("replay.replayToBar.to")} ${startDate.isSame(endDate, "day") ?
				moment(endDate).format("HH:mm") :
				moment(endDate).format("MMMM D, YYYY HH:mm")}`;
		}
		else {
			dateString = `${startDate.format("MMMM D, YYYY h:mm A")} ${getTranslation("replay.replayToBar.to")} ${startDate.isSame(endDate, "day") ?
	...
	return (
		<div>
			<div style={styles.wrapperDiv}>
				<IconButton style={{ marginLeft: -16, marginRight: 8, padding: 12, color: "white" }} onClick={() => dispatch(openSettingsMenu())}>
					<Menu />
				</IconButton>
				<Typography variant="h3" color="inherit" style={styles.typography} > {capitalizedTitle}</Typography>
				...
				{!window.api && (
					<Button onClick={() => updateOpen(true)}
						style={{ marginLeft: "auto", marginRight: 22, height: 30, width: 105, color: "#fff" }}
						variant="contained"
						color="primary"
					>
			...
			>
				...
				>
					<TextField
						...
						InputProps={{ style: { fontSize: 14 }, classes: { underline: classes.underline } }}
