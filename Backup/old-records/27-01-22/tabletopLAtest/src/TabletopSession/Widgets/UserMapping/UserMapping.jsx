	...
	return (
		<div>
					...
					{_.keys(teamsConfig).map(team => {
						return (
							<ToggleButton key={`${team}-size`} value={team} aria-label={team} onClick={handleTeamclick}
								...
								<div style={{marginLeft: 20, textTransform: "none"}}>{`${team.charAt(0).toUpperCase()}${team.substring(1).toLowerCase()} Team`}</div>
							</ToggleButton>
						);
					})}
				</div>
					...
					<div className={"userMappingsList"} key={userMapped.userId} id={userMapped.userId} style={{display:"block"}}>
						...
						{userMapped.selfAgentIds && userMapped.selfAgentIds.length > 0 &&
							<div className="usersList">
								<p className="b2-white"><Star style={{color: "white", verticalAlign: "middle", marginRight: 10}} /><span>Player’s primary roles</span></p>
						...
						{userMapped.controlledAgentIds && userMapped.controlledAgentIds.length > 0 &&
							<div className="usersList">
								<p className="b2-white"><StarOutline style={{color: "white", verticalAlign: "middle", marginRight: 10}} /><span>Player can send orders and tasking to</span></p>
			...
			{userMappingList.length === 0 &&
				<div className="b1-white" style={{textAlign: "center", marginTop: 30}}>No player mappings to display.</div>
			}
		</div>
	);
};
