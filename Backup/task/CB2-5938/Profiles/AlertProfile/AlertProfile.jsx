...
import { PinnedItemsWidget, Activities, CamerasWidget, FileWidget } from "../Widgets/index";
...
const AlertProfile = (props) => {
	...
	const renderTabs = () => {
		...
		return (
			<CardContent style={{ padding: 0 }}>
				...
				{tab === 2 && (
					<PinnedItemsWidget contextId={id} readOnly={readOnly} subscriberRef="dock" isAlertProfile={true} />
				)}
				{tab === 3 && (
					<Activities
	...
	const { message, activityDate, createdDate, isPriority, geometry, closed, target, object } = notification;
					...
					<Card style={{ marginBottom: 12, borderRadius: 5 }}>
						<CardHeader
							...
							avatar={
								...
								>
									{geometry ? <TargetingIcon geometry={geometry} defaultVisibility={true} /> : null}
									{isPriority ? (
