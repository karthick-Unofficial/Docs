...
const CamerasDock = ({ map, readOnly, selectFloorPlanOn, floorPlansWithFacilityFeed }) => {
	...
	const cameraModules = dockedCameras.map((id, index) => {
		...
		return (
			<div className={"docked-camera-wrapper " + (!camera && sidebarOpen ? "dashed-border" : "")} key={index}>
				<CameraDockModule
