...
const NewMapReplay = ({ editing, toggleEditing, params }) => {
	...
	useEffect(() => {
		setBeginningDate(moment().subtract(clipDuration, "minutes"));
	}, [clipDuration]);
	...
	const getDurationString = (startDate, endDate) => {
		...
		return diff.asHours() >= 1
			? `${parseInt(diff.asHours()).toLocaleString(locale)} ${getTranslation("newReplay.newMapReplay.hours")}`
			: `${parseInt(diff.asMinutes()).toLocaleString(locale)} ${getTranslation(
					"newReplay.newMapReplay.minutes"
			  )}`;
	};
			...
			>
				<div
					style={{
						...
						width: replayFrameValue === "clip-duration" ? "50%" : "100%"
					}}
				>
					<SelectField
						id="authType"
						...
						<Fragment>
							...
							<SelectField
								...
								items={durationOptions.map((option) => {
									return {
										value: `${option.toLocaleString(locale)} ${getTranslation(
											"newReplay.newMapReplay.minutes"
										)}`,
										id: option
									};
				...
				>
					...
					<Button
						onClick={() => {
							...
							const location =
								`${window.location.origin}/replay-app/#/replay?type=map&coordinates=${mapCoordinates}&startDate=${startDate}&endDate=${finalDate}` +
								`${params && params.name && params.id ? `&name=${params.name}&id=${params.id}` : ""}`;
							if (editing) {
