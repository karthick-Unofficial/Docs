...
const _getMain = (rule) => {
	...
	switch (rule.trigger) {
		case "berth-assignment-created":
			main = getTranslation(
				"global.ruleBuilder.vesselEvent.newBerthCreated"
			);
			break;
		case "berth-assignment-approval":
			main = getTranslation(
				"global.ruleBuilder.vesselEvent.berthApproved"
			);
			break;
		case "berth-assignment-update":
			main = getTranslation(
				"global.ruleBuilder.vesselEvent.assignmentUpdated"
			);
			break;
		case "arrival":
		...
		case "berth-security-violation":
			main = getTranslation(
				"global.ruleBuilder.vesselEvent.securityViolation"
			);
			break;
		default:
			break;
