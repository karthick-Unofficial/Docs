...
class ViewRule extends Component {
	...
	render() {
		...
		const actions = [
			<FlatButton label={<Translate value="viewRule.actions.flatButtonLabel.cancel" />} primary={true} onClick={this._handleClose} />,
			<FlatButton
				label={<Translate value="viewRule.actions.flatButtonLabel.yesDelete" />}
				primary={true}
		...
		let alertAudio = this.placeholderConverter("viewRule.alertAudio.speak");
		if (rule.audioSettings) {
			if (rule.audioSettings.speakAlertText && rule.audioSettings.speakAlertNotification)
				alertAudio += "\"" + rule.audioSettings.alertText + "\", " + this.placeholderConverter("viewRule.alertAudio.followedBy") + "";
				...
				alertAudio = this.placeholderConverter("viewRule.alertAudio.speakAlert");
		} else {
			alertAudio += "\"" + this.placeholderConverter("viewRule.alertAudio.Alert") + ", " + this.placeholderConverter("viewRule.alertAudio.speakAlert") + "";
		}

		const dismissText = rule.dismissForOrg ? this.placeholderConverter("viewRule.dismissText.dismissAlertsOrg")
			: this.placeholderConverter("viewRule.dismissText.dismissAlertsUser");
		...
		return (rule.id ? <div className="rules-wrapper" style={{ overflowY: "scroll", height: `calc(100vh - ${WavCamOpen ? "308px" : "48px"})` }}>
			...
			<div className="row">
				<div className="row-item fullwidth">
					<div className="alert-table">
						...
						{this.state.edited && (
							<RaisedButton
								label={<Translate value="viewRule.alertTable.raisedButton.save" />}
								primary={true}
			...
			<Dialog
				...
				title={<Translate value="viewRule.dialog.title" />}
				actions={actions}
				modal={false}
