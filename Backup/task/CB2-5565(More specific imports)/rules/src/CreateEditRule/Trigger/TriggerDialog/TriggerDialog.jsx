			...
			dialogRows = shapeList.filter((shape) => {
				...
				.filter(shape => {
					if (typeAheadFilterValue === "") return shape;
						...
						if (includes(shape.entityData.properties.name.toLowerCase(), typeAheadFilterValue.toLowerCase())) return shape;
						else return false;
					}
				});
			...
			dialogRows = availableTargets.filter((item) => {
				...
				.filter(item => {
					if (typeAheadFilterValue === "") return item;
						...
						if (includes(item.name.toLowerCase(), typeAheadFilterValue.toLowerCase())) return item;
						else return false;
					}
				});
