...
const Lookup = ({ type, endpoint, fields, canManage, dir }) => {
	...
	useEffect(() => {
		restClient.exec_get(
			`/berth-schedule-app/api/${endpoint}/query/by${upperFirst(
				type
			)}Data?searchProps=${filter(
				keys(fields),
	...
	const localTypeSingle = startCase(type);
	...
	const filteredValues = filter(fields, field => field.searchBy);
