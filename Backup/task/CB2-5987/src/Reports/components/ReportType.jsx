...
const ReportItem = ({ report, eventName, eventId, id, dir }) => {
	...
	if (id && id === "sitrep" && id === report.id) {
		...
		return (
			<Link state={{ id: report.id, name: report.name }} to={`/report-builder/${report.id}`} key={report.id}>
				<ListItem style={styles.listItem}>
					...
					<ListItemText
						...
						primary={getTranslation(report.pdfOnly ? "mainpage.list.eventTitle" : report.name)}
						secondary={getTranslation(report.desc)}
					/>
				</ListItem>
			</Link>
		);
