...
export const searchValueSelector = createSelector(listPanelState, (state) => {
	return state.searchValue.toLowerCase();
});

export const mapFiltersById = createSelector(mapFiltersState, (filters) => {
	if (!filters) {
		return [];
	}
	const filterIds = [];
...
export const filteredFeaturesSelector = createDeepEqualSelector(mapFiltersById, mapEntities, (ids, entities) => {
	if (!ids.length) {
		return;
	}
	const features = {};
...
export const mapFiltersSelector = createDeepEqualSelector(filteredFeaturesSelector, (entities) => entities);

export const eventSearchSelector = createSelector(listPanelState, (state) => {
	return state && state.eventSearch && state.eventSearch.toLowerCase();
});

export const eventTemplateSearchSelector = createSelector(listPanelState, (state) => {
