...
const SymbolCollection = ({ name, collection, expanded, handleSelect, handleExpand, search, classes }) => {
	return (
		<Accordion expanded={expanded} onChange={handleExpand} sx={{ margin: "0 0 8px 0", boxShadow: "none" }}>
			<AccordionSummary
					...
					{collection
						.filter((symbol) => {
							...
							const filtered =
								...
								!!keywords.filter((keyword) => keyword.toLowerCase().includes(search)).length > 0;
							return filtered;
						})
						.map((symbol) => {
							const { name, path } = symbol;
							return (
								<Grid item key={name} xs={2.4} onClick={() => handleSelect(name)}>
									<img
										style={{
											width: 50,
											height: 50,
