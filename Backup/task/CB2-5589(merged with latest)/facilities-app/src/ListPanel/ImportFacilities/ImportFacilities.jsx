...
import untar from "js-untar"; // cSpell:ignore untar
import { Translate, getTranslation } from "orion-components/i18n";
import { useDispatch, useSelector } from "react-redux";
import { createFacility, createFloorPlan, facilitiesImportDone } from "./importFacilitiesActions";
...
const ImportFacilities = ({ facilities, close }) => {
	...
	const validateFloorPlan = (floorplan) => {
		...
		if (!floorplan.name) {
			...
			!filesRef.current.find(
				(containedFile) => containedFile.name.toLowerCase() === `images/${floorplan.image.toLowerCase()}`
			)
		) {
	...
	const importFacility = useCallback(
		(facility) => {
			const facilityToAdd = {
	...
	const processFacility = useCallback(() => {
	...
	}, [dispatch, facilities, importFacility, setRerender]);

	const importFloorPlans = useCallback(() => {
		const currentState = importStateRef.current;
		if (
			facilitiesImportData &&
			hasOwn(facilitiesImportData, currentState.facilityId) &&
		...
		const index =
			facilitiesImportData && facilitiesImportData[currentState.facilityId]
				? facilitiesImportData[currentState.facilityId].length
				: 0;
		...
		const extractedFile = filesRef.current.find(
			(containedFile) => containedFile.name.toLowerCase() === `images/${floorPlan.image.toLowerCase()}`
		);
		if (extractedFile) {
	...
	}, [dispatch, facilitiesImportData, processFacility, setRerender]);

	useEffect(() => {
		const currentState = importStateRef.current;
		if (facilities && currentState.facility && !currentState.facilityId) {
			const facility = values(facilities).find(
				(existingFacility) =>
					existingFacility.entityData.properties.name.toLowerCase() ===
	...
	}, [facilities, importFloorPlans, setRerender]);

	useEffect(() => {
		if (facilitiesImportData) {
			if (facilitiesImportData.error) {
				statusRef.current += `\nERROR: ${
					typeof facilitiesImportData.error === "string"
						? facilitiesImportData.error
						: JSON.stringify(facilitiesImportData.error)
				}`;
				setRerender(Math.random());
	...
	}, [facilitiesImportData, importFloorPlans, setRerender]);
	...
	const handleUpload = (file) => {
		...
		reader.onload = () => {
			...
			untar(binaryStr).then(
				async (extractedFiles) => {
					// onSuccess
					// Find the file 'Facilities.json' and load it
					const facilitiesConfig = extractedFiles.find(
						(extractedFile) => extractedFile.name.toLowerCase() === "facilities.json"
					);
					if (!facilitiesConfig) {
	...
	return (
		<Dialog
			...
			paperPropStyles={{
				width: 500,
				height: `${file ? "400px" : "170px"}`
			}}
		>
			<div>
				{!file && (
					<Dropzone accept="application/x-tar" onDrop={(acceptedFiles) => handleUpload(acceptedFiles)}>
				...
				>
					...
					{importDone && (
						<Button color="primary" onClick={close}>
							<Translate value="listPanel.importFac.dialog.close" />
						</Button>
					)}
				</div>
			</div>
