...
export const openPanel = (appId, panelName) => {
	return (dispatch, getState) => {
		...
		const globalDockState =
			state.appState &&
			state.appState.persisted &&
			state.appState.persisted.globalDockState;
		if (!globalDockState) {
			return;
		}
		...
		if (
			...
			globalDockState.leftDock.availablePanels.includes(panelName)
		) {
			...
			globalDockState.rightDock.availablePanels.includes(panelName)
		) {
		...
		if (newDockState) {
			dispatch(
				updatePersistedState(appId, dockStateKey, newDockState, false)
			);
		}
	};
};
...
export const closePanel = (appId, panelName) => {
	return (dispatch, getState) => {
		...
		if (
			globalDockState &&
			globalDockState.leftDock &&
			globalDockState.leftDock.currentPanel === panelName
		) {
			newDockState = {
				...globalDockState,
				leftDock: {
			...
			globalDockState &&
			globalDockState.rightDock &&
			globalDockState.rightDock.currentPanel === panelName
		) {
			newDockState = {
				...globalDockState,
				rightDock: {
...
export const moveToOtherDock = (
	...
	return (dispatch) => {
		...
		if (dockDirection === "left") {
			const newLeftPanels =
				globalDockState.leftDock.availablePanels.filter(
			...
			const newRightPanels =
				globalDockState.rightDock.availablePanels.filter(
					(panel) => panel !== panelName
				);
			newDockState = {
				leftDock: {
...
export const saveDockState = (appId, globalDockState) => {
	return (dispatch) => {
		dispatch(
			updatePersistedState(appId, dockStateKey, globalDockState, false)
		);
	};
};
