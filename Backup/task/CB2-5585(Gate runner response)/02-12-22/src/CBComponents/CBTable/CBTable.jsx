...
let CBTableHead = ({ classes, columns, removing, hasAttachments, dir }) => {
	...
	return (
		<TableHead>
			<TableRow>
				{removing && (
					<TableCell className={classes.head} variant="head" padding="none" style={tableHeadStyles.tableCell} />
				)}
				...
				switch (column.type) {
					case "date-time":
						if (!row.data[column.id]) value = "";
							...
							value = timeConversion.convertToUserTime(moment(row.data[column.id]).locale(locale), `full_${timeFormatPreference}`);
							if (timeFormatPreference === "12-hour") {
								...
								value = getLocalize(`${value.split(" ")[0]} ${value.split(" ")[1]}`, "global.date.full24-hour") + value.split(" ")[2];
							}
						}
						else if (!column.includeTime) {
							value = timeConversion.convertToUserTime(moment(row.data[column.id]).locale(locale), "MM/DD/YYYY");
