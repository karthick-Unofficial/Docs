...
const MissionControlWidget = ({ order, enabled, details, dir }) => {
	...
	const getStatusPanel = useCallback(() => {
		...
		const slopeIcon =
			grade >= 0 ? <SlopeUphill className={"detail-icon"} /> : <SlopeDownhill className={"detail-icon"} />;
		const panelDetails =
			state !== "charging" ? (
				<div className="details-container">
					...
					<div className="detail">
						...
						<p className={"detail-content"} style={{ marginBottom: "-3px" }}>
							{speed.toFixed(1)}
						</p>
						<p className={"detail-content attribute"} style={{ marginBottom: "-1px" }}>
	...
	return (
		<BaseWidget
			enabled={enabled}
			order={order}
			title={getTranslation("global.profiles.widgets.missionControl.missionControl")}
			dir={dir}
		>
			{/* STATUS PANEL */}
			{getStatusPanel()}
