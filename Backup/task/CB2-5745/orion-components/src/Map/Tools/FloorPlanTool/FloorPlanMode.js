...
FloorPlanMode.getHandle = function (coordinates) {
	...
	const handlePoint = destination(
		polyCenter,
		distanceFromSide + distanceFromCenter,
		polyBearing
	);
	return handlePoint;
};
...
FloorPlanMode.setRotationHandle = function (state) {
	...
	state.rotateArm.updateCoordinate(
		"0",
		sideMidpoint.geometry.coordinates[0],
		sideMidpoint.geometry.coordinates[1]
	);
	state.rotateArm.updateCoordinate(
		"1",
		newHandle.geometry.coordinates[0],
		newHandle.geometry.coordinates[1]
	);
	state.rotateHandle.updateCoordinate(
		"0",
		newHandle.geometry.coordinates[0],
		newHandle.geometry.coordinates[1]
	);
...
FloorPlanMode.onSetup = function (opts) {
	...
	const { handles, rotateHandle, rotateArm } = this.addHandles(
		floorPlan.coordinates[0],
		{ initial: true }
	);
				...
				activeHandle.properties.index > 1
					? Object.values(handles).find(
							(handle) =>
								...
								activeHandle.properties.index - 2
					  )
					: Object.values(handles).find(
							(handle) =>
								handle.properties.index ===
...
FloorPlanMode.onDrag = function (state, e) {
	...
	const { activeHandle, dragFloorPlan, rotating, floorPlan, rotateHandle } =
		state;
	...
	if (rotating) {
		...
		const prevBearing = bearing(
			polyCenter,
			point(rotateHandle.coordinates)
		);
