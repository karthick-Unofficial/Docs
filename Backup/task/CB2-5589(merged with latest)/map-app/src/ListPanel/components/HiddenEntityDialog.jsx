...
const propTypes = {
	...
	unignoreEntities: PropTypes.func,
	exclusions: PropTypes.obj
};

const HiddenEntityDialog = ({ open, toggleDialog, unignoreEntities, exclusions }) => {
	...
	return (
		...
		>
			<div className="hiddenEntityScroll scrollbar" tabIndex="0" ref={focusRef} style={{ width: "360px" }}>
				{exclusions
					.sort((a, b) => {
					...
					.map((item) => {
						return (
							<div
								className={`ec-dialog-item dockItem ${removalArray.includes(item.entityId) ? "ec-remove" : "ec-keep"
									}`}
								key={item.entityId}
							>
								...
								<DockItemLabel
									...
									secondary={
										item.iconType === ("Emergent" || "Planned")
											? getTranslation("listPanel.hiddenEntityDialog.event")
											: item.iconType
									}
								/>
								<i className="material-icons" onClick={() => handleRemoveMember(item.entityId)}>
									{removalArray.includes(item.entityId) ? "add_circle" : "cancel"}
								</i>
							</div>
						);
					})}
...
HiddenEntityDialog.propTypes = propTypes;
