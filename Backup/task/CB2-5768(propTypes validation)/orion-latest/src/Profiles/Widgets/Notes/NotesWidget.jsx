...
import { isWidgetLaunchableAndExpandable } from "orion-components/Profiles/Selectors";
...
class NotesWidget extends Component {
	...
	static getDerivedStateFromProps(nextProps, prevState) {
		...
		if (
			...
			(nextProps.notes && nextProps.notes.html !== prevState.originalHTML && !prevState.hasChanged)
		) {
			return {
	...
	replaceWhitespace = (str) => {
		let newStr = str.slice().split(" ").join("&nbsp;");
	...
	saveEdit = async (forceUpdate) => {
		...
		if (forceUpdate) {
			const notes = {
				path: this.props.notes ? (this.props.notes.path ? this.props.notes.path : "") : "",
			...
			if (this.state.currentHtml === "<p><br></p>") {
				...
				if (this.props.updateNotesStatus) this.props.updateNotesStatus(false);
			} else {
	...
	render() {
		...
		return selected || !enabled ? (
			...
			<section id="notes-wrapper" className={`${expanded ? "expanded" : "collapsed"}  widget-wrapper`}>
				{!expanded && (
					<div id="notes-widget" className="widget-content">
						<div className="widget-header">
							<div className="cb-font-b2">
								<Translate value="global.profiles.widgets.notes.title" />
							</div>
							<FileDocumentEditOutline
								style={{
									color: `${event.notes ? "#4eb5f3" : "#b5b9be"}`,
									...(dir === "rtl" && {
										marginRight: "5px"
									}),
									...(dir === "ltr" && { marginLeft: "5px" })
								}}
							/>
							<div className="widget-header-buttons">
								{secondaryExpanded && this.state.notesChanged && (
									<div className={"widget-option-button"}>
										<Button variant="text" color="primary" onClick={() => this.cancelEdit()}>
											{getTranslation("global.profiles.widgets.notes.cancel")}
										</Button>
										<Button variant="text" color="primary" onClick={() => this.saveEdit()}>
											{getTranslation("global.profiles.widgets.notes.save")}
										</Button>
									</div>
								)}
				...
				{expanded && (
					<div id="notes-widget" className="widget-content">
							...
							textContent={
								this.state.actor[0]
									? this.state.actor.length < 2
										? getTranslation(
												"global.profiles.widgets.notes.textContent1",
												this.state.actor[0].name
										  )
						...
						<Dialog
							...
							textContent={getTranslation("global.profiles.widgets.notes.uploadingNewImages")}
							dir={dir}
						/>
					</div>
				)}
...
const mapStateToProps = (state) => {
	const launchableAndExpandable = isWidgetLaunchableAndExpandable(state);
