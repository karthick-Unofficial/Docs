...
const EventCard = (props) => {
	const { event, selectFloorPlanOn, getPinnedItems, profileIconTemplate } =
		props;
	const dispatch = useDispatch();

	const eventStats = useSelector(
		(state) => state.globalData.events.eventStatistics[event.id]
	);
	...
	if (isTemplate) {
		...
		const expired = event.endDate
			? moment(event.endDate) < moment()
			: false;
		const dateTimeFormat = `MMM D, YYYY ${
			timeFormatPreference === "24-hour" ? "H" : "h"
		}:mm ${timeFormatPreference === "24-hour" ? "" : "A "}z`;
		const startDate = moment(event.startDate);
		const endDisplayDate = endDate
			? getTranslation("global.events.closed") +
			  timeConversion.convertToUserTime(startDate, dateTimeFormat)
			: null;
	...
	const childSelected =
		selectedEntity &&
		pinnedItems &&
		Object.keys(pinnedItems).indexOf(selectedEntity.id) >= 0;
		...
		>
			{pinnedItems &&
				Object.values(pinnedItems).map((item) => {
					...
					properties.profileIconTemplate =
						profileIconTemplates[item.feedId];
