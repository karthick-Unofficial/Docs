...
const TargetingIcon = (props) => {
	...
	useEffect(() => {
		if (feedLayerCheck && map) {
			const layers = map.getStyle().layers;
			const layerByFeedId = layers.filter((layer) => layer.source && layer.source.includes(props.feedId));
			setVisibility(size(layerByFeedId) ? true : false);
		}
	}, []);
	...
	const shouldRender = !isDisabled && map && mapVisible && geometry && (!filters || !filters.length || filters.includes(id)) && visibility;

	return (
		shouldRender ? (
			<React.Fragment>
				{multipleTargets ?
					geometry && geometry.map((data) => {
						return (
