...
const EventShareDialog = ({
	...
	const handleShareToggle = orgId => {
		sharedWith.includes(orgId)
			? setSharedWith(sharedWith.filter(org => {
				return org !== orgId;
			}))
	...
	return (
		<React.Fragment>
			...
			<Dialog
				...
				abort={{ label: getTranslation("global.profiles.eventProfile.eventShareDialog.cancel"), action: handleClosePublishDialog }}
			...
			<Dialog
				...
				abort={{ label: getTranslation("global.profiles.eventProfile.eventShareDialog.cancel"), action: handleCloseShareDialog }}
				title={
					orgs.length >= 1
