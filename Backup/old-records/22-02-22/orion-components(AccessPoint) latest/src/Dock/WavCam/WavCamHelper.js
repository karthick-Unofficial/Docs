...
class WavCamHelper
{
	constructor(metadata) {
		...
		this.CurrentAz = this.CAMERA_AZ = parseFloat(metadata["x-wavcam-camera-az"]);
		this.CurrentEl = this.CAMERA_EL = parseFloat(metadata["x-wavcam-camera-el"]);
		this.currentVFOV = this.CAMERA_VFOV = parseFloat(metadata["x-wavcam-vfov"]);
		this.currentHFOV = this.CAMERA_HFOV = parseFloat(metadata["x-wavcam-hfov"]);
	...
	updateParams(metadata) {
		...
		this.VIEWABLE_AREA_HEIGHT = parseFloat(metadata["x-wavcam-viewable-area-height"]);
		this.VIEWABLE_AREA_WIDTH = parseFloat(metadata["x-wavcam-viewable-area-width"]);
	...
	convertLatLonToAzEl(lat, lng)
	{
		...
		const detEl = Math.atan2(EarthRadiusUnits["METERS"] * Math.sin(thetaR),
			this.CAMERA_ALT + EarthRadiusUnits["METERS"] * (1.0 - Math.cos(thetaR))) - (Math.PI / 2.0);

		const distance = this.sphericalDistance(this.CAMERA_LAT, this.CAMERA_LON, lat, lng, DistanceUnit.METERS);
	...
	convertAzElToPercentXY(az, el)
	{
		...
		const adjAz = DEG_TO_RAD * (this.currentHFOV / 2.0 - this.currentHFOV * x + 90.0);
		const xCoord = Math.cos(adjAz);
		const yCoord = Math.cos(Math.abs(DEG_TO_RAD * this.CurrentEl)) * Math.sin(adjAz);
