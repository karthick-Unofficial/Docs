...
const MapControls = ({
	...
	const entityData = context
		? { entity: context.entity, fov: context.fov }
		: {};
	...
	if (entityData.entity) {
		canEdit =
			...
			user.integrations.find(
				(int) => int.intId === entityData.entity.feedId
			) &&
			user.integrations.find(
				(int) => int.intId === entityData.entity.feedId
			).permissions &&
			user.integrations
				.find((int) => int.intId === entityData.entity.feedId)
			...
			<ToolTray dockOpen={dockOpen} dir={dir}>
				...
				{toolType && (
					<SaveCancel
						...
						handleConfirm={handleConfirm}
						disabled={
							feature == null || !validateShape(feature.geometry)
								? true
								: false
						}
					/>
