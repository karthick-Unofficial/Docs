...
const TileOptions = ({ setMapStyle }) => {
	const dispatch = useDispatch();

	const mapSettings = useSelector(
		(state) => state.appState.persisted.mapSettings
	);
	const baseMaps = useSelector((state) => state.baseMaps);
	const selected =
		mapSettings && mapSettings.mapStyle
	...
	useEffect(() => {
		if (baseMaps.length > 0) {
			const selectedThumbnail = baseMaps.filter(
			...
			tileLayer.name !== layer && (
				...
				>
					<div
						className="thumb"
						style={{
							backgroundImage: `url(${tileLayer.thumbnail})`
						}}
					/>
					<div className={dir === "rtl" ? "labelRTL" : "label"}>
						{tileLayer.label}
					</div>
				</div>
			)
