...
const WavCamOverlay = ({ imageMetadata, fovItems, visible }, ref) => {
	const canvasContainerRef = React.useRef();
	const canvasRef = React.useRef();
	const [fovEntities, setFovEntities] = useState([]);
	...
	const renderOverlay = (metadata) => {
		...
		if (md && fovItems) {
			...
			canvasRef.current.style.width =
				canvasContainerRef.current.style.width = containerW + "px";
			canvasRef.current.style.height =
				canvasContainerRef.current.style.height = containerH + "px";
			canvasRef.current.style.left =
				canvasContainerRef.current.style.left = containerX + "px";
			canvasRef.current.style.top = canvasContainerRef.current.style.top =
				containerY + "px";

			const tempFovEntities = [];
			...
			if (fovItems) {
				for (const key of Object.keys(fovItems)) {
					...
					if (displayName) {
						const azEl = helper.convertLatLonToAzEl(
							geometry.coordinates[1],
							geometry.coordinates[0]
						);
						const percentXY = helper.convertAzElToPercentXY(
							azEl[0],
							azEl[1]
						);
	...
	return (
		<div
			style={
				visible ? styles.containerStyle : styles.containerStyleHidden
			}
			ref={canvasContainerRef}
		>
			<canvas ref={canvasRef} />

			{fovEntities.map((item) => (
				<div
					key={item.id}
					style={{
						position: "absolute",
