...
const SpotlightProximity = ({ globalState, updateGlobalSettings, dir }) => {
	...
	const handleUpdate = (e) => {
		...
		if (isNaN(value) || Number(value) === 0) {
			setError(
				"global.appBar.optionsDrawer.spotlightProx.errorText.invalidProx"
			);
		} else if (decimals && decimals.length > 3) {
			setError(
				"global.appBar.optionsDrawer.spotlightProx.errorText.limitedValue"
			);
		} else {
			setError(false);
		}
			...
			<div style={{ display: "flex", alignItems: "flex-start" }}>
				<TextField
					label={getTranslation(
						"global.appBar.optionsDrawer.spotlightProx.defaultProx"
					)}
					value={proximity}
					...
					endAdornment={
						outputUnit == "km"
							? getTranslation(
									"global.appBar.optionsDrawer.spotlightProx.km"
							  )
							: getTranslation(
									"global.appBar.optionsDrawer.spotlightProx.mi"
							  )
					}
					endAdornmentStyles={{}}
