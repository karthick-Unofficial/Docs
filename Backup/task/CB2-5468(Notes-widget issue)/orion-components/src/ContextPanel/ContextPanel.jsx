...
const ContextPanel = (props) => {
	...
	useEffect(() => {
		...
		if (prevProps) {
			...
			if (prevProps.secondaryOpen && !secondaryOpen) {
				dispatch(setMapOffset(secondaryWidth ? -secondaryWidth : state.secondaryWidth ? -state.secondaryWidth : 0));
	...
	return !hidden ? (
		<Fragment>
			...
			>
				...
				{showMobileToggle && (
					<div id="toggle-mobile">
						{primaryOpen || secondaryOpen ? (
							...
							>
								{context ? getTranslation("global.contextPanel.openProfile") : getTranslation("global.contextPanel.openListPanel")}
							</Button>
						)}
					</div>
				)}
				...
				{!readOnly && (<div className={`dock-control-inner ${className}`}>
					...
					<div>
						...
						>
							{dir && dir == "rtl" ?
								<ChevronRight className={`close-arrow
								${primaryOpen ? "" : "close-arrow-removed"}
								`} color="#fff" style={dir && dir == "rtl" ? { right: "-150%" } : {}} /> :
					...
					<div className="profile-wrapper">
						...
						{history.length >= 2 && (
							<div id="profile-navigation">
								...
								>
									<div>
										{dir && dir == "rtl" ? <ChevronRight style={{ marginLeft: 0, verticalAlign: "middle" }} /> : <ChevronLeft style={{ marginLeft: 0, verticalAlign: "middle" }} />}
										<span style={{ verticalAlign: "middle", fontSize: "14px", fontWeight: "bold", padding: "0 16px 0 8px", letterSpacing: "0px" }}>
											{
												history[1].name
			...
			>
				<IconButton>
					<ViewList color="#fff" onClick={() => dispatch(openPrimary())} />
				</IconButton>
				{actionButtons &&
