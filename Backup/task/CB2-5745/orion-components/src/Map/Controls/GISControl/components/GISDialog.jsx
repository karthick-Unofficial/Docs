...
import { useMediaQuery, Typography, MenuItem } from "@mui/material";
	...
	return (
		<Dialog
			...
			confirm={{
				label: error
					? getTranslation(
							"global.map.controls.gisControl.gisDialog.retry"
					  )
					: getTranslation(
							"global.map.controls.gisControl.gisDialog.add"
					  ),
				action: handleSave,
				disabled:
					!newService.name ||
			...
			abort={{
				label: getTranslation(
					"global.map.controls.gisControl.gisDialog.cancel"
				),
		...
		>
			<div style={{ width: isXS ? "auto" : 350 }}>
				<TextField
					id="name"
					label={getTranslation(
						"global.map.controls.gisControl.gisDialog.fieldLabel.serviceName"
					)}
				...
				<TextField
					id="url"
					label={getTranslation(
						"global.map.controls.gisControl.gisDialog.fieldLabel.esriServiceEndpoint"
					)}
					...
					helperText={getTranslation(
						"global.map.controls.gisDialog.helperText.exampleURL",
				...
				<SelectField
					id="auth-select"
					label={getTranslation(
						"global.map.controls.gisControl.gisDialog.fieldLabel.authType"
					)}
				...
				{authType === "login" && (
					<Fragment>
						<TextField
							id="username"
							label={getTranslation(
								"global.map.controls.gisControl.gisDialog.fieldLabel.username"
							)}
						...
						<TextField
							id="password"
							label={getTranslation(
								"global.map.controls.gisControl.gisDialog.fieldLabel.password"
							)}
				...
				{authType === "token" && (
					<TextField
						id="token"
						label={getTranslation(
							"global.map.controls.gisControl.gisDialog.fieldLabel.token"
						)}
						value={creds.token}
