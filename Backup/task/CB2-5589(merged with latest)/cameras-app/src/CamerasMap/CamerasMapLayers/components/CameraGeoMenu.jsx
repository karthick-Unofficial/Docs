...
import { DialogContentText, MenuItem, Popover, ListItemIcon, ListItemText, Fab } from "@mui/material";
import { AddLocation as Add, PinDrop as Assign, EditLocation as Edit, Delete, Camera, Undo } from "@mui/icons-material";
...
const CameraGeoMenu = ({ handleSelect, handleConfirm, entity, closeDialog, openDialog, dialog }) => {
	...
	return (
		<Fragment>
			<Fab
				onClick={!geometry ? () => handleSelection("camera-add") : open ? handleMenuClose : handleMenuOpen}
				color="primary"
			>
				{geometry ? <Assign style={{ color: "#FFF" }} /> : <Add style={{ color: "#FFF" }} />}
			</Fab>
			<Popover
				open={open}
				anchorEl={anchorEl}
			...
			>
				{entityType === "accessPoint" && (
					<MenuItem
						onClick={() =>
							facilityEntity
								? dispatch(openDialog("facilityWarningDialog"))
				...
				{entityType === "camera" &&
					!(
						...
						<Fragment>
							<MenuItem onClick={() => handleSelection(`fov-${fov ? "edit" : "add"}`)}>
								<ListItemIcon>
								...
								<ListItemText
									primary={
										fov
											? getTranslation("camerasMapLayers.listItemText.editFOV")
											: getTranslation("camerasMapLayers.listItemText.drawFOV")
									}
								/>
							</MenuItem>
							{fov && (
								<MenuItem onClick={() => handleToggleStageDelete("fov-delete")}>
									<ListItemIcon>
							...
							{!!spotlightShape && (
								<MenuItem onClick={() => handleToggleStageDelete("spotlight-reset")}>
									<ListItemIcon>
										<Undo style={{ color: "#FFF" }} />
									</ListItemIcon>
									<ListItemText
										primary={getTranslation("camerasMapLayers.listItemText.resetSpotlight")}
									/>
								</MenuItem>
							)}
						</Fragment>
			...
			{stageDelete && (
				<Dialog
					...
					title={
						mode === "fov-delete"
							? getTranslation("camerasMapLayers.stageDeleteDialog.title.deleteFOV")
							: getTranslation("camerasMapLayers.stageDeleteDialog.title.resetSpotlight")
					}
					textContent={getTranslation(
						"camerasMapLayers.stageDeleteDialog.textContent",
						"",
						mode === "fov-delete"
							? getTranslation("camerasMapLayers.stageDeleteDialog.delete")
							: getTranslation("camerasMapLayers.stageDeleteDialog.reset"),
						mode === "fov-delete"
							? getTranslation("camerasMapLayers.stageDeleteDialog.fov")
							: getTranslation("camerasMapLayers.stageDeleteDialog.spotlight")
					)}
			...
			{entityType === "accessPoint" ? (
				<Dialog
					...
					confirm={{
						...
						action: () => {
							dispatch(closeDialog("facilityWarningDialog"));
							accessPointService.removeAccessPointFromDisplayTarget(
								entity.id,
								entityData.displayTargetId
							);
							handleSelection("accessPoint-edit");
					...
					abort={{
						label: getTranslation("accessPointMapLayers.facilityAPDialog.cancel"),
						action: () => {
							dispatch(closeDialog("facilityWarningDialog"));
						}
					}}
				>
					{facilityHidden ? (
						<DialogContentText>
							<Translate
								value="accessPointMapLayers.facilityAPDialog.hiddenText"
								count={entityData.properties.name}
							/>
						</DialogContentText>
					) : (
						<DialogContentText>
							<Translate
								value="accessPointMapLayers.facilityAPDialog.locatedText"
								count={entityData.properties.name}
							/>{" "}
							<a
								href={`/facilities-app/#/?${entityData.displayTargetId}`}
							>{`${facilityName}: ${floorPlanName}`}</a>
							<br />
							<Translate value="accessPointMapLayers.facilityAPDialog.continuingText" />
						</DialogContentText>
					)}
				</Dialog>
			) : (
				<Dialog
					...
					abort={{
						label: getTranslation("camerasMapLayers.facilityCamDialog.cancel"),
						action: () => {
							dispatch(closeDialog("facilityWarningDialog"));
						}
					}}
				>
					{facilityHidden ? (
						<DialogContentText>
							<Translate
								value="camerasMapLayers.facilityCamDialog.hiddenText"
								count={entityData.properties.name}
							/>
						</DialogContentText>
					) : (
						<DialogContentText>
							<Translate
								value="camerasMapLayers.facilityCamDialog.locatedText"
								count={entityData.properties.name}
							/>{" "}
							<a
								href={`/facilities-app/#/?${entityData.displayTargetId}`}
							>{`${facilityName}: ${floorPlanName}`}</a>
							<br />
							<Translate value="camerasMapLayers.facilityCamDialog.continuingText" />
						</DialogContentText>
					)}
				</Dialog>
			)}
