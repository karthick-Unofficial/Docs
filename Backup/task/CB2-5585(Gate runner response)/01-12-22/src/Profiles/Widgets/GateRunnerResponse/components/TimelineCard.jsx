...
const TimelineCard = ({ canExpand, handleCardExpand, event, dir, geometry, timeline, open, captured, initialActivityDate, attachments }) => {
    ...
    useEffect(() => {
        if (open) {
            brcService.streamRecommendations(id, (err, response) => {
                if (err) console.log(err);
                if (response) {
                    switch (response.type) {
                        case "initial":
                        case "add":
                            setUnits([...units, response.new_val]);
                            break;
                        default:
                            break;
                    }
                    console.log("?streamRecommendations", response);
                }
            });
        }
    }, [open]);

    console.log("#carImage", attachments, carImage);
    const styles = {
        marginAuto: {
