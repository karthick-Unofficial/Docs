...
export const builderError = (err) => {
	const errorMessgae = err ? err.message ? err.message : err : "";
	return {
		type: t.BUILDER_ERROR,
		error: errorMessgae !== "" ? new Error(errorMessgae) : "There was an error processing your request."
	};
};
...
export const submittingReport = payload => {
	return {
				...
				if (err) {
					...
					if (!res.ok) {
						...
						if (error && error.toLowerCase() === "request timeout") {
							dispatch(builderError("There was a timeout while generating your report. The service will now attempt to send the report via email."));
						} else {
			...
			(err, res) => {
				if (err) {
					...
					if ("ok" in res) {
						...
						if ("data" in res[0]) {
							res.forEach(element => {
								dispatch(pageReceived(element.data));
							});
						}
						else {
							dispatch(builderError());
...
export const previousPage = (type, data) => {
	return {
...
export const generatePDFExport = (type, data) => {
	...
	return async (dispatch, getState) => {
		...
		restClient.exec_post(
			`/reports-app/api/${type}/export-pdf`,
			JSON.stringify(data),
			(err, response) => {
				dispatch(toggleGeneratingFile());
				if (err) {
...
export const requestDialogSubmitError = (error) => {
	return {
...
export const requestPDFDialogSubmitError = (error) => {
	return {
...
export const requestPDFEmail = (type, data) => {
	return async (dispatch, getState) => {
		...
		data.timeFormatPreference = timeFormatPreference ? timeFormatPreference : "12-hour";
		restClient.exec_post(
			`/reports-app/api/${type}/pdf-email`,
			JSON.stringify(data),
			(err, response) => {
				if (err) {
					console.log(err);
