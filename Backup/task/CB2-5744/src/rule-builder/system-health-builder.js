...
const systemHealthBuilder = (rule, collections, targetAction, hasLinks) => {
	...
	return (
		<div id="rule-statement" className="cb-font-b9">
			<Translate value="global.ruleBuilder.systemHealthBuilder.alertMe" />
...
const _getTargets = (rule, collections, targetAction, hasLinks) => {
	if (rule.targets.length === 0) {
		return (
			<Translate value="global.ruleBuilder.systemHealthBuilder.anySystem" />
		);
	} else {
		...
		return rule.targets.map((system, index) => {
			return (
				<span key={system.id} className="rule-statement-container">
					{index > 0 && (
						<span>
							<Translate value="global.ruleBuilder.systemHealthBuilder.or" />
						</span>
					)}
					<span
						className={`${hasLinks ? "cb-font-link" : ""}`}
						onClick={() => targetAction(system)}
					>
						{system.name}
					</span>
				</span>
