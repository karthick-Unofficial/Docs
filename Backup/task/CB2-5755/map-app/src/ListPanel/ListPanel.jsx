...
const ListPanel = () => {
	...
	const drawingToolsActive = useSelector(
		(state) => state.mapState.mapTools.type === "drawing"
	);
	const ownedEvents = useSelector((state) =>
		activeOwnedEventsSelector(state)
	);
	const sharedEvents = useSelector((state) =>
		activeSharedEventsSelector(state)
	);
			...
			case "camera":
				return (
					<ErrorBoundary>
						<CameraProfile
							selectFloorPlan={
								facilityProfileActionCreators.selectFloorPlan
							}
	...
	const panelActions = [
		...
		<Event
			...
			key="events-action-button"
		/>,
		<Search
			onClick={(e) => handleTabSelect(e, "search")}
			key="search-action-button"
		/>
	];
				...
				>
					<Tab
						label={getTranslation(
							"listPanel.listPanel.tabLabel.collections"
						)}
						value="collections"
						sx={styles.tabStyle}
					...
					<Tab
						label={getTranslation(
							"listPanel.listPanel.tabLabel.events"
						)}
						value="events"
						sx={styles.tabStyle}
						classes={{ selected: classes.selected }}
					...
					<Tab
						label={getTranslation(
							"listPanel.listPanel.tabLabel.search"
						)}
						value="search"
						sx={styles.tabStyle}
						classes={{ selected: classes.selected }}
						...
						<Fragment>
							<SearchField
								...
								placeholder={getTranslation(
									"listPanel.listPanel.placeholder.searchColl"
								)}
							...
							<div
								style={{
									paddingTop: "1rem",
									height: `calc(100vh - ${
										WavCamOpen ? "430px " : "190px"
									})`
								}}
								className={
									dir == "rtl"
										? "dockItemsRTL scrollbar"
										: "dockItems scrollbar"
								}
							>
								...
								<List className="customList">
									<EntityCollection
										...
										collection={{
											id: "shared",
											name: getTranslation(
												"listPanel.entityCollection.sharedWithMe"
											)
										}}
									...
									<Fragment>
										...
										<List className="customList">
											<ErrorBoundary key={"hidden-items"}>
												<Collection
													primaryText={getTranslation(
														"listPanel.listPanel.hiddenItems"
													)}
													secondaryText={
														<Fragment>
															<Button
				...
				{canViewEvents && (
					<TabPanel value={"events"} selectedTab={tab}>
						<SearchField
							...
							handleClear={handleClearSearch}
							placeholder={getTranslation(
								"listPanel.listPanel.placeholder.searchEvents"
							)}
							value={search["events"] || ""}
							dir={dir}
						/>
						{tab === "events" && (
							...
							>
								<Typography variant="h6">
									<Translate value="listPanel.listPanel.myEvents" />
								</Typography>
					...
					{tab === "search" && (
						<Fragment>
							<SearchField
								...
								handleClear={handleClearSearch}
								placeholder={getTranslation(
									"listPanel.listPanel.placeholder.wantToFind"
								)}
								value={search["search"] || ""}
								dir={dir}
							/>
