...
export { getFacilities } from "orion-components/Map/Selectors/FacilitiesSelectors";
...
export const floorPlanSelector = createSelector(floorPlan, (floorPlan) => {
	const { selectedFloor, coordinates, creating, preLoaded, image } = floorPlan || {};
	...
	return createDeepEqualSelector(
		...
		(context, cameraGeo, cameraProps, filters) => {
			...
			if (context && context.entity) {
				...
				const cameras = cameraGeo ? merge(cameraProps, cameraGeo) : cameraProps;
				if (filters && Object.keys(filters).length) {
					mapFeatures = pickBy(cameras, (camera) => Object.keys(filters).includes(camera.id));
				} else {
					mapFeatures = cameras;
				}
			}
			...
			Object.keys(mapFeatures).forEach((key) => {
				if ((mapFeatures[key].entityData.displayType || "map").toLowerCase() === "map") {
					...
					if (!mapFeatures[key].entityData.properties.entityType) {
						filteredMapFeatures[key].entityData.properties["entityType"] =
							filteredMapFeatures[key].entityType;
					}
				}
			});
	...
	return createDeepEqualSelector(
		...
		(context, accessPointGeo, accessPointProps, filters) => {
			...
			if (context && context.entity) {
				...
				const accessPoints = accessPointGeo ? merge(accessPointProps, accessPointGeo) : accessPointProps;
				if (filters && Object.keys(filters).length) {
					mapFeatures = pickBy(accessPoints, (accessPoint) => Object.keys(filters).includes(accessPoint.id));
				} else {
...
const roadsAndLabels = (map, roadAndLabelLayerOpacity, roadsVisible) => {
	// Get all layers that contain roads or labels
	const roadsData = map.getStyle().layers.filter((layer) => {
		return (
			layer["source-layer"] && (layer["source-layer"].includes("road") || layer["source-layer"].includes("label"))
		);
	});
	return roadsData.map((layer) => {
		// Set layer visibility & opacity
		if (roadsVisible) {
			if (layer.type === "line") {
				return map
					.setLayoutProperty(layer.id, "visibility", "visible")
					.setPaintProperty(layer.id, "line-opacity", roadAndLabelLayerOpacity);
			} else if (layer.type === "symbol") {
				return map
					.setLayoutProperty(layer.id, "visibility", "visible")
					.setPaintProperty(layer.id, "text-opacity", roadAndLabelLayerOpacity)
					.setPaintProperty(layer.id, "icon-opacity", roadAndLabelLayerOpacity);
			}
		} else {
...
export const getRoadsAndLabels = () => {
	return createDeepEqualSelector(roadsAndLabels, (data) => {
		return data;
	});
};
