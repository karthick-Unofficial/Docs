...
import { Grid, InputAdornment, OutlinedInput, CircularProgress, Checkbox, IconButton, Dialog, DialogContent } from "@mui/material";
...
const ManageModal = ({ open, close, lookupType, contextId, settings, assignedData, dir }) => {
	...
	useEffect(() => {
		...
		if (lookupType === "resources") {
			...
			setTitle(getTranslation("global.profiles.eventProfile.main.equipment"));
		...
		integrationService.getExternalSystemLookup(
			...
			(err, response) => {
				if (err) {
					...
					if (lookupType === "resources") {
						merge(keyBy(data, "MilitaryNumber"), keyBy(assignedData, "MilitaryNumber"));
					}
					else {
				...
				<div style={{ padding: "8px" }}>
					<Grid container>
						...
						<Grid item xs={1} sm={1} md={1} lg={1} >
							<div style={style.dailogCloseIcon}>
