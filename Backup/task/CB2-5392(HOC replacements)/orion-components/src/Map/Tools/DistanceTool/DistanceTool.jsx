...
import { setMapTools, updatePath, updatePaths, deletePath, setActivePath } from "orion-components/Map/Tools/Actions";
...
const DistanceToolWrapper = ({ activePath, before }) => {
	const track = useSelector(state => _.pickBy(feedEntityGeoSelector(state), entity => !!entity && entity.entityType === "track"));
...
const DistanceTool = memo((props) => {
	...
	console.log("#1state", stateRef.current);
	...
	const handleDelete = index => {
		...
		dispatch(deletePath(paths[index].id));
	};
	...
	const { landUnitSystem, toolType, dir, before } = props;
