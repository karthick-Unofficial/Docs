import React, { Fragment, createRef, useState, useEffect, useRef, memo } from "react";
...
const CamerasMapLayers = memo(props => {
	...
	useEffect(() => {
		console.log("#itemschanged", JSON.stringify(items));
	}, [items]);

	useEffect(() => {
		console.log("#profileIcontemplate changed", JSON.stringify(profileIconTemplates));
	}, [profileIconTemplates]);
	...
	Object.keys(prevProps).forEach(key => {
		if (!_.isEqual(prevProps[key], nextProps[key]) && key !== "profileIconTemplates") {
			updated = true;
		}
	});
	if (!_.isEqual(JSON.stringify(prevProps.profileIconTemplates), JSON.stringify(nextProps.profileIconTemplates))) {
		return false;
	}
	if (!updated) {
		return true;
