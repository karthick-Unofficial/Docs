...
const ModificationsHistory = ( { isFacilitator, isController, userInfo, userMappings, sessionId, simulations, simId, simTimePrecision,
	agents, agentGroups, simulationData, modificationsConfig,  floorPlans, dir } ) => {
	...
	return (
		<div>
			<div style={dir == "rtl" ? {display:"flex", flexDirection:"row-reverse", paddingLeft:"10px"} : {display:"flex", flexDirection:"row-reverse", paddingRight:"10px"}}>
				<Button disabled={!hasModificationhistory} style={{minHeight:"41px", width:"258px"}} variant="contained" color="primary" onClick={openPdf}><Translate value="tableopSession.panels.modifications.modificationHistory.export"/></Button>
			</div>
			<div id="modificationHistory">
				<div id="report-title">
					<p className="pdfHeading"><Translate value="tableopSession.panels.modifications.modificationHistory.modHistReport"/></p>
					<p>{getBaseSimName()}</p>
					<p><span><Translate value="tableopSession.panels.modifications.modificationHistory.reportDate"/> </span><span>{currentdate.toLocaleString()}</span></p>
				</div>
				{
									...
									<div className="modificationHistoryflex" style={{width: "100%"}}>
										...
										{ (simkey === 0) && 
											<div className="nameContainer">
												<div className="b2-bright-gray" data-html2canvas-ignore="true"><Translate value="tableopSession.panels.modifications.modificationHistory.currentExercise"/></div>
										...
										<div className="b1-white" style={dir == "rtl" ? {width:"20%", alignSelf: "center", textAlign: "left", marginLeft:5 } : {width:"20%", alignSelf: "center", textAlign: "right", marginRight:5 }}>
										<Translate value="tableopSession.panels.modifications.modificationHistory.time"/>{` ${simu.parentDivergeTime ? utilities.truncate(simu.parentDivergeTime, simTimePrecision) : 0}`}</div>
