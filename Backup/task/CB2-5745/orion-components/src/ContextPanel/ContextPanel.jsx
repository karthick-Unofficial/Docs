...
const ContextPanelWrapper = ({
	...
	const secondaryOpen =
		panelState &&
		($(".secondary-panel").length || $(".secondary-panelRTL").length)
	...
	const mapVisible = useSelector((state) =>
		state.mapState ? state.mapState.baseMap.visible : false
	);
...
const ContextPanel = (props) => {
	...
	useEffect(() => {
		const primaryWidth =
			dir && dir == "rtl"
				? $(".dock-controlRTL").outerWidth() / 2
				: $(".dock-control").outerWidth() / 2;
	...
	return !hidden ? (
		<Fragment>
			<div
				...
				className={`${
					dir && dir == "rtl" ? "dock-controlRTL" : "dock-control"
				} ${
					primaryOpen || (showMobileToggle && secondaryOpen)
						? "open"
						: "closed"
				}`}
			>
					...
					<div id="toggle-mobile">
						{primaryOpen || secondaryOpen ? (
							...
							>
								{context
									? getTranslation(
											"global.contextPanel.openProfile"
									  )
									: getTranslation(
											"global.contextPanel.openListPanel"
									  )}
							</Button>
						)}
					</div>
