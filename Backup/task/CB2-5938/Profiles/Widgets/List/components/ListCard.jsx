...
const ListCard = ({
	...
	const handleExpandClick = () => {
		...
		handleCollapseAndExpand(id, !expandedState);
	};
		...
		if (
			...
			if (firstColumn.type === "checkbox") {
				...
				if (numberIncomplete > 0) {
					return <Avatar style={avatarStyle}>{numberIncomplete}</Avatar>;
				} else {
	...
	const hasAttachments = _.size(_.filter(list.rows, (row) => _.size(row.attachments) > 0)) > 0;
	return list ? (
			...
			{dialog && (
				<Fragment>
					...
					>
						...
						{showLoading ? <CircularProgress size={60} thickness={5} /> : null}
					</Dialog>
					<RowEdit
						key={list.id + "-add-row"}
						list={list}
