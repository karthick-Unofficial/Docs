...
export const startCamerasInRangeStream = (
	...
	return dispatch => {
		cameraService
			.streamCamerasInRange(contextId, entityType, (err, response) => {
				if (err) console.log(err);
				else {
					if (response.add) {
						dispatch(
...
export const startTrackHistoryStream = (contextId, subscriberRef, duration, forReplay) => {
	return dispatch => {
		if (forReplay) {
			dispatch(
				subscriptionDataBatchReceived(
					...
					if (err) console.log(err);
						...
						Promise.resolve(
							...
							.then(subscription => {
								...
								feedService
									.streamEntityByType(contextId, "track", (err, response) => {
										if (err) console.log(err);
										else {
											if (!response) return;
											switch (response.type) {
												case "initial":
													...
													dispatch(addContext(contextId, response.new_val));
													break;
												case "change":
													// Update context entity data (ie updated track details)
													dispatch(updateContext(contextId, response.new_val));
													break;
												default:
													break;
											}
							...
							.then(() => {
								dispatch(addSubscriber(contextId, "trackHistory", "map"));
							});
					}
				}
			);
