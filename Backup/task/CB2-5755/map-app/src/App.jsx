...
const AppWrapper = () => {
	...
	const useFeedQueue = useSelector(
		(state) => state.clientConfig.useFeedQueue || false
	);
	...
	const hydrateAndSubscribe = () => {
		const hydrate = async () => {
			...
			const feeds = await dispatch(
				subscribeAppFeedPermissions(identity.userId, "map-app")
			);
		...
		const getIntegrations = () => {
			return new Promise((resolve, reject) => {
				feedService.getUserAppIntegration(
					userId,
					"map-app",
					(err, res) => {
						if (err) {
			...
			.then((values) => {
				...
				each(globalFeeds, (globalFeed) => {
					...
					if (Object.keys(globalFeed)[0] === "fovs") {
						appendReducer(
							"globalGeo.fovs",
							dataByFeed("fovs", "globalGeo", useFeedQueue)
						);
				...
				each(values[0], (feed) => {
					...
					const geoReducer = dataByFeed(
						feed.intId,
						"globalGeo",
						useFeedQueue
					);
					const dataReducer = dataByFeed(
						feed.intId,
						"globalData",
						false
					);
