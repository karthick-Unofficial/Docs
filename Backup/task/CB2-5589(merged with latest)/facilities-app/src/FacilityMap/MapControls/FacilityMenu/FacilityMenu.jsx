...
import { MenuItem, Popover, ListItemIcon, ListItemText, Fab } from "@mui/material";
// cSpell:ignore videocam
import { Add, EditLocation as Edit, Videocam, SelectAll } from "@mui/icons-material";
...
import { addImage, clearFloorPlan, toggleCreate, setMapTools } from "./facilityMenuActions";
...
const FacilityMenu = ({ type }) => {
	...
	const canEdit = entity
		? session.user.profile.integrations.find((int) => int.intId === entity.feedId) &&
		  session.user.profile.integrations.find((int) => int.intId === entity.feedId).permissions &&
		  session.user.profile.integrations.find((int) => int.intId === entity.feedId).permissions.includes("manage")
		: false;
			...
			menuOptions.push(
				<Fragment>
					<MenuItem onClick={handleEdit}>
						...
						<ListItemText
							primary={getTranslation("facilityMap.mapControls.facilityMenu.editFacLocation")}
						/>
					</MenuItem>
			...
			menuOptions.push(
				<Fragment>
					...
					<MenuItem onClick={handleAddAccessPoint}>
						...
						<ListItemText
							primary={getTranslation("facilityMap.mapControls.facilityMenu.addAccessPointToFloor")}
						/>
					</MenuItem>
				</Fragment>
			);
	...
	return !type ? (
		<Fragment>
			<Fab
				disabled={!!type || (!context && !canCreate) || !menuOptions.length}
				onClick={handleClick}
				color="primary"
			>
				<Add />
