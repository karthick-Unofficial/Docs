...
const TrackHistoryLayers = () => {
	...
	const getTrackHistoryGeoJSON = () => {
		const historyGeoJSON = map(keys(trackHistory), (id) => {
			return {
				geometry: {
					type: "LineString",
					coordinates: map(trackHistory[id], (entry) => entry.entityData.geometry.coordinates)
				}
			};
		});
	...
	const getTrackHistoryGeoJSONPoints = () => {
		...
		each(trackHistory, (history) => each(history, (entry) => trackHistoryPoints.push(entry.entityData)));
	...
	return (
		<Fragment>
			<Source id="thLineSource" geoJsonSource={getTrackHistoryGeoJSON()} />

			<Source id="thPointSource" geoJsonSource={getTrackHistoryGeoJSONPoints()} />
