...
class AccessPointMapLayer extends PureComponent {
	constructor(props) {
		super(props);
		this.state = {
			reloadWeather: false
	...
	addClickHandlers = () => {
		...
		const filterAndLoadProfile = (id, context) => {
			const { items, loadProfile, mapState } = this.props;
			const entitiesFiltered = Object.values(items)
				.filter(entity => {
					return _.includes(entity.id, id);
				})
				// Prevent click on selected accessPoint's FOV polygon
				.filter(entity => {
	...
	componentDidMount = () => {
		const { map, items, roadsVisible } = this.props;
		// Roads and labels
		this.setRoadsAndLabels(roadsVisible);
		// Hide nautical charts if zoomed out beyond useful level
		map.on("zoom", () => {
			this.currentZoomLevel = map.getZoom();
		});
		map.on("mousemove", e => this.handleMouseMove(e.point));
		...
		this.setMapEntities(Object.values(items));
	...
	setMapEntities = items => {
		const { setMapEntities } = this.props;
		const byFeed = _.groupBy(items, "feedId");
		_.each(
			_.keys(byFeed),
	...
	componentDidUpdate = (prevProps, prevState) => {
		const { map, mapName, items, roadsVisible, roadAndLabelLayerOpacity, baseMaps } = this.props;
	...
	handleReorder = e => {
		const { map, mapState, feedId } = this.props;
	...
	getGeoJSON = entityType => {
		const { mapState, profileIconTemplates } = this.props;
		...
		const items = Object.values(this.props.items)
			...
			.filter(item => {
				// Remove current FOV or accessPoint from map while editing
				if (!!mode && !!feature) {
	...
	render() {
		...
		return (
			<div>
				...
				{(nauticalChartsVisible && this.currentZoomLevel >= 7) && (
					<Layer
						type="raster"
						id="nautical"
						...
						before={
							map.getLayer("unclustered-fovs")
								? "unclustered-fovs"
								: map.getLayer("unclustered-accessPoints")
									? "unclustered-accessPoints"
									: null
						}
					/>
				)}
				...
				{!this.state.reloadWeather && weatherVisible && (
					<Layer
						type="raster"
						id="current-radar"
				...
				{contextLoaded && (
					<Source
						id="eventPointSource"
						geoJsonSource={this.getGeoJSON("Event")}
					/>
				)}
...
export default AccessPointMapLayer;
