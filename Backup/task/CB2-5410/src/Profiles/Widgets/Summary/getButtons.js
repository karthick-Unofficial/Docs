...
const getButtons = (user, context, appId, actions, readOnly) => {
	...
	switch (entityType) {
		...
		case "camera": {
			...
			return [
				...buttons,
				{
					...
					icon: "launch",
					toggled: false,
					viewable: !readOnly && appId !== "cameras-app"
				},
				{
					name: activeFOV
		...
		case "event": {
			...
			const { isTemplate, id } = entity;
			let isTemplateToggled = false;
			if (isTemplate) {
				const eventTemplateFavorites = useSelector(
					(state) => state.appState?.userGlobal?.eventTemplateFavorites
				);
				if (eventTemplateFavorites?.length > 0) {
					isTemplateToggled = eventTemplateFavorites.includes(id);
				}
			}
			...
			return [
				{
					...
					icon: "star",
					toggled: isTemplateToggled,
					viewable: !readOnly && isTemplate,
					toggleColor: "#F8F29F"
				},
				{
					...
					icon: "launch",
					toggled: false,
					viewable: !readOnly && appId !== "events-app"
				},
				{
				...
				},
				{
					name:
						isPublic && !canShareEvents
							? getTranslation("global.profiles.eventProfile.main.shared")
							: getTranslation("global.profiles.eventProfile.main.share"),
					icon: "share",
				...
				switch (action.nameText.toLowerCase()) {
					case "launch":
						return {
							...
							icon: "launch",
							toggled: false,
							viewable: !readOnly && appId !== "facilities-app"
						};
					case "edit":
						return {
							name: getTranslation("global.profiles.entityProfile.main.edit"),
