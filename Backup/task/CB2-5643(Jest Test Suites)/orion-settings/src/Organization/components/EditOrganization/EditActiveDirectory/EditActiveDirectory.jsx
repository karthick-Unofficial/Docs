...
const handleConfirm = (data, users, auth, loggedInUserOrg, refreshEcosystem, setState, dispatch) => {
...
const EditActiveDirectory = () => {
	...
	const { refreshEcosystem } = actionCreators;
	const org = useSelector(state => state.session.user.profile.orgId);
	...
	return (
		...
		>
			<ArticleContainer
				headerTitle={getTranslation("mainContent.manageOrganization.orgSettings.activeDir.formTitle")}
				headerDescription={""} editing={true} >
				<Fragment>
					<Container style={{
						marginTop: 30,
						display: "flex",
						...
						<div style={styles.body}>
							...
							{users && (
								<React.Fragment>
									...
									{
										users.length > 0
											? <Oldlist listItems={users} dir={dir} />
							...
							<Container style={styles.container}>
								...
								<Button
									...
									color="primary"
									onClick={() => {
										handleConfirm(data, users, auth, org, refreshEcosystem, setState, dispatch);
									}}
									classes={{
										disabled: classes.disabledBtn
									}}
