...
const EntityProfile = ({
			...
			if (feedId === customWidget.feedId && customWidget.hasOwnProperty('widgets')) {
				...
				for (let i = 0; i < widgets.length; i++) {
					if (widgets[i].id === widgetName) {
						customWidgetsArr.push(widgets[i]);
						break;
					}
				}
			}
	...
	const getWidgetConfig = (widgetId = "") => {
		...
		if (widgetId === "drone-association") {
			const checkDroneWidget = getCustomWidgetConfig(widgetId);
			// remove drone-association widget from irrelevant feeds			
			if (checkDroneWidget.length === 0 && entityType === "track") {
