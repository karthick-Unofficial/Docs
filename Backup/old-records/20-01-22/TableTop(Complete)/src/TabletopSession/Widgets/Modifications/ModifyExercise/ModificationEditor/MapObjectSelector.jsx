...
const MapObjectSelector = React.forwardRef(({id, definitionDesc, itemSelectionFor, enableItemSelectionOnMap, map, 
	...
	const getSelectorLabel =() =>{
		...
		let labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.chooseItemTarget"/>;
		
		switch (controlDataTypes.toUpperCase()) {
			case "AGENT":
				labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.choosePlayer"/>;
				break;
			case "POSITION":
				{
					if (!headerText || headerText === ""){
						labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.chooseLocation"/>;
					} else {
						labelTmp = headerText;
					}	
				}
			...
			case "INTERDICTIONSITE":
				labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.chooseIntSide"/>;
				break;
			case "OBJECTIVE":
				labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.chooseObjective"/>;
				break;
			default:
				labelTmp = <Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.chooseItem"/>;
				break;
		}
		...
		<div id={id} className="modificationflexColumn" style={{backgroundColor:"#3C4656", borderRadius: 5}}>
			<div className="modificationflex" style={{width:"100%", justifyContent:"flex-start", alignItems: "center"}}>
				...
				>
					...
					<Translate value="tableopSession.widgets.modifications.modificationEditor.mapObjectSelector.selectOnMap"/>
				</Button>
			</div>
			{
				mapEntities &&
