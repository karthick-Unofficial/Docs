...
const Activities = (props) => {
	...
	useEffect(() => {
		if (prevProps) {
			if (!isEqual(activities, prevProps.activities)) {
				setActivitiesState(
					uniqBy([...activitiesState, ...activities], "id")
				);
			}
		}
	}, [activities]);
	...
	const handleLaunch = () => {
		...
		if (entityType === "event") {
			window.open(
				`/events-app/#/entity/${contextId}/widget/activity-timeline`
			);
		} else if (entityType === "camera") {
			window.open(
				`/cameras-app/#/entity/${contextId}/widget/activity-timeline`
			);
	...
	const handleChange = (event) => {
		if (event.target.value.length > 280) {
			setError(
				getTranslation(
					"global.profiles.widgets.activities.main.errorText.commentsTxt"
				)
			);
			return;
		}
			...
			switch (string) {
				case "camera": {
					return getTranslation(
						"global.profiles.widgets.activities.main.camera"
					);
				}
				case "event": {
					return getTranslation(
						"global.profiles.widgets.activities.main.event"
					);
				}
				case "shapes": {
					return getTranslation(
						"global.profiles.widgets.activities.main.shapes"
					);
				}
				case "accessPoint": {
					return getTranslation(
						"global.profiles.widgets.activities.main.accessPoints"
					);
				}
				default:
					return string;
	...
	const canComment =
		!readOnly && (!owner && EntityType === "activity" ? true : canManage);
	/**
	...
	return selected || !enabled ? (
		...
		<div className={`collapsed ${"index-" + order}`}>
			...
			{!isAlertProfile || canComment ? (
				<div id="activity-wrapper" className="widget-wrapper">
							...
							<div className="widget-header-buttons">
								{widgetsExpandable ? (
									<div className="widget-expand-button">
										<IconButton
											style={
												inlineStyles.widgetExpandButton
											}
					...
					<div className="widget-content">
								...
								<div id="comment-field-wrapper">
									<div id="comment-input-wrapper">
										...
										<div id="comment-post-button">
											...
											>
												{getTranslation(
													"global.profiles.widgets.activities.main.post"
												)}
											</Button>
										</div>
									</div>
						...
						<Dialog
							open={
								dialog === "activityFilterDialog" ? true : false
							}
							actions={true}
								...
								<FormControlLabel
									control={
										<Checkbox
											checked={
												filters &&
												filters.includes("comment")
											}
											onChange={() =>
												handleUpdateFilter(["comment"])
											}
										/>
									}
									style={{ color: "#7E7F7F" }}
									label={getTranslation(
										"global.profiles.widgets.activities.main.postedMessages"
									)}
								/>
				...
				{filteredActivities.length > 0 ? (
					<Fragment>
						{filteredActivities
							.sort((a, b) => {
								return (
									moment.utc(b.published) -
									moment.utc(a.published)
								);
							})
							.map((activity) => {
								return (
						...
						{loadMore && (
							...
							>
								{getTranslation(
									"global.profiles.widgets.activities.main.loadMore"
								)}
							</Button>
						)}
					</Fragment>
