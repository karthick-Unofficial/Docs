...
SpotlightMode.removeHandles = function(state, e) {
	const { handles } = state;
...
SpotlightMode.onDrag = function(state, e) {
	...
	if (dragSpotlight) {
		...
		const getHandle = index =>
			Object.values(handles).find(handle => handle.properties.index === index);
...
SpotlightMode.onMouseMove = function(state, e) {
	...
	if (featureTarget) {
		...
		if (handle) {
			this.map.getCanvas().style.cursor =
				handle.properties.movement === "lng" ? "ew-resize" : "ns-resize";
		} else {
