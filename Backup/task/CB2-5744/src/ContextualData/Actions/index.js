	...
	return (dispatch) => {
		...
		if (!subscription) {
			...
			(subscription.subscribers.length === 1 &&
				includes(subscription.subscribers, subscriberRef))
		) {
...
export const unsubscribeFromFeed = (
	...
	return (dispatch, getState) => {
		...
		const subscriptions =
			getState().contextualData[contextId].subscriptions;
		...
		if (!(context.trackHistory && subscriptionId === "entity")) {
			...
			if (
				...
				dispatch(
					removeSubscriber(contextId, subscriptionId, subscriberRef)
				);
			}
		}
	};
	...
	return (dispatch, getState) => {
		...
		each(contextualData, (context, contextId) => {
			if (
				...
				Object.values(selectedContext).includes(contextId)
			) {
