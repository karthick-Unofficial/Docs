...
import { addContext, updateContext } from "../Actions";

import * as t from "./actionTypes";
import each from "lodash/each";

const subscriptionDataReceived = (contextId, data, subscriptionId, iterable = true, splitGeo = false) => {
	return {
...
export const subscriptionDataBatchReceived = (contextId, data, subscriptionId, iterable = true, splitGeo = false) => {
	return {
...
export const subscriptionDataRemoved = (contextId, dataId, subscriptionId, iterable = true) => {
	return {
...
const subscriptionDataUpdated = (contextId, data, subscriptionId, iterable = true, splitGeo = false) => {
	return {
...
export const subscriptionReceived = (contextId, subscription, subscriptionId, subscriberRef) => {
	return {
 ...
 */
export const startLiveCameraStream = (contextId, videoProfile, subscriberRef) => {
	return async (dispatch) => {
		try {
			const response = await cameraService.streamVideo(contextId, videoProfile);
...
export const startActivityStream = (id, targetType, subscriberRef) => {
	return (dispatch) => {
		activityService
			.subscribeByTarget(id, targetType, null, (err, response) => {
				if (err) {
					console.log(err);
				} else {
 ...
 * @param: subscriberRef -- reference to component that is using feed
 */
export const startEventActivityStream = (eventId, subscriberRef, amount = 5) => {
	return (dispatch) => {
		activityService
			.subscribeByEvent(eventId, "event", amount, null, (err, response) => {
				if (err) {
					console.log(err);
				} else {
 ...
 * @param: subscriberRef -- reference to component that is using feed
 */
export const startGateRunnerActivityStream = (id, targetType, filterOptions, subscriberRef) => {
	return (dispatch) => {
		activityService
			.subscribeByTarget(id, targetType, filterOptions, (err, response) => {
				if (err) {
					console.log(err);
				} else {
					dispatch(subscriptionDataReceived(id, response.new_val, "activities"));
...
export const startEventCameraStream = (eventId, subscriberRef) => {
	return (dispatch) => {
		eventService
			.subscribeEventCameras(eventId, (err, res) => {
...
export const startFloorPlanCameraStream = (facilityId, floorPlanId, subscriberRef) => {
	return (dispatch) => {
		facilityService
			.streamFloorplanCameras(facilityId, floorPlanId, (err, res) => {
				if (err) console.log(err);
				else {
					res.forEach((change) => {
						switch (change.type) {
...
export const startFloorPlanAccessPointsStream = (facilityId, floorPlanId, subscriberRef) => {
	return (dispatch) => {
		facilityService
			.streamFloorplanAccessPoints(facilityId, floorPlanId, (err, res) => {
				if (err) console.log(err);
				else {
					res.forEach((change) => {
						switch (change.type) {
							...
							case "remove":
								dispatch(
									subscriptionDataRemoved(facilityId, change.old_val.id, "floorPlanAccessPoints")
								);
								break;
							default:
								break;
			...
			.then((subscription) => {
				dispatch(
					subscriptionReceived(facilityId, subscription.channel, "floorPlanAccessPoints", subscriberRef)
				);
			});
	};
};
...
export const startCamerasLinkedItemsStream = (contextId, subscriberRef) => {
	return (dispatch) => {
		linkedEntitiesService
			.subscribeLinkedEntities(contextId, "camera", null, (err, response) => {
				if (err) {
					console.log("ERR", err);
...
const getItemsInFOV = (contextId, subscriberRef, poly) => {
	return (dispatch, getState) => {
		...
		const integrations = Object.values(state.session.userFeeds).filter((int) => int.canView);
						...
						if (err) {
							...
							if (response.ignoreBatches) {
								switch (response.change.type) {
									...
									case "change":
										dispatch(
											subscriptionDataReceived(contextId, response.change.new_val, "fovItems")
										);
										break;

									case "remove":
										dispatch(
											subscriptionDataRemoved(contextId, response.change.old_val.id, "fovItems")
										);
										break;
									default:
										break;
								...
								if (changes.length) {
									each(changes, (change) =>
										dispatch(subscriptionDataUpdated(contextId, change, "fovItems"))
									);
								}
								if (additions.length) {
									each(additions, (addition) =>
										dispatch(subscriptionDataReceived(contextId, addition, "fovItems"))
									);
								}
							}
						}
 ...
 */
export const startCamerasInRangeStream = (contextId, entityType, subscriberRef) => {
	return (dispatch) => {
		associationService
 ...
 */
export const startCameraInRangeVideoStream = (contextId, cameraId, videoProfile, subscriberRef) => {
	return (dispatch) => {
		cameraService
...
export const startTrackHistoryStream = (contextEntity, subscriberRef, duration, forReplay) => {
	return (dispatch) => {
		if (forReplay) {
			...
			restClient.exec_post("/ecosystem/api/ecolink/track-history", JSON.stringify(body), (err, result) => {
				if (err) console.log(err);
				else {
					const { data } = result;
					...
					Promise.resolve(
						dispatch(subscriptionDataBatchReceived(contextEntity.id, rowArray, "trackHistory", false))
					)
						.then((subscription) => {
...
export const unsubscribe = (channel) => {
	return () => {
		realtimeClient._unsubscribe(channel).then((data) => console.log("Unsubscribed from channel: " + channel, data));
	};
};
