...
const optimistMiddleware = (store) => (next) => (action) => {
	...
	if (!action) {
		console.error(
			"optimistMiddleware received an action that was not defined. Action would have been assigned transaction ID: ",
			nextTransactionID
		);
	} else {
		...
		if (action.type === "CLOSE_NOTIFICATION") {
			...
			notificationService.closeBulkNotifications(
			...
			notificationService.reopenNotification(
				action.notificationId,
				(err, response) => {
					if (err) {
						console.log(err);
			...
			notificationService.reopenBulkNotifications(
				action.notificationIds,
				(err, response) => {
					if (err) {
						console.log(err);
