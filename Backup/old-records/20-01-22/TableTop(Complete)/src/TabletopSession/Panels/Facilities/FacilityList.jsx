...
const FacilityList = ( { classes, facilities, facilityFloorPlans, navigate, dir } ) => {
	if (!facilities) {
		return <div><Translate value="tableopSession.panels.facilities.facilityList.noFac"/></div>;
	}
			...
			{	_.values(facilities).map((facility, index) => {
				...
				return (
					<Fragment key={`facil-${facility.entityData.properties.id}`}>
						<ListItem style={dir == "rtl" ? {backgroundColor:"#3C4656", borderRadius: 5, paddingTop: 10, paddingLeft: 10, paddingBottom: 10, marginTop: 10, marginBottom: 10} : {backgroundColor:"#3C4656", borderRadius: 5, paddingTop: 10, paddingRight: 10, paddingBottom: 10, marginTop: 10, marginBottom: 10}} key={`facListItem-${index}`}>
							<ListItemIcon>
							...
							<ListItemText
								...
								primary={
									<div className={classes.contentContainer} style={{cursor:"pointer"}} onClick={() => navigate(facility, null)}>
										...
										<div className="b2-bright-gray">{<Translate value="tableopSession.panels.facilities.facility.floors" count={flrPlansCount}/>}</div>
									</div>
								}
							/>
						</ListItem>
