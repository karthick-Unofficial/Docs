...
const getUnitsMemberByUnitId = (state, unitId, feedSettings) => {
	...
	let result = {};
	forOwn(unitMembers, function (value) {
...
export const unitMemberSelector = createSelector(
	(state, unitId, feedSettings) =>
		getUnitsMemberByUnitId(state, unitId, feedSettings),
	(members) => {
		return members;
	}
);

export const unitMemberMemoized = () =>
	createDeepEqualSelector(unitMemberSelector, (result) => {
		return result;
	});

export const getUnits = () => {
	return createDeepEqualSelector(getAllUnits, (units) => {
		return units;
	});
};
