...
const Facility = ( { classes, facility, floorPlan, floorPlans, navigate, dir } ) => {
	...
	return (
		<div>
			<div style={{display: "flex", margin: 10}} >
				...
				<div className="b1-white" style={dir == "rtl" ? {marginRight:10, color:"#4DB5F4"} : {marginLeft:10, color:"#4DB5F4"}}><Translate value="tableopSession.panels.facilities.facility.title"/></div>
			</div>
			<div className={classes.flexContainer} style={{backgroundColor:"#3C4656", borderRadius: 5, paddingTop:10, paddingBottom:10}}>
				...
				<div className={classes.contentContainer} style={{cursor:"pointer"}}>
					...
					<div className="b1-bright-gray">{<Translate value="tableopSession.panels.facilities.facility.floors" count={flrPlansCount}/>}</div>
				</div>
			</div>
						...
						<div className={classes.flexContainer} style={{height:37, background:"#161617", marginLeft:0, marginRight:0}}>
							...
							<div className={classes.flexContainer} style={{width:"100%"}}>
								<IconButton aria-label="down" onClick={() => handleFloorUpDown(floorPlan.order, "DOWN")} disabled={(floorPlan.order > 1)? false: true} >
									<ArrowDownDropCircleOutline style={dir == "rtl" ? {marginRight:10, marginLeft:5} : {marginLeft:10, marginRight:5}}/>
								</IconButton>
								...
								<IconButton aria-label="up" onClick={() => handleFloorUpDown(floorPlan.order,  "UP")} disabled={(floorPlan.order < floorPlans.length)? false: true} >
									<ArrowUpDropCircleOutline style={dir == "rtl" ?{marginLeft:10, marginRight:5} : {marginLeft:5, marginRight:10}}/>
								</IconButton>
							</div>
