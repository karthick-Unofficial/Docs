...
import { FacilitiesLayer, BasicLayer, AlertLayer, TrackHistoryLayer } from "orion-components/Map/Layers";
...
import { userFeedsSelector, gisLayerSelector } from "orion-components/GlobalData/Selectors"; // Remove once all entities selector added
...
const ReplayLayerSourcesWrapper = () => {
	...
	const activeFeeds = Map(
		filter(Map(userFeeds), (feed) => {
			return feed && !includes(disabledFeeds, feed.feedId) && feed.entityType !== "facility";
		}),
		"feedId"
	);
			...
			<Fragment>
				{Map(gisLayers, (layer) => (
					<BasicLayer
						...
						map={map}
						layer={layer}
						handleClick={(featureId, featureName, layerId) =>
							handleLoadGIS(featureId, featureName, layerId, loadGISProfile, dispatch)
						}
						labelsVisible={labelsVisible}
				...
				{eventsVisible && <Events map={map} />}
				{<TrackHistoryLayer map={map} />}
				<AlertLayer map={map} alerts={alerts} forReplay={true} before="---ac2-alerts-position-end" />
			</Fragment>
		) : (
			<div></div>
		);
