...
const LinkDialog = ({
				...
				serviceCall(
					...
					(err, res) => {
						...
						if (err) {
							...
							setSearch({
								error: getTranslation(
									"global.sharedComponents.linkDialog.errorText.errorOccurred"
								),
								querying: false
							});
						}
						if (!res || !res.success) {
							...
							setSearch({
								error: getTranslation(
									"global.sharedComponents.linkDialog.errorText.noItemsFound"
								),
								querying: false
							});
						} else {
	...
	const sortedEntities = entities.sort((a, b) => {
		if (
			...
			if (
				a.entityData.properties.name.toUpperCase() >
				b.entityData.properties.name.toUpperCase()
			) {
				return 1;
			}
			if (
		...
		<Dialog
			...
			confirm={{
				label: getTranslation(
					"global.sharedComponents.linkDialog.continue"
				),
				action: () => {
					handleSubmit(
						entity,
			...
			abort={{
				label: getTranslation(
					"global.sharedComponents.linkDialog.cancel"
				),
				action: () => {
					setLinkUpdates({ added: [] });
					setEntities([]);
		...
		>
			<SearchField
				...
				placeholder={getTranslation(
					"global.sharedComponents.linkDialog.wantToFind"
				)}
				autoFocus={autoFocus}
				dir={dir}
			/>
						...
						sortedEntities.map((entity) => {
							const checkbox = (
								<FormControlLabel
									...
									control={
										<Checkbox
											checked={linkUpdates.added.some(
												(added) =>
													added.id === entity.id
											)}
											onChange={(e, checked) =>
