	...
	useEffect(() => {
		return () => {
			if (unsubscribeFromFeed) {
				...
				unsubscribeFromFeed(contextId, entityType === "event" ? "eventCameras" : "camerasInRange", subscriberRef);
			}
		};
	}, [unsubscribeFromFeed, entityType, contextId, subscriberRef]);

	const handleExpand = () => {
		selectWidget("Cameras");
	};
				...
				<div className="widget-header">
					...
					{["camera", "shapes", "track", "accessPoint"].includes(entityType) && canLink && (
						<div className="widget-option-button" style={{ marginLeft: "auto" }}>
							<Button onClick={() => openDialog("link-entity-dialog")} style={styles.label} color="primary">
					...
					{canSlewAll && !readOnly && !disableSlew && (
						...
						>
							<Button style={styles.label} color="primary" onClick={handleSlewAll}>
