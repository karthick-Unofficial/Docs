...
const TrackHistoryInfo = ({ map, trackHistoryContexts, timeFormatPreference }) => {
    ...
    useEffect(() => {
        ...
        popupOrder.forEach((popupKey) => {
            if (
                popups[popupKey] &&
                trackHistoryContexts.indexOf(popups[popupKey].properties.id) >
                -1
            ) {
            ...
            {popupOrderNullCheck.map((popupId, index) => {
                return (
                    <Popup
                        ...
                        coordinates={
                            popups[popupId]
                                ? popups[popupId].geometry.coordinates
                                : []
                        }
                        offset={12}
                        onClick={() => handlePopupClick(popupId)}
                    >
                        <div>
                            <IconButton
                                style={{
                            ...
                            {popups[popupId].properties["lat"] && (
                                <p key={`${popupId}-lat`}>
                                    <Translate value="global.map.layers.trackHistoryInfo.lat" />{" "}
                                    <UnitParser
                            ...
                            {popups[popupId].properties["lng"] && (
                                <p key={`${popupId}-lng`}>
                                    <Translate value="global.map.layers.trackHistoryInfo.lon" />{" "}
                                    <UnitParser
                                        sourceUnit={"decimal-degrees"}
                                        value={
                                            popups[popupId].properties["lng"]
                            ...
                            {/* Speed */}
                            {(popups[popupId].properties["speed"] ||
                                popups[popupId].properties["speed"] === 0) && (
                                    <p key={`${popupId}-speed`}>
                                        <Translate value="global.map.layers.trackHistoryInfo.speed" />{" "}
                                        {popups[popupId].properties["speed"]}
                                    </p>
                                )}

                            {/* Heading */}
                            {(popups[popupId].properties["hdg"] ||
                                popups[popupId].properties["heading"]) && (
                                    <p key={`${popupId}-heading`}>
                                        <Translate value="global.map.layers.trackHistoryInfo.heading" />{" "}
                                        {popups[popupId].properties["hdg"]
                                            ? popups[popupId].properties["hdg"]
                                            : popups[popupId].properties["heading"]}
                                        °
                            ...
                            {popups[popupId].properties["acquisitionTime"] && (
                                <p key={`${popupId}-time`}>
                                    {timeConversion.convertToUserTime(
                                        popups[popupId].properties[
                                        "acquisitionTime"
                                        ],
                                        timeFormatPreference ? `full_${timeFormatPreference}` : "full"
                                    )}
                                </p>
                            )}
                        </div>
