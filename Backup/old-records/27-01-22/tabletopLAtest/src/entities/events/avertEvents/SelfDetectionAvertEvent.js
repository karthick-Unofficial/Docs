...
class SelfDetectionAvertEvent extends AvertEvent {
	constructor(event) {
		super(event);
	}
	...
	getTeam(simulationData) {
		const agent = simulationData.agents[this.event.participantId];
		if (agent) {
	...
	getDescription(agents, simulationData, simTimePrecision) {
		const detectedAgent = agents[this.event.actorId];
		const detectorAgent = agents[this.event.participantId];
	...
	apply(frameAgents) {
		const detectedAgent = frameAgents[this.event.actorId];
...
export default SelfDetectionAvertEvent;