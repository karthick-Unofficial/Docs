			...
			if (response.success === false) {
				...
				if (gisState) {
					each(response, (service) => {
						...
						if (size(layerIds))
							dispatch(getGISLayers(service.id, layerIds));
					});
				}
			}
		});
...
export const getGISLayers = (serviceId, layerIds, requestOffset = 0) => {
	if (currentlyRequesting) {
		...
		const requestString = `${serviceId},${
			Array.isArray(layerIds) ? layerIds[0] : layerIds
		}`;
		...
		return (dispatch, getState) => {
			...
			idArray.forEach((id) => {
				...
				restClient.exec_get(
					`/gis-app/api/gis-layers/${serviceId}/${toString(
						id
					...
					(err, response) => {
						...
						const layerIndex =
							cancelRequestList.indexOf(serviceLayerId);
						if (layerIndex > -1) {
							cancelRequestList.splice(layerIndex, 1);
							const requestIndex =
								currentRequestList.indexOf(serviceLayerId);
						...
						if (response[0].nextPageURL) {
							...
							const timeout =
								response[0].features.length === 0 ? 5000 : 10;
							setTimeout(() => {
...
export const updateVisibleGIS = (app, serviceId, update) => {
	return (dispatch, getState) => {
		...
		userService.setAppState(
			app,
			{ ["gisState"]: { [serviceId]: update } },
			(err, result) => {
				if (err) {
